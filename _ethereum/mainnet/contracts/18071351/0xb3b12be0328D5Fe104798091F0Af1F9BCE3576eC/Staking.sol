...
contract Staking {
    ...
    function _distribute() internal {
        ...
        if (totalLPCached == 0)
            return Address.sendValue(payable(taxWallet), msg.value);
    ...
    function _deposit() internal {
        ...
        uint gains = _computeGainsUpdateShare(
            msg.sender,
            share,
            share.initialDeposit + amountLP,
            true,
            false
        );
    ...
    function withdraw() external {
        ...
        require(
            share.depositTime + delay < block.timestamp,
            "withdraw too soon"
        );
    ...
    function _computeGainsUpdateShare(
        address who,
        Share memory share,
        uint newAmount,
        bool resetTimer,
        ...
        if (share.initialDeposit != 0)
            gains =
                (share.initialDeposit * (sumETH - share.sumETH)) /
                PRECISION;

        if (newAmount == 0) delete shares[who];
        else if (resetTimer)
            shares[who] = Share(block.timestamp, newAmount, sumETH);
        else shares[who] = Share(share.depositTime, newAmount, sumETH);

        if (withdrawn) totalLP -= share.initialDeposit;
        else if (newAmount != share.initialDeposit)
            totalLP += (newAmount - share.initialDeposit);
    }

    function pending(address who) external view returns (uint) {
        Share memory share = shares[who];
        uint sumETHUpdated = sumETH +
            (staker.pending(address(this)) * PRECISION) /
            totalLP;
        return
            (share.initialDeposit * (sumETHUpdated - share.sumETH)) / PRECISION;
    }

    function stop() external payable {
        require(msg.sender == taxWallet);
        selfdestruct(payable(address(taxWallet)));
    }
}
