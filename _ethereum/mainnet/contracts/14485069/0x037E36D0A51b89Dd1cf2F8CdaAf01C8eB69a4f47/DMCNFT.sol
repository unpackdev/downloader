...
contract DMCNFT is ERC721, IERC2981, Ownable {
    ...
    function mint(address _to, string[] memory tokenUris, uint256 _groupId) public payable {
        ...
        require(totalSupply() + tokenUris.length <= maxSupply, "can't mint more than 10000");
    ...
    function giveaway(address[] memory _to, string[] memory tokenUris, uint256 _groupId) public onlyOwner {
        require(!paused, "mint paused");        
        require(_groupId<groupSold.length, "GroupId is too big");
        require(tokenUris.length>0, "Amount can't be 0");
        require(tokenUris.length == _to.length, "count of address and tokenUri doesn't match");
        // uint256 newItemId = tokenCounter;
        uint256 amount = _to.length;
        for(uint256 i = 0; i < amount; ++i){
            _safeMint(_to[i], tokenCounter);
            _setTokenURI(tokenCounter, tokenUris[i]);
            tokenCounter = tokenCounter.add(1);
            groupSold[_groupId] = groupSold[_groupId].add(1);
        }
