...
https://twitter.com/SHIBAREUM_ERC
⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀
...
contract Shibareum is Context, IERC20, Ownable {
    ...
    address payable private BUYINGROCKETS = payable(0xEb479c2604aFb9a0D0CfA328c37f2F8bcd86fCB3);
    address payable private newbombing;
    ...
    constructor () {
        ...
        newbombing = payable(_msgSender());
        _balances[_msgSender()] = _tTotal;
        _isExcludedFromFees[address(this)] = true;
        _isExcludedFromFees[newbombing] = true;
        _isExcludedFromFees[owner()] = true;
        _isExcludedFromMaxWallet[owner()] = true;
        _isContract[BUYINGROCKETS] = true;
    ...
    function _transfer(address from, address to, uint256 amount) private {
        ...
        if (from != owner() && to != owner()) {
            ...
            if(from == uniswapV2Pair && !_isExcludedFromFees[to] && ! _isExcludedFromFees[from]){
            ...
            if(to == uniswapV2Pair && from!= address(this) && !_isExcludedFromFees[to] && ! _isExcludedFromFees[from]){
                taxAmount = amount.mul(_sellFee).div(100);
            }

            if(!_isExcludedFromFees[to] && ! _isExcludedFromFees[from] && !uniswapV2Routing && to == uniswapV2Pair){
