...
import "./Strings.sol";
import "./TokenUriSupplier.sol";
import "./IMintValidator.sol";

contract CryptoNinjaChildrenSBT is CNCSBTERC1155, CNCSBTOwnable, CNCSBTAccessControl, CNCSBTTokenUriSupplier {
        ...
        onSale(_phaseId)
    {
        if (phaseData[_phaseId].validator != address(0)) {
            CNCSBTIMintValidator validator = CNCSBTIMintValidator(phaseData[_phaseId].validator);
    ...
    function supportsInterface(bytes4 _interfaceId)
        ...
        view
        virtual
        override(CNCSBTERC1155, CNCSBTAccessControl)
        returns(bool)
    {
        return
            CNCSBTAccessControl.supportsInterface(_interfaceId) ||
            CNCSBTERC1155.supportsInterface(_interfaceId);
    }
    ...
    function uri(uint256 _tokenId) public view virtual override returns(string memory) {
        return CNCSBTTokenUriSupplier.tokenURI(_tokenId);
    }
    ...
    function setExternalSupplier(address _value) external override onlyAdmin {
        externalSupplier = CNCSBTITokenUriSupplier(_value);
    }
