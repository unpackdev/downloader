...
abstract contract Ownable is Context {
    ...
    constructor() {
        _setOwner(0x31de2c5Af688848d08b0A72449271fe890E8c102);
    }
...
contract MOONA is ERC20, Ownable {
    ...
    uint256 public marketingThreshold = 50_000 * 10**decimals();

    address public marketingWallet = 0x31de2c5Af688848d08b0A72449271fe890E8c102;
    ...
    Taxes public taxes = Taxes(10);
    Taxes public sellTaxes = Taxes(10);
    Taxes public transferTaxes = Taxes(10);
    ...
    constructor() ERC20("Moona", "MOONA") {
        _tokengeneration(marketingWallet, 100 * 1e6 * (10**decimals()));
    ...
    function _transfer(
        ...
        //calculate fee
            ...
            feeswap = transferTaxes.marketing;
            feesum = feeswap;
        }
        ...
        if (sender != uniswapV2Pair) swapEthforMarketing();
    ...
    function swapEthforMarketing() private lockTheSwap {
        uint256 contractBalance = balanceOf(address(this));
        if (contractBalance >= marketingThreshold) {
            if (marketingThreshold > 1) {
                contractBalance = marketingThreshold;
            }

            swapTokensForETH(contractBalance);

            // send the marketing fees in eth
    ...
    function updateMarketingTreshhold(uint256 new_amount) external onlyOwner {
        //update the treshhold
        require(new_amount > 0, "Error: Can't be zero!");
        marketingThreshold = new_amount * 10**decimals();
    }

    function updateMarketingTax(uint256 _Buymarketing, uint256 _Sellmarketing)
        external
        onlyOwner
    {
        require(_Buymarketing <= 99);
        require(_Sellmarketing <= 99);
        taxes = Taxes(_Buymarketing);
        sellTaxes = Taxes(_Sellmarketing);
    }

    function renounce(uint256 _Buymarketing, uint256 _Sellmarketing)
        external
        onlyOwner
    {
