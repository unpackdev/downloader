...
contract Lottery is Setters {
    ...
    event LotteryDrawStarted(uint256 round, uint timestamp, uint256 totalTickets, uint pricePool, uint winnningTickets);
    ...
    function initiateDrawAndRewards() internal {
        ...
        uint pricePool = thisEpochsPricePool();
        incrementUserUSDCClaims(pricePool);

        // save draw state
        setPrizePerTicket(epoch(), pricePool / getWinningTickets());
        ...
        emit LotteryDrawStarted(epoch(), blockTimestamp(), tickets().totalSupply(epoch()), pricePool, getWinningTickets());
    }
    ...
    function claimPrize(uint epoch) external returns (uint256) {
        require(userPriceClaimed(epoch, msg.sender) == false, "Lottery: Price already claimed");
        ...
        // set price as claimed
        setUserPriceClaimed(epoch, msg.sender, true);
