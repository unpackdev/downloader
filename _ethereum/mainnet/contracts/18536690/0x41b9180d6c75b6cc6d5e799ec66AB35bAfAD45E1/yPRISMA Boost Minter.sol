...
@external
def set_fees(_fee: uint256, _discount: uint256):
    assert (
        msg.sender == IYLOCKER(ylocker).governance() or 
        msg.sender == ylocker
    ), "!authorized"
    assert _fee <= 10_000
    assert _discount <= 10_000
    self.fee = _fee
    self.discount = _discount
    log FeeSet(_fee, _discount)
...
def getFeePct(
    ...
    if (
        ERC20(yprisma_staker).balanceOf(claimant) >= 100 * DECIMALS or
        ERC20(yprisma_lp_staker).balanceOf(claimant) >= 100 * DECIMALS or
        ERC20(yprisma).balanceOf(claimant) >= 100 * DECIMALS
    ):
        return self.fee * (10_000 - self.discount)
