...
contract testingone is ERC721A, Ownable, ReentrancyGuard {
    enum Status {
        Waiting,
        Started,
        Finished
    ...
    uint256 public constant freemintsupply = 4444;
    uint256 public instantfreeminted = 1;

    event Minted(address minter, uint256 amount);

    constructor(string memory initBaseURI) ERC721A("testing one", "TeSt") {
        baseURI = initBaseURI;
    ...
    function mint(uint256 quantity) external payable nonReentrant {
        ...
        uint256 _cost;
        if (instantfreeminted < freemintsupply) {
