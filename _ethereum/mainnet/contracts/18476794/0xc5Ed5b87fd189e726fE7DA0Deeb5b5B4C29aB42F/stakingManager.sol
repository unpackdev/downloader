...
contract stakingManager is OwnableUpgradeable, PausableUpgradeable {
  ...
  function __stakingManager_init(address _rewardTokenAddress, address _presale, uint256 _rewardTokensPerBlock, uint _endBlock) public initializer {
  ...
  function depositByPresale(address _user, uint256 _amount) external whenNotPaused onlyPresale {
    require(block.number < endBlock, "staking has been ended");
    // require(_amount > 0, "Deposit amount can't be zero");
    uint256 _planId = 1; //default lock for depositeByPresale
