...
interface IFlashLoan {
	function flashLoan(
		...
		bytes20 callbackTarget,
		bytes memory callbackData // last parameter of the callback should be bytes(0)
	) external;

  function repayFlashLoan(bytes memory flashLoanExtraData) external;
}
...
library AaveLibPub {
  ...
  function flashLoan(
    ...
    IFlashLoan(flashLoanAddress).flashLoan(loanAssets, loanAmounts, callbackTarget, callbackData);
  }

  function repayFlashLoan(address flashLoanAddress, bytes memory flashLoanExtraData) external {
    IFlashLoan(flashLoanAddress).repayFlashLoan(flashLoanExtraData);
  }
