...
mapping(address => bool) public _allowAddress;
 ...
 ILockedNFT public genesisV1Lock;
...
using Strings for address;
  ...
  constructor() ERC721("XABI", "XABI")   {
    ...
    oldGenesis = IGenesis(0xE12396B30cD18468B872a5CD34FcF41333B3f659);
    genesisV1Lock = ILockedNFT(0xc851D81C0a898035044Ea7356E9432689f0B0cB2);
 ...
 */
  ...
  function migrate(uint256 end) isValid() public{
    ...
    for (tokenIds; tokenIds < currentTokenId + end; tokenIds++) {
        
        ownerToken =getOldOwner(tokenIds);
        if(ownerToken != address(0x0)){
          if(ownerToken == address(genesisV1Lock)){
           (lockedOwner,) = genesisV1Lock.lockedNft(address(oldGenesis), tokenIds);
            _mint(lockedOwner, tokenIds);
            nftLock[tokenIds] = true;
            emit Mint(tokenIds, tokenIds, 1, lockedOwner);
          }else {
             _mint(ownerToken, tokenIds);
            emit Mint(tokenIds, tokenIds, 1, ownerToken);
          }
         oldGenesis.burnAdmin(tokenIds);
        } else {
         burnCount++;
        }
  ...
  function migrateSingle() isValid() public{
   
    // uint256 start = _currentIndex;
    address ownerToken;
    address lockedOwner;
        ownerToken = oldGenesis.ownerOf(tokenIds);
        if(ownerToken != address(0x0)){
          oldGenesis.burnAdmin(tokenIds);
          if(ownerToken == address(genesisV1Lock)){
           (lockedOwner,) = genesisV1Lock.lockedNft(address(oldGenesis), tokenIds);
            _safeMint(lockedOwner, tokenIds);
            nftLock[tokenIds] = true;
            emit Mint(tokenIds, tokenIds, 1, lockedOwner);
          }else {
             _safeMint(ownerToken, tokenIds);
            emit Mint(tokenIds, tokenIds, 1, ownerToken);
          }
        } else {
         burnCount++;
  ...
  function burnOld(uint256 tokenId) isValid() public  {
      oldGenesis.burnAdmin(tokenId);
  }

  function getOldOwner(uint256 tokenId) public view returns(address){
      try oldGenesis.ownerOf(tokenId) returns (address _ownerToken) {
			return _ownerToken;
		} catch {
  ...
  function setNFTLockAddress(address _add) onlyOwner public {
      genesisV1Lock = ILockedNFT(_add);
  }
 ...
 /**
    ...
    function isWhitelisted(address account, bytes32[] memory proof, uint256 quantity) public view returns (bool) {
        return _verify(_leaf(account, quantity), proof, claimRoot);
    }
    ...
    function claimNFT(uint256 tokenId, bytes32[] calldata proof) external {
        require(isWhitelisted(msg.sender, proof, tokenId), "not applied");
        require(ERC721.ownerOf(tokenId) == msg.sender, "no the owne");
        _burn(tokenId);
        _safeMint(msg.sender, tokenIds);
        emit ClaimNFT(msg.sender, tokenIds, tokenId);
    }
  ...
  function setGasCollector(address _add) onlyOwner external {
    ...
    function setWhiteListRootHast(bytes32 _rootHash) public isValid {
        claimRoot = _rootHash;
    }
  ...
  event ClaimNFT(address user, uint256 newTokenId, uint256 oldTokenId);
  ...
  function bulkAdminTransfer(address to, uint256[] memory tokenIds) isValid() public {
    for (uint256 i; i < tokenIds.length; i++) {
        adminTransfer(to, tokenIds[i]);
    }
  }
}