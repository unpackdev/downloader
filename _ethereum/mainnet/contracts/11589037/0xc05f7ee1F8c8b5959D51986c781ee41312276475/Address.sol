...
contract KatDataStore {
    ...
    address public katAddr;//kat代币地址
    address public usdtAddr;//usdt地址
    address public katMan;
    using SafeMath for uint256;
    using SafeERC20 for IERC20;
    
    uint public curEpoch = 1;//开始周期1
    uint public oneEth = 1 ether ;
    uint public buyBackMin ;//最少回购数
    uint public priceFirst;//官方价格
    uint public priceSecond;//实际价格
    uint public communitylen = 0;
    bool public isInit;//是否已初始化周期标识
    ...
    struct UserInfo {
        address invitor;//邀请地址
        uint registBlock;//用户注册高度
        uint totalInvest;//用户总投入
        uint cid;//用户所在社区ID
    }
    
    struct DepositInfo {//当前周期内用户信息
        uint depositVal;//充值金额
        uint depositTimes;//充值次数
        uint personReward;//分配奖励
        uint dynamicReward;//动态奖励
        bool isWithdraw;//是否提现
        
    }
    struct EpochInfo {//周期
        uint totalAmountToken;//总代币数量
        uint totalLimitEth;//周期最大eth金额
        uint totalDeposit;//周期累计充值
        bool isOver;//周期是否结束
    }
    
    struct CommunityInfo{//社区
        uint totalDeposit;//社区累计充值
        uint totalUserCount;//社区人数
        uint registBlock;//社区注册高度
        string cname;//社区名字
        address cowner;//负责人地址
    }
    ...
    mapping(address => address[]) public referArr;//直推人
    mapping(address => mapping(uint => DepositInfo)) public depositInfo;//周期内用户充值信息
    mapping(address => UserInfo) public userInfo;//用户信息
    mapping(uint => EpochInfo) public epochInfo;//周期信息
    mapping(uint => CommunityInfo) public communityInfo;//社区信息
