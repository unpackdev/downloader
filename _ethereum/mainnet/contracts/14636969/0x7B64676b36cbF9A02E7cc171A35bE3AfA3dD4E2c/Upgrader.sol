...
// Part: TransferableTokenHelper

contract TransferableTokenHelper{
  uint256 public decimals;
}

// Part: YieldHandlerInterface
...
contract Upgrader is Ownable {
    ...
    address new_owner;
    CFVaultV2 vault;
    CFControllerV2 old_controller;
    CFControllerV3 new_controller;
    CFPoolV3 new_pool;
    ...
    function prepare_upgrade(address _vault, address _old_controller, address _new_controller, address _new_pool) public onlyOwner() {
        vault = CFVaultV2(_vault.toPayable());
        old_controller = CFControllerV2(_old_controller.toPayable());
        new_controller = CFControllerV3(_new_controller.toPayable());
        new_pool = CFPoolV3(_new_pool);
    ...
    function upgrade() public onlyOwner() {
        new_pool.setVault(address(vault));

        IERC20 target_token = IERC20(0xA0b86991c6218b36c1d19D4a2e9Eb0cE3606eB48);
        old_controller.setVault(address(this));
        old_controller.withdraw(old_controller.get_current_pool().get_lp_token_balance());
        ...
        target_token.transfer(address(new_controller.get_current_pool()), target_token_amount);
        new_controller.deposit(target_token_amount);
        emit minted(new_controller.get_current_pool().get_lp_token_balance());
        ...
        new_controller.transferOwnership(new_owner);
        new_pool.transferOwnership(new_owner);
    }
}
