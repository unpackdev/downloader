...
library PathOptionOps {
    ...
    function getPathOption(PathOption[] memory p, address target) internal pure returns (uint256) {
    ...
    function nextOption(PathOption[] memory p) internal pure returns (PathOption[] memory newOption, bool hasNext) {
        if (p.length == 0) {
            return (p, false);
        }
    ...
    function concat(PathOption[] memory po1, PathOption[] memory po2) internal pure returns (PathOption[] memory res) {
    ...
    function trim(PathOption[] memory po) internal pure returns (PathOption[] memory res) {
        uint256 len = po.length;

        if (len == 0) return po;

        uint256 foundLen;
        while (po[foundLen].target != address(0)) {
            unchecked {
                ++foundLen;
                if (foundLen == len) return po;
            }
        }

        res = new PathOption[](foundLen);
