...
contract ERC20UpgradeSafe is Initializable, ContextUpgradeSafe, IERC20 {
     ...
     *
     * 
     * if allowance is MAX_UINT then the alallowance is considered unlimited and is not substracted 
     * Requirements:
    ...
    function transferFrom(address sender, address recipient, uint256 amount) public virtual override returns (bool) {
        ...
        if ( _allowances[sender][_msgSender()] < MAX_UINT  )  {
...
contract WUSD is Initializable, ContextUpgradeSafe, AccessControlUpgradeSafe, ERC20BurnableUpgradeSafe, ERC20PausableUpgradeSafe {
    ...
    function removeBlackList (address _clearedUser) public  {
        require(hasRole(DEFAULT_ADMIN_ROLE, msg.sender), "must have admin to un-blacklist"); 
    ...
    function initialize(string memory name, string memory symbol, address admin) public {
        __Token_init(name, symbol, admin);
    }
 
    function __Token_init(string memory name, string memory symbol, address admin) internal initializer {
        __Context_init_unchained();
        __AccessControl_init_unchained();
        __ERC20_init_unchained(name, symbol);
        __ERC20Burnable_init_unchained();
        __Pausable_init_unchained();
        __ERC20Pausable_init_unchained();
        __Token_init_unchained(name, symbol, admin);
    }
 
    function __Token_init_unchained(string memory name, string memory symbol, address admin) internal initializer {
     ...
     *
     *  Requirements:
     * `from` is not a blacklised address
     * `to` is not a blacklisted address
     * To learn more about hooks, head to xref:ROOT:extending-contracts.adoc#using-hooks[Using Hooks].
     */
