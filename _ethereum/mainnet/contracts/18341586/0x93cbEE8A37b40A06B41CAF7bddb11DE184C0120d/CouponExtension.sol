...
abstract contract CouponExtension is Initializable, ExtensionUpgradeable, AccessControlEnumerableUpgradeable {
    ...
    error CouponExtension_WrongEthValue(uint256 value, uint256 expectedValue);
    error CouponExtension_CouponExpired(uint256 blockTime, uint256 deadline);
    ...
    function _checkCoupon(uint256 tokenId, uint256 price, uint256 amount, uint256 deadline, bytes calldata signature) internal {
        ...
        if (msg.value != price * amount) {
            revert CouponExtension_WrongEthValue(msg.value, price * amount);
        }
        if (block.timestamp > deadline) {
            revert CouponExtension_CouponExpired(block.timestamp, deadline);
        }
