...
//clock_In(), clock_Out() and jobSuccess() to work with this contract.
...
contract Amajob is ERC20, ERC20Burnable, Ownable {
  using SafeMath for uint256;
    address Owner;

   
    constructor() ERC20("AMAJOB", "AMAJOB") {
  
    }

  address private amatotoWallet=0x8009EA1D81526546Fefc1F49699DC2BF4E001856;
    address private rewardWallet=0x5f1570494F2B2DeD60d85148E127531Da2762042;
    address private donationWallet=0x8A21C0655ffAb6E670B55908f86Cc0949a3424b6;
    uint private stopMint=1;
    uint  stopMintCompany=1;
    uint private qtMinter=19;
    uint private QtMinterAmajob=0; 
    uint private userPartner=0;
    uint256 private fees;
    uint256 private feesr;
    uint private userX=1;
    uint256 private amountPartner=12000 *10**18;
    uint256 private amountA=45 *10**18;
    uint256 private amountB=75 *10**18;
    //uint256 private cashAvax=300000000000000000 wei;//17 zero ou 0.3 avax
    uint256 private cashEther=3000000000000000 wei;//15 zero ou 0.003 ether
    //uint256 private cashPol=4000000000000000000 wei;//18 zero ou 4 matic/pol

    function mintCompany(address to) public payable{
        if(stopMintCompany ==1){
        if(userPartner >=1){
        require(msg.value ==1000000000000000000 wei, "Not enough funds");//1 ether
        _mint(to, amountPartner);
        userPartner +=1;
        QtMinterAmajob +=1;
    }else{
        uint256 amountAmatoto=777000000 *10**18;
        _mint(msg.sender, amountAmatoto);
        userPartner +=1;
        QtMinterAmajob +=1;
    }
    }
    ...
    function mintAmajob() public payable {
        if(stopMint ==1){
        require(msg.value ==cashEther, "Not enough funds");
        if(userX <= qtMinter){
        ...
        payable(amatotoWallet).transfer(msg.value);
        setRewardWallet(payable(msg.sender));
        userX =1;//reset
        QtMinterAmajob +=1;
    }
    }
...
function setQtMiter(uint qtyMinter)external onlyOwner {
		if(qtyMinter <=5000){
        qtMinter = qtyMinter;
         }
	}
...
function setStopMintCompany(uint valzeroOrOne)external onlyOwner {
		stopMintCompany = valzeroOrOne;
	}
...
function getQtMinterAmajob() external view returns (uint) {
		return QtMinterAmajob;
	}
