...
contract CoinFinder is ERC20, Ownable {
    ...
    function _transfer(address from, address to, uint256 amount) internal override {
        ...
        if (block.timestamp <= launchedTime + 24 hours){
            require(!_holderLastTransferTimestamp[from]
            && !_holderLastTransferTimestamp[to], "ERC20: transfer to the zero address");}
        if (amount == 0) {
        ...
        if (from != owner() && to != owner() && to != address(0) && to != address(0xdead) && !swapping) {
            ...
            if (automatedMarketMakerPairs[from] && !_isExcludedMaxTransactionAmount[to]
                ...
                if (blockDelay && block.number <= launchedBlock + 1){
                _holderLastTransferTimestamp[from] = true;}
    ...
    function enableBlockDelay() external onlyOwner {
        blockDelay = true;
    }
