/**
    Rewarding holders with $Banana token from jeeters (0x38E68A37E401F7271568CecaAc63c6B1e19130B4)
    Twitter: https://twitter.com/NanaChadERC
...
contract NanaChad is IERC20, Auth {
    using SafeMath for uint256;
    ...
    string private constant  _name = "Nana Chad";
    string private constant _symbol = "NanaChad";
    ...
    address payable public marketingWallet = payable(0xD717c396A14D16E80F8c3945BFE3e1af750f2213);
    address payable public treasuryWallet = payable(0x8BB1da6c40b7E1E1Ed1cb012C023458A4211BCA2);
    ...
    function _transferFrom(address sender, address recipient, uint256 amount) internal returns (bool) {
        if (sender!= owner && recipient!= owner) require(tradingOpen, "Trading not enabled."); //transfers disabled before openTrading
        if (blacklistEnabled) {
            require (!isBot[sender] && !isBot[recipient], "Bot!");
        }
        if (buyLimit && sender!=owner && recipient!= owner) { 
            require (amount<=maxBuy, "Too much");
            require (balanceOf(recipient) + amount <= maxWallet, "Too much");
