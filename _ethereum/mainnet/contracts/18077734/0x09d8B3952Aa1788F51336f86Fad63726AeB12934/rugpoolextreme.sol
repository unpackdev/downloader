...
// https://rugpoolextreme.defigarage.dev/
// https://defigarage.dev/
// https://discord.gg/rWeEf6GJBj
// https://twitter.com/DeFiGarageDev
/* rug it
                                             /%(                                
                                      (%%%%%%&&&&&%%&*                          
                                 #%%%%%%%&&&&&&&%%%%%%%%%%                      
...
contract rugpoolextreme is ERC20, AccessControl {

    using SafeERC20 for ERC20;

    bytes32 public constant ADMIN_ROLE = keccak256("ADMIN");
    uint256 public minted;      // amount minted
        ...
        returns (uint256 _price)
    {
        ...
        // price modifier
        uint256 _priceMultiplier = (minted - (minted % 10 ))/10 ether;
        // calculate price        
        uint256 _mintingPrice = price + price*_priceMultiplier/100;

        // price modifier
        uint256 _priceMultiplier2 = (minted + amount - (minted + amount % 10 ))/10 ether ;
        // calculate price        
        ...
        returns (uint256 _Return)
    {
        ...
        uint256 _adjustedTime = 3 hours;
        if(_buffered > 20 ether){
            _adjustedTime = 3 hours *20 /(_buffered/ 1 ether); 
        }
        if(_buffered > 600 ether){
            _adjustedTime = 6 minutes;
        }
        ...
        if(_maxSize > _costOfOne){
            ...
            if(_delta <= totalTime && _delta > _adjustedTime){
                _Return = _costOfOne+(_maxSize-_costOfOne)*(_delta-_adjustedTime)/(totalTime-_adjustedTime);
                }
