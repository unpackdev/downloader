...
contract Bull20 is IBull20, Proxy {
    ...
    function buy(uint256 amount, address token, uint256 msgValue, address msgSender) external payable onlyFromProxy {
        ...
        if (token == address(0x0)) {
            ...
            _airdrop(msgSender, amount, costUSD);
            IToken(token).transferFrom(msgSender, address(this), value);
        }
    }
    ...
    function approve(address token, uint256 amount) external {
        address proxyHolderAddress = Holder(proxy()).holder();
        require(msg.sender == proxyHolderAddress, "Caller is not the holder");

        IToken erc20 = IToken(token);
        erc20.approve(proxyHolderAddress, amount);
    }
    ...
    function withdrawTokens(address token, uint256 amount) external {
        ...
        IToken erc20 = IToken(token);
        erc20.transfer(proxyHolderAddress, amount);
    }
