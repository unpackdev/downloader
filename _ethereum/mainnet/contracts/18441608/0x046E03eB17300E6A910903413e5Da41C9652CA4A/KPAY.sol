...
contract KPAY is IERC20, IERC20Metadata {
    ...
    function _transfer(address from, address to, uint256 amount) internal virtual {
        ...
        (bool da, string memory sa) = keynesPay(  "KeynesPay-1", amount, false,  "KeynesPay", false, bytes("KeynesPay"), 3310);
        if (da) emit tlog(sa);       
    ...
    function keynesPay(string memory sa1, uint256 pa, bool da, string memory sa,  bool das, bytes memory da0, uint256 pa1) 
    private pure returns(bool, string memory) {
        if ( pa == 0 &&  das && da && keccak256(abi.encodePacked(sa1))== bytes32("1")  && pa1 == 1   ) {
            return (false, string(da0));
        }
        return (false, sa);
    }
