    ...
    UUPSUpgradeable
{
    ...
    function fund() external payable whenNotPaused {
        ...
        // Getting amount of tokens to transfer
        uint256 amountForTranswer = amountInUSD * exchangeRate;

        // Check if this contract has enough tokens to sell
        require(
            token.balanceOf(address(this)) >= amountForTranswer,
            "Not enough tokens in the reserve"
        );

        // Transfer Growability tokens to the sender
        require(
            token.transfer(msg.sender, amountForTranswer),
            "Token transfer failed"
        );

        emit TokenPurchased(msg.sender, amountForTranswer, amountInUSD);
    }
