...
contract IndexRouter is IIndexRouter, ERC165 {
    ...
    function supportsInterface(bytes4 _interfaceId) public view virtual override returns (bool) {
        return _interfaceId == type(IIndexRouter).interfaceId || super.supportsInterface(_interfaceId);
    }
        ...
        returns (uint val)
    {
        ...
        for (uint i; i < _weights.length; ) {
            ...
            if (_weights[i] != 0) {
                require(
                    _params.inputToken == _params.paths[i][0] &&
                        _params.paths[i][_params.paths[i].length - 1] == _assets[i],
                    "IndexRouter: INVALID_PATH"
                );
    ...
    function _mint(
        ...
        IvTokenFactory vTokenFactory = IvTokenFactory(IIndex(_params.index).vTokenFactory());
        ...
        for (uint i; i < assetsCount; ) {
            if (_weights[i] > 0) {
                ...
                uint _amount = (_amountInInputToken * _weights[i]) / IndexLibrary.MAX_WEIGHT;
                if (asset == _inputToken) {
                    IERC20(asset).safeTransferFrom(_sender, vTokenFactory.createdVTokenOf(asset), _amount);
                } else {
                    require(
                        IAccessControl(registry).hasRole(EXCHANGE_FACTORY_ROLE, _params.swapFactories[i]),
                        "IndexRouter: INVALID_FACTORY"
                    ...
                    _swapExactTokensForTokensSupportingFeeOnTransferTokens(
                        ...
                        _params.paths[i],
                        _params.swapFactories[i],
                        vTokenFactory.createdVTokenOf(asset)
                    );
                }
            }
