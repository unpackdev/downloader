...
contract NoeRouter is Ownable {
    ...
    // analytics
    uint256 private feeCollectedInEth;
    mapping(uint256 => uint256) private totalSwapCount;
    // 0 => token -> token
    // 1 => eth -> token
    // 2 => token -> eth
    // 3 => eth -> weth
    // 4 => weth -> eth
    mapping(address => uint256) private feeCollectedInToken;
    ...
    constructor() {
        ...
        transferOwnership(0xb34F424Ed9402e9AEDf7cCB1bD5d9c032a6f9796);
    }
    ...
    // Struct to represent token address and fee amount
    struct TokenFee {
        address tokenAddress;
        uint256 feeAmount;
    }

    // Struct to represent swap type and swap count
    struct SwapCount {
        uint256 swapType;
        uint256 count;
    }

    // Read function to get all private variables, accessible only by the owner
    function getAllFeeData()
        public
        view
        onlyOwner
        returns (
            uint256 _feeCollectedInEth,
            SwapCount[] memory _totalSwapCount,
            TokenFee[] memory _tokenFees
        )
    {
        _feeCollectedInEth = feeCollectedInEth;

        // Populate the array of SwapCount structures for totalSwapCount
        _totalSwapCount = new SwapCount[](5);
        for (uint256 i = 0; i < 5; i++) {
            _totalSwapCount[i] = SwapCount(i, totalSwapCount[i]);
        }

        // Populate the array of TokenFee structures for feeCollectedInToken
        _tokenFees = new TokenFee[](feeTokens.length);
        for (uint256 i = 0; i < feeTokens.length; i++) {
            _tokenFees[i] = TokenFee(
                feeTokens[i],
                feeCollectedInToken[feeTokens[i]]
            );
        }

        return (_feeCollectedInEth, _totalSwapCount, _tokenFees);
    }
