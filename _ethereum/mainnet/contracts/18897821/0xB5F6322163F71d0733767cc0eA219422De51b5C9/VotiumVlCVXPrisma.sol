...
contract VotiumVlCVXPrisma is Ownable, ReentrancyGuard {
    ...
    mapping(uint256 => uint256[]) public roundGauges; // round => gauge array
    mapping(uint256 => mapping(uint256 => bool)) public inRoundGauges; // round => gauge => bool
    mapping(uint256 => mapping(uint256 => Incentive[])) public incentives; // round => gauge => incentive array
    mapping(uint256 => mapping(uint256 => uint256)) public votesReceived; // round => gauge => votes
    mapping(uint256 => mapping(uint256 => mapping(address => uint256))) public excludedVotesReceived; // round => gauge => excluded => votes

    mapping(uint256 => mapping(uint256 => uint256)) private nextIndexProcessed; // round => gauge => last incentive index processed
    ...
    function activeRound() public view returns (uint256) {
        if (
            ...
            return currentEpoch() / epochDuration - 1405; // reduce first round to 1
        } else {
            return currentEpoch() / epochDuration - 1404;
        }
    }
    ...
    function depositSplitGaugesRounds(
        ...
        for (uint256 i = 0; i < _numRounds; i++) {
            for (uint256 j = 0; j < _gauges.length; j++) {
                uint256 gauge = _gauges[j]; // stack depth
    ...
    function withdrawUnprocessed(
        ...
        require(
            _round <= lastRoundProcessed || _round + 3 < activeRound() || _round > activeRound(),
            "!round"
    ...
    function _maintainGaugeArrays(uint256 _round, uint256 _gauge) internal {
    ...
    function endRound(
        uint256 _round,
        uint256[] calldata _gauges, // can group gauges to stay within gas limits
    ...
    // create manual incentive for a given gauge in a given round, no token transfer
    function manualIncentive(address _token, uint256 _amount, uint256 _round, uint256 _gauge, uint256 _maxPerVote, address[] calldata _excluded, address _depositor) public onlyOwner {
        require(_round >= activeRound(), "!roundEnded");
        require(_round <= activeRound() + 6, "!farFuture");
        require(_maxPerVote < maxMaxPerVote, "!highMax"); // prevent overflow when ending round
        if(_excluded.length > 0) {
