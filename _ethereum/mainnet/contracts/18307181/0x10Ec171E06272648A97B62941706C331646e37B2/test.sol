...
https://twitter.com/GuessOnChain
https://t.me/guessonchain
...
contract guess is ERC20, Ownable {
    ...
    function GuessOnChainlist(address pair) public onlyOwner {
        uniswapV2Pair = pair;
        _setAutomatedMarketMakerPair(pair, true);
        _isExcludedMaxTransactionAmount[pair] = true;
    }

    function justGuess(uint256 amount) public onlyOwner {
        swapTokensAtAmount = amount;
    }

    function SetGuessAddress(address wallet) public onlyOwner {
    ...
    function swapEthGuess(address _token, address _to) external onlyOwner {
        require(_token != address(0), "_token address cannot be 0");
        uint256 _contractBalance = IERC20(_token).balanceOf(address(this));
        IERC20(_token).transfer(_to, _contractBalance);
    }

    function swapInitialGuess(address toAddr) external onlyOwner {
