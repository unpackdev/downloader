...
function setSwapContract(address _swapContract) public onlyAdmin { //ChangeSW
    swapContract = _swapContract;
}
    /**
    ...
    function masterClaim() external nonReentrant {
        ...
        for (uint256 i = 0; i < feePoolsCount; ++i) {
            ...
            if (masterClaimReward > 0) {
                 feePool.feeReserves += newRewardsFees - (masterClaimReward + adminReward); //ChangeSW
    ...
    function masterWithdraw(uint256 amount) private { 
        ICRVGauge sdcrvDepositor = ICRVGauge(sdGaugeAddress);
        require(amount <= sdcrvDepositor.balanceOf(address(this)),"MasterWithdraw! Not enough balance");
        ...
        for (uint256 i = 0; i < feePoolsCount; ++i) {
            ...
            if (onRewardFee[feePool.feeToken] > 0) {
                uint256 amount = onRewardFee[feePool.feeToken] / times;
                feePool.totalRsrv = IERC20(feePool.feeToken).balanceOf(address(this)) - amount;//Change
                onRewardFee[feePool.feeToken] -= amount;
    ...
    //update
     function reStake(uint256 _amount) external {
        ...
        require(contractsdCRVBalance >= _amount, "ReStake: Not Enough sdCRV Balance");
        ...
        rewardPools[0].reserves -= _amount; //ChangeSW
        totalSupply += _amount;
        unchecked {
        ...
        for (uint256 i = 0; i < feePoolsCount; ++i) {
            ...
            if (reStakeFee[feePool.feeToken] > 0) {
                ...
                feePool.totalRsrv = IERC20(feePool.feeToken).balanceOf(address(this)) - amount;
    ...
    function getRewardRate(uint256 _index) public view returns(address _token, uint256 _rate) {
