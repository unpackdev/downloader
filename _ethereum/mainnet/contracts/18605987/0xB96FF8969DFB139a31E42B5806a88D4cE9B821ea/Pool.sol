...
abstract contract Pool is ReentrancyGuardUpgradeable, Admin {
    ...
    struct DstChainInfo {
        ...
        uint32 dynamicFeeNum;
        // Limit of a single transfer
        uint256 maxTransferLimit;
    }

    uint256 public constant DYNAMIC_FEE_DEN = 1000000; // denominator of dynamic fee
        ...
        nonReentrant
    {
        _checkDstChain(poolId, dstChainId);
        require(
            poolId == NATIVE_TOKEN_POOL_ID || dynamicFeeNum == 0,
            "Pool: Dynamic fee can only be set for native token pools"
        );

        _dstChains[poolId][dstChainId].staticFee = uint128(staticFee);
        _dstChains[poolId][dstChainId].dynamicFeeNum = uint32(dynamicFeeNum);
