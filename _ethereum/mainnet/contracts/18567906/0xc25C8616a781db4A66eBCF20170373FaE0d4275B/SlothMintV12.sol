...
import "./ISpecialSlothItemV3.sol";
import "./ISlothMintV4.sol";
import "./OwnableUpgradeable.sol";
import "./Initializable.sol";

contract SlothMintV12 is Initializable, OwnableUpgradeable, ISlothMintV4 {
    ...
    function publicMintBody(uint8 quantity) payable external {
        ...
        ISlothV3(_slothAddr).mint(msg.sender, quantity);
    ...
    function _mintSlothCollectionNovember(address transferAddress, uint256 quantity, uint8 clothType) internal {
        ISpecialSlothItemV3(_specialSlothItemAddr).mintSlothCollectionNovember(transferAddress, quantity, clothType);
    }
    ...
    function mintCollaboWithBody(uint8 quantity, uint256 specialType) internal {
        ...
        _publicMint(quantity, msg.sender);
        ISpecialSlothItemV3(_specialSlothItemAddr).mintCollaboCloth(msg.sender, quantity, specialType);
    ...
    function publicMintWithClothesAndCollaboForPiement(address transferAddress, uint256 specialType) payable external {
        ...
        if (msg.sender == owner()) {
          ...
          ISpecialSlothItemV3(_specialSlothItemAddr).mintCollaboCloth(transferAddress, 1, specialType);
    ...
    function _mintHalloweenSlothCollection(address transferAddress, uint256 quantity, uint256 clothType) internal {
        if (clothType == 1) {
          ISpecialSlothItemV3(_specialSlothItemAddr).mintHalloweenJiangshiSet(transferAddress, quantity);
        }
        if (clothType == 2) {
          ISpecialSlothItemV3(_specialSlothItemAddr).mintHalloweenJacKOLanternSet(transferAddress, quantity);
        }
        if (clothType == 3) {
          ISpecialSlothItemV3(_specialSlothItemAddr).mintHalloweenGhostSet(transferAddress, quantity);
        }
    }
    ...
    function publicMintHalloweenSlothCollection(uint256 quantity, uint256 clothType) payable external {
        ...
        emit mintSlothCollection(quantity, 39, clothType, false);
    }
    function publicMintHalloweenSlothCollectionForPiement(address transferAddress, uint256 quantity, uint256 clothType) payable external {
        ...
        require(msg.sender == _piementAddress, "worng address");
        _mintHalloweenSlothCollection(transferAddress, quantity, clothType);
        emit mintSlothCollection(quantity, 39, clothType, true);
    }
    ...
    function publicMintAllHalloweenSlothCollection(uint256 quantity) payable external {
        ...
        for (uint256 i = 1; i <= numberOfCollectionKind; i++) {
          ...
          emit mintSlothCollection(quantity, 39, i, false);
        }
    }
    function publicMintAllHalloweenSlothCollectionForPiement(address transferAddress, uint256 quantity) payable external {
        ...
        for (uint256 i = 1; i <= 4; i++) {
          _mintHalloweenSlothCollection(transferAddress, quantity, i);
          emit mintSlothCollection(quantity, 39, i, true);
        }
    }
