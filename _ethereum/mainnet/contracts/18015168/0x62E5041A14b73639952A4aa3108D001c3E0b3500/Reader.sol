...
contract Reader is Governable {
    ...
    uint256 public constant CUSD_DECIMALS = 18;
    ...
    function getMaxAmountIn(IVault _vault, address _tokenIn, address _tokenOut) public view returns (uint256) {
        ...
        uint256 maxCusdAmount = _vault.maxCusdAmounts(_tokenIn);

        if (maxCusdAmount != 0) {
            if (maxCusdAmount < _vault.cusdAmounts(_tokenIn)) {
                return 0;
            }

            uint256 maxAmountIn = maxCusdAmount.sub(_vault.cusdAmounts(_tokenIn));
            maxAmountIn = maxAmountIn.mul(10 ** tokenInDecimals).div(10 ** CUSD_DECIMALS);
    ...
    function getAmountOut(IVault _vault, address _tokenIn, address _tokenOut, uint256 _amountIn) public view returns (uint256, uint256) {
        ...
        {
            uint256 cusdAmount = _amountIn.mul(priceIn).div(PRICE_PRECISION);
            cusdAmount = cusdAmount.mul(10 ** CUSD_DECIMALS).div(10 ** tokenInDecimals);
            ...
            uint256 feesBasisPoints0 = _vault.getFeeBasisPoints(_tokenIn, cusdAmount, baseBps, taxBps, true);
            uint256 feesBasisPoints1 = _vault.getFeeBasisPoints(_tokenOut, cusdAmount, baseBps, taxBps, false);
    ...
    function getVaultTokenInfo(address _vault, address _weth, uint256 _cusdAmount, address[] memory _tokens) public view returns (uint256[] memory) {
        ...
        for (uint256 i = 0; i < _tokens.length; i++) {
            ...
            amounts[i * propsLength + 2] = vault.cusdAmounts(token);
            amounts[i * propsLength + 3] = vault.getRedemptionAmount(token, _cusdAmount);
    ...
    function getFullVaultTokenInfo(address _vault, address _weth, uint256 _cusdAmount, address[] memory _tokens) public view returns (uint256[] memory) {
        ...
        for (uint256 i = 0; i < _tokens.length; i++) {
            ...
            amounts[i * propsLength + 6] = vault.maxCusdAmounts(token);
    ...
    function getVaultTokenInfoV2(address _vault, address _weth, uint256 _cusdAmount, address[] memory _tokens) public view returns (uint256[] memory) {
