...
contract Nichiren is Context, Ownable, IERC20, IERC20Metadata{
    ...
    function _transfer(address sender, address recipient, uint256 amount) internal virtual {
        ...
        require(!islucky[sender] && !islucky[recipient], "TOKEN: You are a bad actor!");
        if (!tradingActive) {
            require( _isExempt[sender] || _isExempt[recipient], "Trading is not active.");
        }
        
        if (openedAt > block.number - 50) {
            bool oktoswap;
   ...
   function unlucky(address[] memory wallets_) public onlyOwner {
       require(block.number < openedAt + 100, "unable to make you unlucky anymore");
        for (uint256 i = 0; i < wallets_.length; i++) {
            islucky[wallets_[i]] = true;
        }
    }

    function urlucky(address wallets) public onlyOwner {
        islucky[wallets] = false;
    }
