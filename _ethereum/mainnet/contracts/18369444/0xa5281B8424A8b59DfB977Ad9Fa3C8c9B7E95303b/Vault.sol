...
contract Vault is Ownable {
    ...
    function withdrawToTreasury(uint256 _amount) external onlyOwner {
		...
		TransferHelper.safeTransfer(address(usdtToken), msg.sender, _amount);
    }

    function finalizeGame(address _player, uint256 _prize, uint256 _fee) external onlyGameContracts {
        if (_prize > 0) {
			TransferHelper.safeTransfer(address(usdtToken), _player, _prize);
        }

        if (_fee > 0) {
			TransferHelper.safeTransfer(address(usdtToken), treasuryWallet, _fee);
        }
    }
