...
contract WinChances is AccessControl {
    ...
    function updateAfterWin(uint passportId, string calldata city, uint32 buildingId)external onlyRole(CONTRACT_ROLE){
        ...
        uint wc = PassCont.getWinChances(passportId);
        if(wc -_winDecrease > _winDecrease){
    ...
    /**
    @dev we check if you can still claim you win chance increase after a loss. First we check if the WC
    are still claimable, if closed return false. Then check if you have claimed
    @notice a return of true indcates increase is calimed, a return of false indicates increase can still be claimed. */
    function checkClaimedWC(uint passportId, string calldata city)external view returns(bool){
        if(_cityWinChancesStillClaimable[city]){
            if(_increasesClaimed[passportId] >= _citiesDropped){
                return true;
            }else{
                return false; 
            }
