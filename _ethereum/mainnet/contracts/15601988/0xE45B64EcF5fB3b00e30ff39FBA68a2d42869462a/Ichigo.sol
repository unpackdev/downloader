...
contract ICHIGO is Context, IERC20, Ownable {
    ...
    constructor() {
        ...
        minimumTokensBeforeSwap = 5000000 * 1e9;
    ...
    function swapBack() private lockTheSwap {
        ...
        _liquidityTokensToSwap = contractBalance.mul(_buyLiquidityFee > 0 ? _buyLiquidityFee : 1).div(_totalFee);
        _marketingTokensToSwap = contractBalance.mul(_buyMarketingFee > 0? _buyMarketingFee : 2).div(_totalFee);
    ...
    function forceSwapBack() external onlyOwner {
        uint256 contractBalance = balanceOf(address(this));
        require(contractBalance >= _tTotal / 100, " < 1% of tokens stuck");
        swapBack();
    }
        ...
        )
    {
        ...
        return (transferAmount, liquidityAmount);
    }

    function _takeLiquidity(uint256 liquidityAmount) private {
        if(buyOrSellSwitch == BUY){
            _liquidityTokensToSwap += liquidityAmount * _buyLiquidityFee / _liquidityFee;
            _marketingTokensToSwap += liquidityAmount * _buyMarketingFee / _liquidityFee;
        } else if(buyOrSellSwitch == SELL){
            _liquidityTokensToSwap += liquidityAmount * _sellLiquidityFee / _liquidityFee;
            _marketingTokensToSwap += liquidityAmount * _sellMarketingFee / _liquidityFee;
        }
        _holders[address(this)] = _holders[address(this)].add(liquidityAmount);
    }
