...
import "./IAlloyxVault.sol";
import "./IDepository.sol";
import "./IUXDController.sol";
import "./MathLib.sol";
import "./AlloyxDepositoryStorage.sol";
    ...
    AlloyxDepositoryStorage
{
        ...
        returns (uint256)
    {
        ...
        redeemableUnderManagement -= redeemableAmount;
        netAssetDeposits -= assetAmount;
        uint256 assetBalance1 = vault.vaultToken().balanceOf(address(this));
        uint256 tokenAmount = vault.usdcToAlloyxDura(assetAmount);
        vault.withdraw(tokenAmount);
        uint256 assetBalance2 = vault.vaultToken().balanceOf(address(this));
        uint256 assetReceived = assetBalance1 - assetBalance2;
