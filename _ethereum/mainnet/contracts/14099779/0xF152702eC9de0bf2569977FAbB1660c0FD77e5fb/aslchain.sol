...
/*
 ...
 //_\\  / /\ \  / / /    / /  | '_ \ / _` | | '_ \ 
/  _  \/ / _\ \/ / /___ / /___| | | | (_| | | | | |
\_/ \_/_/  \__/_/\____/ \____/|_| |_|\__,_|_|_| |_|
...
// Shout out to LOOT.
contract ASLChain is ERC721Enum, Ownable, ReentrancyGuard {
    ...
    function getAge(uint256 tokenId) internal pure returns (string memory) {
        uint256 rand = uint256(
            keccak256(
                abi.encodePacked(
                    string(abi.encodePacked("AGE", toString(tokenId)))
                )
            )
        ) % 103;
        return toString(rand + 12);
    }
    ...
    function getSex(uint256 tokenId) internal view returns (string memory) {
    ...
    function pluck(
        ...
        string memory output = arr[rand % arr.length];
        ...
        returns (string memory)
    {
        ...
        parts[0] = '<svg xmlns="http://www.w3.org/2000/svg" preserveAspectRatio="xMinYMin meet" viewBox="0 0 350 350"><style>.base { fill: black; font-family: monospace; font-size: 24px; text-align: center; }</style><rect width="100%" height="100%" fill="white" /><text x="50%" y="50%" dominant-baseline="middle" text-anchor="middle" class="base">';
            ...
            bytes(
                string(
                    abi.encodePacked(
                        '{"name": "A/S/L Chain #',
                        toString(tokenId),
                        '", "description": "A/S/L Chain genesis collection is a random combination of 103 Ages, 3+ Sexes and 50 (USA) States which all live immutably on chain. The A/S/L you own can represent any part of yourself.", "image": "data:image/svg+xml;base64,',
