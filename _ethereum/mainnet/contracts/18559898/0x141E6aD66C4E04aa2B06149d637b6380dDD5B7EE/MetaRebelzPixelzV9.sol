...
import "./ERC721A.sol";
import "./MetaRebelzV2.sol";

contract MetaRebelzPixelzV9 is ERC721AQueryableUpgradeable, OwnableUpgradeable, PausableUpgradeable, ReentrancyGuardUpgradeable, DefaultOperatorFiltererUpgradeable, ERC721ABurnableUpgradeable {
    uint256 public cost;   
   ...
   /// @custom:oz-upgrades-unsafe-allow constructor
    ...
    function claimOriginal(uint256[] memory tokenIds) public payable {
        require(claimable == true, "Claims disabled");
        require(balanceOf(msg.sender) >= requiredClaimAmount, "Must own minimum required claim");
        require(ERC721A(smallBrosContract).balanceOf(msg.sender) >= 1, "Must own at least 1 token of SmallBros");
        require(ERC721A(okaybearsContract).balanceOf(msg.sender) >= 1, "Must own at least 1 token of OkayBears");
        require(ERC721AUpgradeable(hongozContract).balanceOf(msg.sender) >= 1, "Must own at least 1 token of Hongoz");
        require(msg.value == 0, "No ether should be sent with this transaction");
        
        for (uint i = 0; i < tokenIds.length; i++) {
            require(ownerOf(tokenIds[i]) == msg.sender, "Sender does not own all tokens");
            require(isUnlocked(tokenIds[i]), "Token is locked");
            burn(tokenIds[i]);
        }

        MetaRebelzV2(0x94c2911cEB9B3684204C63b8E204A660990b7318).mint(claimGiveAmount, msg.sender);
    }
