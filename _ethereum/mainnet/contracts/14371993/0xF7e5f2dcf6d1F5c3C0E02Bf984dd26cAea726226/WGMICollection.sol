...
contract WGMICollection is ERC721A, Ownable, Pausable, ReentrancyGuard, Allowlist, PaymentSplitter {
    uint private constant MAX_SUPPLY = 1475;
    uint private constant MAX_PER_TX = 3;
    ...
    ) ERC721A("WGMICollection", "WGMI", 20) PaymentSplitter(_receivers, _shares) Allowlist() {
    ...
    function preMint(uint amount, bytes32[] calldata _freeProof, bytes32[] calldata _ogProof, bytes32[] calldata _wlProof) external payable onlyAllowedPresale(_freeProof, _ogProof, _wlProof, amount) whenNotPaused nonReentrant {
        ...
        require(amountTest < MAX_PER_TX + 1, "amount can't exceed 3");
    ...
    function publicMint(uint amount) external payable whenNotPaused nonReentrant {
        uint supply = totalSupply();
        require(amount < MAX_PER_TX + 1, "amount can't exceed 3");
