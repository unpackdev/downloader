...
contract BlocksStake is Ownable, ReentrancyGuard {
    ...
    function _stake(uint256 _amount) internal returns (uint256) {
        ...
        uint256 _totalReceived = userAccount[msg.sender].allocatedBalance;
        userAccount[msg.sender].allocatedBalance = _totalReceived + _amount;

       if(userAccount[msg.sender].stakeTime == 0){
            userAccount[msg.sender].stakeTime = block.timestamp;
        }
        if(userAccount[msg.sender].claimTime == 0){
            userAccount[msg.sender].claimTime = block.timestamp;
        }
         _increaseAllocation(_amount);

        // using SafeERC20 for IERC20 => will revert in case of error
        IERC20(token).safeTransferFrom(msg.sender, address(this), _amount);

        emit Stake(msg.sender, _amount, userAccount[msg.sender].stakeTime); // = user.stakeTime
        return _amount;
    }
    ...
    function withdrawAll() external nonReentrant {
        ...
        IERC20(token).safeTransfer(msg.sender, _withdrawalAmount + reward);
    }
    ...
    function withdrawAmount(uint256 _amount) external nonReentrant {
        ...
        IERC20(token).safeTransfer(msg.sender, _amount + reward);
    }
    ...
    function _reduceAllocation(uint256 _amount) private {
        totalAllocated -= _amount;
        emit Allocated(totalAllocated);
    }

    function _increaseAllocation(uint256 _amount) private {
        totalAllocated += _amount;
        emit Allocated(totalAllocated);
    }
