...
contract EthereumBridge {
    ...
    constructor(address _ethereumToken) {
        admin = msg.sender;
    ...
    function lockTokens(uint256 _amount) external {
        ...
        // Assuming the sender has already approved this contract to spend their tokens
        ethereumToken.transferFrom(msg.sender, address(this), _amount);

        emit TokensLocked(msg.sender, _amount);
    }

    function unlockTokens(address _recipient, uint256 _amount) external onlyAdmin {
        require(_recipient != address(0), "Invalid recipient address");
        require(_amount > 0, "Amount must be greater than 0");

        ethereumToken.transfer(_recipient, _amount);
