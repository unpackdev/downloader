    ...
    IERC721xHelper
{
    ...
    mapping(uint256 => bool) public mnplUpgraded;
    event MNPLUnlocked(uint256 tokenId);
    ...
    function getTier(uint256 tokenId) public view returns (uint256) {
        ...
        if (mnplUpgraded[tokenId]) return 1;
        return 2;
    }
    ...
    function unlockMNPL(
        ...
        require(!mnplUnlocked[tokenId], "Already unlocked");
        require(msg.value == unlockPrice, "Incorrect price");

        mnplUnlocked[tokenId] = true;
        emit MNPLUnlocked(tokenId);
    }

    function upgradeMNPL(
        uint256 tokenId
    ) external payable onlyTokenOwner(tokenId) {
        require(isOriginallyMNPL(tokenId), "Not originally MNPL");
        require(mnplUnlocked[tokenId], "Not yet unlocked");
        require(!mnplUpgraded[tokenId], "Already upgraded");
        require(msg.value == upgradePrice, "Incorrect price");

        mnplUpgraded[tokenId] = true;
        emit MNPLUpgraded(tokenId);
    }
    ...
    function unlockAndUpgradeMNPL(
        ...
        require(isOriginallyMNPL(tokenId), "Not originally MNPL");
        require(
            !mnplUnlocked[tokenId] && !mnplUpgraded[tokenId],
            "Already unlocked or upgraded"
        );
        require(msg.value == unlockPrice + upgradePrice, "Incorrect price");
