...
contract TLNX is ERC1400, IExtensionTypes {

    uint256 public constant ISSUED_TOKENS = 12_460_317 * 10 ** 18;
    uint256 public constant FOR_SALE_TOKENS = 8_722_222 * 10 ** 18;

    bytes32 public constant TLNX_DEFAULT_PARTITION = keccak256(abi.encodePacked("TLNX_DEFAULT_PARTITION"));

    bytes32[] private TLNX_DEFAULT_PARTITIONS = [bytes32(TLNX_DEFAULT_PARTITION)];
   ...
   */
    ...
    ERC1400("ATLANT-X", "TLNX", GRANULARITY, TLNX_DEFAULT_PARTITIONS)
    {
        require(newOwner != address(0), "TLNX: wrong address");
        require(seller != address(0), "TLNX: wrong address");

        transferOwnership(newOwner);

        _issueByPartition(TLNX_DEFAULT_PARTITION, msg.sender, newOwner, ISSUED_TOKENS, "");
        _isIssuable = false;
