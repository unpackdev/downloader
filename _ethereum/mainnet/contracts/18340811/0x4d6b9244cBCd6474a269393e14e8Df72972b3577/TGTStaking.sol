...
contract TGTStaking is Ownable, ReentrancyGuard {
    ...
    function getStakingMultiplier(address _user) public view returns (uint256) {
        ...
        if (timeDiff >= 365 hours) {
            return 1e18;
        } else if (timeDiff >= (30 hours * 6) && timeDiff < 365 hours) {
            if (timeDiff > (30 hours * 6)) {
                return (75e16 + calculatePart(25e16, calculatePercentage(timeDiff - 30 hours * 6, 30 hours * 6)));
            }
            else return 75e16;
        }
        else if (timeDiff >= 7 hours && timeDiff < (30 hours * 6)) {
            if (timeDiff > 7 hours) {
                return (5e17 + calculatePart(25e16, calculatePercentage(timeDiff - 7 hours, 30 hours * 6)));
            }
            else return 5e17;
        }
        return 0;
