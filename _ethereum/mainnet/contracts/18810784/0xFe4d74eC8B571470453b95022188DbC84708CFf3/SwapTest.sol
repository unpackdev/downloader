...
IERC721ReceiverUpgradeable
{
    ...
    IENSController public ensController;
    bool public isEnsActive;
    uint256 public ensRegisterFee;
    ...
    function setupFee(uint256 _sellerFee, uint256 _buyerFee, uint256 _ensRegisterFee) public onlyOwner {
        require(_sellerFee <= 20 && _buyerFee <= 20, 'Swap:fee too much. max 20 (2%)');

        sellerFee = _sellerFee;
        buyerFee = _buyerFee;
        ensRegisterFee = _ensRegisterFee;
    }
    ...
    function setIsEnsActive(bool isActive_) public onlyOwner {
        isEnsActive = isActive_;
    }

    function setEnsControllerAddress(address _address) public onlyOwner {
        ensController = IENSController(_address);
    }

    function getEnsCommitment(
        OrderEIP712.OrderSig calldata orderWithSig,
        OrderEIP712.EnsData[] calldata data
        ...
        for (uint256 i = 0; i < data.length; i++) {
            hashes[i] = ensController.makeCommitment(
                data[i].name,
                orderWithSig.signer,
                data[i].duration,
                data[i].secret,
    ...
    function registerEns(
        ...
        require(isEnsActive, 'Swap:ens deactivated');
        require(orderWithSig.swapType == OrderEIP712.SwapType.ENS, 'Swap:Wrong swap type');
        require(address(ensController) != address(0), 'Swap:Wrong ens controller address');

        bytes32 orderHash = hashBySig(orderWithSig);

        uint256 actualPrice = ensController.getRentPrice(data.name, orderWithSig.signer, data.duration, data.data);
        require(orderWithSig.price >= actualPrice, 'Swap:Wrong order price');

        validateOrder(orderWithSig, orderHash, /* ignore check gas price */ false);

        uint256 withdrawalEthGas = gasleft();
        getEthFrom(orderWithSig.signer, actualPrice);
        uint256 transferGasUsage = withdrawalEthGas - gasleft();

        (bool success,) = address(ensController).call{value: actualPrice}(
            abi.encodeWithSignature(
                "register(string,address,uint256,bytes32,bytes[])",
                data.name, orderWithSig.signer, data.duration, data.secret, data.data
            )
        );

        require(success, 'Swap:EnsController call failed');

        uint256 tokenId = uint256(keccak256(bytes(data.name)));

        require(IERC721Upgradeable(ensController.getBase(data.data)).ownerOf(tokenId) == orderWithSig.signer, 'Swap:Wrong token owner');

        dealsByHash[orderHash]++;

        uint256 fee = actualPrice.div(1000).mul(ensRegisterFee);
        ...
        (bool transferSuccess,) = msg.sender.call{value: approxEthForGasUsageWithFee}('');
        require(transferSuccess, 'Swap:Transfer failed');
    }

    function makeEnsCommits(OrderEIP712.OrderSig calldata orderWithSig, OrderEIP712.EnsData[] calldata data) public onlyOperators {
        uint256 startGas = gasleft();

        require(isEnsActive, 'Swap:ens deactivated');
        require(orderWithSig.swapType == OrderEIP712.SwapType.ENS, 'Swap:Wrong order type');

        bytes32 orderHash = hashBySig(orderWithSig);

        validateOrder(orderWithSig, orderHash, /* ignore check gas price */ false);

        bytes32[] memory hashes = getEnsCommitment(orderWithSig, data);

        for (uint256 i = 0; i < hashes.length; i++) {
            (uint256 availableAt, uint256 expiredAt) = ensController.commit(hashes[i]);
            emit EnsCommit(orderHash, hashes[i], availableAt, expiredAt, block.timestamp);
        }

        uint256 approxEthForGasUsage = (startGas - gasleft())
        .add(80000) // weth.transferFrom(from, address(this), value);
        .add(45000) // weth.withdraw(value);
        .add(21000)
        .mul(tx.gasprice);

        getEthFrom(orderWithSig.signer, approxEthForGasUsage);

        (bool transferSuccess,) = msg.sender.call{value: approxEthForGasUsage}('');
        require(transferSuccess, 'Swap:Transfer failed');
    }
    ...
    function p2pSwap(OrderEIP712.OrderSig[] calldata orders, uint256[][] calldata tokenIds) public nonReentrant payable {
        ...
        (bool senderTransferSuccess,) = msg.sender.call{value: sumOfExchange}(''); // send eth to seller
        require(senderTransferSuccess, 'Swap:Sender transfer failed');

        (bool ownerTransferSuccess,) = owner().call{value: sellerFeeActual.add(buyerFeeActual)}(''); // send all fee to owner
        require(ownerTransferSuccess, 'Swap:Owner transfer failed');
    ...
    function transferTokenToSigner(
        ...
        if (assetClass == ERC721_ASSET_CLASS) {
            ...
            revert("Unexpected condition token transfer");
        }
    }
