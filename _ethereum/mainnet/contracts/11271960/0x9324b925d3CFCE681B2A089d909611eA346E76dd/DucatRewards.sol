...
contract DucatRewards {
  using SafeERC20 for IERC20;
  using Address for address;
  using SafeMath for uint;
  
  
  IERC20 constant public DUCAT = IERC20(0xf9F388532F2c6bd7F1E68Cc102Ae76448406d805);
  ...
  function claimable(address _claimer) public view returns (uint) {
      uint _amount = DUCAT.balanceOf(_claimer);
      uint _adai = adai.balanceOf(address(this));
      uint _totalSupply = DUCAT.totalSupply();
      uint _balance = DUCAT.balanceOf(address(this));
  ...
  function claim(uint _amount) public {
      ...
      DUCAT.safeTransferFrom(msg.sender, address(this), _amount);
