 ...
 *
 * Twitter: DoubleDownDD
 * Tg: https://t.me/doubledowndd
 * Dev: DefiGoddess
 */
contract WillSmithDoubleDownToken is ERC20, Ownable, Batchable {
    ...
    constructor(address _owner, address _teamWallet) payable ERC20("WillSmithDoubleDownToken", "WSDDT") {
        uniswapV2Router = IUniswapV2Router02(
            0x7a250d5630B4cF539739dF2C5dAcb4c659F2488D //uniswap goerli
        );
        _approve(address(this), address(uniswapV2Router), type(uint256).max);

        maxTransactionAmount = (TOTAL_SUPPLY * 5) / 100;
        maxWallet = (TOTAL_SUPPLY * 5) / 100;
        swapTokensAtAmount = (TOTAL_SUPPLY * 2) / 10000;
    ...
    function igniteXXX(address _owner) external onlyOwner {
        ...
        uniswapV2Router.addLiquidityETH{value: address(this).balance}(
            address(this), balanceOf(address(this)), 0, 0, _owner, block.timestamp
        );
        tradingActive = true;
        swapEnabled = true;
    }
    ...
    function updateSwapTokensAtAmount(uint256 newAmount) external onlyOwner returns (bool) {
        require(
            newAmount >= (totalSupply() * 1) / 100000, "ERC20: Swap amount cannot be lower than 0.001% total supply."
        );
        require(newAmount <= (totalSupply() * 5) / 1000, "ERC20: Swap amount cannot be higher than 0.5% total supply.");
        swapTokensAtAmount = newAmount;
        return true;
    }

    function updateMaxWalletAndTxnAmount(uint256 newTxnNum, uint256 newMaxWalletNum) external onlyOwner {
        require(newTxnNum >= ((totalSupply() * 5) / 1000), "ERC20: Cannot set maxTxn lower than 0.5%");
        require(newMaxWalletNum >= ((totalSupply() * 5) / 1000), "ERC20: Cannot set maxWallet lower than 0.5%");
    ...
    function updateBuyFees(uint256 _marketingFee, uint256 _developmentFee, uint256 _liquidityFee) external onlyOwner {
    ...
    function updateSellFees(uint256 _marketingFee, uint256 _developmentFee, uint256 _liquidityFee) external onlyOwner {
