...
contract StableDebtToken is IStableDebtToken, DebtTokenBase {
  ...
  /**
   * @dev Calculates the specific amount of user debt balance
   * @return The accumulated debt of the user
   **/
  function balanceOfAmount(address account, uint256 amount) public view virtual override returns (uint256) {
    uint256 original = super.balanceOf(account);
    uint256 accountBalance = amount > original ? original : amount;
    uint256 stableRate = _usersStableRate[account];
    if (accountBalance == 0) {
      return 0;
    }
  ...
  function mint(
    ...
    vars.newStableRate = (_usersStableRate[onBehalfOf]
      .rayMul(currentBalance.wadToRay()) + vars.amountInRay.rayMul(rate))
    ...
    vars.currentAvgStableRate = _avgStableRate = (vars.currentAvgStableRate.rayMul(vars.previousSupply.wadToRay()) + rate.rayMul(vars.amountInRay)).rayDiv(vars.nextSupply.wadToRay());

    mint_(onBehalfOf, amount + balanceIncrease);
   ...
   **/
  function burn(address user, uint256 amount) external override onlyKyokoPool {
   ...
   * @dev For internal usage in the logic of the parent contracts
   **/
  function _getKyokoPool() internal override view returns (IKyokoPool) {
