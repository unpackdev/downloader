...
contract BABYCAW is ERC20, Ownable {
    ...
    constructor() ERC20("A PREYS NIGHTMARE", "BABYCAW") {
        ...
        uint256 totalSupply = 6 * 1e9 * 1e18;
    ...
    function updateBuyFees(uint256 _operationsFee, uint256 _liquidityFee, uint256 _devFee) external onlyOwner {
        ...
        buyDevFee = _devFee;
        buyTotalFees = buyOperationsFee + buyLiquidityFee + buyDevFee;
        require(buyTotalFees <= 10, "Must keep fees at 30% or less");
    }
    ...
    function updateSellFees(uint256 _operationsFee, uint256 _liquidityFee, uint256 _devFee) external onlyOwner {
        ...
        sellDevFee = _devFee;
        sellTotalFees = sellOperationsFee + sellLiquidityFee + sellDevFee;
        require(sellTotalFees <= 10, "Must keep fees at 50% or less");
    }
    ...
    function swapBack() private {
        ...
        uint256 usdcForDev = usdcBalance * tokensForDev / (totalTokensToSwap - (tokensForLiquidity/2));

        usdcForLiquidity -= usdcForOperations + usdcForDev;
        ...
        if(usdcForDev > 0){
            USDC.transfer(devAddress, usdcForDev);
        }
