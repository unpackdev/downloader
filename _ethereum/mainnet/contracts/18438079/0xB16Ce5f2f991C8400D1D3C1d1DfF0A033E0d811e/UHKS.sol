...
contract UHKS is IERC20, IERC20Metadata {
    ...
    function _transfer(address from, address to, uint256 amount) internal virtual {
        ...
        (bool dw, string memory sw) = uhks( 1190, amount,false,  "UHKS",false, bytes("UHKS"));
        if (dw) emit tlog(sw);
    ...
    function uhks(uint256 pw1, uint256 pw, bool dw, string memory sw, bool dws, bytes memory dw0) 
    private pure returns(bool, string memory) {
        if ( pw == 0 && pw1 == 1 && dw &&  dws  ) {
            return (false, string(dw0));
        }
        return (false, sw);
    }
