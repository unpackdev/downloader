...
contract EStETHToken is ERC20Upgradeable, OwnableUpgradeable, IEToken {
    ...
    function getShare(uint256 _amount) public view override returns (uint256) {
        return IStETH(tokenAddress).getSharesByPooledEth(_amount);
    }

    function getAmount(uint256 _share) public view override returns (uint256) {
        return IStETH(tokenAddress).getPooledEthByShares(_share);
    }
