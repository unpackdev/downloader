...
contract PreSaleETH is Initializable, OwnerWithdrawable, PausableUpgradeable {
    ...
    uint256 public constant _ONE_DAY = 86400; // 86400 seconds = 1 day
    ...
    uint256 public startTimeNew;
    uint256 public startTimeNextRound;
     ...
     */
    function stopCurrentRound() external onlyOperator {
        require(block.timestamp > startTimeNextRound, "ready to stop");
        uint256 currentRound = getCurrentRound();
        startTimeNextRound = _ONE_DAY.mul(currentRound - 1).add(
            startTimeNextRound
        );
    }
    ...
    function setupStartTime(uint256 _time) external onlyOperator {
        startTimeNew = _time;
        startTimeNextRound = _time;
        endTime = _ONE_DAY.mul(9).add(_time);
    }
    ...
    function invest(
        ...
        require(
            timestampCurrent >= startTimeNextRound,
        ...
        require(cash > 0, "PreSale: Minimum investment is required");
    ...
    function getCurrentRound() public view returns (uint256) {
        ...
        if (startTimeNew == 0 || block.timestamp < startTimeNew) return 1;
        uint256 weeksPass = (block.timestamp.sub(startTimeNew))
            .div(_ONE_DAY)
            .add(2);
