...
// File: contracts/Dawning_Chain.sol
...
contract Dawning_Chain is Ownable, Auditable, ERC721 {
    using SafeMath for uint256;
    ...
    // The NFT is a non-fungible token and this one is non-transferable 
    event TransferAttempted(string _message);

    constructor(address _auditor) Ownable() Auditable(_auditor, address(this)) ERC721("The Church of the Chain incorporated inaugural donation NFT", "DAWNCHAIN") public {}

    function mint(address _donator) public payable onlyOwner() isAudited() {
        if (donators[_donator] == true) {
            return;
        }
        ...
        emit MintedToken(_donator, tokenID, now);
    ...
    function _transfer(address from, address to, uint256 tokenId) internal virtual override {
        // may not need require statements
        require(ownerOf(tokenId) == from, "ERC721: cannot transfer a token that is not yours");
        require(to != address(0), "ERC721: cannot transfer to the zero address");

        emit TransferAttempted("NFT is non-transferable");
    }
}