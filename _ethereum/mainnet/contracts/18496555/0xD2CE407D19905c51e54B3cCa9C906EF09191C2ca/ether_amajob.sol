/**
 *Submitted for verification at snowtrace.io on 2023-10-08
*/
...
contract ether_amajob is ERC20, ERC20Burnable, Ownable {
  using SafeMath for uint256;
    address Owner;

   address private walletX=0x1810Cac7d1183fba6Ef9eFf343CFe82825e7A7E1;
	address private anyWallet=0x1810Cac7d1183fba6Ef9eFf343CFe82825e7A7E1;

    address zero_address = 0x0000000000000000000000000000000000000000;
    address ethTokenAddress = 0xF413Ddc5996Dd1F96d361E1132906562C82974a9;
    ...
    uint256 tokenPaid=3000000000 *10**18;
    uint256 tokenPaid_B=25;
    uint256 tokenPaid_Bx;
    mapping(address => bool) whitelistedAddresses;

    uint256 limitToken=1;
    uint qtToken=3;
    uint mintOrStock=1;
    uint stopmint=0;
    uint mintUser=0;
 ...
 function setAmatotoContract(address amatotoContract)external onlyOwner {
  ...
  function clock_In()external payable{
    require(msg.value >=3e16 wei, "Not enough funds");
...
function clock_xMint() external payable{
      require(msg.value >=5e16 wei, "Not enough funds");
      require(clockIn[msg.sender]>0,"User did not clock in");
      require(clockOut[msg.sender]>0,"User did not clock out");
      if (mintUser <=qtMiner){
     if(mintUser==qtMiner){mintUser=0;
...
function resetMyClock() external payable{ 
require(msg.value >=5e16 wei, "Not enough funds");
clockIn[msg.sender] =0;
clockOut[msg.sender] =0;
   }
...
function setAnyWallet(address payable anyWalletPay) internal {
 ...
 function setStopmint(uint valzeroOrOne)external onlyOwner {
    ...
    function setStopmintX(uint valzeroOrOne)external onlyOwner {
		stopmintx = valzeroOrOne; 
	}
function Wallet_X()external view returns (address){
      return walletX;
      } 

  function any_Wallet()external view returns (address){
      return anyWallet;
      } 
 
function Stopmint()external view returns (uint){
      return stopmint;
      } 
function StopmintX()external view returns (uint){
      return stopmintx;
      } 

  function resetAllWallet()external onlyOwner{
    walletX=0x1810Cac7d1183fba6Ef9eFf343CFe82825e7A7E1;
	anyWallet=0x1810Cac7d1183fba6Ef9eFf343CFe82825e7A7E1;
  }
  function checkIfBusy() external view returns (string memory){
      if(block.timestamp > amatotoDate + timeReset){
          return "You can clock in to mint";}else{
              return "Please wait!";
