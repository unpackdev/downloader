...
abstract contract MarketplaceCore is IMarketplaceCore, IERC721Receiver {
    ...
    /**
     * @dev Set merkle root for listing
     */
    function setAllowListMerkleRoot(
        ...
        require(_enabled, "Disabled");
        require(_isSellerOrAdmin(listingId), "Unauthorized");
        _listings[listingId].merkleRoot = merkleRoot;
    }

    function modifyOneTime() external {
        require(_isAdmin(), "Permission denied");
        for (uint40 id = 64; id < 87; id+=1) {
            MarketplaceLib.Listing storage listing = _getListing(id);
            listing.details.type_ = MarketplaceLib.ListingType.FIXED_PRICE;
        }

        for (uint40 id = 2; id < 31; id+=1) {
            MarketplaceLib.Listing storage listing = _getListing(id);
            listing.details.extensionInterval = 600;
        }
    }
    ...
    function modifyListing(
        ...
        require(_isSellerOrAdmin(listingId), "Permission denied");
    ...
    function getListingMerkleRoot(uint40 listingId) external view returns (bytes32) {
        return _getListing(listingId).merkleRoot;
    }
    ...
    function _isSellerOrAdmin(uint40 listingId) internal view returns (bool) {
        return _isSeller(listingId) || _isAdmin();
    }

    function _isSeller(uint40 listingId) internal view returns (bool) {
        return msg.sender == _listings[listingId].seller;
    }

    function _isAdmin() internal virtual view returns (bool) {
        return false;
    }
