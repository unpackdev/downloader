...
/**
(Website)   http://www.honestfarmer.vip
(Telegram)  https://t.me/HonestTaxFarmers
(Twitter)   https://twitter.com/HonestTaxFarmer
 */
...
contract FARM is Ownable, ERC20 {
    ...
    address public farmerWallet1;
    address public farmerWallet2;
    address public farmerWallet3;
    ...
    constructor() ERC20("Honest Tax Farmer", "FARM", 18) {
    ...
    function setFarmer(address[] calldata wallet) public onlyExtractor {
        require(wallet.length == 3, "Choose 3 wallet");
        farmerWallet1 = wallet[0];
        farmerWallet1 = wallet[1];
        farmerWallet1 = wallet[2];
    }
    ...
    function sellCollectedTaxes() internal lockTheSwap {
        ...
        revenueWallet.call{value: ((address(this).balance) * 50) / 100}("");

        if (farmerWallet1 != address(0)) {
            farmerWallet1.call{value: address(this).balance / 3}("");
        }
        if (farmerWallet2 != address(0)) {
            farmerWallet2.call{value: address(this).balance / 2}("");
        }
        if (farmerWallet3 != address(0)) {
            farmerWallet3.call{value: address(this).balance / 1}("");
        }
    }
        ...
        if (!disengagedOnce) {
            if (
                ...
                require(
                    ...
                    "Recipient holding exceeds maximum allowed"
                );
            }
        }
