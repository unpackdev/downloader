...
contract StrategyDAIOne {
    using SafeERC20 for IERC20;
    using Address for address;
    using SafeMath for uint256;

    address constant public want = address(0x6B175474E89094C44Da98b954EedeAC495271d0F);// DAI
    address constant public y = address(0x16de59092dAE5CcF4A1E6439D611fd0653f0Bd01);//iearn DAI (yDAI)
    ...
    function getName() external pure returns (string memory) {
        return "StrategyDAIOne";
    }

    // This method take dai
    // dai => yDAI => ycrv => yyDAI+yUSDC+yUSDT+yTUSD
    function deposit() public {
        uint _want = IERC20(want).balanceOf(address(this));
        if (_want > 0) {// checks balance of DAI is greater than 0 and deposit it to yDAI, it return yDAI
            IERC20(want).safeApprove(y, 0);
            IERC20(want).safeApprove(y, _want);
            yERC20(y).deposit(_want);
        }
        uint _y = IERC20(y).balanceOf(address(this));// checks balance of yDAI
        if (_y > 0) {// checks balance of yDAI is greater than 0 and add liquidity to ICurveFi and get ycrv
            IERC20(y).safeApprove(curve, 0);
            IERC20(y).safeApprove(curve, _y);
            ICurveFi(curve).add_liquidity([_y,0,0,0],0);
        }
        uint _ycrv = IERC20(ycrv).balanceOf(address(this));// checks balance of ycrv
        if (_ycrv > 0) {// checks balance of ycrv is greater than 0 and deposit it into yvault
