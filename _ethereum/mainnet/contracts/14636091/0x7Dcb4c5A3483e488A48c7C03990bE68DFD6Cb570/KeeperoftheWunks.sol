...
contract KeeperoftheWunks is AccessControl, ERC721URIStorage, EIP712 {
  using   SafeMath for uint256;

  bytes32 public  constant MINTER_ROLE       = keccak256("MINTER");
  string  private constant SIGNING_DOMAIN    = "Blam-LazyMint";
  string  private constant SIGNATURE_VERSION = "1";
  address public           proxyRegistryAddress;

  constructor() ERC721("Keeper of the Wunks", "KEEPERWUNK") EIP712(SIGNING_DOMAIN, SIGNATURE_VERSION) {
    _setupRole(MINTER_ROLE, payable(_msgSender()));
    proxyRegistryAddress = address(0xa5409ec958C83C3f309868babACA7c86DCB077c1);
  }

  function airdrop(Voucher calldata voucher, address recipient) public {
    address signer = _verify(voucher);

    require(hasRole(MINTER_ROLE, signer), "Invalid signature");
    require(hasRole(MINTER_ROLE, _msgSender()), "Only authorized minters can airdrop.");

    _mint(signer, voucher.tokenId);
    _setTokenURI(voucher.tokenId, voucher.uri);
    _transfer(signer, recipient, voucher.tokenId);
  }
  ...
  function redeem(Voucher calldata voucher) public payable returns (uint256) {
    ...
    require(hasRole(MINTER_ROLE, signer), "Invalid signature");
    require(msg.value == voucher.price, "Sent incorrect eth amount");
  ...
  function _hash(Voucher calldata voucher) internal view returns (bytes32) {
    return _hashTypedDataV4(keccak256(abi.encode(
      keccak256("Voucher(uint256 tokenId,uint256 price,string uri)"),
      voucher.tokenId,
      voucher.price,
  ...
  function setProxyRegistryAddress(address _proxyRegistryAddress) external {
    require(hasRole(MINTER_ROLE, _msgSender()), "Only authorized minters can set proxy");
