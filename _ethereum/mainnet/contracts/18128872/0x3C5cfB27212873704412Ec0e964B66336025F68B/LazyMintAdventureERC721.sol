...
abstract contract LazyMintAdventureBase is AdventureWhitelist, IAdventurous {

    error LazyMintAdventureBase__AdventureApprovalToCaller();
    error LazyMintAdventureBase__AlreadyOnQuest();
    error LazyMintAdventureBase__AnActiveQuestIsPreventingTransfers();
    error LazyMintAdventureBase__CallerNotApprovedForAdventure();
    error LazyMintAdventureBase__CallerNotTokenOwner();
    error LazyMintAdventureBase__MaxSimultaneousQuestsCannotBeZero();
    error LazyMintAdventureBase__MaxSimultaneousQuestsExceeded();
    error LazyMintAdventureBase__NotOnQuest();
    error LazyMintAdventureBase__QuestIdOutOfRange();
    error LazyMintAdventureBase__TooManyActiveQuests();
    ...
    function adventureTransferFrom(address from, address to, uint256 tokenId) external override {
        ...
        (address owner_, TokenOwner storage tokenOwner_) = _requireCallerApprovedForAdventure(tokenId);
        transferType = TRANSFERRING_VIA_ADVENTURE;
        _doTransfer(owner_, tokenOwner_, from, to, tokenId);
    ...
    function adventureSafeTransferFrom(address from, address to, uint256 tokenId) external override {
        ...
        (address owner_, TokenOwner storage tokenOwner_) = _requireCallerApprovedForAdventure(tokenId);
        transferType = TRANSFERRING_VIA_ADVENTURE;
        _doSafeTransfer(owner_, tokenOwner_, from, to, tokenId, "");
        transferType = TRANSFERRING_VIA_ERC721;
    }
    ...
    function adventureBurn(uint256 tokenId) external override {
        _requireCallerIsWhitelistedAdventure();
        (address owner_,) = _requireCallerApprovedForAdventure(tokenId);
        transferType = TRANSFERRING_VIA_ADVENTURE;
        _doBurn(owner_, tokenId);
        transferType = TRANSFERRING_VIA_ERC721;
    }
    ...
    function enterQuest(uint256 tokenId, uint256 questId) external override {
        _requireCallerIsWhitelistedAdventure();
        (address owner_,) = _requireCallerApprovedForAdventure(tokenId);
        _enterQuest(owner_, tokenId, _msgSender(), questId);
    }
    ...
    function exitQuest(uint256 tokenId, uint256 questId) external override {
        _requireCallerIsWhitelistedAdventure();
        (address owner_,) = _requireCallerApprovedForAdventure(tokenId);
        _exitQuest(owner_, tokenId, _msgSender(), questId);
    }
    ...
    function bootFromAllQuests(uint256 tokenId, address adventure) external {
        ...
        (address owner_,) = _ownerOfToken(tokenId);
        _exitAllQuests(owner_, tokenId, adventure, true);
    }
    ...
    function userExitQuest(uint256 tokenId, address adventure, uint256 questId) external {
        _requireAdventureRemovedFromWhitelist(adventure);
        address owner_ = _requireCallerOwnsToken(tokenId);
        _exitQuest(owner_, tokenId, adventure, questId);
    }
    ...
    function userExitAllQuests(uint256 tokenId, address adventure) external {
        _requireAdventureRemovedFromWhitelist(adventure);
        address owner_ =_requireCallerOwnsToken(tokenId);
        _exitAllQuests(owner_, tokenId, adventure, false);
    }
    ...
    function isParticipatingInQuest(uint256 tokenId, address adventure, uint256 questId) public override view returns (bool participatingInQuest, uint256 startTimestamp, uint256 index) {
        if(questId > type(uint32).max) {
            revert LazyMintAdventureBase__QuestIdOutOfRange();
        }
    ...
    /// Emits a QuestUpdated event for off-chain processing.
    function _exitQuest(address ownerOfToken, uint256 tokenId, address adventure, uint256 questId) internal {
        (bool participatingInQuest, uint256 startTimestamp, uint256 index) = isParticipatingInQuest(tokenId, adventure, questId);
        if(!participatingInQuest) {
            revert LazyMintAdventureBase__NotOnQuest();
        }
    ...
    function _exitAllQuests(address tokenOwner, uint256 tokenId, address adventure, bool booted) internal {
        uint256 questCount = getQuestCount(tokenId, adventure);
    ...
    function _requireCallerApprovedForAdventure(uint256 tokenId) internal view returns (address owner_, TokenOwner storage tokenOwner_) {
        (owner_, tokenOwner_) = _ownerOfToken(tokenId);
        if(!areAdventuresApprovedForAll(owner_, _msgSender())) {
            revert LazyMintAdventureBase__CallerNotApprovedForAdventure();
        }
    }

    /// @dev Validates that the caller owns the specified token
    /// Throws when the caller does not own the specified token.
    function _requireCallerOwnsToken(uint256 tokenId) internal view returns (address owner_) {
        (owner_,) = _ownerOfToken(tokenId);
        if(owner_ != _msgSender()) {
            revert LazyMintAdventureBase__CallerNotTokenOwner();
        }
    }
    ...
    function _validateMaxSimultaneousQuests(uint256 maxSimultaneousQuests_) internal pure {
        if(maxSimultaneousQuests_ == 0) {
            revert LazyMintAdventureBase__MaxSimultaneousQuestsCannotBeZero();
        }

        if(maxSimultaneousQuests_ > MAX_CONCURRENT_QUESTS) {
            revert LazyMintAdventureBase__MaxSimultaneousQuestsExceeded();
        }
    }
    ...
    function _doBurn(address owner_, uint256 tokenId) internal virtual;

    function _doTransfer(address owner_, TokenOwner storage tokenOwner_, address from, address to, uint256 tokenId) internal virtual;

    function _doSafeTransfer(address owner_, TokenOwner storage tokenOwner_, address from, address to, uint256 tokenId, bytes memory data) internal virtual;

    function _ownerOfToken(uint256 tokenId) internal view virtual returns (address, TokenOwner storage);
}

abstract contract LazyMintAdventureERC721 is 
    LazyMintAdventureBase, 
    LazyMintERC721 {
    ...
    function supportsInterface(bytes4 interfaceId) public view virtual override(LazyMintERC721Base, IERC165) returns (bool) {
        return 
    ...
    function _doBurn(address owner_, uint256 tokenId) internal virtual override {
        _burn(owner_, tokenId);
    }

    function _doTransfer(address owner_, TokenOwner storage tokenOwner_, address from, address to, uint256 tokenId) internal virtual override {
        _transfer(owner_, tokenOwner_, from, to, tokenId);
    }

    function _doSafeTransfer(address owner_, TokenOwner storage tokenOwner_, address from, address to, uint256 tokenId, bytes memory data) internal virtual override {
        _safeTransfer(owner_, tokenOwner_, from, to, tokenId, data);
    }

    function _ownerOfToken(uint256 tokenId) internal view virtual override returns (address, TokenOwner storage) {
    ...
    function _beforeTokenTransfer(address /*from*/, address /*to*/, uint256 firstTokenId, uint256 batchSize) internal virtual override {
        for (uint256 i = 0; i < batchSize;) {
            if(blockingQuestCounts[firstTokenId + i] > 0) {
                revert LazyMintAdventureBase__AnActiveQuestIsPreventingTransfers();
            }
...
abstract contract LazyMintAdventureERC721Initializable is 
    LazyMintAdventureBase, 
    LazyMintERC721Initializable {
