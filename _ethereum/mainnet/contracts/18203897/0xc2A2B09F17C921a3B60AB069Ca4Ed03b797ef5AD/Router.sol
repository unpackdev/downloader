...
contract Router is Initializable, OwnableUpgradeable, ReentrancyGuardUpgradeable {
    ...
    /// @dev srcpoolid => chainId => maximum transfer amount
    mapping(uint256 => mapping(uint16 => uint256)) public maxTransferLimits;
    ...
    function setMaxTransferLimit(uint256 poolId_, uint16 dstChainId_, uint256 maxTransferLimit_)
        external
        onlyOwner
        nonReentrant
    {
        ...
        maxTransferLimits[poolId_][dstChainId_] = maxTransferLimit_;
    }
    ...
    function transferETH(
        ...
        require(maxTransferLimits[nativeTokenPoolId][dstChainId_] >= amount_, "Router: exceeding the maximum limit");
    ...
    function transferToken(
        ...
        amount_ = (amount_ / convertRate) * convertRate;
        require(amount_ > 0, "Router: amount too small");
        require(maxTransferLimits[srcPoolId_][dstChainId_] >= amount_, "Router: exceeding the maximum limit");
