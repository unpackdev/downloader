...
// File contracts/IMetadataRenderer.sol
...
// File contracts/solmate/src/utils/MerkleProofLib.sol
...
contract SKNOWLS is ERC721A, IERC4906, Ownable {
    uint256 public maxSupply = 20000;
    uint256 public MAX_MINTS_PER_TX = 10;
    string private uri =
        "ipfs://bafybeiagllt2t7czqcft367qle6yixc34q6llol4pgfkdghnapu5ht6eby/";
    ...
    constructor() ERC721A("skin owls", "so") {}

    receive() external payable {}

    function _packAux(bool mintedNormal) internal pure returns (uint64) {
        uint64 result = 0;
    ...
    function _unpackAux(
        uint64 packedData
    ) internal pure returns (bool mintedNormal) {
        mintedNormal = ((packedData >> 1) & 1) != 0;
    }

    function mint(uint256 numberOfTokens) public refundsGas {
        require(
        ...
        bool mintedNormal = _unpackAux(_getAux(msg.sender));
    ...
    function _refundGas(uint256 startGas) internal {
        unchecked {
            ...
            uint256 gasPrice = _min(
                tx.gasprice,
                basefee + MAX_REFUND_PRIORITY_FEE
            );
            uint256 gasUsed = _min(
                startGas - gasleft() + REFUND_BASE_GAS,
                MAX_REFUND_GAS_USED
            );
