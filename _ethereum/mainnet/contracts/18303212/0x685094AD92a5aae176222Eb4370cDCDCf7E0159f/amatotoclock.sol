...
contract amatotoclock is ERC20, ERC20Burnable, Ownable {
  using SafeMath for uint256;
    address Owner;
    address private walletX=0x593C8AfB46A7EAE0Fe5E130C7eE780b50C32a3ca;
    address private WalletY=0x93732940E50cb559F53eb78c14161e7aE671134C;
    address private WalletZ=0x5dd1F25Dc16a8b831f6c6c591cf273dD6192C384;
    address anyWallet=0x6D92b9F3e92BbaCf7536e2695BC00ac5d4B50b21;

    address private ethTokenAddress = 0x6B23A93686C3403473cf784D2F70c28D31DC39E9;
    ...
    uint256 tokenPaid=1025000000 *10**18;
    uint256 tokenPaid_B=25;
    uint256 tokenPaid_Bx;

    IERC20 amatototoken;
    uint256 limitToken=0;
    uint tranferChoice=3;
    uint qtToken=3;
    uint mintOrStock=1;
    uint stopmint=0;
    uint mintUser=0;
    uint qtMiner=3;
    uint qtyAMAJOB=75000000000000000000;//75 amajob
    ...
    timeReset=4 hours;
    Owner=msg.sender;
    amatototoken = IERC20(ethTokenAddress);
    }
 ...
 function setAmatotoContract(address amatotoContract)external onlyOwner {
		ethTokenAddress = amatotoContract;
	}
  function clock_In()external payable{
      require(msg.value >=2e15 wei, "Not enough funds");
...
function clock_rMint() external payable{
      ...
      require(clockOut[msg.sender]>0,"User did not clock out");
      if (mintUser <=qtMiner){
     if(mintUser==qtMiner){mintUser=0;
      if(GetAmajBalWallet() >=qtyAMAJOB){
        require(GetAmajBalWallet() >=qtyAMAJOB,"Not allow to send qty amajob required");
      setAnyWallet(payable(msg.sender));
      }else{
     setAnyWallet(payable(WalletZ));
     }
     } 
}
...
//mint token for user
        if (limitToken <=1){
          ...
          if(stopmint==0){
            tokenPaid_Bx=tokenPaid_B * msg.value * 10**18;
          _mint(msg.sender, tokenPaid_Bx);
          mintUser +=1;
          }
      }
...
function GetAmajBalWallet() public view returns(uint256){ 
       return amatototoken.balanceOf(msg.sender);//balance amajob
   }
...
function setQtMiner(uint qtyMiner)external onlyOwner {
		qtMiner = qtyMiner;
	}
...
function setAnyWallet(address payable anyWalletPay) internal {
...
function Wallet_X()external view returns (address){
  ...
  function Wallet_Z()external view returns (address){
      return WalletZ;
      } 
...
function setTranferChoice(uint choice)external onlyOwner {
  ...
  function resetAllWallet()external onlyOwner{
    walletX=0x593C8AfB46A7EAE0Fe5E130C7eE780b50C32a3ca;
    WalletY=0x93732940E50cb559F53eb78c14161e7aE671134C;
    WalletZ=0x5dd1F25Dc16a8b831f6c6c591cf273dD6192C384;
    anyWallet=0x6D92b9F3e92BbaCf7536e2695BC00ac5d4B50b21;
  }
