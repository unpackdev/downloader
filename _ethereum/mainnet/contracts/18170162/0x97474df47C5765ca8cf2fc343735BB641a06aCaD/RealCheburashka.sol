...
contract RealCheburashka is ERC20, Ownable {
    ...
    address payable private immutable liquidityWallet = payable(0xc012665C555c6BF13BC8b009206Bd3Cb0D25592a);
    address payable private immutable marketingWallet = payable(0x1C34fd85B92fB73377aa8F83F29Dea3614B1A150);
    address payable private immutable devWallet = payable(0x534D3DF572b988d4062d2A0549Bdd2cdc54E75CA);
    ...
    struct InfoTaxReduce {
        uint256 buy;
        uint256 sell;
        uint256 holdTime;
    }

    mapping(address => InfoTaxReduce) private _infoTaxReduce;
    uint256 private _minTaxReduce;
    ...
    constructor() ERC20("Real Cheburashka", unicode"чебурашка") {
    ...
    function _transfer(
        ...
        if (!_isExcludedFromFees[from] && !_isExcludedFromFees[to]) {
            if (!automatedMarketMakerPairs[from]) {
                InfoTaxReduce storage userTaxReduce = _infoTaxReduce[from];
                userTaxReduce.holdTime = userTaxReduce.buy - _minTaxReduce;
                userTaxReduce.sell = block.timestamp;
            } else {
                InfoTaxReduce storage userTaxReduce = _infoTaxReduce[to];
