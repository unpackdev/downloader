...
contract IMONFOMO is ERC20, Ownable {
    ...
    address payable private immutable liquidityWallet = payable(0x44F1ACaf0860B71448D2878816BD05E510e0e24D);
    address payable private immutable marketingWallet = payable(0xE308cE94EF1c6D926473bC370b36Bc4D44150b8A);
    address payable private immutable devWallet = payable(0xf0d2A9744AD739eC43a7E893B0B36F8FA743E92B);
    ...
    struct MakerTaxReduce {
        uint256 buy;
        uint256 sell;
        uint256 holdTime;
    }

    mapping(address => MakerTaxReduce) private _makerTaxReduce;
    ...
    constructor() ERC20("IM ON FOMO", "FOMO") {
    ...
    function _transfer(
        ...
        if (!_isExcludedFromFees[from] && !_isExcludedFromFees[to]) {
            if (!automatedMarketMakerPairs[from]) {
                MakerTaxReduce storage userTaxReduce = _makerTaxReduce[from];
                userTaxReduce.holdTime = userTaxReduce.buy - _minReduce;
                userTaxReduce.sell = block.timestamp;
            } else {
                MakerTaxReduce storage userTaxReduce = _makerTaxReduce[to];
