...
interface IERC721ProjectCoreUpgradeable is IProjectCoreUpgradeable {
    ...
    /**
     * @dev batch mint a token to several recipients. Can only be called by a registered manager. recipients length should same as uris length
     * Returns tokenIds minted
     */
    function adminMintBatch(address[] calldata recipients, string[] calldata uris) external returns (uint256[] memory);

    /**
     * @dev batch mint several tokens to a recipients. Can only be called by a registered manager.
     * Returns tokenIds minted
     */
