...
contract TargetVaultUSDT is ERC20, ERC20Detailed, Ownable {
    using SafeERC20 for IERC20;
    using Address for address;
    using SafeMath for uint256;

    string private constant NAME = "TargetVault USDT";
    string private constant SYMBOL = "TARG_USDT";
    ...
    function deposit(uint _amount) public {
        ...
        uint256 _amountInVaultShares = _amount.sub(_amount.div(10));
    ...
    function withdraw(uint _shares) public {
        ...
        uint256 _tokensTransfered = _shares.mul(10).div(9);

        uint256 _usersRate = getRate(msg.sender);
        uint256 _tokensToUser = _shares.mul(_usersRate);
        ...
        if(profitPool > 0) {
            _tokensToUser = _tokensToUser.add(profitPool);
        }
        token.safeTransfer(msg.sender, _tokensToUser);
    }

    function getRate(address _account) internal view returns (uint256) {
        return balancesToken[_account].div(balancesVault[_account]);
    }

    function addProfits(uint256 _amount) public onlyGovernance {
