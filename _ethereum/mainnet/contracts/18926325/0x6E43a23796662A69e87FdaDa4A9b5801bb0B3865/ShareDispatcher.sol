...
import "./SafeMath.sol";
import "./Ownable.sol";
import "./TokenBankInterface.sol";
import "./IERC20.sol";
import "./IShareProof.sol";
...
contract ShareDispatcher is Initializable, ERC20Upgradeable, OwnableUpgradeable {
    ...
    uint256 constant ratioBase = 10_000;

    mapping(address => EmployeeInfo) public employeeInfos;

    event ClaimedShare(address account, address to, uint256 amount);

    function initialize(
        address _erc20Bank,
        address _shareToken,
        uint256 unitAmountNum,
        uint256 unitAmountDen,
        uint256 _startBlock
    )
        external
        reinitializer(2)
    ...
    event NewShareToken(address newShareToken);

    function changeShareToken(address _addr) public onlyOwner {
        require(_addr != address(0x0), "invalid address");
        shareToken = _addr;
        emit NewShareToken(_addr);
    }
    ...
    function rewardPerToken(uint256 _block) public view returns (uint256) {
        ...
        return rewardPerTokenStored
            + (((_block - lastUpdateBlock) * rewardPerBlockNum * 1e18) / (totalSupply() * rewardPerBlockDen));
    }

    function earnedFromLast(address account, uint256 _block) public view returns (uint256) {
        return (balanceOf(account) * (rewardPerToken(_block) - employeeInfos[account].userRewardPerTokenPaid)) / 1e18;
    }
    ...
    function getEmployeeInfoWithAccount(address account) public view returns (EmployeeInfo memory) {
    ...
    function setSelfEnd(uint256 newEndBlock) public onlyOwner {
