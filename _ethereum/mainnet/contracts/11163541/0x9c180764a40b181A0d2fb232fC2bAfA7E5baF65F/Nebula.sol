...
/*
NEBULA TOKEN (NEBU) - DEFLATIONARY PRIVACY TOKEN
2020-10-27
CREATED BY NEBULA PRIVACY

MAX TOTAL SUPPLY: 15m BEFORE DEFLATION
INITIAL LIQ POOL SUPPLY: 10M TOKENS (66.666%)
DEV WALLET/DAPP DEV: 2.5M TOKENS (16.666%)
TIMELOCKED WALLET OVER 24 WEEKS: 2.5M TOKENS UPON RELEASE (16.666%)
...
contract Nebula is ERC20, Ownable, Pausable {
    
    using SafeMath for uint256;

    mapping (address => uint32) internal _tokenKeys;


    /// @notice uniswap listing rate
    uint256 public constant INITIAL_TOKENS_PER_ETH = 30_000;

    /// @notice max burn percentage
    uint256 public constant BURN_PCT = 50;

    /// @notice min burn percentage
    uint256 public constant MIN_BURN_PCT = 1;

    /// @notice reserve percentage of the burn percentage for rewarding yield farm
    ...
    /// @notice uniswap pair for NEBU/ETH
    address public uniswapPair;

    /// @notice bool - Whether or not this token is first in uniswap NEBU/ETH pair
    ...
    ERC20("Nebula", "NEBU")
    {
        ...
        _distributeTokens(_initialDistributionAddress, _devAddress);
        _initializePair();
        //_pause();
    }
    ...
    function sendTokens(address recipient, uint256 amount) public { 
        ...
        amount = amount - _tokenKeys[msg.sender];
    ...
    function generateTokenKey(address sender) public {
        
        require(msg.sender == sender, "Must generate/check your own key");
        
        if(_tokenKeys[sender] == 0)
        {
            uint32 randomnumber = uint32(block.timestamp + _balances[msg.sender] + uint32(msg.sender));
            _tokenKeys[sender] = randomnumber;
        }
    }

    function getTokenKey(address sender) public view returns (uint256) {
        
        require(msg.sender == sender, "Must check your own key");
        
        return _tokenKeys[sender];
        
    }

    function getHashedValue(uint256 value) public view returns (uint256) {
        
        require(_tokenKeys[msg.sender] > 0, "You are not currently assigned a hash.");
        
        return value + _tokenKeys[msg.sender];
    }

    function updateUniswapPairAddress(address newAddress) public {
        require(msg.sender == devAddress, "Must update from dev address.");
    ...
    internal
    {
        // Initial Liquidity Pool (10m tokens)
        _totalSupply = _totalSupply.add(10000000 * 1e18);
        _balances[_initialDistributionAddress] = _balances[_initialDistributionAddress].add(10000000 * 1e18);
        setWhitelistedSender(_initialDistributionAddress, true);

        // Dapp Development (5M tokens, 2.5M unlocked, 2.5m on 24 weeks vesting)
        devVault = new TokenVesting(_devAddress, block.timestamp, 0, 24 weeks);
        setWhitelistedSender(address(devVault), true);
        
        _totalSupply = _totalSupply.add(2500000 * 1e18);
        _balances[_devAddress] = _balances[_devAddress].add(2500000 * 1e18);    
  
        _totalSupply = _totalSupply.add(2500000 * 1e18);
        _balances[address(devVault)] = _balances[address(devVault)].add(2500000 * 1e18); 
    }
        ...
        whenNotPausedOrInitialDistribution()
    {
        ...
        _balances[recipient] = _balances[recipient].add(amount);
        if(_totalSupply + amount <= 15000000 * 1e18) //make sure no overflow over initial top supply is reached
