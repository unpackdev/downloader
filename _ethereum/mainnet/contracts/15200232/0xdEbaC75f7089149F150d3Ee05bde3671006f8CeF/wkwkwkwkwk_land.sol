...
contract wkwkwkwkwk_land is ERC721A, Ownable {
    ...
    function mint(uint256 qty) public payable {
        ...
        if (totalSupply() >= MAX_FREE_SUPPLY) {
            require(totalSupply() + qty <= MAX_SUPPLY, "sold out");
            require(qty > 0, "no");
            require(qty <= MAX_QTY, "3");
            require(msg.value >= COST * qty, "insufficient");
            _mint(msg.sender, qty);
        } else {
            require(balanceOf(msg.sender) < 1, "don't be greedy");
            require(qty == 1, "1");
            _mint(msg.sender, qty);
        }
    }
