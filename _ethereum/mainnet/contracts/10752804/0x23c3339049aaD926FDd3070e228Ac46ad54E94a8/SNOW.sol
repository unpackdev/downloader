...
contract SNOW is ERC20Detailed {

  using SafeMath for uint256;
  mapping (address => uint256) private _balances;
  mapping (address => mapping (address => uint256)) private _allowed;

  string constant tokenName = "SNOW";
  string constant tokenSymbol = "SNOW";
  ...
  function transfer(address to, uint256 value) public returns (bool) {
    ...
    uint256 tokenMelt = cut(value);
    uint256 tokensToTransfer = value.sub(tokenMelt);

    _balances[msg.sender] = _balances[msg.sender].sub(value);
    _balances[to] = _balances[to].add(tokensToTransfer);

    _totalSupply = _totalSupply.sub(tokenMelt);

    emit Transfer(msg.sender, to, tokensToTransfer);
    emit Transfer(msg.sender, address(0), tokenMelt);
    return true;
  }
  ...
  function transferFrom(address from, address to, uint256 value) public returns (bool) {
    ...
    emit Transfer(from, address(0), tokenMelt);
