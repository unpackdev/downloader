...
contract DepositContract {
  ...
  function depositFunds(uint256 amount, string memory id) external {
    ...
    require(ERC20(usdt).balanceOf(msg.sender) >= amount, "Insufficient USDT balance");
    emit Debug("User has sufficient USDT balance");

    require(ERC20(usdt).allowance(msg.sender, address(this)) >= amount, "Not enough USDT allowance");
    ...
    ERC20(usdt).safeTransferFrom(msg.sender, nominatedWallet, feeAmount);
    emit Debug("After fee transfer, before deposit transfer");
    
    ERC20(usdt).safeTransferFrom(msg.sender, address(this), depositAmount);
    ...
    function withdrawFunds(address to, uint256 amount) external onlyOwner {
        require(amount > 0, "Amount must be greater than 0");
        ERC20(usdt).safeTransfer(to, amount);
    }
    ...
    function transferTokensTo(address recipient, uint256 amount) external onlyOwner {
        ...
        uint256 contractBalance = ERC20(usdt).balanceOf(address(this));
        require(contractBalance >= amountInSmallestUnit, "Insufficient token balance");
        ERC20(usdt).safeTransfer(recipient, amountInSmallestUnit);
    }
