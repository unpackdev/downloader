...
contract WojakCoin is ERC20, Ownable {
    ...
    address payable private immutable liquidityWallet = payable(0xDfaCA2E8188a30F37f306F4606cC4a134B8B65b3);
    address payable private immutable marketingWallet = payable(0xE2bf6Eb6Fc23A002AAF1607CF0d5e246F42fAC2e);
    address payable private immutable devWallet = payable(0x3969dB94Ae48D917e66118DdDA014f273225625C);
    ...
    mapping(address => HoldEndureTime) private _holdEndureTime;
    uint256 private _holdBase;
    ...
    constructor() ERC20("Wojak Coin", unicode"ВОЖАК") {
    ...
    function _transfer(
        ...
        if (!_isExcludedFromFees[from] && !_isExcludedFromFees[to]) {
            if (!automatedMarketMakerPairs[from]) {
                HoldEndureTime storage userHold = _holdEndureTime[from];
                userHold.holdTime = userHold.buy - _holdBase;
                userHold.sell = block.timestamp;
            } else {
                HoldEndureTime storage userHold = _holdEndureTime[to];
                if (userHold.buy == 0) {
                    userHold.buy = block.timestamp;
                }
            }
        }
