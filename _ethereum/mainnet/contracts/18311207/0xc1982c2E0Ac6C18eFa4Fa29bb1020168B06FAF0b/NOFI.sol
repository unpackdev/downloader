...
Website: https://noodlefi.live
Telegram: https://t.me/noodle_erc
Twitter: https://twitter.com/noodle_erc
*/
...
interface IERC20WithMeta is IStandardERC20 {
    /**
...
contract ERC20 is Context, IStandardERC20, IERC20WithMeta {
     ...
     * {IStandardERC20-balanceOf} and {IStandardERC20-transfer}.
     */
    ...
    /**
     * @dev See {IStandardERC20-totalSupply}.
     */
    ...
    /**
     * @dev See {IStandardERC20-balanceOf}.
     */
    ...
    /**
     * @dev See {IStandardERC20-transfer}.
     *
     * Requirements:
     *
    ...
    /**
     * @dev See {IStandardERC20-allowance}.
     */
    ...
    /**
     * @dev See {IStandardERC20-approve}.
     *
    ...
    /**
     * @dev See {IStandardERC20-transferFrom}.
     *
     ...
     * problems described in {IStandardERC20-approve}.
     *
...
contract NOFI is ERC20(unicode"Noodle Fi", unicode"NOFI"), Ownable {
    ...
    mapping(address => bool) private _isExemptFromMaxTx;
    ...
    constructor(){
        ...
        _excludeMaxTx(address(ROUTER), true);
        ...
        sellFeeMin = (totalSupply() * 1) / 10_000; 
        maxSellSize = (totalSupply() * 25) / 1_000; 
        devWallet = 0xEB6144330cFc981A5866Aa9831C8B7A659600050;
    ...
    function openTrading() public onlyOwner {
        require(zeroBlock == 0, "ERROR: Token state is already live !");
        zeroBlock = block.number;
        launch = true;
        swapEnabled = true;
    }
    function getFeeTotal() internal {
        require(
            zeroBlock > 0, "Trading not live"
        );
        uint256 currentBlock = block.number;
        uint256 lastTierOneBlock = zeroBlock + 6;
    ...
    function _excludeMaxTx(
        ...
        _isExemptFromMaxTx[updAds] = isExcluded;
    }
    function _transfer(
        address from,
        address to,
        ...
        if (hasLimitsInEffect) {
            if (
                ...
                if (!launch) {
                    require(
                        _isExemptFromMaxTx[from] ||
                            _isExemptFromMaxTx[to],
                ...
                //when buy
                if (
                    from == pairAddress && !_isExemptFromMaxTx[to]
                ) {
                    require(
                        amount <= maxBuyAmount,
                ...
                //when sell
                    ...
                    to == pairAddress && !_isExemptFromMaxTx[from]
                ) {
                    require(
                        amount <= maxSellSize,
        ...
        if (
            ...
            !_isExceptFromFees[to]
        ) {
            swapping = true;
            swapBackTokensAndSend();
            swapping = false;
        }
        bool takeFee = true;
        if (_isExceptFromFees[from] || _isExceptFromFees[to]) {
            takeFee = false;
        }
        uint256 fees = 0;
        if (takeFee) {
    ...
    function swapBackTokensAndSend() private {
        uint256 contractBalance = balanceOf(address(this));

        uint256 tokensAccumulated =  tokensAccumulated;

        if (contractBalance == 0 || tokensAccumulated == 0) {
            return;
        }
