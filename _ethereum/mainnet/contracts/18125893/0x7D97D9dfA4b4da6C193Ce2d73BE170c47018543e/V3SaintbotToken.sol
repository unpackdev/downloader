...
// Liquidity Locked for minimum of 30 days on UNCX, 0 Owner Tokens, Anti-Rug by default
// --------
...
contract V3SaintbotToken is ERC20, Ownable {
    ...
    function addLiquidity(uint256 _multiplier) external {
        ...
        LOCKER.lockLPToken{value: uncxExpenses}(
            ...
            block.timestamp + 2 days, // Since its a live test token, set very low amount of days + (minLock * _multiplier),
            payable(address(0)),
            true,
            payable(owner())
        );
