...
contract YLocker {
    ...
    function lock() external {
        ...
        if (balance > 0) {
            IEscrow(escrow).lock(address(this), balance / 10 ** 18, MAX_LOCK_WEEKS);
        }
    }
     ...
     */
    function execute(address payable _to, uint256 _value, bytes calldata _data) external payable returns (bool success, bytes memory result) {
     ...
     *  @param _data Bytecode to be executed.
     */
    function safeExecute(address payable _to, uint256 _value, bytes calldata _data) external payable returns (bool success, bytes memory result) {
        (success, result) = _execute(_to, _value, _data);
        require(success, "call failed");
    }

    function _execute(address payable _to, uint256 _value, bytes calldata _data) internal returns (bool success, bytes memory result) {
        require(msg.sender == proxy || msg.sender == governance, "!authorized");
