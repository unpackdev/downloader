...
contract Zenithra is ERC20, Ownable {
    ...
    function _distributeBNB(uint256 remainingBNB) private {
        ...
        uint256 amountBNBinsurance = remainingBNB * (insurancefee) / (totalFee);

        if(amountBNBinsurance > 0){
        (bool insuranceSuccess, /* bytes memory data */) = payable(insuranceReciever).call{value: amountBNBinsurance, gas: 30000}("");
