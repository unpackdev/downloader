...
contract DMXHero is ERC721Upgradeable, OwnableUpgradeable , DefaultOperatorFiltererUpgradeable {
    ...
    function setTurnInAddress(address TurnInAddress) public onlyOwner {
        turnInAddress = TurnInAddress;
    }
    ...
    function turnIn(uint32[] memory heroIds, uint32 heroId) public {
        require(heroIds.length <= 10, "Too many heroes being turned in");
        for (uint i=0; i<heroIds.length; i++) {
          require(msg.sender == ownerOf(heroIds[i]), "You must own the heroes");
        }

        require(msg.sender == ownerOf(heroId), "You must own the hero");

        for (uint i=0; i<heroIds.length; i++) {
            uint32 tokenId = heroIds[i];
            _transfer(msg.sender, turnInAddress, tokenId);
