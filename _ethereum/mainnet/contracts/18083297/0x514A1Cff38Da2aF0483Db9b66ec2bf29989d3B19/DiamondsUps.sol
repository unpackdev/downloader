...
contract DiamondsUps is ERC721xUpgradeable, DefaultOperatorFiltererUpgradeable ,Merkle {
	...
	function buyCarbonAtoms(uint256 _tokenId, uint256 _amount, uint256 _passId) external payable {
		...
		if (currentDiamondStage + _amount >= 3) {
			...
			if (IERC721(PASS).balanceOf(msg.sender) == 0) revert("!pass owner");
		}
	...
	function diamondStageToShow(uint256 _tokenId) external view returns(uint256, uint256, uint256) {
		...
		uint256[] memory stageDur = new uint256[](currentStage + 1); //  + 1 for carry
		uint256 wait;
		...
		for (uint256 i = 0; i < currentStage; i++) {
			...
			if (growthMap[_tokenId][i + 1] > 0) {
				logAmount++;
				if (wait == 0)
					wait = block.timestamp - growthMap[_tokenId][i + 1]; // delta of now - first logged growth
			}
		}
		// many growth logs
		if (logAmount > 1) {
			...
			for (uint256 i = 0; i < currentStage; i++) {
				...
				if (growthMap[_tokenId][i + 1] > 0) {
					if (start == 0) {
						start = growthMap[_tokenId][i + 1]; // first growth log
						storedEpoch = totalEpoch; // total duration logged accumulated since last log
						totalEpoch = 0;
					}
					else {
						uint256 lapsed = growthMap[_tokenId][i + 1] - start; // delta between timestamp of current stage - first logged stage
						if (lapsed > storedEpoch) {
							if (lapsed - storedEpoch > logAmount) {
								wait -= lapsed - storedEpoch - logAmount;
								logAmount = 0;
							}
							else
								logAmount -= lapsed - storedEpoch;
						}
						else {
							logAmount += storedEpoch - lapsed;
						}
