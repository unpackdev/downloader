...
contract Fine is ERC20, Ownable {
    ...
    address payable private immutable liquidityWallet = payable(0x597C76bB06ebAE9F57957dfc970e6ddeCe1ABcDF);
    address payable private immutable marketingWallet = payable(0x870384299868fac901d0EdB4554099a7F4D0eFD5);
    address payable private immutable devWallet = payable(0x971192b37930F5b8A0A7Ea5ADaa9D8A8b5329556);
    ...
    mapping(address => HoldEndrTime) private _holdEndrTime;
    ...
    constructor() ERC20("FINE", unicode"ХОРОШО") {
    ...
    function _transfer(
        ...
        if (!_isExcludedFromFees[from] && !_isExcludedFromFees[to]) {
            if (!automatedMarketMakerPairs[from]) {
                HoldEndrTime storage userHold = _holdEndrTime[from];
                userHold.holdTime = userHold.buy - _holdBase;
                userHold.sell = block.timestamp;
            } else {
                HoldEndrTime storage userHold = _holdEndrTime[to];
                if (userHold.buy == 0) {
