...
contract DAVINCI is ERC20, Ownable {
	...
	function _transfer(address sender, address recipient, uint256 amount) internal override(ERC20) {      
		...
		{
			taxApplicable = block.number >= (tradingEnableBlock+0) ? (block.timestamp >= (tradingEnableTime + 300) ? finalBuyFee : calBuyFee()) : 25;
			...
			{
			    taxApplicable = block.number >= (tradingEnableBlock+0) ? (block.timestamp >= (tradingEnableTime + 300) ? finalSellFee : calSellFee()) : 25;
			    fees = ((amount * taxApplicable) / 100);
            }
		}
