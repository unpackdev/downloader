...
contract YieldilizerLiquidityFarm is Ownable, ReentrancyGuard {
    ...
    struct PoolInfo {
        ...
        uint256 allocPoint;       // How many allocation points assigned to this pool. ldrs to distribute per block.
        uint256 duration;
        uint256 startBlock;
        uint256 bonusEndBlock;
        uint256 lastRewardBlock;  // Last block number that ldrs distribution occurs.
        uint256 accTokenPerShare;   // Accumulated ldrs per share, times 1e12. See below.
        uint256 accReflectionPerShare;   // Accumulated ldrs per share, times 1e12. See below.
    ...
    // ldr tokens created per block.
    uint256 public rewardPerBlock;
    // Bonus muliplier for early ldr makers.
    uint256 public constant BONUS_MULTIPLIER = 1;
    // Deposit Fee address
    address public feeAddress;
    address public treasuryWallet = 0x63ED01F0dfe1cbD3Eb2631E9BE998B40a211BA54;
    ...
    // The block number when ldr mining starts.
    ...
    constructor(IERC20 _ldrs, address _reflectionToken, uint256 _rewardPerBlock, bool _hasDividend) {
        ldrs = _ldrs;
    ...
    // Update the given pool's ldrs allocation point and deposit fee. Can only be called by the owner.
    ...
    // View function to see pending ldrs on frontend.
    function pendingRewards(uint256 _pid, address _user) external view returns (uint256) {
        ...
        if (block.number > pool.lastRewardBlock && lpSupply != 0) {
            ...
            uint256 ldrsReward = multiplier.mul(rewardPerBlock).mul(pool.allocPoint).div(totalAllocPoint);
            accTokenPerShare = accTokenPerShare.add(ldrsReward.mul(1e12).div(lpSupply));
        }
    ...
    function updatePool(uint256 _pid) public {
        ...
        uint256 multiplier = getMultiplier(pool.lastRewardBlock, block.number, pool.bonusEndBlock);
        uint256 ldrsReward = multiplier.mul(rewardPerBlock).mul(pool.allocPoint).div(totalAllocPoint);
        pool.accTokenPerShare = pool.accTokenPerShare.add(ldrsReward.mul(1e12).div(lpSupply));
    ...
    function compoundReward(uint256 _pid) external payable nonReentrant {
        ...
        if(address(ldrs) != address(pool.lpToken)) {
            uint256 tokenAmt = pending / 2;
            uint256 tokenAmt0 = tokenAmt;
            address token0 = address(ldrs);
    ...
    function _calculateTotalStaked(uint256 _pid, IERC20 _lpToken, uint256 _amount, bool _deposit) internal {
        if(_deposit) {
            ...
            if(address(_lpToken) == address(ldrs)) {
    ...
    function availableRewardTokens() public view returns (uint256) {
        if(address(ldrs) == reflectionToken) return totalEarned;

        uint256 _amount = ldrs.balanceOf(address(this));
    ...
    // Safe ldrs transfer function, just in case if rounding error causes pool to not have enough ldrs.
    function safeTokenTransfer(address _to, uint256 _amount) internal {
        uint256 ldrsBal = ldrs.balanceOf(address(this));
        bool transferSuccess = false;
        if (_amount > ldrsBal) {
            transferSuccess = ldrs.transfer(_to, ldrsBal);
        } else {
            transferSuccess = ldrs.transfer(_to, _amount);
        }
    ...
    //ldrs has to add hidden dummy pools inorder to alter the emission, here we make it simple and transparent to all.
    ...
    function depositRewards(uint _amount) external nonReentrant {
        ...
        uint256 beforeAmt = ldrs.balanceOf(address(this));
        ldrs.safeTransferFrom(msg.sender, address(this), _amount);
        uint256 afterAmt = ldrs.balanceOf(address(this));
    ...
    function recoverWrongToken(address _token) external onlyOwner {
        require(_token != address(ldrs) && _token != reflectionToken, "cannot recover reward token or reflection token");
