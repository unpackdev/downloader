...
contract NFITokenWrapper {
    ...
    // !! Mainnet NUX token address
    IERC20 public stakeToken = IERC20(0x55bC1a7cbca22CFD83B4395FDEcF97a40EDfAc0c);
...
contract NFINUXPool is NFITokenWrapper, IRewardDistributionRecipient {
    ...
    function stake(uint256 amount) public checkOpen checkStart updateReward(msg.sender){
        require(amount > 0, "NFI-NUX-POOL: Cannot stake 0");
        super.stake(amount);
        emit Staked(msg.sender, amount);
    }

    function withdraw(uint256 amount) public checkStart updateReward(msg.sender){
        require(amount > 0, "NFI-NUX-POOL: Cannot withdraw 0");
    ...
    modifier checkStart(){
        require(block.timestamp > startTime,"NFI-NUX-POOL: Not start");
        _;
    }

    modifier checkOpen() {
        require(open, "NFI-NUX-POOL: Pool is closed");
        _;
    }
