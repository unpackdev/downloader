...
contract PirateApes is OwnableUpgradeable, ERC721Upgradeable {
  ...
  {
    return
      tokenId <= supply && (tokenState[tokenId].locationId != LOCATION_HEAVEN);
  }

  function isTransferUnlocked(address from, address to)
    public
    view
    returns (bool)
  {
    return
      ...
      markets[_msgSender()] ||
      // some marketplaces might require transfers to their addresses for delegation
      // it can be also used to estimateGas (eg x2y2)
      markets[to];
  }
  ...
  function burnBatchOperator(address from, uint256[] calldata tokenIds)
    external
    onlyOperator
  {
    for (uint256 i = 0; i < tokenIds.length; i++) {
      ...
      tokenState[tokenId].locationId = LOCATION_HEAVEN;
    ...
    tokenOwnerState[from].balance -= uint16(tokenIds.length);

    burned += tokenIds.length;
  }
