    ...
    IValeria
{
    ...
    /// @notice The land upgrade price
    uint256 public upgradePrice;

    /// @notice The max supply of land upgrades
    uint256 public upgradeMaxSupply;
     ...
     *      Charge VAL fee, drop into sink wallet for potential burn, restrict to max upgrade supply.
     *      Whitelist merkle restricted on initial launch for 24hrs
     * @param tokenIds The token ids of the lands
     * @param owner The owner of the land token ids
     * @param allowance The current users allowance
     * @param proof The merkle proof for whitelist check if needed
     */
    function _upgradeLands(
        ...
        uint256 requiredPayment = upgradeAmount * upgradePrice;

        require(upgradedCount + upgradeAmount < upgradeMaxSupply, "!supply");

        // Check land balance restrictions if toggled on
        if (isUpgradeRestricted) {
             ...
             *    100+ = 150 Upgrades
             */
            require(ownerLandUpgrades[owner] + upgradeAmount <= allowance);
            require(
        ...
        // Upgrade owner land upgrade count
        ownerLandUpgrades[owner] += upgradeAmount;
    ...
    function upgradeLands(
        ...
        _upgradeLands(tokenIds, _msgSender(), _msgSender(), allowance, proof);
    }
    ...
    function delegateUpgradeLands(
        ...
        _upgradeLands(tokenIds, _msgSender(), vault, allowance, proof);
    }
    ...
    /**
     * @notice Sets the merkle root for whitelist
     * @param _whitelistMerkleRoot The merkle root to store
     */
    ...
    /**
     * @notice Sets the configuration for the land upgrades
     * @param _upgradePrice The price of the upgrade
     * @param _upgradeMaxSupply The max upgrade amount
     */
    function setUpgradeConfiguration(
        uint256 _upgradePrice,
        uint256 _upgradeMaxSupply
    ) external onlyOwner {
        upgradePrice = _upgradePrice;
        upgradeMaxSupply = _upgradeMaxSupply;
    }
}
