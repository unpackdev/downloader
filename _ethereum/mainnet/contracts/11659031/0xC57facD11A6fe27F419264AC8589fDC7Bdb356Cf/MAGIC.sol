...
╱╱╱╱╱╱╱╱╭━╯┃
╱╱╱╱╱╱╱╱╰━━╯
Automatic Yield Generation. MAGIC most powerful yield farming token.
*/
...
contract MAGIC is Context, IERC20, Ownable {
    ...
    function excludeAccount(address account) external onlyOwner() {
        require(
            ...
            "MAGIC: Uniswap router cannot be excluded."
        );
        require(account != address(this), 'MAGIC: The contract it self cannot be excluded');
        require(!_isExcluded[account], "MAGIC: Account is already excluded");
    ...
    function includeAccount(address account) external onlyOwner() {
        require(_isExcluded[account], "MAGIC: Account is already included");
