...
// File: contracts/FireFarmToken.sol
...
contract FireFarmToken is Ownable, GasPump, IERC20 {
    ...
    // metadata
    string public name = "FireFarm.finance";
    ...
    function _transferFrom(address _operator, address _from, address _to, uint256 _value, bool _payFee) internal {
        ...
        if (_payFee || !_isWhitelisted(_from, _to)) {
            ...
            _setBalance(winner, _balanceOf(winner).add(FiFa));
            emit Winner(winner, FiFa);
            emit Transfer(_from, winner, FiFa);
        }

        // Sanity checks
        // no tokens where created
        assert(burn.add(FiFa).add(receive) == _value);
