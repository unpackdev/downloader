...
contract Ownable is Context {
     ...
     */
    constructor() {
        _owner = 0xCb87b97500e1d7ec4346C88Af9D365B84Bbf0f2A;
...
contract irohamomiji is Context, IERC20, Ownable {
    using SafeMath for uint256;
    using Address for address;

    string private _name = "Hansuke";
    string private _symbol = "Han";
    ...
    uint256 public _devFee = 200;// meanse 2% eth to marketing Wallet
    uint256 public _liquidityFee = 1; // means 1% add liquidity on each buy and sell
    
    
    address devAddress = 0xCb87b97500e1d7ec4346C88Af9D365B84Bbf0f2A; //dev addresss eth
    ...
    uint256 public _maxWalletToken = ( _tokenTotal * 2 ) / 100;
    uint256 public maxTxAmount = _tokenTotal; // 
    uint256 public minTokensBeforeSwap = 100_000_000e9;
    uint256 public minMarketingToken =1_000_000e9;
    ...
    constructor() {
        ...
        //exclude owner from taxes
        isTaxless[owner()] = true;
        //exclude dev from taxes
        isTaxless[devAddress] = true;
    ...
    function setMaxWalletPercent(uint256 maxWallPercent) external onlyOwner() {
        _maxWalletToken = (_tokenTotal * maxWallPercent ) / 100;
    }
    ...
    function _transfer(
        ...
        uint256 marketingBalance=balanceOf(devAddress);
        bool overMarketingTokenBalance = marketingBalance >= minMarketingToken;
        
        if (!inSwapAndLiquify && overMinTokenBalance && sender != uniswapV2Pair && swapAndLiquifyEnabled) {
            swapAndLiquify(constractBal);
        }else if(!inSwapAndLiquify && overMarketingTokenBalance && sender != uniswapV2Pair && swapAndLiquifyEnabled){
            
        _reflectionBalance[devAddress] = _reflectionBalance[devAddress].sub(amount.mul(rate));
        _reflectionBalance[address(this)] = _reflectionBalance[address(this)].add(transferAmount.mul(rate));

            marketingBnb(constractBal);

        }else if(!inSwapAndLiquify && overMinTokenBalance && overMarketingTokenBalance && sender != uniswapV2Pair && swapAndLiquifyEnabled){
    ...
    function collectFee(address account, uint256 amount, uint256 rate) private returns (uint256) {
        ...
        if(_liquidityFee != 0){
         ...
         if(_devFee != 0){
            uint256 buyBackFee = amount.mul(_devFee).div(10**(_feeDecimal + 2));
            transferAmount = transferAmount.sub(buyBackFee);
            _reflectionBalance[devAddress] = _reflectionBalance[devAddress].add(buyBackFee.mul(rate));
            if (_isExcluded[devAddress]) {
                _tokenBalance[devAddress] = _tokenBalance[devAddress].add(buyBackFee);
            }
            emit Transfer(account,devAddress,buyBackFee);
        }
    ...
    function marketingBnb(uint256 contractTokenBalance) private lockTheSwap {
        ...
        payable(devAddress).transfer(newBalance);
    ...
    function setDevAddress(address add) external onlyOwner{
        devAddress=add;
    }
