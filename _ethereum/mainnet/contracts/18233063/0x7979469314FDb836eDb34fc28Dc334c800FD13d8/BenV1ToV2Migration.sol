...
contract BenV1ToV2Migration is Initializable {
  ...
  function initializeStep1(BenCoinV2 _benV2) external notInitializedStep1 {
    initializedStep = 1;
    uint oldBurntV1SupplyAsV2 = conversionRate(oldBurntV1Supply);
    if (address(_benV2) == address(0)) {
      revert CannotUseZeroAddress();
    }

    // Mint the bulk of the tokens to this address for conversion
    benV2 = _benV2;
    uint conversionTokens = maxV2Supply - oldBurntV1SupplyAsV2;
    _benV2.mint(address(this), conversionTokens);

    // Mint the previously burnt V1 tokens to the treasury
    _benV2.mint(treasury, oldBurntV1SupplyAsV2);
  }
