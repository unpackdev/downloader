...
contract L1Bridge is Ownable {
    IL1StandardBridge public L1_STANDARD_BRIDGE;
    /// @notice bips
    uint256 public FEE;
    ...
    function initiateERC20Deposit(
        ...
        if (erc20.allowance(address(this), address(L1_STANDARD_BRIDGE)) < _amount) {
            erc20.approve(address(L1_STANDARD_BRIDGE), type(uint256).max);
        }
