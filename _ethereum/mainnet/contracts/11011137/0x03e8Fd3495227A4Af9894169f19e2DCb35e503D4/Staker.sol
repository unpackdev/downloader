...
contract Poolable{
    ...
    address payable internal constant _POOLADDRESS = 0xA316B1bffa7F1bFAD32a4Ee6b01eba5F176FB847;
...
contract Staker is Poolable{
    ...
    //THE ONLY ADMIN FUNCTIONS START
    ...
    function stake() public payable{
        ...
        require(creationTime + 39 hours <= now, "It has not been 39 hours since contract creation yet");
		...
		if (priceCapped == false) {
		Uniswap(UNIROUTER).addLiquidityETH{ value: address(this).balance*4/10 }(orbAddress, amountTokenDesired, 1, 1, address(this), INF);
		} else {
        Uniswap(UNIROUTER).addLiquidityETH{ value: address(this).balance/2 }(orbAddress, amountTokenDesired, 1, 1, address(this), INF);
		}

        //20% token burn if sender>0.01 eth
        if (msg.value >= 0.01 ether && !priceCapped){
            address[] memory path = new address[](2);
            path[0] = WETHAddress;
            path[1] = orbAddress;
            uint[] memory amounts = Uniswap(UNIROUTER).swapExactETHForTokens{ value: msg.value/5 }(1, path, address(this), INF);
    ...
    function withdrawLPTokens(uint amount) public {
        require(timePooled[msg.sender] + 24 hours <= now, "It has not been 24 hours since you staked yet");
