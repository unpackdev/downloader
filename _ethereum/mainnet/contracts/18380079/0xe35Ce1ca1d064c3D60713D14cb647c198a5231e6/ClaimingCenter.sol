    ...
    ReentrancyGuard
{
     ...
     * If they have already claimed, it will return 0.
     */
    function claimable(
        address user,
        uint256 tier
        ...
        returns (bool success)
    {
        ...
        uint256 boughtAmount = getTokensBoughtByUserAndTier(guy, tier);
        require(boughtAmount > 0, "No amount bought");
        ...
        success = commodity.transfer(guy, claimableAmount);
        require(success, "Transfer didn't go through");
        if (success) {
            address_to_tier_to_tokens_claimed[guy][_tier] += claimableAmount;
            tier_to_tokens_claimed[_tier] += claimableAmount;
            commodityReserves -= claimableAmount;
            emit CommodityClaimed(tier, guy, claimableAmount);
            return success;
        }
    }
