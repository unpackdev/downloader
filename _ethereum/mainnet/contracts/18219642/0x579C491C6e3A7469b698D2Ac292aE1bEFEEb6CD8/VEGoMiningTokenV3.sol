...
contract VEGoMiningTokenV3 is IVEGoMiningToken, PausableUpgradeable, AccessControlUpgradeable, ReentrancyGuardUpgradeable, UUPSUpgradeable {
    ...
    function futureTotalSupply() external view returns (uint256[] memory) {
        uint256[] memory totalValues = new uint256[](1460);
        uint256 _epoch = epoch;
        Point memory lastPoint = pointHistory[_epoch];
        uint256 t = block.timestamp;
        for (uint256 i = 0; i < 1460; i++) {
            totalValues[i] = supplyAt(lastPoint, t);
            t += 1 days;
        }
