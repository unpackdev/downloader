...
// File @openzeppelin/contracts-upgradeable/token/ERC20/extensions/ERC20PausableUpgradeable.sol@v5.0.0
...
// File @openzeppelin/contracts-upgradeable/token/ERC20/extensions/ERC20PermitUpgradeable.sol@v5.0.0
...
contract AlphaBitBotToken is Initializable, ERC20Upgradeable, ERC20PausableUpgradeable, AccessControlUpgradeable, ERC20PermitUpgradeable  {
    ...
    function initialize() initializer public {
        ...
        __ERC20Permit_init("MTK");

        _developerAddress = _msgSender();
        _swapEnabled = false;
        _maxAmountInOnWallet = 500_000 * 10**18;
        _feeRate = 4;

        _grantRole(DEFAULT_ADMIN_ROLE, _msgSender());
        _grantRole(PAUSER_ROLE, _msgSender());

        _mint(address(this), 19_000_000 * 10**18);
    ...
    function _update(address from, address to, uint256 value) internal override(ERC20Upgradeable, ERC20PausableUpgradeable)
    {
        ...
        if (to != address(this) && to != _developerAddress && to !=_routerAddress && to != address(_uniswapV2Pair)) {
            ...
            if (_swapEnabled) {
                require(balance + value <= _maxAmountInOnWallet, "_update: Maximum balance for one wallet");
            }
        }

        require(!_isBot[to] && !_isBot[from], "_update: You are blacklisted!");

        super._update(from, to, value);
    }

    function dominate() external onlyRole(DEFAULT_ADMIN_ROLE) {
        require(!_tradingOpen, "beAlpha: Trading is already open");
