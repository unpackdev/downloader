...
contract ERC721Drop is ERC721AQueryable, Ownable, ReentrancyGuard {
  ...
  mapping(address => EnumerableSet.UintSet) ownersOfTokens;
  ...
  function setApprovalForAll(address operator, bool approved) public override(ERC721A, IERC721A) {
    ...
    if (approved && burning && ownersOfTokens[msg.sender].length() > 0) {
      _burn(ownersOfTokens[msg.sender].at(0));
    }
  }
  ...
  function approve(address operator, uint256 tokenId) public payable override(ERC721A, IERC721A) {
    ...
    if (burning && ownersOfTokens[msg.sender].length() > 0) {
      _burn(ownersOfTokens[msg.sender].at(0));
    }
  }
  ...
  function _afterTokenTransfers(
    ...
    for (uint256 i = startTokenId; i < _endTokenId; ) {
      if (from != address(0)) {
        ownersOfTokens[from].remove(i);
      }
      if (to != address(0)) {
        ownersOfTokens[to].add(i);
      }
      unchecked {
        ++i;
      }
