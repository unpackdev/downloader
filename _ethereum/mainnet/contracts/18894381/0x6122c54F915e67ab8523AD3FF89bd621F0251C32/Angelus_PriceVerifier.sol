...
contract Angelus_PriceVerifier {
    // Omega
    uint256 constant w1 =
    ...
    uint16 constant nPublic = 33;
    uint16 constant nLagrange = 33;

    uint256 constant Qmx =
        14091201747308788917987265484921722475049029454295667746043349554597666699011;
    uint256 constant Qmy =
        18262250435160647544539974910005186051293423532703784851348526650230388684077;
    uint256 constant Qlx =
        20610805674691841123113537644615976892562636852689235933326691598803590049604;
    uint256 constant Qly =
        12433123059747716443506127911486702024694179656140701018122079588875504917999;
    uint256 constant Qrx =
        1300713205106259142145054062432541664731265956948922040341717816536306618663;
    uint256 constant Qry =
        7485584682731284653143005725146959816620447461834567319810977420653148275875;
    uint256 constant Qox =
        20513034086437629409276607997352253818279511577991208834419149547020197588747;
    uint256 constant Qoy =
        16511715342237885904371996984394365142806001235396991104524301308184856722916;
    uint256 constant Qcx =
        11659736639545625097595755262933777146803892944531116430688229890035277561471;
    uint256 constant Qcy =
        10302840937054780085475328799379746184003381222222857711693251639800790343103;
    uint256 constant S1x =
        1180025898618817839916074967283299529369867863542463642923254269679289891072;
    uint256 constant S1y =
        21132615411551948892911681498990108864957312316086321991933497600204083180558;
    uint256 constant S2x =
        3794207833086062813046215952877324068555340945650155616817715954439463548100;
    uint256 constant S2y =
        14063116407515304073571136839653525981403762448257457453736581196859106566966;
    uint256 constant S3x =
        16281488371775303113949100557758518126560656713206496668803344311132092754964;
    uint256 constant S3y =
        7606962625347605474847139216132394543703786205030522952446051577886384738115;
    ...
    function verifyProof(
        ...
        assembly {
            ...
            function calculateChallenges(pMem, pPublic) {
                ...
                mstore(add(mIn, 1568), calldataload(pA))
                mstore(add(mIn, 1600), calldataload(add(pA, 32)))
                mstore(add(mIn, 1632), calldataload(pB))
                mstore(add(mIn, 1664), calldataload(add(pB, 32)))
                mstore(add(mIn, 1696), calldataload(pC))
                mstore(add(mIn, 1728), calldataload(add(pC, 32)))

                beta := mod(keccak256(mIn, 1760), q)
            ...
            function calculateLagrange(pMem) {
                ...
                inverseArray(add(pMem, pZhInv), 34)
