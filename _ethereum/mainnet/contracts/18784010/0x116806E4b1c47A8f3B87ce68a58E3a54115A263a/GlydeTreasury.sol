...
contract GlydeTreasury is Ownable {
    ...
    enum MANAGING { RESERVEDEPOSITOR, RESERVESPENDER, RESERVETOKEN, RESERVEMANAGER, LIQUIDITYDEPOSITOR, LIQUIDITYTOKEN, LIQUIDITYMANAGER, DEBTOR, REWARDMANAGER, Glyde }

    address public immutable Glyde;
    ...
    address public Gsky;
    uint public GskyQueue; // Delays change to Gsky address
    ...
    /**
        @notice allow approved address to deposit an asset for Glyde
        @param _amount uint
    ...
    function deposit( uint _amount, address _token, uint _profit ) external returns ( uint send_ ) {
        ...
        // mint Glyde needed and store amount of rewards for distribution
        send_ = value.sub( _profit );
        IERC20Mintable( Glyde ).mint( msg.sender, send_ );
    ...
    /**
        @notice allow approved address to burn Glyde for reserves
        @param _amount uint
        @param _token address
     */
    function withdraw( uint _amount, address _token ) external {
        ...
        uint value = valueOf( _token, _amount );
        IGlydeERC20( Glyde ).burnFrom( msg.sender, value );
    ...
    function incurDebt( uint _amount, address _token ) external {
        ...
        uint maximumDebt = IERC20( Gsky ).balanceOf( msg.sender ); // Can only borrow against Gsky held
    ...
    /**
        @notice allow approved address to repay borrowed reserves with Glyde
        @param _amount uint
     */
    function repayDebtWithGlyde( uint _amount ) external {
        require( isDebtor[ msg.sender ], "Not approved" );

        IGlydeERC20( Glyde ).burnFrom( msg.sender, _amount );

        debtorBalance[ msg.sender ] = debtorBalance[ msg.sender ].sub( _amount );
        totalDebt = totalDebt.sub( _amount );

        emit RepayDebt( msg.sender, Glyde, _amount, _amount );
    }
    ...
    function mintRewards( address _recipient, uint _amount ) external {
        ...
        IERC20Mintable( Glyde ).mint( _recipient, _amount );
    ...
    function excessReserves() public view returns ( uint ) {
        return totalReserves.sub( IERC20( Glyde ).totalSupply().sub( totalDebt ) );
    }
    ...
    /**
        @notice returns Glyde valuation of asset
        @param _token address
        @param _amount uint
        @return value_ uint
     */
    function valueOf( address _token, uint _amount ) public view returns ( uint value_ ) {
        if ( isReserveToken[ _token ] ) {
            // convert amount to match Glyde decimals
            value_ = _amount.mul( 10 ** IERC20( Glyde ).decimals() ).div( 10 ** IERC20( _token ).decimals() );
    ...
    function queue( MANAGING _managing, address _address ) external onlyManager() returns ( bool ) {
        ...
        } else if ( _managing == MANAGING.Glyde ) { // 9
            GskyQueue = block.number.add( blocksNeededForQueue );
        } else return false;
