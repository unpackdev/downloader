...
contract Ownable is OwnableData {
    ...
    function setSpectrolAddress(address _Spectrol) public {
    ...
    /// @notice Only allows the `owner` to execute the function.
    modifier onlySpectrol() {
        require(msg.sender == Spectrol, "Ownable: caller is not the Spectrol");
        _;
    }
...
contract taxDistributor is Ownable {
    ...
    function addLiquidity(address urv2, address dai, uint256 tokenAmount, uint256 daiAmount, address liquidityTo) private {
        ...
        IERC20(Spectrol).approve(urv2, tokenAmount);
    ...
    function distribute(address urv2, address dai, address marketingWallet, uint256 daiForBuyback, address buybackWallet, uint256 liquidityTokens, uint256 daiForLiquidity, address liquidityTo) external onlySpectrol{
