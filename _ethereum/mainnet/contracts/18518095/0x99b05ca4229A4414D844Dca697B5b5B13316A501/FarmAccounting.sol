...
library FarmAccounting {
        ...
        returns (uint256 amount)
    {
        unchecked {
            ...
            if (duration > 0) {
                uint256 elapsed = _min(block.timestamp, finished) - _min(checkpoint, finished);
    ...
    function startFarming(Info storage info, uint256 amount, uint256 period) internal returns (uint256) {
        ...
        (uint40 finished, uint32 duration, uint184 reward, uint256 balance) =
        ...
        (info.finished, info.duration, info.reward, info.balance) =
            (uint40(block.timestamp + period), uint32(period), uint184(amount), balance + amount);
        return amount;
    }
    ...
    function stopFarming(Info storage info) internal returns (uint256 leftover) {
        ...
        (info.finished, info.duration, info.reward, info.balance) =
            (uint40(block.timestamp), uint32(0), uint184(0), info.balance - leftover);
    }
    ...
    function _min(uint256 x, uint256 y) internal pure returns (uint256 z) {
        // slither-disable-line low-visibility
