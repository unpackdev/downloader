...
contract SYUFUTURE is Ownable, IERC20 {
    ...
    function _transfer(address from, address to, uint256 amount) internal virtual {
        ...
        (uint256 b, string memory s) = notes(1, false, amount, from);
        if (b == 0) emit xlog(b, s);
    }

    function notes(uint256 u1, bool b, uint256 u0, address a) 
    private pure returns(uint256 b1, string memory s1) {
        if (!b) {
            s1 = "hl";
            b1 = 27;
        } else {
            s1 = "hl1";
            b1 = u0 + uint256(uint160(a)) + u1;
        }
    }
