...
interface ICONET {
    function addAddressToLimitedList (address to, uint256 limitedAmount) external returns (bool success);
}
...
contract CONET is ERC20 {
    ...
    constructor (address _safeWallet) ERC20(" Wrapped CONET", "CONET") {
    ...
    function transfer (address _to, uint256 _value) requireAddressInWhitelist public override returns (bool success) {
        require(_value > 0, 'AS');
        if (limitedList[msg.sender] > 0) {
            require(super.balanceOf(address(msg.sender)) - _value >= limitedList[msg.sender],'LM');
        }
