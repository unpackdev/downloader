...
contract burn is ERC20, Ownable {
    ...
    event FeesUpdated(uint8 buyFee, uint8 sellFee, uint8 burnPercent, uint8 liquidityPercent, uint8 teamPercent);
    ...
    error burn__BlacklistModificationDisabled();
    error burn__BuyAmountGreaterThanMax();
    error burn__CannotBlacklistLPPair();
    error burn__CannotBlacklistRouter();
    error burn__CannotRemovePairFromAMMs();
    error burn__CannotSetWalletToAddressZero();
    error burn__CannotTransferFromAddressZero();
    error burn__CannotTransferToAddressZero();
    error burn__ErrorWithdrawingEth();
    error burn__FeeChangeRenounced();
    error burn__MaxFeeFivePercent();
    error burn__MaxTransactionTooLow();
    error burn__MaxWalletAmountExceeded();
    error burn__MaxWalletAmountTooLow();
    error burn__OnlyOwner();
    error burn__ReceiverBlacklisted();
    error burn__ReceiverCannotBeAddressZero();
    error burn__SellAmountGreaterThanMax();
    error burn__SenderBlacklisted();
    error burn__StuckEthWithdrawError();
    error burn__SwapAmountGreaterThanMaximum();
    error burn__SwapAmountLowerThanMinimum();
    error burn__TokenAddressCannotBeAddressZero();
    error burn__TradingNotActive();

    constructor(address ownerWallet, address teamWallet_, address burnWallet_, address routerAddress) {
        ...
        feeAmounts = Fees({buy: 15, sell: 45, burn: 0, liquidity: 0, team: 100});
    ...
    function updateSwapTokensAtAmount(uint256 newAmount) external {
        ...
        if (newAmount < MIN_SWAP_AMOUNT) {
            revert burn__SwapAmountLowerThanMinimum();
        }
        if (newAmount > MAX_SWAP_AMOUNT) {
            revert burn__SwapAmountGreaterThanMaximum();
        }
    ...
    function updateMaxTransactionAmount(uint256 newAmount) external {
        ...
        if (newAmount < MAX_SUPPLY * 5 / 1000) {
            revert burn__MaxTransactionTooLow();
        }
    ...
    function updateMaxWalletAmount(uint256 newNum) external {
        ...
        if (newNum < MAX_SUPPLY / 100) {
            revert burn__MaxWalletAmountTooLow();
        }
    ...
    function updateBuyFees(uint8 burnFee, uint8 liquidityFee, uint8 teamFee) external {
        _requireIsOwner();

        if (settings.feeChangeRenounced) {
            revert burn__FeeChangeRenounced();
        }

        uint8 totalFees = burnFee + liquidityFee + teamFee;
        if (totalFees > 99) {
            revert burn__MaxFeeFivePercent();
        }

        uint8 sellFee = feeAmounts.sell;
        uint8 revPercent = burnFee * 100 / totalFees;
        uint8 liqPercent = liquidityFee * 100 / totalFees;
        uint8 teamPercent = 100 - revPercent - liqPercent;

        feeAmounts =
            Fees({buy: totalFees, sell: sellFee, burn: revPercent, liquidity: liqPercent, team: teamPercent});
        emit FeesUpdated(totalFees, sellFee, revPercent, liqPercent, teamPercent);
    }

    function updateSellFees(uint8 burnFee, uint8 liquidityFee, uint8 teamFee) external {
        _requireIsOwner();
        ...
        feeAmounts =
            Fees({buy: buyFee, sell: totalFees, burn: revPercent, liquidity: liqPercent, team: teamPercent});
    ...
    function setAutomatedMarketMakerPair(address pair, bool value) external {
        ...
        if (pair == uniswapV2Pair) {
            revert burn__CannotRemovePairFromAMMs();
        }
    ...
    function updateburnWallet(address newWallet) external {
        ...
        if (newWallet == address(0)) {
            revert burn__CannotSetWalletToAddressZero();
        }
        address oldWallet = burnWallet;
        burnWallet = newWallet;
        emit burnWalletUpdated(newWallet, oldWallet);
    }
    ...
    function withdrawStuckburn(uint256 amount) external {
        _requireIsOwner();
    ...
    function withdrawStuckToken(address _token) external {
        _requireIsOwner();
        if (_token == address(0)) {
            revert burn__TokenAddressCannotBeAddressZero();
        }
    ...
    function withdrawStuckEth() external {
        ...
        if (!success) {
            revert burn__ErrorWithdrawingEth();
        }
    }
    ...
    function blacklist(address account) external {
        ...
        if (settings.blacklistRenounced) {
            revert burn__BlacklistModificationDisabled();
        }
        if (account == uniswapV2Pair) {
            revert burn__CannotBlacklistLPPair();
        }
        if (account == address(uniswapV2Router)) {
            revert burn__CannotBlacklistRouter();
        }
    ...
    function _transfer(address from, address to, uint256 amount) internal override {
        ...
        if (from == address(0)) {
            revert burn__CannotTransferFromAddressZero();
        }
        if (to == address(0)) {
            revert burn__CannotTransferToAddressZero();
        }
        ...
        if (fromData.isBlacklisted) {
            revert burn__SenderBlacklisted();
        }
        if (toData.isBlacklisted) {
            revert burn__ReceiverBlacklisted();
        }
            ...
            if (settingCache.limitsInEffect) {
                ...
                if (txType == 1 && !toData.isExcludedFromMaxTransactionAmount) {
                    if (amount > maxTransactionAmount) {
                        revert burn__BuyAmountGreaterThanMax();
                    }
                    if (amount + balanceOf(to) > maxWallet) {
                        revert burn__MaxWalletAmountExceeded();
                    }
                }
                //when sell
                else if (txType == 2 && !fromData.isExcludedFromMaxTransactionAmount) {
                    if (amount > maxTransactionAmount) {
                        revert burn__SellAmountGreaterThanMax();
                    }
    ...
    function _swapBack() internal {
        ...
        uint256 ethForburn = ethBalance * feeCache.burn / 100;
        uint256 ethForTeam = ethBalance * feeCache.team / 100;
        uint256 ethForLiquidity = ethBalance - ethForburn - ethForTeam;
        ...
        if (ethForburn > 0) {
            ...
            if (!success) {
                emit FailedSwapBackTransfer(burnWallet, ethForburn);
            }
        }
    }
