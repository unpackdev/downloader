...
import "./ICallableLoan.sol";
import "./ICallableLoanErrors.sol";
import "./ILoan.sol";
import "./IRequiresUID.sol";
...
import "./BaseUpgradeablePausable08x.sol";

import "./CallableLoanConfigHelper.sol";
import "./Waterfall.sol";
// solhint-disable-next-line max-line-length
import "./CallableCreditLine.sol";
import "./StaleCallableCreditLine.sol";
import "./SaturatingSub.sol";
import "./PaymentSchedule.sol";
import "./CallableLoanAccountant.sol";
  ...
  IVersioned
{
  ...
  /**
   * @notice Pay the full amount of interest and principal owed on the loan.
   * !!Warning!! Any previously prepaid interest (in a previous #pay call)
   * !!          beyond the currently accrued interest will NOT be refunded.
   */
  function payFull() external nonReentrant whenNotPaused {
    uint256 amount = _staleCreditLine.interestOwed() +
      _staleCreditLine.totalPrincipalOutstanding() +
      _staleCreditLine.interestAccrued();
    _pay(amount);
    uint256 _termEndTime = Math.max(_staleCreditLine.termEndTime(), block.timestamp);
    assert(_staleCreditLine.interestOwed() == 0);
    assert(_staleCreditLine.principalOwed() == 0);
    assert(_staleCreditLine.principalOwedAt(_termEndTime) == 0);
    assert(_staleCreditLine.interestOwedAt(_termEndTime) == 0);
  }
  ...
  function _pay(uint256 amount) internal returns (ILoan.PaymentAllocation memory) {
    ...
    ILoan.PaymentAllocation memory pa;
    bool isClosingPayment;

    {
      // We run into stack frame issues so we must re-invoke interestOwed, interestAccrued and
      // totalPrincipalOutstanding rather than assign to a variable in-memory.
      isClosingPayment =
        amount >= cl.interestOwed() + cl.interestAccrued() + cl.totalPrincipalOutstanding();
      pa = CallableLoanAccountant.allocatePayment({
        ...
        timeUntilNextPrincipalSettlement: isClosingPayment
          ? 0
          : cl.nextPrincipalDueTimeAt(block.timestamp).saturatingSub(block.timestamp),
        guaranteedFutureInterestPaid: cl.totalInterestPaid().saturatingSub(
    ...
    cl.pay(totalPrincipalPayment, totalInterestPayment, isClosingPayment);
    emit PaymentApplied({
      payer: msg.sender,
      pool: address(this),
