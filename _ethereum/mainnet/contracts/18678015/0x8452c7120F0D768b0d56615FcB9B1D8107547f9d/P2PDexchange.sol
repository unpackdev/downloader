...
contract P2PDexchange is ReentrancyGuard, Ownable {
    ...
    **/
    function swapTokenForToken(uint256 _id, uint256 _amount) public nonReentrant {
        ...
        // current fee is applied to buyer
        // we want to apply the fee to buyer and seller
        ...
        uint256 feeToOwner1 = 0;
        if(fee > 0){

            feeToOwner1 = _amount * fee / 10000;
            uint256 toSend = _amount * tokenRate / PRECISION_FACTOR;
            uint256 feeToOwner2 = toSend * fee / 10000;

            if(address(_pair.t1) == weth){
                payable(owner()).transfer(feeToOwner1);
            } else {
                TransferHelper.safeTransfer(address(_pair.t1), owner(), feeToOwner1);
            }
            TransferHelper.safeTransferFrom(address(_pair.t2), msg.sender, owner(), feeToOwner2);
            TransferHelper.safeTransferFrom(address(_pair.t2), msg.sender, _pair.seller, toSend-feeToOwner2);
        } else {
            uint256 toSend = _amount * tokenRate / PRECISION_FACTOR;
            TransferHelper.safeTransferFrom(address(_pair.t2), msg.sender, _pair.seller, toSend); // funds transferred to the seller
        }
        ...
        if(address(_pair.t1) == weth){
            payable(msg.sender).transfer(_amount - feeToOwner1);
        } else {
            TransferHelper.safeTransfer(address(_pair.t1), msg.sender, _amount - feeToOwner1);
        }
    ...
    **/
    function swapEthForToken(uint256 _id, uint256 _amount) public payable nonReentrant {
        ...
        if(fee > 0){
            ...
            TransferHelper.safeTransfer(address(_pair.t1), owner(), feeToOwner1);
            payable(owner()).transfer(feeToOwner2);
            payable(_pair.seller).transfer(toSend-feeToOwner2);
