...
contract DNFDayBreakStaking is Initializable, Upgradeable, Releasable {
    ...
    // value of now.
    function stake(uint256[] calldata _tokenIds) external whenNotStakingPaused {
    ...
    function withdraw(uint256[] calldata _tokenIds) external whenNotStakingPaused {
        require(
    ...
    // to the user.
    function claimRewards(uint256 tokenId) external whenNotStakingPaused {

        require(
            stakedAssets[tokenId].owner == msg.sender,
            "You don't own this token!"
        );
    ...
    function claimAllRewards() external whenNotStakingPaused {
        ...
        for (uint256 index = 0; index < stakeHolderTokens[msg.sender].tokenId.length; index++) {
            if(stakedAssets[stakeHolderTokens[msg.sender].tokenId[index]].owner == msg.sender) {
    ...
    function _requireStakingPaused() internal view virtual {
        require(stakingPaused(), "Pausable: not paused");
    }
     ...
     * - The contract must not be paused.
     */
    function stakingPause() external onlyOwner virtual whenNotStakingPaused {
        _stakingPaused = true;
    }
     ...
     * - The contract must be paused.
     */
    function stakingUnpause() external onlyOwner virtual whenStakingPaused {
