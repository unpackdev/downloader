...
contract UniswapPriceSource {
    ...
    {
        require(
            pool.token0() == quoteToken || pool.token1() == quoteToken, "at least one token from the pool must be WETH"
        );
    ...
    function getTokenPriceInEth(IUniswapV3Pool pool, uint32 twapInterval) public view returns (uint256) {
        require(pool.token0() == WETH || pool.token1() == WETH, "at least one token from the pool must be WETH");
        uint256 decimals = getAdjustedDecimals(pool, WETH);
        return getActualPrice(getPriceX96FromSqrtPriceX96(getSqrtTwapX96(pool, twapInterval, WETH)), decimals);
    }
    ...
    function getTokenPriceInUsdt(IUniswapV3Pool pool, uint32 twapInterval) public view returns (uint256) {
        uint256 tokenEthPrice = getTokenPriceInEth(pool, twapInterval);

        uint256 usdtEthPrice = getEthPriceInUsdt();
        return (usdtEthPrice * tokenEthPrice) / 10 ** 18;
    }
    ...
    function getEthPriceInUsdt() public view returns (uint256) {
        uint256 decimals = getAdjustedDecimals(usdtWethPool, USDT);
        return getActualPrice(getPriceX96FromSqrtPriceX96(getSqrtTwapX96(usdtWethPool, 5 minutes, USDT)), decimals);
    }
}