...
contract Csr is DefaultOperatorFilterer {
    ...
    string internal nftName = "Circle Square Rectangle";
    string internal nftSymbol = "CSRGA";
    ...
    /**
     * @dev Guarantees that the msg.sender is an owner.
     */
    modifier onlyOwner() {
        require(
            _own == payable(msg.sender),
    ...
    /**
     * @dev Transfer contract balance to owner balance.
     */
     ...
     * @return nfts string, kind of nft
     */
    function drawByKind(uint _tokenId)
        internal
        view
        returns (string memory, uint8)
    {
        ...
        if (nfts[_tokenId].elements[0].figure == 0) {
            _generate = true;
            _kind = 2;
        } else if (bytes(nfts[_tokenId].elements[0].color).length == 0) {
            _kind = 1;
        }
        ...
        return (string.concat(out, s2), _kind);
    }
        ...
        returns (string memory)
    {
        ...
        return
            string.concat(
                ...
                '","attributes":[{"trait_type":"Color change","value":"',
                (kind == 1 ? "Yes" : "No"),
                '"},{"trait_type":"Changeable","value":"',
                (kind == 2 ? "Yes" : "No"),
                '"}]}'
            );
    }
