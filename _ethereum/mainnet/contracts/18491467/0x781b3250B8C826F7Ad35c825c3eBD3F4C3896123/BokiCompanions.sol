...
contract BokiCompanions is ERC721A, Ownable, ERC2981 {
    ...
    function hasMerkleRoot() external view returns (bool) {
    ...
    function mint(
        ...
        uint16 _amount
    ) external payable saleActive isMerkleRootSet {
        bytes32 leaf = keccak256(abi.encode(msg.sender, _maxMintAmount));
        bool verified = MerkleProof.verify(proof, merkleRoot, leaf);
        require(verified, "Not a boki holder");
        require(_amount <= LIMIT, "Surpass txn limit");
        require(
            _numberMinted(msg.sender) + _amount <= _maxMintAmount,
