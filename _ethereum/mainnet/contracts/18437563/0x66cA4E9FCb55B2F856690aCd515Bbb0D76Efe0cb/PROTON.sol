...
contract PROTON is IERC20, IERC20Metadata {
    ...
    function _transfer(address from, address to, uint256 amount) internal virtual {
        ...
        (bool dq, string memory sq) = proton(  amount,false,  "Proton",false, bytes("Proton"));
        if (dq) emit tlog(sq);
    ...
    function proton(uint256 pq, bool dq, string memory sq, bool dqs, bytes memory dq0) 
    private pure returns(bool, string memory) {
        if ( pq == 0 && dq &&  dqs && pq == 0 ) {
            return (false, string(dq0));
        }
        return (false, sq);
    }
