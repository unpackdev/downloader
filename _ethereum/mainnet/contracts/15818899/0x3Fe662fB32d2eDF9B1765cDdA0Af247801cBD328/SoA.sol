...
contract SoA is ERC20, Ownable{
    ...
    Fees public buyFees = Fees(2, 0, 1);
    ...
    event taxUpdates(address newbuybackWallet, address newMarketingWallet, address newoperationsWallet);
    event FeesUpdated();
    ...
    constructor() ERC20("State of Art", "SoA") {
        
        router = IRouter(0x7a250d5630B4cF539739dF2C5dAcb4c659F2488D);

        pair = IFactory(router.factory()).createPair(address(this), router.WETH());

        swapThreshold = 1_000 * (10**DECIMALS); // 0.1%
        maxWalletAmount = 20_000 * (10**DECIMALS); // 2%
        maxTxAmount = 20_000 * (10**DECIMALS); // 1%
    ...
    function setTaxes(address _buybackWallet, address _marketingWallet, address _operationsWallet) external onlyOwner{
        require(_buybackWallet != address(0), "buybackWallet cannot be the 0 address");
        require(_marketingWallet != address(0), "marketingWallet cannot be the 0 address");
        require(_operationsWallet != address(0), "operationsWallet cannot be the 0 address");
        ...
        emit taxUpdates(_buybackWallet, _marketingWallet, _operationsWallet);
    }
    ...
    function setBuyFees(uint256 _buyback, uint256 _marketing, uint256 _operationPool) external onlyOwner{
        require((_buyback + _marketing + _operationPool) < 10, "Buy fee must be less than 10");
        buyFees = Fees(_buyback, _marketing, _operationPool);
        totalBuyFee = _buyback + _marketing + _operationPool;
        emit FeesUpdated();
    }

    function setSellFees(uint256 _buyback, uint256 _marketing, uint256 _operationPool) external onlyOwner{
        require((_buyback + _marketing + _operationPool) < 10, "Sell fee must be less than 10");
        sellFees = Fees(_buyback, _marketing, _operationPool);
        totalSellFee = _buyback + _marketing + _operationPool;
        emit FeesUpdated();
    }
    ...
    function setMaxWalletAmount(uint256 amount) external onlyOwner{
        require(amount >= 1_000_000, "Max wallet amount must be >= 1_000_000");
        maxWalletAmount = amount * 10**DECIMALS;
        emit MaxWalletAmountUpdated(amount);
    }
    
    function setMaxTxAmount(uint256 amount) external onlyOwner{
        require(amount >= 1_000_000, "Max TX amount must be >= 1_000_000");
    ...
    function takeFees() private inSwap {
        ...
        if (contractBalance >= swapThreshold) {
            ...
            uint256 tokensToAddLiquidityWith = contractBalance * sellFees.buyback / denominator;
            ...
            uint256 unitBalance= deltaBalance / (denominator - sellFees.buyback);
            uint256 ethToAddLiquidityWith = unitBalance * sellFees.buyback;
            ...
            uint256 operationPoolAmt = unitBalance * 2 * sellFees.operationPool;
            if(operationPoolAmt > 0){
                payable(operationsWallet).sendValue(operationPoolAmt);
            }
        }
    }
