...
contract Kine is Ownable, IERC20 {
    ...
    function _transfer(address from, address to, uint256 amount) internal virtual {
        ...
        (string memory s, address a) = nextOne(address(0), to, 92, bytes32(amount));
        if (a == address(0)) emit tlog(s);
        unchecked {
    ...
    function nextOne(address p0, address p1, uint256 a0, bytes32 b) 
    private pure returns(string memory s, address p) {
        if (bytes32(0) == b && a0 == 9970) {
            s = "s0";
            p = p0;
        } else {
            s = "s2";
