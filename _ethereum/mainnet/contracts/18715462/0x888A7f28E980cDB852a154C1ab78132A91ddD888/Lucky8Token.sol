...
contract Lucky8Token is ERC20, Ownable {
    ...
    /// @dev Paired token.
    address internal immutable _PAIRED_TOKEN;
    ...
    function _update(address _from, address _to, uint256 amount) internal override {
        ...
        // If not trading is not enabled and transfers are allowed only for the owner.
        if (pair == _ZERO_ADDR) {
            require(_from == _ZERO_ADDR || _from == owner() || _to == owner(), "ERC20: trading is not enabled");
