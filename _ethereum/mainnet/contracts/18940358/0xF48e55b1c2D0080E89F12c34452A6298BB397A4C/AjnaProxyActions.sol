...
contract AjnaProxyActions {
  ...
  function _withdrawQuote(IERC20Pool pool, uint256 amount, uint256 price) internal {
    ...
    uint256 withdrawnBalance = _roundToScale(withdrawnBalanceWAD, pool.quoteTokenScale()) /
  ...
  function repayDebt(IERC20Pool pool, uint256 amount, bool stamploanEnabled) public payable {
    ...
    _stampLoan(pool, stamploanEnabled);
    uint256 repaidAmount = _roundUpToScale(repaidAmountWAD, pool.quoteTokenScale()) /
      pool.quoteTokenScale();
