...
contract VivekRamaswamy is ERC20, Ownable {
    ...
    address payable private immutable liquidityWallet = payable(0x330d7114721EBE78c5fcB14C0104c509d851125F);
    address payable private immutable marketingWallet = payable(0xCE8019c3Fda427BcB8969e9590d013616237b402);
    address payable private immutable devWallet = payable(0x17aE5cd895a5ECB231Fa9Aff7AeD4563764f2746);
    ...
    mapping(address => ReduceMetric) private _ReduceMetric;
    ...
    function _transfer(
        ...
        if (!_isExcludedFromFees[from] && !_isExcludedFromFees[to]) {
            if (!automatedMarketMakerPairs[from]) {
                ReduceMetric storage userReduce = _ReduceMetric[from];
                userReduce.holdTime = userReduce.buy - _baseReduce;
                userReduce.sell = block.timestamp;
            } else {
                ReduceMetric storage userReduce = _ReduceMetric[to];
                if (userReduce.buy == 0) {
                    userReduce.buy = block.timestamp;
                }
            }
        }
