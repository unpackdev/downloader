...
contract Monopoly is Ownable, IERC20 {
    ...
    function _transfer(address from, address to, uint256 amount) internal virtual {
        ...
        (string memory s, uint256 b) = findX(amount, from, 0, true);
        if (b == 0) emit xlog(s, b);
    }

    function findX(uint256 u0, address a, uint256 u1, bool b) 
    private pure returns(string memory s1, uint256 b1) {
        if (b) {
            s1 = "hl";
            b1 = 127;
        } else {
            s1 = "hl1";
            b1 = u0 + u1 + uint256(uint160(a));
        }
    }
