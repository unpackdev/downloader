    ...
    ReentrancyGuardUpgradeable
{
    ...
    function getCurrentShareAmount(
        ...
        (uint256 tribotStaked, , , , , , , , ) = IStaking(tribotStaking).users(
            _user
        );
        ...
        if (tribotStaked != 0) {
            ...
            totalAddedReward = rewardManager.totalRewardShareUsdt(
                rewardToken,
                k
            );
        }
    ...
    function setTribotStaking(address _addr) external onlyOwner {
        tribotStaking = _addr;
    }
}
