...
import "./ISteakStaking.sol";

import "./IERC20.sol";
import "./SafeERC20.sol";
import "./Ownable.sol";

contract SteakStaking is Ownable, ISteakStaking {
  string public steakCut = "Porterhouse";
  ...
  function removeReward(uint256 _amount) public onlyOwner {
    ...
    IERC20(steakToken).safeTransfer(address(msg.sender), _amount);
  }
  ...
  function stake(uint256 _amount) public {
    ...
    IERC20(steakToken).safeTransferFrom(
      address(msg.sender),
      address(this),
      _amount);
  ...
  function unstake(uint256 _amount) public {
    ...
    IERC20(steakToken).safeTransfer(
      address(msg.sender),
      _amount);
  ...
  function payAndUpdateUser(UserInfo storage user, uint256 newAmount) internal {
    ...
    if (pending > 0) {
      IERC20(steakToken).safeTransfer(address(msg.sender), pending);
    }
