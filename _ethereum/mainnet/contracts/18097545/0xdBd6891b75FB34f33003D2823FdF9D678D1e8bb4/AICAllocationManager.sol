...
interface IAllocationManager {
    ...
    function getallocationduration(uint256 id) external view returns (uint256);

    function getallocationcliffoff(uint256 id) external view returns (uint256);

    function getallocationstart(uint256 id) external view returns (uint256);
}
...
contract VestingWallet is Context {
    ...
    address private immutable _allocationmanageradress;
    ...
    constructor(
        ...
        _allocationmanageradress = allocationmnm;
    }
    ...
    function allostart() public view virtual returns (uint256) {
        return
            IAllocationManager(_allocationmanageradress).getallocationstart(
                _allocationid
            );
    }

    function alloduration() public view virtual returns (uint256) {
        return
            IAllocationManager(_allocationmanageradress).getallocationduration(
                _allocationid
            );
    }

    function allocliffoff() public view virtual returns (uint256) {
        return
            IAllocationManager(_allocationmanageradress).getallocationcliffoff(
                _allocationid
            );
    }
    ...
    function released() public view virtual returns (uint256) {
        address token = 0xD2Ec5461A79b960f5b8570156D60CAcd11a99c98;
    ...
    function release() public virtual {
        ...
        require(block.timestamp > allostart(), "Vesting not started yet");
    ...
    function _vestingSchedule(
        ...
        if (timestamp < allostart()) {
            return 0;
        } else {
            require(
                (allostart() + allocliffoff()) < timestamp,
                "You still have to wait, sorry."
            );

            if (timestamp > (allostart() + allocliffoff() + alloduration())) {
                return totalAllocation;
            }

            return
                (totalAllocation *
                    (timestamp - (allostart() + allocliffoff()))) /
                alloduration();
...
contract AICAllocationManager is IAllocationManager, Ownable {
    using SafeMath for uint256;

    mapping(uint256 => uint256) public allocation;
    mapping(address => uint256) public whitelist;
    mapping(address => uint256) public whitelistallocation;
    mapping(uint256 => uint256) public allocationduration;
    mapping(uint256 => uint256) public allocationcliffoff;
    mapping(uint256 => uint256) public allocationstart;
    ...
    constructor(address addressCai) {
    ...
    function setWhitelist(
        ...
        whitelist[_addr] = amount;
        whitelistallocation[_addr] = allocationID;
    }
    ...
    function setAllocation(
        ...
        allocation[id] = id;
        allocationduration[id] = duration;
        allocationcliffoff[id] = cliffoff;
        allocationstart[id] = starttimestamp;
    }

    function AllocationClaim() public virtual {
        require(whitelist[msg.sender] > 0, "You are not in the Whitelist");
        uint256 amountCai = whitelist[msg.sender];

        _execute(msg.sender, amountCai, whitelistallocation[msg.sender]);
    ...
    function getallocationstart(
        uint256 id
    ) public view virtual override returns (uint256) {
        return allocationstart[id];
    }

    function getallocationduration(
        uint256 id
    ) public view virtual override returns (uint256) {
        return allocationduration[id];
    }

    function getallocationcliffoff(
        uint256 id
    ) public view virtual override returns (uint256) {
        return allocationcliffoff[id];
    }
