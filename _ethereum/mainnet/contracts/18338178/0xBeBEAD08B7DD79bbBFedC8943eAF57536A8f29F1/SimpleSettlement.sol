...
contract SimpleSettlement is OwnableUpgradeable, ReentrancyGuardUpgradeable, UUPSUpgradeable {
     ...
     * @param _marginEngine to transact against
     ...
     */
    function settle(IMarginEngine _marginEngine, Bid calldata _bid, address[] calldata _collaterals, uint256[] calldata _amounts)
        external
        nonReentrant
    {
        ...
        // verify if seller is vault, will need to be fixed when Account Abstraction gets adopted
        bool isVault = _bid.vault.code.length > 0;
        if (isVault) IAuctionVault(_bid.vault).verifyOptions(_bid.options);

        // deposit actions from counterparty
        ActionArgs[] memory bActions = _createDeposits(_collaterals, _amounts, _marginEngine);

        // mint or transfer actions from vault/margin account
        (ActionArgs[] memory sActions, ActionArgs[] memory bMints) =
            _createMintsOrTransfers(_bid.vault, msg.sender, _bid.options, _bid.weights, isCashSettled, isVault);
        ...
        _marginEngine.batchExecute(batch);
    }
    ...
    function settleBatch(
        ...
        // deposit actions from counterparty
        ActionArgs[] memory depositActions = _createDeposits(_collaterals, _amounts, _marginEngine);

        (BatchExecute[] memory batch, ActionArgs[] memory bActions) = _setupBidActionsBulk(_marginEngine, _bids);
    ...
    function wrap(address _marginEngine, address _subAccount, uint8 _collateralId, uint256 _amount) external nonReentrant {
        ...
        // wrap underlying
        (address wrapper, uint256 amount) = _wrapToken(address(this), underlying, _amount, isCashSettled);
        ...
        returns (BatchExecute[] memory batch, ActionArgs[] memory bActions)
    {
        ...
        for (uint256 i; i < bidCount;) {
            ...
            // verify if seller is vault, will need to be fixed when Account Abstraction gets adopted
            bool isVault = _bids[i].vault.code.length > 0;
            if (isVault) IAuctionVault(_bids[i].vault).verifyOptions(_bids[i].options);

            // mint or transfer actions from vault/margin account
            (ActionArgs[] memory sActions, ActionArgs[] memory bMints) =
                _createMintsOrTransfers(_bids[i].vault, msg.sender, _bids[i].options, _bids[i].weights, isCashSettled, isVault);
     ...
     */
    function _createDeposits(address[] memory _collaterals, uint256[] memory _amounts, IMarginEngine _marginEngine)
        internal
        returns (ActionArgs[] memory actions)
    {
        ...
        IAssetRegistry assetRegistry = isCashSettled ? _marginEngine.grappa() : _marginEngine.pomace();

        actions = new ActionArgs[](_collaterals.length);

        for (uint256 i; i < _collaterals.length;) {
            (address wrapper, uint256 amount) = _wrapToken(msg.sender, _collaterals[i], _amounts[i], isCashSettled);
    ...
    function _createMintsOrTransfers(
        ...
        unchecked {
            ...
            if (_isCashSettled) {
                counterpartyAction = CASH_MINT_LONG_INTO_ACCOUNT_ACTION;
                vaultAction = _isVault ? CASH_MINT_LONG_INTO_ACCOUNT_ACTION : CASH_TRANSFER_LONG_ACTION;
            } else {
                counterpartyAction = PHYSICAL_MINT_LONG_INTO_ACCOUNT_ACTION;
                vaultAction = _isVault ? PHYSICAL_MINT_LONG_INTO_ACCOUNT_ACTION : PHYSICAL_TRANSFER_LONG_ACTION;
            }
    ...
    /**
     * @notice Wraps asset on the fly
     */
    function _wrapToken(address from, address _underlying, uint256 _amount, bool _isCashSettled)
        internal
