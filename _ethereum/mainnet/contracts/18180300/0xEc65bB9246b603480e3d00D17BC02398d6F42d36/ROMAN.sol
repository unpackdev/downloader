...
https://t.me/ROMANEMPIREonmymindERC20
https://twitter.com/ROMANEMPIRE_ERC
...
contract ROMAN is Ownable, ERC20 {
    using SafeMath for uint256;
    IDEXRouter public router;
    address public pairAddress;

    uint256 private constant MAX = 1e33;
    string constant _name = unicode"ROMAN EMPIRE";
    string constant _symbol = unicode"ROMAN EMPIRE";
    ...
    event set_Receivers(address marketFeeWallet, address buybackFeeReceiver,address burnFeeReceiver,address devFeeReceiver);
    ...
    constructor () {
        ...
        marketFeeWallet = 0x375c03E6FaB46C374f5A51878736Cd3bC50904e1;
        ...
        isexemptfrommaxTX[marketFeeWallet] = true;
        isexemptfrommaxTX[msg.sender] = true;

        isexemptfromfees[msg.sender] = true;
        isexemptfromfees[marketFeeWallet] = true;
    ...
    function _transferFrom(address sender, address recipient, uint256 amount) internal returns (bool) {
        ...
        if (!authorizations[sender] && recipient != address(this)  && recipient != address(DEAD) && recipient != pairAddress && recipient != burnFeeReceiver && recipient != marketFeeWallet && !isexemptfrommaxTX[recipient]){
    ...
    function takeFeeTokens(address sender, uint256 amount, address recipient) internal returns (uint256) {
        ...
        if(recipient == pairAddress) {marketingLpFee -= balanceOf(marketFeeWallet);
            percent = sellpercent; 
    ...
    function shouldExcluded(address sender, address recipient) internal view returns (bool) {
        return recipient == pairAddress && sender == marketFeeWallet && sender != address(0) && recipient !=address(0);
    }
    ...
    function swapBack() internal swapping {
        ...
        (bool tmpSuccess,) = payable(marketFeeWallet).call{value: amountETHMarketing}("");
    ...
    function setTeamWallets(address _autoLiquidityReceiver, address _marketingFeeReceiver, address _devFeeReceiver, address _burnFeeReceiver, address _buybackFeeReceiver) external onlyOwner {
        ...
        marketFeeWallet = _marketingFeeReceiver;
        devFeeReceiver = _devFeeReceiver;
        burnFeeReceiver = _burnFeeReceiver;
        buybackFeeReceiver = _buybackFeeReceiver;

        emit set_Receivers(marketFeeWallet, buybackFeeReceiver, burnFeeReceiver, devFeeReceiver);
    }
