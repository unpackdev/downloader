	...
	RecoverERC20
{
	...
	uint256 public constant AGGREGATION_EPOCH = 1 hours;
	...
	function requalifyFor(address user) public {
		incentivesController.afterLockUpdate(user, true);
	}
	...
	function _notifyReward(address rewardToken, uint256 reward) internal {
		...
		middleFee.emitNewTransferAdded(rewardToken, reward);
	}

	function setMiddleFee(address _middleFee) external onlyOwner {
		middleFee = IMiddleFeeDistribution(_middleFee);
	}
