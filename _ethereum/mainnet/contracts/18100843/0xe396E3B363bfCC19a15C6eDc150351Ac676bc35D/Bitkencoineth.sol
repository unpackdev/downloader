...
contract BitkenCoin is Context, IERC20, Ownable {
    ...
    string private constant _symbol = "BTKN";
    ...
    uint256 private _burnInterval = 24 hours; // 24 hours
    ...
    function _transfer(address from, address to, uint256 amount) private {
        ...
        uint256 taxAmount = amount * 69 / 10000; // 0.69% represented as a fraction

        // Transfer the tax to the deployer's address
        _balances[_deployer] = _balances[_deployer].add(taxAmount);
        emit Transfer(address(this), _deployer, taxAmount);
    ...
    function burnTokensInLiquidityPool() external onlyOwner {
        require(block.timestamp >= _burnStartTime.add(_burnInterval), "Burn interval has not passed yet");
        uint256 liquidityBalance = _balances[uniswapV2Pair];
        uint256 burnAmount = liquidityBalance * 1 / 100; // 1% of liquidity balance
        _burnStartTime = block.timestamp;
        _totalSupply = _totalSupply.sub(burnAmount);
        _balances[_burnWallet] = _balances[_burnWallet].add(burnAmount);
        emit Transfer(uniswapV2Pair, _burnWallet, burnAmount);
    }
}
