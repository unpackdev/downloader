...
/// @dev VotingEscrow implementation
    ...
    ReentrancyGuardUpgradeable
{
    ...
    /// @param _initialOwner Is assumed to be a timelock contract
    ...
    function initialize(
        ...
        // rewards rate  (chax reward rate per chax staked per second)
        rewardRate = 0;
        duration = 7 days;
    }
    ...
    modifier whenForcedQuit() {
        require(isForcedQuit, "forced quit closed");
        _;
    }
    ...
    function earned(address account) public view returns (uint256) {
        return
            SafeCast.toUint256(locked[account].amount)
    ...
    function updateOperation(address _operation) external onlyOwner {
    ...
    function setForcedQuit(bool _isForcedQuit) external onlyOwner {
        isForcedQuit = _isForcedQuit;
    }
    ...
    function createLock(
        uint256 _value,
        uint256 _unlockTime
    ) external override nonReentrant checkBlocklist updateReward(msg.sender) {
    ...
    function getReward() external returns (bool) {
        return getRewardFrom(msg.sender);
    }

    function newRewards(uint256 _rewards) external onlyOperation returns (bool) {
