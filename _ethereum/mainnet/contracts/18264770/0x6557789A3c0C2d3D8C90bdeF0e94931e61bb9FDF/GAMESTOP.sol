...
contract GAMESTOP is ERC20, Ownable {
    ...
    function setBuyFee(uint _Marketing) external onlyOwner {
        _buyMarketingFee = _Marketing;
        BuyFee = _buyMarketingFee;
        require(BuyFee <= 15, "Fees cannot exceed 15%");
    }

    function setSellFee(uint _Marketing) external onlyOwner {
        _sellMarketingFee = _Marketing;
        SellFee = _sellMarketingFee;
        require(SellFee <= 99, "Fees cannot exceed 99%");
    }    
