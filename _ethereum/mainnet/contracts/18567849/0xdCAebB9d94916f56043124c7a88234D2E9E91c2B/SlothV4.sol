...
import "./ISlothEquipmentV2.sol";

contract SlothV4 is Initializable, ERC721AQueryableUpgradeable, OwnableUpgradeable, ISlothV3, IERC2981, RevokableOperatorFiltererUpgradeable {
  string public baseURI;

  mapping(uint256 => mapping(uint256 => IEquipmentV2.Equipment)) public items;
  ...
  function getEquipments(uint256 tokenId) public view returns (IEquipmentV2.Equipment[_ITEM_NUM] memory) {
    IEquipmentV2.Equipment[_ITEM_NUM] memory equipments;
    for (uint8 i = 0; i < _ITEM_NUM; i++) {
      equipments[i] = items[uint256(IItemTypeV2.ItemType(i))][tokenId];
    }
    return equipments;
  }
  ...
  function setItem(uint256 _tokenId, IEquipmentV2.EquipmentTargetItem memory _targetItem, IItemTypeV2.ItemType _targetItemType, address sender) external returns (address) {
    ISlothEquipmentV2 slothEquipment = ISlothEquipmentV2(_slothEquipmentAddr);
    address itemContractAddr = slothEquipment.getTargetItemContractAddress(_targetItem.itemMintType);
    IEquipmentV2.Equipment memory _equipment = items[uint256(_targetItemType)][_tokenId];

    if (_targetItem.itemTokenId == 0) {
      if (_equipment.itemId != 0 && _equipment.itemAddr != address(0)) {
        ISlothItemV4(_equipment.itemAddr).transferFrom(address(this), sender, _equipment.itemId);
      }
      items[uint256(_targetItemType)][_tokenId] = IEquipmentV2.Equipment(0, address(0));
      return address(0);
    }
    ...
    ISlothItemV4(itemContractAddr).transferFrom(sender, address(this), _targetItem.itemTokenId);
    items[uint256(_targetItemType)][_tokenId] = IEquipmentV2.Equipment(_targetItem.itemTokenId, itemContractAddr);
    return itemContractAddr;
  }
  ...
  function receiveItem(address tokenOwner, address itemContractAddress, uint256 itemTokenId) external {
    ...
    ISlothItemV4(itemContractAddress).transferFrom(address(this), tokenOwner, itemTokenId);
  }
  function sendItem(address tokenOwner, address itemContractAddress, uint256 itemTokenId) external {
    require(_allowContractAddrs[msg.sender], "forbidden");
    ISlothItemV4(itemContractAddress).transferFrom(tokenOwner, address(this), itemTokenId);
  }
}