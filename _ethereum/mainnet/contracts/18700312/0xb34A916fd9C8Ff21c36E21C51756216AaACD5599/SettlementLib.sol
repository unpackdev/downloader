...
library SettlementLib {
    ...
    function performBid(uint40 listingId, MarketplaceLib.Listing storage listing, uint256 bidAmount, address payable referrer, bool increase, mapping(address => mapping(address => uint256)) storage escrow, bytes memory data) public {
        ...
        if (MarketplaceLib.hasBid(listing.flags)) {
            if (currentBid.bidder == bidder) {
                ...
                require(bidAmount >= computeMinBid(listing.details.erc20, listing.details.initialAmount, currentBid.amount, listing.details.minIncrementBPS), "Minimum bid not met");
                receiveTokens(listing, bidder, bidAmount, false, true);
                // Refund bid amount
        ...
        if (listing.details.type_ == MarketplaceLib.ListingType.DYNAMIC_PRICE) {
            ...
            if (MarketplaceLib.hasBid(listing.flags)) {
                if (listing.details.type_ == MarketplaceLib.ListingType.INDIVIDUAL_AUCTION) {
                    currentPrice = computeMinBid(listing.details.erc20, listing.details.initialAmount, listing.bid.amount, listing.details.minIncrementBPS);
                }
            }
        }
        return currentPrice;
     ...
     * Get the min bid
     */
    function computeMinBid(address erc20, uint256 baseAmount, uint256 currentAmount, uint16 minIncrementBPS) pure public returns (uint256) {
        if (currentAmount == 0) {
            return baseAmount;
        }
        ...
        if (erc20 == address(0)) {
            // ETH related bid tiers
            if (currentAmount < 250000000000000000) {
                // If current amount is less than 0.25 ETH, increment by 0.1 ETH
                return currentAmount+100000000000000000;
            } else if (currentAmount < 1000000000000000000) {
                // If current amount is less than 1 ETH, increment by 0.25 ETH
                return currentAmount+250000000000000000;
            } else if (currentAmount < 5000000000000000000) {
                // If current amount is less than 5 ETH, increment by 0.5 ETH
                return currentAmount+500000000000000000;
            }
        }
