...
contract PropdatesV2 is Initializable, Ownable2StepUpgradeable, UUPSUpgradeable{
    ...
    /// @dev reverts if propIds length is not equal to newAdmins length
    /// @dev if current admin for any is zero address, reverts unless msg.sender is prop proposer
    /// @dev if current admin for any is not zero address, reverts unless msg.sender is current admin
    /// @param propIds array of propIds
    /// @param newAdmins array of new admins for those propIds
