...
contract PolicyBook is IPolicyBook, ERC20PermitUpgradeable, AbstractDependant {
    ...
    event Liquidityforwarded(address user, uint256 amount);
    ...
    function setForwardingAllowance(address _v2PolicyBookAddress) external {
        require(_v2PolicyBookAddress != address(0), "No v2 destiny address");
        uint256 balanceToApprove = stblToken.balanceOf(address(this));

        stblToken.safeApprove(_v2PolicyBookAddress, balanceToApprove);
    }
    ...
    function forwardUserLiquidity(
        ...
        IV2PolicyBook v2PolicyBook = IV2PolicyBook(_v2PolicyBookAddress);
        IV2PolicyBookFacade facade = IV2PolicyBookFacade(v2PolicyBook.policyBookFacade());

        facade.addLiquidityFromDistributorFor(_user, _stblAmountStnd);

        emit Liquidityforwarded(_user, _stblAmountStnd);
    }

    function withdrawSTBLTo(address to) external override onlyPolicyBookAdmin {
        uint256 usdtBalance = stblToken.balanceOf(address(this));
        stblToken.safeTransfer(to, usdtBalance);
    }
    ...
    function unlockTokens() public override {
        ...
        returns (uint256 _stblAmountTether)
    {
        ...
        if (withdrawalsInfo[_sender].withdrawalAmount > 0) {
            unlockTokens();
        }

        uint256 _holderTotalBMIX = balanceOf(_sender);

        if (allowance(_sender, address(this)) < _holderTotalBMIX) {
            migrate(_sender, _holderTotalBMIX);
        }

        this.transferFrom(_sender, address(this), _holderTotalBMIX);
