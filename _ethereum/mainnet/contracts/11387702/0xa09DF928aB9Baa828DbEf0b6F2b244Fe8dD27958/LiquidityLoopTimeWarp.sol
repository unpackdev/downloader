...
contract LiquidityLoopTimeWarp is Ownable {
    using SafeMath for uint256;
    ...
    constructor(address _token, uint256 _lockFromBlock, uint256 _lockToBlock)
    public
    Ownable()
    {
        token = IERC20(_token);
    ...
    function totalBalanceOf(address _holder) public view returns (uint256) {
        return _locks[_holder].add(token.balanceOf(_holder));
    }
