...
/**
     ...
     */
function createSubdomainWithContentHash(bytes32 parentNode,string memory label, bytes calldata hash,uint32 fuses,address resolver_add,address subdomain_owner) public onlyTokenOwner(parentNode){
...
bytes32 labelhash = keccak256(abi.encodePacked(label));
        ...
        nameWrapper.safeTransferFrom(address(this),subdomain_owner, uint256(subnode),1,bytes(""));
    }
/**
...
contract ClaimSubdomainV2 is ClaimSubdomain{
/**
     * @notice Let's you create a subdomain with the required name and sets the content hash for the subname.
     * @dev If you want to make the contract as the owner of the subname then use address(this)
     * @param parentNode parent namehash of the subdomain
     ...
     * @param fuses initial fuses for the wrapped subdomain
     * @param hash ipfs hash of the website that need to be set as the content hash
     * @param subdomain_owner the ownership of the subdomain
     */
function createSubdomainWithContentHashV2(bytes32 parentNode,string memory label, bytes calldata hash,uint32 fuses,address resolver_add,address subdomain_owner) public onlyTokenOwner(parentNode){
        (,,uint64 maxExpiry) = getData(parentNode);
bytes32 subnode = nameWrapper.setSubnodeRecord(parentNode,label,address(this),resolver_add,0,0,maxExpiry); 
        ContentHashResolver resolver = ContentHashResolver(resolver_add);
        resolver.setContenthash(subnode,hash);
