...
contract StrategyCRV  {
    ...
    function _withdrawSome(uint256 _amount) internal returns(uint256) {
        // calculate amount of rencrv to withdraw for amount of _want_
        uint _rencrv = _amount.mul(1e18).div(ICurveFi(curve).get_virtual_price());
    ...
    function withdrawUnderlying(uint256 _amount) internal returns (uint) {
        ...
        ICurveFi(curve).remove_liquidity(_amount, [_amount, 0]);
