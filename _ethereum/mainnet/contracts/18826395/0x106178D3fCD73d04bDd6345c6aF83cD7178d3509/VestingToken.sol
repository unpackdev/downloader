...
contract VestingToken is Ownable{
    ...
    function getClaimSingleAlloc(address _recipient, uint _index) public view returns (uint amount) {
        uint claimAmount;
        if (allocations[_recipient][_index].revocable || block.timestamp <= allocations[_recipient][_index].endCliff) {
            claimAmount = 0;
        }
        else {
    ...
    function calculateClaimAmount(address _recipient, uint _index) internal view returns (uint amount) {
        ...
        if (allocations[_recipient][_index].revocable || block.timestamp <= allocations[_recipient][_index].endCliff) {
            ...
            newClaimAmount = allocations[_recipient][_index].totalAllocated;
                 ...
                 newClaimAmount += ((allocations[_recipient][_index].totalAllocated)
                	* (block.timestamp - allocations[_recipient][_index].endCliff))
                    / (allocations[_recipient][_index].endVesting - allocations[_recipient][_index].endCliff);
            }
    ...
    * @param _recipient recipient of allocation
    ...
    function transferSingleAlloc(address _recipient, uint _index) external {
        ...
        require(allocations[_recipient][_index].endCliff <= block.timestamp, "Start time of claim should be later than start of vesting time");
        //transfer tokens after subtracting tokens claimed
