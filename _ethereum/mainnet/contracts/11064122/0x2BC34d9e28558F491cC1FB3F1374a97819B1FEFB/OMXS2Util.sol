   ...
   function settle(uint _startIndex,uint _count,uint256 each)public onlyOwner () {
       OMXS2Token omxs2 =OMXS2Token(omxS2Addr);
        ...
        for(uint i=_startIndex;i<(_startIndex+_count);i++){
            ...
            uint oSize=token.balanceOf(omxs2.users(i)).div(exchangeRate * 10 ** ongDecimals);
          
            if(pSize>0&&oSize>0){
                if(pSize>oSize){
                    amount=oSize.mul(each).mul(1*10**ongDecimals);
                }else{
                   amount=pSize.mul(each).mul(1*10**ongDecimals);
                }
