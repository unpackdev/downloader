...
contract SlippageAuction is ReentrancyGuard, Timelock2Step {
    ...
    function getPreSlippagePrice() external view returns (uint256 _price) {
        getPreSlippagePrice(details[details.length - 1]);
    }
    ...
    function _getAmountIn(
        ...
        _slippagePerTokenSell = (_detail.priceSlippage * amountIn) / PRECISION;
        _postPriceSlippage = _detail.priceLast + (2 * _slippagePerTokenSell); // Price impact is twice the slippage
    }
    ...
    function startAuction(StartAuctionParams calldata _params) external nonReentrant returns (uint256 _auctionNumber) {
        ...
        // Ensure that a nonzero amount of tokens are auctioned
        if (_params.amountListed == 0) revert ZeroAmount();

        // Ensure that priceStart > priceMin
        if (_params.priceStart < _params.priceMin) revert PriceStartLessThanPriceMin();

        // Ensure slippage does not exceed max
        if (_params.priceSlippage >= PRECISION) revert PriceSlippageTooHigh();
    ...
    /// @notice Emitted when a user attempts to start an auction selling 0 tokens
    error ZeroAmount();
