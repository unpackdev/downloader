...
import "./IChainlinkPriceConsumer.sol";
import "./Tiers.sol";
import "./ReentrancyGuard.sol";
import "./SD59x18.sol";

contract PurchasingCenter is Ownable, Tiers, ReentrancyGuard {
    ...
    // mapping(Tier => uint256) public tier_to_buyers;
    mapping(Tier => address[]) public tier_to_buyersArray;
    ...
    event BoughtTokens(uint256 tier, address guy, uint256 contribution);
    event PwcUpdated(uint256 tier, uint256 pwc);
    event Oversubscribed(uint256 tier, address guyWhoPushedItOver);
    event BoughtTokens(uint256 tier, uint256 ethIn, uint256 amountOut);
    ...
    function buyTokens(
        uint256 tier
    ) external payable whenStarted nonReentrant returns (bool success) {
        Tier _tier = _t(tier);
    ...
    function ethRefundable(uint256 tier) public view returns (uint256) {
        uint256 totalRefund;
        uint256 buyers = (tier == 1) ? tier1_buyers.length : (tier == 2)
            ? tier2_buyers.length
            : (tier == 3)
            ? tier3_buyers.length
            : (tier == 4)
            ? tier4_buyers.length
            : 0;
        require(buyers != 0, "no buyers yet");
        if (tier == 1) {
