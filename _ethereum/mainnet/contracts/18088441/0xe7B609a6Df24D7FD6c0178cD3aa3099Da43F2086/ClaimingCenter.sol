...
contract ClaimingCenter is Tiers, Ownable, ReentrancyGuard {
    ...
    mapping(address => mapping(Tier => uint256))
        public address_to_tier_eth_refund_claimed;
    ...
    function startClaims() external onlyOwner whenClaimsNotStarted {
        // once started it's impossible unstart / pause.
        claimsStarted = true;
        emit ClaimsStarted(block.timestamp);
    }

    function startRefunds() external onlyOwner whenClaimsNotStarted {
        // once started it's impossible unstart / pause.
        refundsStarted = true;
    ...
    function claim(
        uint256 tier
    ) external nonReentrant whenClaimsStarted returns (bool) {
        Tier _tier = _t(tier);
        ...
        bool success = commodity.transfer(guy, amountToClaim);
    ...
    function withdrawCommodity(
        ...
        bool success = commodity.transfer(to, amount);
    ...
    function claimRefunds(
        uint256 tier
    ) external whenRefundsStarted nonReentrant returns (bool success) {
        address guy = msg.sender;
        uint256 refundAmount = refundClaimable(guy, tier);
        Tier _tier = _t(tier);
        require(
            address_to_tier_eth_refund_claimed[guy][_tier] == 0,
        ...
        if (success) {
            address_to_tier_eth_refund_claimed[guy][_tier] = refundAmount; // Updates the amount of refund a guy has claimed.
