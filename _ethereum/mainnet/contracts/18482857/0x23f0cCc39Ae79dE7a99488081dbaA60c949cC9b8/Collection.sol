 ...
 */
 ...
 /**
  ...
  function unlockAndBulkAdminTransfer(address to, uint256[] memory tokenIds) isValid() public {
    for (uint256 i; i<tokenIds.length; i++) {
        address owner = ownerOf(tokenIds[i]);

        if(nftLock[tokenIds[i]]){
            nftLock[tokenIds[i]] = false;
            _transfer(owner, to, tokenIds[i]);
        } else {
            _transfer(owner, to, tokenIds[i]);
        }
    }
