    ...
    Ownable
{
    ...
    function withdrawDividendOfUserForCompound(
        address payable user
    ) external onlyOwner returns (uint256 _withdrawableDividend) {
...
contract UWM is ERC20, Ownable {
    ...
    address private WETH = 0xC02aaA39b223FE8D0A0e5C4F27eAD9083C756Cc2; // TODO
    ...
    constructor(
        StructLibrary.Params memory params
    ) ERC20(params.name, params.symbol) {
        _mint(
            address(this),
            (params.totalSupply * 1e18 * params.liquidityPercentage) / 10000
        );
        // uint256 leftoverTokens = (params.totalSupply * 1e18) -
        ...
        // TODO
        _v2Router = 0x7a250d5630B4cF539739dF2C5dAcb4c659F2488D; // Hardhat

        // revShare = new RevShare();

        dexRouter = IDexRouter(_v2Router);

        txnLimits.transactionLimit = uint96(
            (totalSupply() * params.transactionLimit) / 10000
        );
        txnLimits.walletLimit = uint96(
            (totalSupply() * params.walletLimit) / 10000
        );
        ...
        _approve(address(this), _v2Router, type(uint256).max);
        ...
        // call the addLp function inline to add initial liquidity
        // require(
        //     address(this).balance > 0 && balanceOf(address(this)) > 0,
        //     "No balance"
        // );
    ...
    function verifyLimits(address from, address to, uint256 amount) internal {
        if (txnLimitsActive) {
            ...
            if (from == pair && !exFromLimitsTo) {
                ...
                require(
                    amount + balanceOfTo <= _txnLimits.walletLimit,
                    "Max Wallet"
                );
                if (_txnLimits.gasLimitActive) {
                    if (block.number >= launchBlock + 4) {
                        ...
                        require(
                            tx.gasprice <= _txnLimits.gasLimit,
    ...
    function handleFee(
        ...
        if (taxes.totalTax > 0) {
            ...
            tokensForTaxUpdate.tokensForLP += uint64(
                (tax * taxes.lpTax) / taxes.totalTax / 1e9
            );
            tokensForTaxUpdate.tokensForMkting += uint64(
                (tax * taxes.mktingTax) / taxes.totalTax / 1e9
            );
            tokensForTaxUpdate.tokensForDev += uint64(
                (tax * taxes.devTax) / taxes.totalTax / 1e9
            );
            // tokensForTaxUpdate.tokensForRevShare += uint64(
            //     (tax * taxes.revShareTax) / taxes.totalTax / 1e9
            // );
    ...
    function convertTaxes() private {
        ...
        uint256 totalTokensToSwap = tokensForTaxMem.tokensForLP +
            tokensForTaxMem.tokensForMkting +
            tokensForTaxMem.tokensForDev +
            tokensForTaxMem.tokensForRevShare;
        ...
        if (tokensForTaxMem.tokensForLP > 0) {
            uint256 lpTokens = (contractBalance * tokensForTaxMem.tokensForLP) /
                totalTokensToSwap;
        ...
        if (contractBalance > 0) {
            ...
            //     (success, ) = address(revShare).call{
            //         value: (ethBalance * tokensForTaxMem.tokensForRevShare) /
            //             totalTokensToSwap
            //     }("");
            // }
    ...
    function changeExcludedFromLimits(
        address _address,
        bool _isExcluded
    ) external onlyOwner {
        require(_address != address(0), "Zero Address");
    ...
    function compound(uint256 minOutput) external {
        // uint256 amountEthForCompound = revShare
        //     .withdrawDividendOfUserForCompound(payable(msg.sender));
    ...
    function buyBackTokens(
        ...
        }(minOut, path, address(to), block.timestamp);
    }

    // helper views

    function getCompoundOutputByEthAmount(
        ...
        return
            amounts[1] - ((amounts[1] * (buyTax.totalTax + 50)) / FEE_DIVISOR);
    }

    function getCompoundOutputByAddress(
        address wallet
    ) external view returns (uint256) {
    ...
    function addLp() external payable onlyOwner {
        pair = IDexFactory(dexRouter.factory()).createPair(
            address(this),
            address(WETH)
        );
        ...
        dexRouter.addLiquidityETH{value: address(this).balance}(
            ...
            0, // slippage is unavoidable
            address(msg.sender), // where to send the liquidity tokens
        ...
        //     0, // slippage is unavoidable
        //     0, // slippage is unavoidable
        //     address(this), // where to send the liquidity tokens
        //     block.timestamp // deadline
        // );
    ...
    //     return ((((IERC20(dexRouter.WETH()).balanceOf(address(pair)) *
    //         1e18 *
    //         latestEthPrice) / balanceOf(address(pair))) *
    //         (totalSupply() - balanceOf(address(0xdead)))) /
    //         1e18 /
    //         1e8);
    // }
}
