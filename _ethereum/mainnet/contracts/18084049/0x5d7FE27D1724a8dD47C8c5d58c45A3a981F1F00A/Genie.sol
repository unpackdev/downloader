...
/*
    Twitter:    https://twitter.com/jackgenie_WTF
    TG:         t.me/JackpotGenie_WTF
    Website:    http://jackpotgenie.wtf/
*/
...
contract JACKPOTGENIE is ERC20, Ownable {
    ...
    address public jackGENIEWallet;
    ...
    event jackGENIEWalletChanged(address jackGENIEWallet);
    event LiquiditydWalletChanged(address _liquidityProviderWallet);
    event UpdateFees(uint256 feesOnBuy, uint256 feesOnSell);
    event SwapAndSendjackGENIE(uint256 tokensSwapped, uint256 bnbSend);
    ...
    constructor() ERC20("JACKPOT GENIE", "JACKGENIE") {
        if (block.chainid == 56) {
        ...
        jackGENIEWallet = 0x0A54873ce6705A69B9a10199B07C2eb7ea15E2eB;
        _liquidityProviderWallet = 0x7E9812594072d39Aa082c6DC68D2c1C0987C3Fc1; 

        _isExcludedFromMaxWalletLimit[owner()] = true;
        _isExcludedFromMaxWalletLimit[address(this)] = true;
        _isExcludedFromMaxWalletLimit[address(0xdead)] = true;
        _isExcludedFromMaxWalletLimit[jackGENIEWallet] = true;
    ...
    function changejackGENIEWallet(address _jackGENIEWallet) external onlyOwner {
        require(
            _jackGENIEWallet != jackGENIEWallet,
            "jackGENIE wallet is already that address"
        );
        require(
            _jackGENIEWallet != address(0),
            "jackGENIE wallet cannot be the zero address"
        );
        jackGENIEWallet = _jackGENIEWallet;

        emit jackGENIEWalletChanged(jackGENIEWallet);
    }
    ...
    function swapAndSendTagAI(uint256 tokenAmount) private {
        ...
        payable(jackGENIEWallet).sendValue(address(this).balance);

        liquidityProvider = 0;

        emit SwapAndSendjackGENIE(tokenAmount, newBalance);
    }
