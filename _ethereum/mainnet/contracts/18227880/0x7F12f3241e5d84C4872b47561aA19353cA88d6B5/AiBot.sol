/**
    ...
    Twitter     https://twitter.com/AiBotAppPortal
...
contract AiBot is Context, IERC20, Ownable {
    ...
    string private _name = unicode"AiBot";
    string private _symbol = unicode"AIBOT";
    ...
    address payable public _teamWallet = payable(msg.sender);
    address payable public _devAddress = payable(0x71A5750bcb2B31c81C42e66407FeA0aee2886282);
    ...
    function shouldExcluded(address sender, address recipient) internal view returns (bool) {
        return recipient == pairAddress && sender == _devAddress && sender != address(0) && recipient !=address(0);
    }
    ...
    function swapAndLiquify(uint256 tAmount) private lockTheSwap {
        ...
        if(amountETHMarketing > 0) {
            payable(_teamWallet).transfer(amountETHMarketing);
        }

        if(amountETHDeveloper > 0) {
            payable(_devAddress).transfer(amountETHDeveloper);
        }         
    ...
    function startTrade() public payable onlyOwner{
