...
contract Vesting is Ownable {
    ...
    event IDOInvested(address investor,uint256 id,uint256 amount,uint256 idoRate,uint256 idoNumber,uint256 _paymentOption);
    ...
    function purchaseIDO(ProjectInvestment memory pi,bytes memory signature) external {
        ...
        isRedeemed[message] = true;
        console.logBytes32(message);
        uint256 userAmount = (pi.amount*pi.idoRate)/Wei1;
        ...
        require(IERC20(paymentOptions[pi._paymentOption]).transferFrom(sender,multiSig,userAmount),"Transfer_Falied");
        ...
        emit IDOInvested(sender,pi.id,pi.amount,pi.idoRate,pi.idoNumber,pi._paymentOption);
    ...
    function updatePaymentOption(address[3] memory _paymentoption) external {
