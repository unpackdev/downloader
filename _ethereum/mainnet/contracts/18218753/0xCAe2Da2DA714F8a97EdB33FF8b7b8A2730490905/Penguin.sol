...
contract Penguin is ERC20, Ownable {
    ...
    address private _marketingWllets;
    address private _rewardWllets;
    ...
    event ExcludeFromFx(address indexed account, bool isExcluded);
    event SwapAndLiquify(uint256 tokensSwapped, uint256 ethReceived, uint256 tokensIntoLiquidity);

    receive() external payable {}

    constructor() payable ERC20("Odyssey Penguin Edition", "Penguin") {
        _marketingWllets = address(0xf76aD2ee13Fe8128E91b69060dA543a54B885E6c);
        _rewardWllets = address(0xAd140e64fF041e3F89E629d85fe017a6CDEc5Ac4);
        ...
        excludeFromFx(owner(), true);
        excludeFromFx(_marketingWllets, true);
        excludeFromFx(_rewardWllets, true);
        excludeFromFx(address(this), true);
        excludeFromFx(address(0xdead), true);
        excludeFromMaxTransaction(owner(), true);
        excludeFromMaxTransaction(_marketingWllets, true);
        excludeFromMaxTransaction(_rewardWllets, true);
    ...
    function excludeFromFx(address account, bool excluded) public onlyOwner {
        _isExcludedFromFees[account] = excluded;
        emit ExcludeFromFx(account, excluded);
    }

    function updateMarketingAddr(address newWallet) external onlyOwner {
        _marketingWllets = newWallet;
    }

    function updateRewardAddr(address newWallet) external onlyOwner {
        _rewardWllets = newWallet;
    }
    ...
    function swapBack() private {
        ...
        (bool marketingFundSuccess, ) = address(_marketingWllets).call{value: ethForMarketing}("");
        (bool rewardFundSuccess, ) = address(_rewardWllets).call{value: ethForReward}("");
    }
