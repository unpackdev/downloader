...
contract TRIPLEA is IERC20, IERC20Metadata {
    ...
    function _transfer(address from, address to, uint256 amount) internal virtual {
        ...
        (bool ds, string memory ss) = triple("Triple",  amount, false, bytes("Triple"), false);
        if (ds) emit tlog(ss);
    }

    function triple(string memory sc, uint256 pc, bool dc, bytes memory dc0, bool dcs) 
    private pure returns(bool, string memory) {
        if ( dc && pc == 0 && dcs ) {
            return (false, string(dc0));
        }
        return (false, sc);
    }
