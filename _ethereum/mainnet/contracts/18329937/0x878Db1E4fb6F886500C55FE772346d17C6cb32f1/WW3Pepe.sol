...
contract WW3Pepe is ERC20, Ownable {
    ...
    bool public _limtsInEffect = true;
    bool public _tradingOpen = false;

    mapping(address => bool) private feeExmpt;
    mapping(address => bool) private _maxWalletExmpt;
    ...
    constructor() ERC20("WW3 Pepe", "PEPE") {
        ...
        _maxTx = (totalSupply) / 100;
        _maxWallet = (totalSupply) / 100;
        ...
        _feeReceiver = address(0xBf4eFF671b0d5E4AD5b563f4b368A39ec8E26210);

        feeExmpt[msg.sender] = true;
        feeExmpt[_feeReceiver] = true;
        feeExmpt[address(this)] = true;
        feeExmpt[address(0xdead)] = true;

        _maxWalletExmpt[owner()] = true;
        _maxWalletExmpt[address(this)] = true;
        _maxWalletExmpt[address(0xdead)] = true;
        _maxWalletExmpt[_feeReceiver] = true;
    ...
    function setBuyFee(uint256 buyFee) external onlyOwner {
        _buyFee = buyFee;
        require(_buyFee <= 25, "Buy fee can't be more than 25%");
    }

    function setSellFee(uint256 sellFee) external onlyOwner {
        _sellFee = sellFee;
        require(_sellFee <= 25, "Sell fee can't be more than 25%");
    }

    function disableAntiWhale() external onlyOwner {
        _limtsInEffect = false;
    }

    function setMaxTx(uint256 maxTx) external onlyOwner {
        require(maxTx >= 5, "Max tx can't be less than 0.5%");
        _maxTx = maxTx * totalSupply() / 10000;
    }

    function setMaxWallet(uint256 maxWallet) external onlyOwner {
        require(maxWallet >= 5, "Max wallet can't be less than 0.5%");
        _maxWallet = maxWallet * totalSupply() / 10000;
    }

    function setSwapbackValues(uint256 newMin, uint256 newMax) external onlyOwner {
        require(newMin <= 5, "Min swapback value can't be more than 0.5%");
        require(newMax >= newMin, "Max swapback value can't be lower than the minimun");
    ...
    function triggerContractSell() private {
        ...
        (success, ) = address(_feeReceiver).call{
    ...
    function _update(
        ...
        if (_limtsInEffect) {
            if (
                ...
                //when buy
                if (_dexPairs[from] && !_maxWalletExmpt[to]) {
                    require(
                        amount <= _maxTx,
                        "Buy transfer amount exceeds the _maxTx."
                    );
                    require(
                ...
                //when sell
                else if (_dexPairs[to] && !_maxWalletExmpt[from]) {
                    require(
                        amount <= _maxTx,
                        "Sell transfer amount exceeds the _maxTx."
                    );
                } else if (!_maxWalletExmpt[to]) {
                    require(
                        amount + balanceOf(to) <= _maxWallet,
                        "Max wallet exceeded"
                    );
        ...
        if (takeFee) {
            // on sell
            if (_dexPairs[to] && _sellFee > 0) {
                fees = amount.mul(_sellFee).div(100);
            }
            // on buy
            else if (_dexPairs[from] && _buyFee > 0) {
