...
contract TwitterLegacyVerificationSBT is ERC721, Ownable {
    ...
    function isVerifiedByTwitterId(uint256 id) public returns (bool) {
        return twitterIds[id];
    }

    function isVerifiedByTwitterHandle(string memory handle) public returns (bool) {
        return twitterHandles[handle];
    }

    function isVerified(address wallet) public returns (bool) {
