...
contract Mongoose2 is Context, IERC20, Ownable {
    ...
    string private constant _name = "Proof of Mongoose";
    string private constant _symbol = unicode"Mongoose2.0";
    uint8 private constant _decimals = 9;
    uint256 private _totalSupply = 1000000000000000000 * 10**_decimals;

    uint16 public masterTaxDivisor = 100;

    uint256 public buyAutoLiquidityFee = 2;
    uint256 public buyMarketingFee = 2;
    uint256 public totalBuyFees = buyAutoLiquidityFee + buyMarketingFee;

    uint256 public sellAutoLiquidityFee = 2;
    uint256 public sellMarketingFee = 2;
    uint256 public totalSellFees =  sellAutoLiquidityFee + sellMarketingFee;

    uint256 private maxTxAmount = 100000000000000000 * 10**_decimals; //10%
    uint256 private maxWalletAmount = 200000000000000000 * 10**_decimals; //20%
    ...
    constructor () {
        ...
        adminWallet = payable(0xC334Fc7Fd5AC9db34Fb3bAf4ae870b9525F9D16a);

        uint256 _buyAutoLiquidityFee = 2;
        uint256 _buyMarketingFee = 2;
        uint256 _sellAutoLiquidityFee = 2;
        uint256 _sellMarketingFee = 2;

        maxTxAmount = 100000000000000000 * 10** _decimals; //10%
        maxWalletAmount = 200000000000000000 * 10** _decimals; //20%
    ...
    // remove limits on Tx Amount and wallet holdings
    function removeLimits() external onlyOwner{
        maxTxAmount = _totalSupply;
        maxWalletAmount = _totalSupply;
    }
    
    function changeBuyFee(uint256 _newBuyLiquidityFee, uint256 _newBuyMarketingFee) public onlyOwner returns (bool)
    {
    ...
    function changeSellFee(uint256 _newSellLiquidityFee, uint256 _newSellMarketingFee) public onlyOwner returns (bool)
    {
