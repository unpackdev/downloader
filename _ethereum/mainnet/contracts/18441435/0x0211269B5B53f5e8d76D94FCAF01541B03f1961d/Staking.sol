...
contract LPMoneyStaking is Ownable, ReentrancyGuard {
    using SafeMath for uint256;
    ...
    function pendingReward(address _user) external view returns (uint256) {
        ...
        if (block.number > lastRewardBlock && tokenStaked != 0) {
            ...
            uint256 adjustedTokenPerShare =
                accTokenPerShare.add(TokenReward.mul(PRECISION_FACTOR).div(tokenStaked));
    ...
    function _updatePool() internal {
        ...
        uint256 multiplier = _getMultiplier(lastRewardBlock, block.number);
        uint256 TokenReward = multiplier.mul(rewardPerBlock);
        accTokenPerShare = accTokenPerShare.add(TokenReward.mul(PRECISION_FACTOR).div(tokenStaked));
        lastRewardBlock = block.number;
    }
