...
contract ArtTokenVault is AccessControlUpgradeable, IArtTokenVault {
    ...
    event ArtBuyoutCreated(address buyoutAddress, address buyer);
    ...
    /// @notice the ERC20 ArtToken address of the vault's token
    IArtToken private tokenAT;
    string private tokenNameAT;
    string private tokenSymbolAT;

    /// @notice the address of the LISA Protocol settings contract
    ...
    /// @notice the URI of the artwork metadata stored in permanent storage
    string public artworkMetadataURI;
    ...
    function initialize(
        ...
        artworkMetadataURI = metadataURI;
    ...
    function _deployArtToken() private {
        ...
        newAtToken.initialize(tokenNameAT, tokenSymbolAT, address(nftContract));
        nftContract.setArtToken(address(newAtToken));
        tokenAT = newAtToken;
    }
    ...
    function deploySimpleCrowdsale(
        ...
        address(newCrowdsale).functionCall(
            abi.encodeWithSignature(
                "initialize((uint256,address,address,address,uint256,uint256,uint256,uint256,uint256,uint256,address,string))",
                params
            )
        );
    ...
    function deployProportionalCrowdsale(
        ...
        address(newCrowdsale).functionCall(
            abi.encodeWithSignature(
                "initialize((uint256,address,address,address,uint256,uint256,uint256,uint256,uint256,uint256,uint256,address,string))",
                params
            )
        );
        emit ERC20CrowdsaleCreated(
    ...
    /// Removes AT token address and sets crowdsale to zero address.
