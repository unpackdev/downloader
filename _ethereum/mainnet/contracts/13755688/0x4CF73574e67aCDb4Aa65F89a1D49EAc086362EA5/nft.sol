...
contract nft is ERC721, Ownable {
    ...
    constructor() ERC721('Trippy Mandrills Club', 'TMC') {
        numberOfTotalTokens = 0;
        maxTotalTokens = 9000;

        unrevealedURI = "ipfs://QmSTuJfvPgFGrGHsNXATSeC23KC2ju2JEJ1JLXo4BvoS8T/";
    }
    ...
    function tokenId() internal view returns(uint256) {
        uint currentId = totalSupply() + 150 - _reservedMints;
        bool exists = true;
        while (exists) {
            currentId += 1;
    ...
    */
    ...
    function reservedMint(uint tokenId_, address recipient) public onlyOwner {
        require(_reservedMints < maxReservedMints, "Not enough Reserved NFTs for Creator left to mint..");
        require(!_exists(tokenId_), 'Token has already been Minted!');
        require(tokenId_ <= 150 && tokenId_ > 0, 'Token is out of the 1-150 range reserved for Giveaways!');
    ...
    function withdraw() public onlyOwner {
        ...
        _withdraw(shareholder1, (balance * 50) / 100);
        _withdraw(shareholder2, accountBalance()); //to avoid dust eth, gets a little more than 20%
    }
