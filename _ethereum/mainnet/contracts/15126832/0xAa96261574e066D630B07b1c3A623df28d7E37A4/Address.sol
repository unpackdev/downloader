...
contract StakingPool is Ownable, Pausable {
    ...
    struct UserInfo {
        ...
        uint tokenAtLastUserAction; // keeps track of token deposited at the last user action
    ...
    uint public constant MAX_PERFORMANCE_FEE = 2000; // 20%
     ...
     */
    constructor( IERC20 _token, IERC20 _receiptToken, IMasterChef _masterchef, address _admin, address _treasury ) {
        token = _token;
    ...
    function harvest() external notContract whenNotPaused {
        ...
        uint bal = available();
        uint currentPerformanceFee = bal.mul(performanceFee).div(10000);
        token.safeTransfer(treasury, currentPerformanceFee);

        uint currentCallFee = bal.mul(callFee).div(10000);
    ...
    /**
     * @notice Withdraw unexpected tokens sent to the StakingPool
     */
     ...
     */
    function calculateHarvestRewards() external view returns (uint) {
        uint amount = IMasterChef(masterchef).pendingTokens(0, address(this));
        amount = amount.add(available());
        uint currentCallFee = amount.mul(callFee).div(10000);
     ...
     */
    function calculateTotalPendingRewards() external view returns (uint) {
        uint amount = IMasterChef(masterchef).pendingTokens(0, address(this));
        amount = amount.add(available());
    ...
    /**
     * @notice Withdraws from funds from the StakingPool
    ...
    function withdraw(uint _shares) public notContract {
        ...
        uint currentAmount = (balanceOf().mul(_shares)).div(totalShares);
        user.shares = user.shares.sub(_shares);
        totalShares = totalShares.sub(_shares);

        uint bal = available();
        if (bal < currentAmount) {
            uint balWithdraw = currentAmount.sub(bal);
            IMasterChef(masterchef).leaveStaking(balWithdraw);
            uint balAfter = available();
            uint diff = balAfter.sub(bal);
            if (diff < balWithdraw) {
        ...
        if (block.timestamp < user.lastDepositedTime.add(withdrawFeePeriod)) {
            uint currentWithdrawFee = currentAmount.mul(withdrawFee).div(10000);
    ...
    function balanceOf() public view returns (uint) {
        (uint amount, ) = IMasterChef(masterchef).userInfo(0, address(this));
