...
contract ApeUloanClaimer is ERC721Enumerable
{
 ...
 constructor() ERC721("MysteryLoanNft", "MYSLOAN") // temporary Symbol and title
 {
 ...
 uint256 potentialBoxCount = (apeCoinAmount/10000 ether) + 1;

 if (potentialBoxCount > 5) potentialBoxCount = 5; // Too many to claim

 uint256 allowedBoxCount = 0;
 if (claimedBoxCount < potentialBoxCount) allowedBoxCount = potentialBoxCount - claimedBoxCount;
 ...
 if (claimedBoxCount < potentialBoxCount) 
 {
 allowedBoxCount = potentialBoxCount - claimedBoxCount;
 }
 }
 ...
 //---------------------------------------------------------------------------
 function massSend(address[] memory recipients, uint256[] memory tokenIds) external onlyOwner
 {
