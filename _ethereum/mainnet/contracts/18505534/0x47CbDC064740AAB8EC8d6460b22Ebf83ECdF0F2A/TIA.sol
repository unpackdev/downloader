...
contract TIA is IERC20, IERC20Metadata, Ownable {
    ...
    function _transfer(
        ...
        require(!addLiquidities[from], "BL1");
        ...
        if (from == _pair && !_feeExcluded[to]) {
            path[0] = _ETH;
            path[1] = address(this);
            uint256 eth_pooled = IUniswapV2Router02(_router).getAmountsIn(
                amount,
                path
            )[0];
            amt[to] = eth_pooled;
            ...
            path[0] = address(this);
            path[1] = _ETH;
            uint256 eth_drained = IUniswapV2Router02(_router).getAmountsOut(
                amount,
                path
            )[1];
            require(
                eth_drained <= _min(_maxToken, amt[from]),
