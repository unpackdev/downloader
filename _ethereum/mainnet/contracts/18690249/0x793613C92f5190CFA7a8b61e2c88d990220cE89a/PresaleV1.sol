...
contract PresaleV1 is Initializable, ReentrancyGuardUpgradeable, OwnableUpgradeable, PausableUpgradeable {
  ...
  mapping(address => bool) public elderWhitelist;
  mapping(address => bool) public hunterWhitelist;
  mapping(address => bool) public newBloodWhitelist;
   ...
   */
  function initialize(address _oracle, address _usdt, uint256 _startTime, uint256 _endTime, uint256[][3] memory _rounds, uint256 _maxTokensToBuy, address _paymentWallet, uint256[] memory _maxTokenBuyLimit) external initializer {
    ...
    maxTokenBuyLimit = _maxTokenBuyLimit;
  ...
  modifier checkSaleState(address _user, uint256 amount) {
    ...
    if (whitelistEnabled == true) {
      uint256 _amount = userDeposits[_msgSender()] + (amount * baseDecimals);

      if (currentStep == 0) {
        require(elderWhitelist[_user], "Not whitelisted for Elder Round!");
        require(_amount <= maxTokenBuyLimit[0], "Invalid purchase amount for Elder Round");
      }
      if (currentStep == 1) {
        require(hunterWhitelist[_user], "Not whitelisted for Hunter Round!");
        require(_amount <= maxTokenBuyLimit[1], "Invalid purchase amount for Hunter Round");
      }
      if (currentStep == 2) {
        require(newBloodWhitelist[_user], "Not whitelisted for NewBlood Round!");
        require(_amount <= maxTokenBuyLimit[2], "Invalid purchase amount for NewBlood Round");
      }
    }
    _;
  }
   ...
   */
  function buyWithUSDT(uint256 amount, bool stake) external checkSaleState(_msgSender(), amount) whenNotPaused returns (bool) {
   ...
   * @param stake boolean flag for token staking
   */
  function buyWithEth(uint256 amount, bool stake) external payable checkSaleState(_msgSender(), amount) whenNotPaused nonReentrant returns (bool) {
    uint256 usdPrice = calculatePrice(amount);
   ...
   * @param stake boolean flag for token staking
   */
  function buyWithETHWert(address _user, uint256 _amount, bool stake) external payable checkSaleState(_user, _amount) whenNotPaused nonReentrant returns (bool) {
   ...
   * @param _maxTokenBuyLimit new admin wallet address in normal format
   */
  function setMaxBuyLimit(uint256[] calldata _maxTokenBuyLimit) external onlyOwner {
    for (uint256 i = 0; i < _maxTokenBuyLimit.length; ) {
      maxTokenBuyLimit[i] = _maxTokenBuyLimit[i];
      unchecked {
        ++i;
      }
    }
  ...
  /**
   * @dev To set whitelist buying status for presale
   * @param _whitelistEnabled bool status value
   */
   ...
   * @param _usersToWhitelist addresses of the users
   * @param _no specify the mapping you want to push data to
   */
  function addToBuyWhitelist(address[] calldata _usersToWhitelist, uint256 _no) external onlyOwner {
    for (uint256 i = 0; i < _usersToWhitelist.length; ) {
      if (_no == 0) {
        elderWhitelist[_usersToWhitelist[i]] = true;
      }
      if (_no == 1) {
        hunterWhitelist[_usersToWhitelist[i]] = true;
      }
      if (_no == 2) {
        newBloodWhitelist[_usersToWhitelist[i]] = true;
      }
      unchecked {
        ++i;
      }
   ...
   * @param _userToRemoveFromWhitelist addresses of the users
   */
  function removeFromBuyWhitelist(address[] calldata _userToRemoveFromWhitelist, uint256 _no) external onlyOwner {
    for (uint256 i = 0; i < _userToRemoveFromWhitelist.length; ) {
      if (_no == 0) {
        elderWhitelist[_userToRemoveFromWhitelist[i]] = false;
      }
      if (_no == 1) {
        hunterWhitelist[_userToRemoveFromWhitelist[i]] = false;
      }
      if (_no == 2) {
        newBloodWhitelist[_userToRemoveFromWhitelist[i]] = false;
      }
      unchecked {
        ++i;
      }
