...
contract Marketplace is Ownable, ReentrancyGuard, DelegateCashEnabled
{
    ...
    struct SaleTypeFixedPrice
    {
        ...
        uint _MintPackAmount;         // [8] -> _MintPackAmount (Transfer Amount Per Purchase)
        address _NFT;                 // [9] -> _NFT
    ...
    mapping(uint=>uint) public ETHRevenueWithdrawn;
   ...
   /**
     ...
     * @param SaleIndex The Sale Index To Edit
     * @param LiveMintProjectIDs The LiveMint ProjectIDs To Edit
     */
     ...
     * @param Recipient The Recipient Of The ERC721s
     * @param TokenIDs The TokenIDs To Withdraw
     */
    function ____WithdrawERC721(address Contract, address Recipient, uint[] calldata TokenIDs) external onlyOwner 
    { 
    ...
    function __FinalizeFixedPriceSale(
        ...
        else if (FixedPriceSales[SaleIndex]._Type == 1) // Factory MintPass MintPack Direct Mint
        {
            for(uint x; x < DesiredAmount; x++)
            {
                IMinter(FixedPriceSales[SaleIndex]._NFT)._MintToFactory(
                    FixedPriceSales[SaleIndex]._MintPassProjectID, 
                    Purchaser, 
                    FixedPriceSales[SaleIndex]._MintPackAmount
                ); 
            }
        }
        ...
        else if (FixedPriceSales[SaleIndex]._Type == 5) // ERC721 transferFrom() Sale
        {
            ...
            for(uint x; x < DesiredAmount; x++) { _NFT.transferFrom(_Operator, Purchaser, _StartingIndex + x); }
