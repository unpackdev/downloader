...
contract TheUniverseOfSmilesX is ERC721A, Ownable, ERC2981, DefaultOperatorFilterer {
    using Strings for uint256;
    ...
    uint256 public publicSaleCost = 0.07 ether; // This is the Normal Price Per NFT
    uint256 public max_per_wallet = 10; // Maximum NFTs can be minted Per Wallet
    uint256 public presaleCost = 0.04 ether; // Price of an NFT in the Presale per a wallet
    uint256 public presale_per_wallet = 2; // Number of NFTs that can be minted in Presale
    uint256 public total_presale_limit = 200; // Number of NFTs you allocate for Presale
    uint256 public total_presale_count;  
    uint256 public whitelistCost = 0.02 ether; // Price of an NFT in the WL
    uint256 public wl_per_wallet = 1; // Number of NFTs that can be minted in WL per a wallet     
    uint256 public total_wl_limit = 200; // Number of NFTs you allocate for WL
    uint256 public PUBLIC_MAX_SUPPLY = 9000; // Number of NFTs in the Collection    
    uint256 public total_wl_count;    
    uint256 private total_owner_minted;
    ...
    constructor(string memory _initBaseURI, string memory _initNotRevealedUri, string memory _contractURI) ERC721A("The Universe Of SmilesX", "TUOS") {
     
    setBaseURI(_initBaseURI);
    setNotRevealedURI(_initNotRevealedUri);   
    setRoyaltyInfo(owner(),500);
    contractURI = _contractURI;  
    ownerMint(0xf68a0d541587B1eff0Cf40bb1215c50a7EBfF3d9,1000);
    }
    ...
    function mint(uint256 quantity) public payable  {
        require((totalSupply() - total_owner_minted) + quantity <= PUBLIC_MAX_SUPPLY,"No More NFTs to Mint");
        
        if (msg.sender != owner()) {

            require(public_mint_status, "Public mint status is off");           
            require(wlMinted[msg.sender] + publicSaleMinted[msg.sender] + presaleMinted[msg.sender] + quantity <= max_per_wallet, "Per Wallet Limit Reached");
    ...
    function ownerMint(address ownerAddress, uint256 quantity) public onlyOwner  {
        
        _safeMint(ownerAddress, quantity);
        total_owner_minted = total_owner_minted + quantity;
    ...
    function presale(uint256 quantity) public payable  {
        ...
        if (msg.sender != owner()) {
            ...
            require(presaleMinted[msg.sender] + quantity <= presale_per_wallet, "Presale Limit per Wallet Reached");
            require(wlMinted[msg.sender] + publicSaleMinted[msg.sender] + presaleMinted[msg.sender] + quantity <= max_per_wallet, "Per Wallet Limit Reached");
    ...
    function setPresale_per_wallet(uint256 _presale_per_wallet) external onlyOwner {
        presale_per_wallet = _presale_per_wallet;
    }
