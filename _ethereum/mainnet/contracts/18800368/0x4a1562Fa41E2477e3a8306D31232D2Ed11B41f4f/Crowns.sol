...
contract Crowns is Initializable, ERC20Upgradeable {
    ...
    struct VestingSchedule {
        ...
        uint256 tgePercent;
        string pool;
        string id; // Optional: ID of the vesting schedule, to set schedules programmatically
    }
    ...
    // emitted when user mint tokens manually
    // @param address The address of the user
    ...
    function mintInStablecoin(
        ...
        // Scaling factor is 1e18 - 1e6 + 1e5 = 1e17
        uint256 tokenAmount = (stablecoinAmount / stagePrices[saleStage]) *
            1e17;
    ...
    function mintForAdmin(
        ...
        emit ManualMint(msg.sender, to, amount, pool);
    }

    /**
     * @dev Deletes a vesting schedule for a specific ID.
     * Only the admin can call this function.
     * @param _address Address of the staker whose vesting schedule is to be deleted.
     * @param _id ID of the vesting schedule to delete.
     */
    function deleteVestingSchedule(
        ...
        require(
            vestingScheduleExists(_address, _id),
            "Vesting schedule does not exist"
        );

        VestingSchedule[] storage schedules = vestingSchedules[_address];
        for (uint256 i = 0; i < schedules.length; i++) {
            if (
                keccak256(abi.encodePacked(schedules[i].id)) ==
    ...
    /**
     * @dev Checks if a vesting schedule with a specific ID exists for an address.
     * @param _address Address to check the vesting schedule for.
     * @param _id ID of the vesting schedule to check.
     * @return exists True if the vesting schedule exists, false otherwise.
     */
    function vestingScheduleExists(
        ...
        exists = false;
        VestingSchedule[] memory schedules = vestingSchedules[_address];
        for (uint256 i = 0; i < schedules.length; i++) {
            if (
                keccak256(abi.encodePacked(schedules[i].id)) ==
                keccak256(abi.encodePacked(_id))
    ...
    function setVestingSchedule(
        ...
        // Require that either the id string empty or vesting schedule not exists
        require(
            bytes(id).length == 0 || !vestingScheduleExists(staker, id),
            "Non-empty ID already exists in vesting schedule"
        );
        ...
        emit NewVestingSchedule(staker, totalAmount, pool, 0, "admin", id);
    }
    ...
    /**
     * @dev Returns all vesting schedules for a given address.
     * @param beneficiary address to retrieve the vesting schedules for
     */
    ...
    /**
     * @dev Returns the vesting schedule count for a given address.
     * @param beneficiary address to retrieve the vesting schedule count for.
     */
