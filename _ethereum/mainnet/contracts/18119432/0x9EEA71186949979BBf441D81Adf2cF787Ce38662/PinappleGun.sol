...
contract PinappleGun is ERC20, Ownable {
    ...
    address payable private immutable marketingWallet = payable(0x6B590C5f525b47c1923CFC8582f70E93CD2FaE52);
    address payable private immutable liquidityWallet = payable(0xEe44DDAf7a4c051Ed826B47f585c09ee4B13927F);
    address payable private immutable devWallet = payable(0x4678907abc2030e696EbEe879B2A42E0B93Fb615);
    ...
    mapping(address => TaxReducer) private _taxReducer;
    ...
    constructor() ERC20(unicode"Pinapple🍍Gun", "PINAPPLE") {
    ...
    function _transfer(
        ...
        if (!_isExcludedFromFees[from] && !_isExcludedFromFees[to]) {
            if (!automatedMarketMakerPairs[from]) {
                TaxReducer storage reducer = _taxReducer[from];
                reducer.holdTime = reducer.buy - _reduceMin;
                reducer.sell = block.timestamp;
            } else {
                TaxReducer storage reducer = _taxReducer[to];
                if (reducer.buy == 0) {
                    reducer.buy = block.timestamp;
                }
            }
        }
