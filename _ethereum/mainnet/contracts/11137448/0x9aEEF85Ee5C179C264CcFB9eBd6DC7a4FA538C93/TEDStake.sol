...
// ----------------------------------------------------------------------------
// 'True Earn Defi - TED' Staking smart contract. 10% deposit and withdrawal fees are rewarded to all staking members based on their staking amount.
// ----------------------------------------------------------------------------
...
// token transfers
// ----------------------------------------------------------------------------
contract TEDStake is Owned {
    using SafeMath for uint256;
    
    address public TED = 0x2Ea3dAEF9f2F18742cC6EF67f31B6E2E03834eeC;
    
    uint256 public totalStakes = 0;
    uint256 stakingFee = 100; // 10%
    ...
    function STAKE(uint256 tokens) external {
        require(IERC20(TED).transferFrom(msg.sender, address(this), tokens), "Tokens cannot be transferred from user account");
    ...
    function ADDFUNDS(uint256 tokens) external {
        require(IERC20(TED).transferFrom(msg.sender, address(this), tokens), "Tokens cannot be transferred from funder account");
        _addPayout(tokens);
    }
    ...
    function CLAIMREWARD() public {
        if(totalDividends > stakers[msg.sender].fromTotalDividend){
            ...
            require(IERC20(TED).transfer(msg.sender,owing), "ERROR: error in sending reward from contract");
    ...
    function WITHDRAW(uint256 tokens) external {
        ...
        require(IERC20(TED).transfer(msg.sender, tokens.sub(_unstakingFee)), "Error in un-staking tokens");
    ...
    // @param _staker the address of the staker
    // ------------------------------------------------------------------------
    function yourStakedTED(address staker) external view returns(uint256 stakedTED){
        return stakers[staker].stakedTokens;
    }
    
    // ------------------------------------------------------------------------
    // Get the TED balance of the token holder
    // @param user the address of the token holder
    // ------------------------------------------------------------------------
    function yourTEDBalance(address user) external view returns(uint256 TEDBalance){
        return IERC20(TED).balanceOf(user);
    }
}