...
contract convexCCYeller is Ownable {
    ...
    AggregatorV3Interface internal priceFeedCRV;
    ...
    function getCRVData() public view returns (uint) {
        (, int price, , , ) = priceFeedCRV.latestRoundData();
        uint decimalsPriceFeed = priceFeedCRV.decimals();
        uint divisor = 10**decimalsPriceFeed;
        uint crvUsdtPrice = (uint(price) * 1e18) / divisor;
    ...
    function getLpValue() public view returns (uint, uint) {
        ...
        uint crvUsdtPrice = getCRVData();

        uint lastPriceInCRV = LPToken(lpToken).last_price();
        uint lpTokenPriceUsdt = crvUsdtPrice * 1e18 / lastPriceInCRV;
