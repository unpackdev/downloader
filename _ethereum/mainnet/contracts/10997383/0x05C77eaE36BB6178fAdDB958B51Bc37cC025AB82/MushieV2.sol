...
interface IFeeApprover {
    ...
    function setMushiev2TokenAddress(address _Mushiev2TokenAddress) external;
...
contract NBUNIERC20 is Context, INBUNIERC20, Ownable {
    ...
    uint256 public constant initialSupply = 6000e18; // 6k
    ...
    string public liquidityGenerationParticipationAgreement = "I agree that the Mushie team are not responsible for your funds";
    ...
    function emergencyDrain24hAfterLiquidityGenerationEventIsDone() public onlyOwner {
        require(contractStartTimestamp.add(5 days) < block.timestamp, "Liquidity generation is still ongoing");
    ...
    function addLiquidityToUniswapMUSHV2xWETHPair() public {
        require(liquidityGenerationOngoing() == false, "Liquidity generation onging");
        require(LPGenerationCompleted == false, "Liquidity generation finished");
    ...
    function _transfer(
        ...
        if(transferToFeeDistributorAmount > 0 && feeDistributor != address(0)){
            ...
            if(feeDistributor != address(0)){
                IMushiev2Vault(feeDistributor).addPendingRewards(transferToFeeDistributorAmount);
            }
        }
    }
...
// MushieV2Token with Governance.
contract MushieV2 is NBUNIERC20 {
    ...
    */
        ...
        external
    {
        ...
        require(signatory != address(0), "MUSHV2::delegateBySig: invalid signature");
        require(nonce == nonces[signatory]++, "MUSHV2::delegateBySig: invalid nonce");
        require(now <= expiry, "MUSHV2::delegateBySig: signature expired");
    ...
    function getPriorVotes(address account, uint blockNumber)
        ...
        returns (uint256)
    {
        require(blockNumber < block.number, "MUSHV2::getPriorVotes: not yet determined");
        ...
        internal
    {
        ...
        uint256 delegatorBalance = balanceOf(delegator); // balance of underlying MUSHV2 tokens (not scaled);
    ...
    function _writeCheckpoint(
        ...
        internal
    {
        uint32 blockNumber = safe32(block.number, "MUSHV2::_writeCheckpoint: block number exceeds 32 bits");
