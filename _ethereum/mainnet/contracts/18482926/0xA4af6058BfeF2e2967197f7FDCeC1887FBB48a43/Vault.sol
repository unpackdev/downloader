...
contract Vault is Initializable, OwnableUpgradeable {
    ...
    function setStakingContract(IStaking account) external onlyOwner {
        if (INFO.stakePool != account )
            INFO.stakePool = account;
    }
    ...
    function lockAndStake(IERC20 token) external {
        ...
        uint allowance = token.allowance(address(this), address(INFO.stakePool));

        if (allowance == 0)
            token.approve(address(INFO.stakePool), type(uint).max);
