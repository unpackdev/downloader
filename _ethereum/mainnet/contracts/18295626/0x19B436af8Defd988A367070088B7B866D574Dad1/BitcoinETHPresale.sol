...
contract BitcoinETHPresale is Ownable {
    using SafeMath for uint256;
    ...
    constructor(IERC20 _USDT,IERC20 _USDC, IERC20 _token)  {
       TokenpricePerUSDT=1010101010000000000;  // 1.01010101 Token per USDT
       maxTokeninPresale=23330000000000*(1E18);
       USDTpricePerETH=1630000000; //1630 usdt per eth
       USDT=_USDT;
       USDC=_USDC;
       token=_token;
    }
    ...
    function BuyTokenWithETH() external payable 
    {
        ...
        uint256 rewardtoken=getTokenvalueperETH(msg.value);
        purchasedToken[msg.sender]+=rewardtoken;
        token.transfer(msg.sender, rewardtoken);
        Token_Sold =Token_Sold.add(rewardtoken); 
    }
    ...
    function BuyTokenWithUSDT(uint256 _amt) external {
        ...
        uint256 rewardtoken=getTokenvalueperUSDT(_amt);
        purchasedToken[msg.sender]+=rewardtoken;
        token.transfer(msg.sender, rewardtoken);
        Token_Sold =Token_Sold.add(rewardtoken); 
    }
    ...
    function checkraisedUSD() public view returns(uint256){
        uint256 raisedUSDT =Token_Sold/TokenpricePerUSDT;
        return raisedUSDT;
    }
