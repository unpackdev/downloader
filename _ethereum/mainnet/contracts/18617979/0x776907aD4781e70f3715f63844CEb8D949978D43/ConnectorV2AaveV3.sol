...
contract ConnectorV2AaveV3 {
    ...
    function delegateDoughCall(uint256 _actionId, address _token, uint256 _amount, bool _opt) external payable {
        ...
        if (_token == address(0)) revert CustomError("ConnectorV2AaveV3: invalid token1");
        ...
        if (_actionId == 0) {
            ...
            if (_token == _WETH) {
                ...
                IERC20(_token).safeTransferFrom(owner, address(this), _amount + _feeAmount);
            }
            // send Fee to Treasury
            if (_feeAmount > 0) {
                IERC20(_token).safeTransfer(IDoughV2Index(doughV2Index).treasury(), _feeAmount);
            }
            // Supply to AaveV3
            IERC20(_token).approve(address(_I_AAVE_V3_POOL), _amount);
            _I_AAVE_V3_POOL.supply(_token, _amount, address(this), 0);
        } else if (_actionId == 1) {
            uint256 _withdrawAmount = _amount;
            if (_opt) {
                (_withdrawAmount, , , , , , , , ) = _I_AAVE_V3_DATA_PROVIDER.getUserReserveData(_token, address(this));
            }
            // Withdraw from AaveV3
            _I_AAVE_V3_POOL.withdraw(_token, _withdrawAmount, address(this));
            // Check Health Factor
            ...
            if (_token == _WETH) {
                ...
                IERC20(_token).safeTransfer(owner, _withdrawAmount - _feeAmount);
            }
        } else if (_actionId == 2) {
            // Borrow from AaveV3
            _I_AAVE_V3_POOL.borrow(_token, _amount, 2, 0, address(this));
            // Check Health Factor
            (, , , , , uint256 _healthFactor) = _I_AAVE_V3_POOL.getUserAccountData(address(this));
            (, uint256 _shieldDsaHfTrigger, , ) = IDoughV2Index(doughV2Index).getShieldInfo(address(this));
            if (_healthFactor <= _shieldDsaHfTrigger) revert CustomError("error: HealthFactor <=  hfTrigger");
            ...
            if (_token == _WETH) {
                ...
                IERC20(_token).safeTransfer(owner, _amount - _feeAmount);
            }
        } else {
            uint256 _repayAmount = _amount;
            if (_opt) {
                (, , _repayAmount, , , , , , ) = _I_AAVE_V3_DATA_PROVIDER.getUserReserveData(_token, address(this));
            }
            // get Fee to Treasury
            ...
            if (_token == _WETH) {
                ...
                IERC20(_token).safeTransferFrom(owner, address(this), _repayAmount + _feeAmount);
            }
            // send Fee to Treasury
            if (_feeAmount > 0) {
                IERC20(_token).safeTransfer(IDoughV2Index(doughV2Index).treasury(), _feeAmount);
            ...
            // Repay to AaveV3
            IERC20(_token).approve(address(_I_AAVE_V3_POOL), _repayAmount);
            _I_AAVE_V3_POOL.repay(_token, _repayAmount, 2, address(this));
        }
    }
}
