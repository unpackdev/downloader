    ...
    AccessControlUpgradeable
{
    ...
    bytes32 public constant ENTREPRENEUR_ADMIN_ROLE = keccak256("ENTREPRENEUR_ADMIN_ROLE");
    event FundWithdrawn(
    ...
    function initialize(address _superAdmin) public initializer {
        ...
        _setRoleAdmin(ENTREPRENEUR_ROLE, ENTREPRENEUR_ADMIN_ROLE);
    }
    ...
    /**
     * @notice Provides functionality to check if the given account has Entrepreneur admin role .
     * @param   _account  .
     * @return  bool  .
     */
    function isEntrepreneurAdmin(address _account) external view returns (bool) {
        return hasRole(ENTREPRENEUR_ADMIN_ROLE, _account);
    }

    /**
     * @notice Provides functionality to update the role admin by Default admin only.
     * @param   role  .
     * @param   adminRole  .
     */
    function updateRoleAdmin(bytes32 role, bytes32 adminRole) external onlyRole(DEFAULT_ADMIN_ROLE) {
