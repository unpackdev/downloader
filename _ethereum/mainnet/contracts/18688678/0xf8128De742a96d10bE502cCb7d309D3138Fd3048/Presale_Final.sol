...
contract BLIVPreSale is Initializable, ReentrancyGuardUpgradeable, OwnableUpgradeable {
    ...
    bool public publicSaleMode; 
    uint256 public publicSaleAmt;
    ...
    function initialize(address _usdt, address _payoutAddress) external initializer {
        ...
        publicSaleAmt = 5000000;
        //todo: CHANGE TO MAINNET ONE;
        aggregatorInterface = AggregatorV3Interface(0xD4a33860578De61DBAbDc8BFdb98FD742fA7028e); 
    }
        ...
        returns (bool)
    {
        ...
        sendValue(payable(payoutAddress), ethAmount);
    ...
    function claimableAmount(address user, uint256 _id)
        ...
        //change total amount to BLIVX balance
    {
        Vesting memory _user = userVesting[user][_id];

        uint256 blivXBal = IERC20Upgradeable(presale[_id].receiptToken). balanceOf(user);
        ...
        uint256 tgeClaimAmount = blivXBal / (40 * BASE_MULTIPLIER);
        require(blivXBal + tgeClaimAmount > 0, "Nothing to claim");
        uint256 amount = (blivXBal / BASE_MULTIPLIER) - _user.claimedAmount - tgeClaimAmount;
        ...
        uint256 perMonthClaim = (((blivXBal / BASE_MULTIPLIER) - tgeClaimAmount)  * BASE_MULTIPLIER * MONTH) /
            (_user.claimEnd - _user.claimStart);
    ...
    function startPublicSale()
        ...
        onlyOwner
    {
        require(publicSaleMode == false, "Public sale already ongoing");
        publicSaleMode = true;
        publicSaleAmt = 5000000;
    }
    ...
    function stopPublicSale()
        ...
        onlyOwner
    {
        require(publicSaleMode == true, "Public sale already stopped");
        publicSaleMode = false;
    }
    ...
    function purchasePublic(uint256 amount, bool purchaseInEth) 
    external payable nonReentrant 
    returns(bool) 
    {
        require(amount >= 300);
        require(publicSaleMode == true, "Public sale paused");
        uint256 usdPrice = amount * 200000000000000000;
        // usdPrice = usdPrice / (10**12);
        ...
        require(msg.value >= ethAmount, "Less payment");
        uint256 excess = msg.value - ethAmount;
        publicSaleAmt -= amount;
        sendValue(payable(payoutAddress), ethAmount);
        if (excess > 0) sendValue(payable(_msgSender()), excess);

        require(
            amount <=
                IERC20Upgradeable(presale[1].saleToken).balanceOf(
                    address(this)
                ),
            "Not enough BLIV tokens in the contract"
        );
        
        bool status = IERC20Upgradeable(presale[1].saleToken).transfer(
            _msgSender(),
        ...
        require(success, "Token payment failed");
        //transfer BLIV here, hardcoded id 1 pls check!
