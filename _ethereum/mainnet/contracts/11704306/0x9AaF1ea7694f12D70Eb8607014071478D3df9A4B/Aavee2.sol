...
contract Aavee2 is IExchange, TokenFetcher {
  using Address for address;
  ...
  constructor(uint16 _refCode, address _lendingPool, address _wethGateway) public {
    refCode = _refCode;
  ...
  returns (uint256)
  {
    ...
    if (address(fromToken) == address(data.aToken)) {
      if (address(toToken) == Utils.ethAddress()) {
        IWETHGateway(wethGateway).withdrawETH(fromAmount, address(this));
      }
      else {
      ...
      if (address(fromToken) == Utils.ethAddress()) {
        IWETHGateway(wethGateway).depositETH{value : fromAmount}(address(this), refCode);
      }
      else {
