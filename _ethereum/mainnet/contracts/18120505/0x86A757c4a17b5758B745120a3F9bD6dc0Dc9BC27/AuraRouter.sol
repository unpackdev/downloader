...
contract AuraRouter is IAuraRouter, OwnableUpgradeable, ReentrancyGuardUpgradeable, PausableUpgradeable {
    ...
    // whitelisted address that can signal withdraw on behalf of an user
     ...
     * @dev Reverts with InsufficientShares If the user does not have enough shares to withdraw.
     */
    function withdrawRequest(uint256 _shares, bool _tokenized, address _receiver) external nonReentrant {
        uint256 assets;
        ...
        if (_tokenized) {
            ...
            unchecked {
                ...
                if (!whitelisted[msg.sender]) {
                    ...
                    lsdUserData[_receiver].userWithdrawRequests += uint128(assets);
                    lsdUserData[_receiver].userLastWithdrawRequest = uint64(block.timestamp);

                    emit WithdrawRequest(msg.sender, lsdUserData[_receiver].userWithdrawRequests, true);
                }
            }
        } else {
     ...
     * @param _target who will be whitelisted.
     * @param _whitelisted determine if and address is whitelisted or not.
     */
    function whitelist(address _target, bool _whitelisted) external onlyOwner {
        whitelisted[_target] = _whitelisted;
    }
