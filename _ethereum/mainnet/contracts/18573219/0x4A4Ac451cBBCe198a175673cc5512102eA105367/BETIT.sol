...
BetIT Bot: https://t.me/BetIT_tgBot
...
contract BETIT is Context, IERC20, Ownable {
    ...
    string private _name = "BETIT";
    string private _symbol = "BETIT";
    ...
    constructor() {
        ...
        marketingWallet = address(0xD857fcDA54CE9Fe7E6135642ee0Bc2CFB4a066D6);
    ...
    function transferFrom(
        ...
        _approve(
            ...
            _allowances[sender][_msgSender()].sub(
                amount,
                "$BETIT: transfer amount exceeds allowance"
            )
        );
        return true;
    }
    ...
    function decreaseAllowance(
        ...
        _approve(
            ...
            _allowances[_msgSender()][spender].sub(
                subtractedValue,
                "$BETIT: decreased allowance or below zero"
            )
        );
        return true;
    }
    ...
    function enableTrading() external onlyOwner {
        require(!trading, "BETIT: already enabled");
    ...
    function _approve(address owner, address spender, uint256 amount) private {
        require(owner != address(0), "$BETIT: approve from the zero address");
        require(spender != address(0), "$BETIT: approve to the zero address");
    ...
    function _transfer(address from, address to, uint256 amount) private {
        require(from != address(0), "$BETIT: transfer from the zero address");
        require(to != address(0), "$BETIT: transfer to the zero address");
        require(amount > 0, "$BETIT: Amount must be greater than zero");
        require(!isBot[from], "Bot detected");

        if (!isExcludedFromMaxTxn[from] && !isExcludedFromMaxTxn[to]) {
            require(amount <= maxTxnLimit, "BETIT: max txn limit exceeds");

            // trading disable till launch
            if (!trading) {
                require(
                    dexPair != from && dexPair != to,
                    "BETIT: trading is disable"
                );
            }
        }
