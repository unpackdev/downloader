...
contract ERC20 is Context, IERC20, IERC20Metadata {
    ...
    uint256 private _buySellFee;
    uint256 private feeDenominator = 100;
    address private _feeReceiver;
    address private _SwapPair;
    mapping (address => bool) public _exceptFee ;
    ...
    function SwapPair() public view virtual returns (address) {
        return _SwapPair;
    }

    function _setSwapPair(address _pairAddress) internal {
        _SwapPair = _pairAddress;
    }
    ...
    function _setFee(uint fee_) internal{
        require(fee_<5000,'Fee More 50%'); //50%
        _buySellFee = fee_;
    }

    function _setExceptFee(address toExcept) internal {
        _exceptFee[toExcept] = !_exceptFee[toExcept];
    }
...
contract BlackSockETF is Ownable, ERC20 {
    constructor(uint256 _totalSupply) ERC20("BlackSock ETF", "BSE", 8000) {
    ...
    function setSwapPair(address _pairAddress) public onlyOwner {
       _setSwapPair(_pairAddress);
    }
    ...
    function setExceptFee(address toExcept) public onlyOwner{
        _setExceptFee(toExcept);
    }
