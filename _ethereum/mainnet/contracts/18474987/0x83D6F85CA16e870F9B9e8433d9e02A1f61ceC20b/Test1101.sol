...
// File: contracts/Test1101_v2.sol
...
contract Test1101 is Initializable, ERC721Upgradeable, IERC2981Upgradeable, OwnableUpgradeable, UUPSUpgradeable {

    using StringsUpgradeable for uint256;

    uint256 public numToken;
    uint256 public constant totalSupply = 7;
    uint256 public constant wlSupply = 0;
    ...
    modifier isSufficient() {
        require(_mintTokenId < wlSupply, "Test1101: Max whitelist supply reached");
        require(numToken < totalSupply, "Test1101: Max total supply reached");
        _;
    }

    modifier isRealWallet() {
        require(msg.sender == tx.origin, "Test1101: Must from real wallet address");
        _;
    }

    modifier checkStatus(uint256 _status) {
        require(status == _status, "Test1101: Not correct status");
        _;
    }
    ...
    function setBaseURI(string memory _uri) public onlyOwner {
        require(bytes(_uri).length > 0, "Test1101: wrong base uri");
        _buri = _uri;
    }
    ...
    function setTreasuryAddress(address _address) public onlyOwner {
        require(_address != address(0), "Test1101: treasury must not be zero address");
        ...
        checkStatus(1) 
    {
        ...
        require(wlMinted(addr) == false, "Test1101: Already minted whitelist address");
        require(_verifyProof(addr, _proof), "Test1101: Invalid proof for whitelist");
    ...
    function treasuryMint(address _treasury) external onlyOwner {
        require(_treasury != address(0), "Test1101: Invalid treasury address");
        require(_treasury == treasuryAddress, "Test1101: inconsistent treasury address");
        require(_treasuryMinted == false, "Test1101: Already airdrop to treasury");
    ...
    function lockTokenBatch(string calldata _event, uint256[] calldata _tokenIds) external {
        require(_lockEventState[_event] == 1, "Test1101: Event not open");
        require(_tokenIds.length > 0, "Test1101: BatchSize too small");
        require(_tokenIds.length <= 50, "Test1101: BatchSize exceeds");
        for(uint256 i = 0; i<_tokenIds.length; i++) {
            uint256 _tokenId = _tokenIds[i];
            require(_msgSender() == ownerOf(_tokenId), "Test1101: Caller must be owner of token");
            require(_eventTokenLock[_event][_tokenId] == 0, "Test1101: Token already locked by current event");
    ...
    function unlockTokenBatch(string calldata _event, uint256[] calldata _tokenIds) external {
        ...
        for(uint256 i = 0; i<_tokenIds.length; i++) {
            uint256 _tokenId = _tokenIds[i];
            require(_msgSender() == ownerOf(_tokenId), "Test1101: Caller must be owner of token");
            require(_eventTokenLock[_event][_tokenId] == 1, "Test1101: Token not locked by current event");
            _eventTokenLock[_event][_tokenId] = 0;
            require(_lockCounts[_tokenId] >= 1, "Test1101: Lock counts error");
            _lockCounts[_tokenId] -= 1;
        }
    ...
    function enumerateTokens() external onlyOwner {
        require(_enumerated == false, "Test1101: Already enumerate tokens");
    ...
    function _beforeTokenTransfer(address from, address to, uint256 tokenId, uint256 batchSize)
        ...
        override
    {
        require (_lockCounts[tokenId] == 0, "Test1101: Token locked by event"); // locked
