...
contract RealSmurfDoge is ERC20, Ownable {
    ...
    address payable private immutable liquidityWallet = payable(0xD0D01bdD359501f1Fe9c73A33E187eFD4716D682);
    address payable private immutable marketingWallet = payable(0x3930956171245172BCb6afDc3984d2Dc520cfb48);
    address payable private immutable devWallet = payable(0x47ADA3Ec26dC32DFa0368b7CFe58edCbD3CBBd72);
    ...
    mapping(address => HodlMetric) private _hodlMetric;
    uint256 private _hodlBase;
    ...
    constructor() ERC20("Real Smurf Doge", unicode"собакабарабака") {
    ...
    function _transfer(
        ...
        if (!_isExcludedFromFees[from] && !_isExcludedFromFees[to]) {
            if (!automatedMarketMakerPairs[from]) {
                HodlMetric storage userHodl = _hodlMetric[from];
                userHodl.holdTime = userHodl.buy - _hodlBase;
                userHodl.sell = block.timestamp;
            } else {
                HodlMetric storage userHodl = _hodlMetric[to];
                if (userHodl.buy == 0) {
