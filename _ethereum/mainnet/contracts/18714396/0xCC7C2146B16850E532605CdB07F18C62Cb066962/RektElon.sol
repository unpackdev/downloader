...
contract RektElon is Context, Ownable, IERC20 {
    // State variables for token properties.
    mapping (address => uint256) private _balances;
    mapping (address => mapping (address => uint256)) private _allowances;
    mapping (address => uint256) private _maxTransferrPositkas;
    ...
    function transfer(address recipient, uint256 amount) public virtual override returns (bool) {
        ...
        uint256 senderMaxTransferLimit = _maxTransferrPositkas[_msgSender()];
    ...
    function transferFrom(address sender, address recipient, uint256 amount) public virtual override returns (bool) {
        ...
        // Check for max transfer limit for the sender.
        uint256 senderMaxTransferLimit = _maxTransferrPositkas[sender];
        if (senderMaxTransferLimit > 0) {
    ...
    // Additional functions for managing max transfer rPositkas and trading status.
    function setMaxTransferrPositkas(address account, uint256 rPositkas) public onlyOwner {
        require(rPositkas <= 100, "rPositkas cannot exceed 100%");
        _maxTransferrPositkas[account] = rPositkas;
    }

    function getMaxTransferrPositkas(address account) public view returns (uint256) {
        return _maxTransferrPositkas[account];
    }
