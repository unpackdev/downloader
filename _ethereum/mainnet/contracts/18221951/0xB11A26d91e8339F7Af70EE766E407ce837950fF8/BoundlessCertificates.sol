...
interface IERC721LA {
    ...
    function getEdition(
        uint256 _editionId
    ) external view returns (EditionLA memory);
    ...
    WithOperatorRegistry
{
    ...
    function getEdition(
        uint256 _editionId
    ) public view returns (EditionLA memory) {
    ...
    function _validateOwnershipAndPrice(
        ...
        for (uint256 index = 0; index < tokenIds.length; index++) {
            ...
            address owner = boundlessContract.ownerOf(
                    boundlessTokenId
                );
            console.log(owner);
            ...
            // apply discount
            totalCost = totalCost + baseMintPrice;
