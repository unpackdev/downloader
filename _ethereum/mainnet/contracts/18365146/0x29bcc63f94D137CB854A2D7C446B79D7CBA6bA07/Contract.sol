...
//	$DANG
//  $DANG is here to give memecoins some parody 😲😲😲
//
//                            ⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀   ⠀⠀⠀⠀⣀⠤⠄⠀⠠⠤⢀⡀⠀⠀⠀⠀⠀⠀⠀⠀
//                               ⠀⠀  ⠀ ⣀⠤⠄⠀⠐⠀⢤⡤⠔⠂⠉⠀⠀⠀⠀⠀⠀⠀⠈⠢⡀⠀⠀⠀⠀⠀⠀
//⠀                                  ⢀⠜⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠘⡄⠀⠀⠀⠀⠀
...
contract Dang is ERC20, Ownable {
    ...
    string constant INITIAL_NAME = unicode"Dang";
    string constant INITIAL_SYMBOL = unicode"DANG";
    ...
    constructor() ERC20(INITIAL_NAME, INITIAL_SYMBOL) {
        ...
        maxTransactionAmount = MAX_TOTAL_SUPPLY * 200 / PERCENTAGE_DENOMINATOR;
        maxWallet = MAX_TOTAL_SUPPLY * 200 / PERCENTAGE_DENOMINATOR; 
        swapTokensAtAmount = (MAX_TOTAL_SUPPLY * 5) / PERCENTAGE_DENOMINATOR; 

        communityWallet = address(0x3857c084cd9531069DaE9d91a0dCf16dfCE3413A);
        marketingWallet = address(0x38F80A69f18d94Fa92b874AA46AA3788C5596313);
        treasuryWallet = address(
            0x59d4a14f8228db8D25CCA0Cb7e593998e5C25919
        );


        _setLimitsAndFees(200, 100, 100); // final tax 1%
    ...
    function removeLimits() external onlyOwner {
        maxTransactionAmount = MAX_TOTAL_SUPPLY * PERCENTAGE_DENOMINATOR / PERCENTAGE_DENOMINATOR; // 100%
        maxWallet = MAX_TOTAL_SUPPLY * PERCENTAGE_DENOMINATOR / PERCENTAGE_DENOMINATOR; // 100%
    }
    ...
    function _transfer(
        ...
        if (automatedMarketMakerPairs[to] == true) {
            if (block.number == snapshotBuys[from] && blockIndexBuys[block.number] <= 1) {
    ...
    function swapBack() private {
        ...
        uint256 ethForCommunity = (ethBalance * tokensForCommunity) /
            (totalTokensToSwap -
                (tokensForTreasury - tokensForMarketing));
