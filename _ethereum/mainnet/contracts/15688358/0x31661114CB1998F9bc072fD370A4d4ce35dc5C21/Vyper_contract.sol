...
@internal
def _take_fee(token: address, amount: uint256, fee_numerator: uint256) -> uint256:
    if fee_numerator != 0:
...
@external
def swap(_from: Tokens, to: Tokens, amount_in: uint256, min_out: uint256, fee_numerator: uint256 = 30) -> uint256:
    ...
    if _from in (Tokens.yCrv | Tokens.styCrv | Tokens.lpyCrv) and to in (Tokens.yCrv | Tokens.styCrv | Tokens.lpyCrv):
        ...
        amount = self._take_fee(YCRV, amount, fee_numerator)
        if to == Tokens.lpyCrv:
        ...
        amount = self._take_fee(SDCRV, amount, fee_numerator)
        amount = self._deposit_into_destination(to, token_to, amount)
    else:
        if _from != Tokens.crv:
            if _from == Tokens.lpyCrv:
        ...
        amount = self._take_fee(CRV, amount, fee_numerator)
        if to != Tokens.crv:
...
@external
@view
def simulate(_from: Tokens, to: Tokens, amount_in: uint256, fee_numerator: uint256 = 30) -> uint256:
    token_from: address = self._get_token(_from)
    token_to: address = self._get_token(to)
