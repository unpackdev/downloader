...
interface ERC721SeaDropStructsErrorsAndEvents {
  ...
  /**
   *  @notice Revert with an error if the drop is active
   */
  error DropActive();

  /**
   *  @notice Revert with an error if the token URI already revealed
   */
  error TokenURIRevealed();
