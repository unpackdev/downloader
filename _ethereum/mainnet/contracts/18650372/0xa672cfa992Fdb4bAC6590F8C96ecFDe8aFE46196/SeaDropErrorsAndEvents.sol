...
interface SeaDropErrorsAndEvents {
     ...
     *      pay for their own mint.
     */
    error OnlySellerCanCancelOwnOrder();

    /**
     * @dev Revert with an error if the msg sender is not allowed.
     */
    error OnlyOffererCanCancelOwnOffer();

    /**
     * @dev Revert with an error if the msg sender is not allowed.
     */
    error OnlyCollectionOffererCanCancelOwnCollectionOffer();

    /**
     * @dev Revert with an error if the msg sender is not allowed.
     */
    error OnlyNFTOwnerCanAccetpTheOffer();
     ...
     *      pay for their own mint.
     */
    error SomethingIsWrongInAuction();
     ...
     * @dev Revert if a supplied payer address is the zero address.
     */
    error RoyaltyOwnersRoyaltyRatesLengthMisMatchOrShouldBeMoreThanZero();

    /**
     * @dev Revert if a supplied payer address is the zero address.
     */
    error TokenTypeNotDefine();

    /**
     * @dev Revert if a supplied payer address is the zero address.
     */
    error TokenNotRegisteredOrNeedToBeOwnerOfToken();
     ...
     * @dev Revert if a supplied payer address is the zero address.
     */
    error FeeRateExecedPercent();

    /**
     * @dev Revert if a supplied payer address is the zero address.
     */
    error InvalidTokenAddressOrNotRegistered();

    /**
     * @dev Revert if a supplied payer address is the zero address.
     */
    error LogicContractNotSet();

    /**
     * @dev Revert if a supplied payer address is the zero address.
     */
    error LogicDropContractNotSet();

    /**
     * @dev Revert if a supplied payer address is the zero address.
     */
    error RoyaltyOwnersRoyaltyRatesLengthMisMatch();

    /**
     * @dev Revert if a supplied payer address is the zero address.
     */
    error CannotChangeAuctionPriceWithValidBid();

    /**
     * @dev Revert if a supplied payer address is the zero address.
     */
    error PartiaFilledOrdersCannotChangePrice();
     ...
     *      match the INonFungibleSeaDropToken interface.
     */
    error CannotCancelAuctionWithValidBid();

    /**
     * @dev Revert with an error if the sender does not
     *      match the INonFungibleSeaDropToken interface.
     */
    error OnlySellerCanChangeOwnOrderPrice();

    /**
     * @dev Revert with an error if the sender does not
     *      match the INonFungibleSeaDropToken interface.
     */
    error MustPayTheExactPriceForOrder();
     ...
     *      for an already-empty TokenGatedDropStage.
     */
     error TokenAlreadyRegisteredOrNotOwnerOfToken();
     ...
     *      been used.
     */
    error NeedToBeOwnerOfToken();

    /**
     * @dev Revert with an error if a signature for a signed mint has already
     *      been used.
     */
    error RoyaltyOwnerLenghtShouldBeMoreThanZero();

    /**
     * @dev Revert with an error if a signature for a signed mint has already
     *      been used.
     */
    error RoyaltyRatesLenghtShouldBeMoreThanZero();

    /**
     * @dev Revert with an error if a signature for a signed mint has already
     *      been used.
     */
    error FeeRateExecedHundredPercent();

    /**
     * @dev Revert with an error if a signature for a signed mint has already
     *      been used.
     */
    error FeeRateExecedTenPercent();

    /**
     * @dev Revert with an error if a signature for a signed mint has already
     *      been used.
     */
    error AmountAndPriceCannotBeZero();
     ...
     * @dev An event with the updated payer for an nft contract.
     */
    event CollectionFeatureUpdated(
        address indexed nftContract,
        bool indexed feature
    );
}
