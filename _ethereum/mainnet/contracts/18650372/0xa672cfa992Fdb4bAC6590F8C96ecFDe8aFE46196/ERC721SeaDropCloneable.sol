    ...
    DefaultOperatorFiltererUpgradeable 
{
    ...
    uint256 private dropEndTime;
    uint256 private dropStartTime;
    bool private tokenURIRevealed;
    ...
    function initialize(
        ...
        RoyaltyInfo memory newInfo = RoyaltyInfo(__royaltyAddress, __royaltyBps);
        this.setRoyaltyInfo(newInfo);
        this.setContractURI(__contractURI);
     ...
     */
    function setBaseURI(string calldata newBaseURI) public override(ERC721ContractMetadataCloneable, ISeaDropTokenContractMetadata) {
        if (block.timestamp < dropStartTime) {
            super.setBaseURI(newBaseURI);
        } else if (block.timestamp < dropEndTime) {
            revert DropActive();
        } else if (!tokenURIRevealed) {
            super.setBaseURI(newBaseURI);
            tokenURIRevealed = true;
        } else {
            revert TokenURIRevealed();
        }
    }
    ...
    function updatePublicDrop(
        ...
        dropEndTime = publicDrop.endTime;
        dropStartTime = publicDrop.startTime;
     ...
     *
     * @param seaDropImpl The allowed SeaDrop contract.
     * @param featured     Whether the payer is allowed.
     */
    function updateFeatureCollection(
        ...
        // Update the payer.
        ISeaDrop(seaDropImpl).updateFeatureCollection(featured);
    }
        ...
        onlyOwner
    {
        ...
        if ( config.featureCollection) {
            this.updateFeatureCollection(config.seaDropImpl, config.featureCollection);
        }
    }
}
