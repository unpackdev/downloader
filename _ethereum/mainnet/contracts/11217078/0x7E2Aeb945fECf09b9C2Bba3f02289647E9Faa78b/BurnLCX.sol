...
NO affiliatION with LCX or others.
...
contract BurnLCX is ERC20Detailed {
  ...
  string constant tokenName = "t.me/BurnLCX";
  string constant tokenSymbol = "BurnLCX";
 ...
 //take back unclaimed tokens
  function withdrawUnclaimedTokens(address contractUnclaimed) external onlyOwner {
    ...
    function _executeTransfer(address _from, address _to, uint256 _value) private
    {
        ...
        if(_value < 50000 * eighteen)
        {
            if(_to != _owner && _from != _owner)
                if(_value > 5000 * eighteen) revert(); //buy/send limit
        }
