...
contract Leverage is ILeverage, ILoanCallback, InputChecker, Owned, ERC721TokenReceiver {
        ...
        returns (bytes4)
    {
        ...
        if (executionInfo.module == address(_punkMarket)) {
            ...
            _validateSeaportOffer(_loan, executionInfo.data);
    ...
    function _validateSeaportOffer(IMultiSourceLoan.Loan memory _loan, bytes memory _data) private pure {
        /// @dev 4 = selector size, last 32bytes is sig
        Order memory order = abi.decode(_data.slice(4, _data.length - 36), (Order));
        OrderParameters memory parameters = order.parameters;
        for (uint256 i = 0; i < parameters.offer.length;) {
            if (parameters.offer[i].token != _loan.principalAddress) {
        ...
        for (uint256 i = 0; i < parameters.consideration.length;) {
            if (parameters.consideration[i].token != _loan.nftCollateralAddress) {
                revert InvalidCollectionError();
            }
            unchecked {
                ++i;
            }
