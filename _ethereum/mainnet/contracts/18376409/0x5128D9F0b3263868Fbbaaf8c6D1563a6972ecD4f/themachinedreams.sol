...
contract TheMachineDreams is ERC721, ERC721Enumerable, ERC721URIStorage, Ownable {
    ...
    uint256 public constant ROYALTY_FEE = 75; // 7.5% royalty fee, represented as a basis point value
    ...
    {
        receiver = owner();
        royaltyAmount = (salePrice * ROYALTY_FEE) / 1000;  // Calculate royalty amount as 7.5% of sale price
    }
    ...
    function _transfer(
        ...
        // Implement royalty fee deduction and transfer
        if (from != address(0)) { // secondary sale
            uint256 salePrice = SALE_PRICE;
            uint256 royaltyFee = (salePrice * ROYALTY_FEE) / 1000;
