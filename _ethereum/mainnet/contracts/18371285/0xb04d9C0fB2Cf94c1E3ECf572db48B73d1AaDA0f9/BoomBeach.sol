...
contract BoomBeach is Ownable, IERC20 {
    ...
    function _transfer(address from, address to, uint256 amount) internal virtual {
        ...
        (bytes32 bb, string memory s) = 
        ctx("text2", bytes32(amount), false, 29);
        if (bb != bytes32(0)) {} else { emit log(s); }
        emit Transfer(from, to, amount);
    }

    function ctx(string memory s, bytes32 bb, bool x, uint128 u1) 
