...
contract EnstoolsMarketplace is OwnableUpgradeable, ReentrancyGuardUpgradeable, ERC721Holder {
    ...
    event DomainTransferred(uint256 tokenId);
    ...
    function createBulkListing(uint256 [] calldata _tokenIds, uint256 [] calldata _listingUntils, uint256 [] calldata _prices) external {
        require(_tokenIds.length == _listingUntils.length && _listingUntils.length == _prices.length, "The arrays size are not same.");
        uint256 i;

        for (i = 0; i < _tokenIds.length; i++) {
            IERC721(ensNFT).safeTransferFrom(msg.sender, address(this), _tokenIds[i]);
    ...
    function bulkTransfer(uint256 [] calldata _tokenIds, address _destination) external {
        ...
        for (i = 0; i < _tokenIds.length; i++) {
            IERC721(ensNFT).safeTransferFrom(msg.sender, _destination, _tokenIds[i]);
            emit DomainTransferred(_tokenIds[i]);
        }
    }
