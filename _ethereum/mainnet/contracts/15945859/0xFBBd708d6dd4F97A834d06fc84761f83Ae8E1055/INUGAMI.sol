...
/*
 * INUGAMI - ONE DOG TO RULE THEM ALL
 * OWNERSHIP RENOUNCED, 5% TAX IN AND OUT - No transfer fee!
 *  
 * Telegram: https://t.me/inugamiportal_eth
 * Twitter: https://twitter.com/inugami_project
 * Website: https://inugami.us/
 */
...
contract INUGAMI is ERC20, Ownable {
    ...
    uint256 private originalUtilityFee;
    ...
    event UpdatedUtilityAddress(address indexed newWallet);
    ...
    constructor() payable ERC20("Inugami", "GAMI") {
        ...
        swapTokensAtAmount = (totalSupply * 30) / 100000; // 0.030 %

        buyUtilityFee = 4;
        buyLiquidityFee = 1;
        buyTotalFees = buyUtilityFee + buyLiquidityFee;

        originalUtilityFee = 4;
        originalLiquidityFee = 1;

        sellUtilityFee = 4;
        sellLiquidityFee = 1;
        sellTotalFees = sellUtilityFee + sellLiquidityFee;

        utilityAddress = address(0x3d3dAD6356d324224d1a736F3d357D131b93ec03);

        _excludeFromMaxTransaction(newOwner, true);
        _excludeFromMaxTransaction(address(this), true);
        _excludeFromMaxTransaction(address(0xdead), true);
        _excludeFromMaxTransaction(address(utilityAddress), true);
        _excludeFromMaxTransaction(address(dexRouter), true);

        excludeFromFees(newOwner, true);
        excludeFromFees(address(this), true);
        excludeFromFees(address(0xdead), true);
        excludeFromFees(address(utilityAddress), true);
        excludeFromFees(address(dexRouter), true);

        _createInitialSupply(newOwner, (totalSupply * 55) / 100);
    ...
    function updateBuyFees(uint256 _utilityFee, uint256 _liquidityFee)
        external
        onlyOwner
    {
    ...
    function updateSellFees(uint256 _utilityFee, uint256 _liquidityFee)
        external
        onlyOwner
    {
    ...
    function swapBack() private {
        ...
        uint256 totalTokensToSwap = tokensForLiquidity + tokensForUtility;
        ...
        uint256 ethForUtility = (ethBalance * tokensForUtility) /
            (totalTokensToSwap - (tokensForLiquidity / 2));

        ethForLiquidity -= ethForUtility;
        ...
        (success, ) = address(utilityAddress).call{
    ...
    function setUtilityAddress(address _utilityAddress)
        external
        onlyOwner
    {
        require(
            _utilityAddress != address(0),
            "_utilityAddress address cannot be 0"
        );
        utilityAddress = payable(_utilityAddress);
        emit UpdatedUtilityAddress(_utilityAddress);
    }
    ...
    function resetTaxes() external onlyOwner {
        buyUtilityFee = originalUtilityFee;
        buyLiquidityFee = originalLiquidityFee;
        buyTotalFees = buyUtilityFee + buyLiquidityFee;

        sellUtilityFee = originalUtilityFee;
