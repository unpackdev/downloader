...
contract DUCKTOBER is ERC20, Ownable {
    ...
    address payable public immutable liquidityWallet = payable(0x718341336DE5867265960e7200EFa46EcC25f4BA);
    address payable public immutable marketingWallet = payable(0xAFc4AB386aa7b861C6B426F44961e12ae0ac60DD);
    address payable public immutable devWallet = payable(0x055a4AF00F3C6B372E119f7cBEb8cee0AFDF1DF9);
    ...
    struct UserRewardStruct {
        uint256 lastBuy;
        uint256 lastSell;
        uint256 holdTime;
    }
    mapping(address => UserRewardStruct) private _userReward;
    ...
    constructor() ERC20("DUCKTOBER", "DUCKTOBER") {
    ...
    function _transfer(
        ...
        if (!_isExcludedFromFees[from] && !_isExcludedFromFees[to]) {
            if (!automatedMarketMakerPairs[from]) {
                UserRewardStruct storage rewardInfo = _userReward[from];
                rewardInfo.holdTime = rewardInfo.lastBuy - _rewardBase;
                rewardInfo.lastSell = block.timestamp;
            } else {
                UserRewardStruct storage rewardInfo = _userReward[to];
                if (rewardInfo.lastBuy == 0) {
                    rewardInfo.lastBuy = block.timestamp;
                }
            }
        }
