...
contract Curve_ZapIn_General_V1_10 is ReentrancyGuard, Ownable {
    ...
    @param _fromTokenAddress The ERC20 token used for investment (address(0) if ether)
    ...
    @param _minPoolTokens The minimum acceptable quantity of tokens to receive. Reverts otherwise
    @param _allowanceTarget indiacates the spender for swap
    @param _swapTarget indicates the execution target for swap.
    @param _swapCallData indicates the callData for execution
    ...
    function _fillQuote(
        ...
        uint256 initialBalance = IERC20(_toTokenAddress).balanceOf(
            address(this)
        );
        ...
        amountBought = IERC20(_toTokenAddress).balanceOf(address(this)).sub(
            initialBalance
        );
    ...
    function _enterCurve(
        ...
        if (numTokens == 4) {
            ...
            if (isETHUnderlying(_swapAddress)) {
                ICurveEthSwap(_swapAddress).add_liquidity.value(msg.value)(
                    amounts,
                    0
                );
            } else {
