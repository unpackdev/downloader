...
contract BadgerLPCollection is PoolTokenWrapper, Ownable, Pausable {
	using SafeMath for uint256;
	...
	function earned(address account, uint256 pool) public view returns (uint256) {
		...
		return
			balanceOf(account, pool).mul(blockTime.sub(p.lastUpdateTime[account]).mul(rewardRate)).div(1e18).add(
				p.points[account]
			);
	}
