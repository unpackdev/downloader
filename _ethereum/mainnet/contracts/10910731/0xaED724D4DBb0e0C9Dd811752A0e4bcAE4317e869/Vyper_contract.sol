...
def add_liquidity(uamounts: uint256[N_COINS], min_mint_amount: uint256):
    ...
    for i in range(N_COINS):
        ...
        if uamount > 0:
            ...
            yvERC20(self.coins[i]).deposit(uamount)
            amounts[i] = yvERC20(self.coins[i]).balanceOf(self)
...
def _send_all(_addr: address, min_uamounts: uint256[N_COINS], one: int128):
    ...
    for i in range(N_COINS):
        if (one < 0) or (i == one):
            ...
            _balance: uint256 = yvERC20(_coin).balanceOf(self)
            if _balance == 0:  # Do nothing for 0 coins
                continue
            yvERC20(_coin).withdraw(_balance)
...
def remove_liquidity_imbalance(uamounts: uint256[N_COINS], max_burn_amount: uint256):
    ...
    for i in range(N_COINS):
        if amounts[i] > 0:
            rate: uint256 = yvERC20(self.coins[i]).getPricePerFullShare()
...
def calc_withdraw_one_coin(_token_amount: uint256, i: int128) -> uint256:
    ...
    for j in range(N_COINS):
        rates[j] = yvERC20(self.coins[j]).getPricePerFullShare()
