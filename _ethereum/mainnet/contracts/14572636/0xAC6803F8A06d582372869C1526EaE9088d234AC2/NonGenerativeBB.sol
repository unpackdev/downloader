...
contract NonGenerativeBB is GenerativeBB {
 using Counters for Counters.Counter;
    ...
    */
    ...
    function calculatePriceSmall(uint256 seriesId, uint256 currencyType) public view returns(uint256 price) {
...
function buyNonGenBoxTest(uint256 seriesId, uint256 currencyType, address collection, string memory ownerId, bytes32 user) validateCurrencyType(seriesId,currencyType, false) internal {
        require(!_isWhiteListed[seriesId] || crypoWhiteList[msg.sender][seriesId], "not authorize");
        require(abi.encodePacked(nonGenSeries[seriesId].name).length > 0,"Series doesn't exist"); 
        require(nonGenSeries[seriesId].maxBoxes > nonGenSeries[seriesId].boxId.current(),"boxes sold out");
        transferToken(seriesId, currencyType, collection, ownerId);
        
    }
    
   function transferToken(uint256 seriesId, uint256 currencyType, address collection, string memory ownerId) private {
       token = USD;
        
        uint256 price1 = calculatePrice(nonGenSeries[seriesId].price, baseCurrency[seriesId], currencyType);
        // if(currencyType == 0){
            // price = price / 1000000000000;
        uint256    price = SafeMath.div(price1, 1000000000000);
        // }
        // escrow alia
        // token.transferFrom(msg.sender, bankAddress[seriesId], price);
        // transfer box to buyer
        ...
        emitBuyBoxNonGen(seriesId, currencyType, price, collection, ownerId);
       emit USDTTest(price, price1, bankAddress[seriesId], msg.sender);
   }
...
/** 
   ...
   // event BlackList(uint256 indexed seriesId, bytes32 indexed combHash, bool flag);
    ...
    event USDTTest(uint256 calculatedPrice, uint256 actualPrice, address to, address from);
}