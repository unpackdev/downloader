...
contract DfxDistributor is DfxDistributorEvents, ReentrancyGuardUpgradeable, AccessControlUpgradeable {
    ...
    function _distributeReward(address gaugeAddr) internal returns (uint256 weeksElapsed, uint256 rewardTally) {
        ...
        if (gaugeType >= 2) {
            ...
            ILiquidityGaugeV4(gaugeAddr).deposit_reward_token(address(rewardToken), rewardTally);
        }
        ...
        onlyRole(GOVERNOR_ROLE)
    {
        ...
        ILiquidityGaugeV4(gaugeAddr).deposit_reward_token(address(rewardTokenAddr), rewardTally);
        emit RewardDistributed(gaugeAddr, rewardTally);
    }
