...
contract LpTokenStaker is ILpTokenStaker, BaseMinter {
    ...
    ) BaseMinter(_tor, _emergencyMinter) {
    ...
    function shutdown() external {
        ...
        for (uint256 i; i < pools.length; i++) {
            _claimTORRewardsForPool(pools[i]);
        }
        isShutdown = true;
        emit Shutdown();
    }
    ...
    function claimTORRewardsForPool(address pool) external override notShutdown {
        require(
            msg.sender == address(ITorusPool(pool).rewardManager()),
            "can only be called by reward manager"
        );
        _claimTORRewardsForPool(pool);
    }

    function _claimTORRewardsForPool(address pool) internal {
        require(controller.isPool(pool), "not a pool");
        checkpoint(pool);
        uint256 torToMint = poolShares[pool];
        if (torToMint == 0) {
            return;
        }
        torus.mint(address(pool), torToMint);
        controller.inflationManager().executeInflationRateUpdate();
        poolShares[pool] = 0;
        emit TokensClaimed(pool, torToMint);
    }
