...
contract AdminModule is Helpers, Events {
    ...
    function initializeV2() external reinitializer(2) onlyAuth {
        uint8[] memory protocolIds_ = new uint8[](2);
        uint256[] memory maxRiskRatio_ = new uint256[](2);

        protocolIds_[0] = 6; // Morpho Aave V3
        protocolIds_[1] = 7; // Saprk

        maxRiskRatio_[0] = 880000; // 88%
        maxRiskRatio_[1] = 880000; // 88%

        // Setting max risk ratio for new protocols.
        updateMaxRiskRatio(protocolIds_, maxRiskRatio_);

        string[] memory targets_ = new string[](1);
        bytes[] memory calldatas_ = new bytes[](1);

        // Enabling Spark e-mode. E-mode can be enabled without any deposit.
        targets_[0] = "SPARK-A";
        calldatas_[0] = abi.encodeWithSignature(
            "setUserEMode(uint8)",
            1 // WETH correlated mode
