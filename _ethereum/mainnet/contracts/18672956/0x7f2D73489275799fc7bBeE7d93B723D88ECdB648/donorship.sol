...
contract DonorshipPaymentProcessor is Ownable, Pausable, ReentrancyGuard {
    ...
    {
        require(_amountIn > 0, "Amount must be greater than 0");
        uint256 secondaryFee = affiliates[_affiliateID].feeAmount;
    ...
    function processTokenDonation(
        ...
        require(doesAffiliateExist(_affiliateID), "Affiliate ID doesnt exist");
        ...
        bool feeEnabledInvidually = affiliates[_affiliateID].feeEnabledIndividually;
        console.log("feeEnabledInvidually: \n", feeEnabledInvidually);
        //always apply primary fee
        (uint256 primaryDeductionAmount, ) = enactPrimaryFee(amountWETH);
        //enum aff receiver
        address affReceiver = affiliates[_affiliateID].receiver;
    ...
    function processDonation(bytes32 _affiliateID, address _charity) public payable nonReentrant whenNotPaused{
        ...
        //get fee status for individual affiliate
        bool feeEnabledInvidually = affiliates[_affiliateID].feeEnabledIndividually;
        console.log("feeEnabledInvidually: ", feeEnabledInvidually);
        // if affiliate fees enabled, enact second fee from the original msg.value
        if (isAffiliateFeeEnabled && feeEnabledInvidually) {
    ...
    function addAffiliate(bytes32 _affiliateID, address _receiver, uint256 _feeAmount) public onlyOwner {
        require(_affiliateID != 0, "Invalid ID");
        require(_receiver != address(0), "Invalid address");
        require(_feeAmount <= 500, "Fee too high");
        require(affiliates[_affiliateID].receiver == address(0), "ID exists");

        affiliates[_affiliateID] = Affiliate(_receiver, _feeAmount, true, _affiliateID);
    ...
    function updateTokenChecker(address _newTokenChecker) public onlyOwner {
        poolchecker = TokenPoolChecker(_newTokenChecker);
    }
