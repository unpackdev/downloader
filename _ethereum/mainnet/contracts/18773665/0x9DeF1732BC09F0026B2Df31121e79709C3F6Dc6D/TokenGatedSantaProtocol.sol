...
import "./ITokenGatedSantaProtocol.sol";
import "./SantaProtocol.sol";
...
contract TokenGatedSantaProtocol is SantaProtocol, ITokenGatedSantaProtocol {
        ...
        returns (bool eligible)
    {
        ...
        if (TOKEN_GATE_CONTRACT.balanceOf(account) == 0) return false;
        if (TOKEN_GATE_CONTRACT.ownerOf(tokenGateTokenId) != account) return false;
    ...
    function _tokenGate(address account, address nft, uint256 tokenId) internal virtual {
        if (nft != address(TOKEN_GATE_CONTRACT)) revert NotTokenGateCollection();
        if (TOKEN_GATE_LIMIT == 0) {
            if (TOKEN_GATE_CONTRACT.balanceOf(account) == 0) revert NotTokenGatedCollectionHolder();
            return;
        }

        if (_tokenGateNftUsed[nft][tokenId] >= TOKEN_GATE_LIMIT) revert TokenGateLimitReached();
        if (TOKEN_GATE_CONTRACT.ownerOf(tokenId) != account) revert NotTokenGatedTokenHolder();
