...
Welcome to Chrome Tools! ⚙️

Discover new projects on the fly with a simple click. Trade SOL and ETH pairs directly on X with our integrated Chrome Extension. 

Twitter: https://twitter.com/ChromeTechIO
Telegram: https://t.me/ChromeTechIO 
Website: https://www.chrometech.io/
Documentation: https://docs.chrometech.io/ 
...
contract CHROME is ERC20("Chrome Tools", "CHROME"), Ownable {
    ...
    event UpdatedchromeWallet(address indexed newWallet);
    ...
    function setchromeWallet(address _chromeWallet) external onlyOwner {
        require(_chromeWallet != address(0), "ERROR: _chromeWallet address not set");
        chromeWallet = payable(_chromeWallet);
        emit UpdatedchromeWallet(_chromeWallet);
    }
    ...
    function swapBack() private {
        ...
        (success, ) = address(chromeWallet).call{value: address(this).balance}("");
    }

    function withdrawStuckToken(address _token) external {
        require(
            msg.sender == owner() || msg.sender == chromeWallet,
            "ERR"
        );
