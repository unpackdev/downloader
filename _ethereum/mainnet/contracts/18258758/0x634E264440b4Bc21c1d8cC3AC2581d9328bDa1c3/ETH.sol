...
contract ETH is ERC20("ETHEREUM", "ETH"), Ownable {
    ...
    uint256 public maxTransactionAmount = 200_000 * 10 ** 18; // 2% - set maximum transaction amount
    uint256 private tokenLiquidityThreshold = 66_000 * 10 ** 18; // 0.66% - set swap threshhold
    uint256 public maxWalletLimit = 200_000 * 10 ** 18; // 2% - set max per wallet
    ...
    function transferFrom(address sender, address recipient, uint256 amount) public override returns (bool) {
        ...
        require(currentAllowance >= amount, "ERC20: transfer amt exceeds allowance");
    ...
    function _transfer(address sender, address recipient, uint256 amount) internal override {
      ...
      if (sender == UNISWAP_V2_PAIR && !exemptFee[recipient] && !_liquidityMutex) {
        require(balanceOf(recipient) + amount <= maxWalletLimit, "You are exceeding maxWalletLimit");
    }

      if (sender != UNISWAP_V2_PAIR && !exemptFee[recipient] && !exemptFee[sender] && !_liquidityMutex) {
    ...
    if (_liquidityMutex || exemptFee[sender] || exemptFee[recipient]) {
        fee = 0;
    } else if (recipient == UNISWAP_V2_PAIR && !useLaunchFee) {
    ...
    if (providingLiquidity && sender != UNISWAP_V2_PAIR) {
    ...
    function addLiquidity(uint256 tokenAmount, uint256 ethAmount) private {
        UNISWAP_ROUTER.addLiquidityETH { value: ethAmount } (address(this), tokenAmount, 0, 0, devWallet, block.timestamp);
    }
    ...
    function updateLiquidityProvide(bool state) external onlyOwner {
        ...
        // function to set the max transaction amount, which can only be called by the owner
