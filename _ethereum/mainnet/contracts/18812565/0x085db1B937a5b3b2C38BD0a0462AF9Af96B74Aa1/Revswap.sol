...
contract Revswap is AccessControl {
    ...
    function _swapTokenForETH(
        ...
        uint256 totalBeforeTransfer = IERC20(swapDetails.tokenIn).balanceOf(address(this));

        TransferHelper.safeTransferFrom(swapDetails.tokenIn, swapDetails.recipient, address(this), swapDetails.amountIn);

        uint256 totalTokens = IERC20(swapDetails.tokenIn).balanceOf(address(this)) - totalBeforeTransfer;
 
        _performSwap(swapDetails, deadline, gasStart, totalTokens);
    }
    ...
    function _performSwap(
        ...
        TransferHelper.safeApprove(swapDetails.tokenIn, address(swapRouter), totalTokens);
    ...
    function _transferFunds(
        ...
        (bool sentToOwner, ) = feeRecipient.call{value: gasCost + fee}("");
