  ...
  OperatorFilterer 
{
  ...
  uint128 internal constant _MAX_PRESALE_STAGES = 5;
  ...
  function initialize(
    ...
    publicStageMode = _initParams.publicStageMode;
    metadataSettings = MetadataSettings(_initParams.enableRandomReveal , _initParams.revealStatus, 0);
  ...
  function configureFullSaleParams(
    ...
    if (_publicSaleParams.startTime > 0) {
      publicSaleStage = _publicSaleParams;
    }

    if (_publicAuctionParams.startTime > 0) {
  ...
  function updatePublicSaleParams(StandardSaleSettings calldata _saleParams) external onlyOwner {
    publicSaleStage = _saleParams;
  }

  function updatePublicAuctionParams(DutchAuctionSettings calldata _auctionParams) external onlyOwner {
    publicAuctionStage = _auctionParams;
  }

  function updateMetadataParams(MetadataSettings calldata _metadata) external onlyOwner {
  ...
  function revealToken(string memory _baseTokenURI) external onlyOwner {
    MetadataSettings memory settings = metadataSettings;

    if (settings.enableRandomReveal && settings.randomOffset == 0) {
      metadataSettings.randomOffset = (block.timestamp + block.prevrandao) % MAX_SUPPLY;
    }
  ...
  function tokenURI(uint256 _tokenId) public view override(ERC721AUpgradeable, IERC721AUpgradeable) returns (string memory) {
    ...
    if (settings.revealed) {
      uint256 shiftedTokenId = (_tokenId + settings.randomOffset) % MAX_SUPPLY;
