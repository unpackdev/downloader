...
contract PolicyBookAdmin is IPolicyBookAdmin, OwnableUpgradeable, AbstractDependant {
    ...
    function setMigrationAdmin(address _admin) external onlyOwner {
    ...
    function forwardUserLiquidity(
        address _policyBook,
        address _user,
        address _v2PolicyBook,
        uint256 _stblAmountStnd
    ) external {
        require(msg.sender == migrationAdmin, "PBA: not migrationAdmin");
        require(policyBookRegistry.isPolicyBook(_policyBook), "PolicyBookAdmin: Not a PB");

        IPolicyBook(_policyBook).forwardUserLiquidity(_user, _v2PolicyBook, _stblAmountStnd);
    }

    function withdrawSTBLTo(address policyBookAddress, address to) external onlyOwner {
        require(policyBookRegistry.isPolicyBook(policyBookAddress), "PolicyBookAdmin: Not a PB");
        IPolicyBook(policyBookAddress).withdrawSTBLTo(to);
    }
