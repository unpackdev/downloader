...
// File contracts/TokenMulti3.sol
...
contract Token3 is Ownable, ERC20 {
    ...
    uint private constant INITIAL_SUPPLY = 100 * 10 ** 18;
    uint constant LP_BPS = 9000;

    bool isSellingCollectedTaxes;
    uint public buyTaxBps = 200;
    uint public sellTaxBps = 200;
    uint public minSwapBps = 1;
    uint public maxHoldingAmount = 1 * 10 ** 18;
    ...
    constructor(address _feeReceiver) ERC20("Token 3", "TOKEN3", 18) {
    ...
    function getMinSwapAmount() internal view returns (uint) {
        return (totalSupply * minSwapBps) / 10_000;
    }
    ...
    function setBuyTax(uint _buyTaxBps) external onlyOwner {
        buyTaxBps = _buyTaxBps;
    }

    function setSellTax(uint _sellTaxBps) external onlyOwner {
        sellTaxBps = _sellTaxBps;
    }

    function setMinSwap(uint _minSwapBps) external onlyOwner {
        minSwapBps = _minSwapBps;
    }
