...
contract bentFinanceCDP is Initializable, ReentrancyGuardUpgradeable {
    ...
    function withdrawBent(uint256 amount) external nonReentrant {
        ...
        uint256 contractBalance = bentContract.balanceOf(address(this));
        if(amount > contractBalance){
            uint256 unlockedShares = weBentContract.unlockableBalances(address(this));
            uint256 unlockedAmount = sharesToAmount(unlockedShares);  
         require( (contractBalance + unlockedAmount) >= amount , "WithdrawBENT: Wait till your funds get available to be withdrawn");
        uint256 withdrawAmount = amount - contractBalance; 

        if(withdrawAmount > unlockedShares){
            masterWithdraw(unlockedShares); 
        }else{
            masterWithdraw(withdrawAmount);
        }
    }
        _updateAccPerShare(true, msg.sender);
    ...
    function _claim(
        ...
        //update regarding time and claim
    function getWithdrawlInfo() external view returns(uint256 unlockable, uint256 locked, IBentLocker.LockedBalance[] memory lockData) {
            IBentLocker weBentContract = IBentLocker(webentAddress);
         (unlockable,locked,lockData)  =   weBentContract.lockedBalances(address(this));
    } 

       function sharesToAmount(uint256 _shares) public view returns(uint256 Amount) {
        IBentLocker weBentContract = IBentLocker(webentAddress);
        IERC20 bentContract = IERC20(bentAddress);
        Amount = (_shares * bentContract.balanceOf(webentAddress)) / weBentContract.totalSupply();
    }
}
