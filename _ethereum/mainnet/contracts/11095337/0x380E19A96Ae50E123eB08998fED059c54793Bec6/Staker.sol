...
contract Staker is Poolable{
    ...
    function stake(address staker, address payable ref) public payable{
        ...
        address poolAddress = Uniswap(FACTORY).getPair(YIELD$Address, WETHAddress);
        uint ethAmount = IERC20(WETHAddress).balanceOf(poolAddress); //Eth in uniswap
        uint tokenAmount = IERC20(YIELD$Address).balanceOf(poolAddress); //token in uniswap
        
        uint toMint = (address(this).balance.mul(tokenAmount)).div(ethAmount);
        IERC20(YIELD$Address).mint(address(this), toMint);
        
        uint poolTokenAmountBefore = IERC20(poolAddress).balanceOf(address(this));
        
        uint amountTokenDesired = IERC20(YIELD$Address).balanceOf(address(this));
        IERC20(YIELD$Address).approve(UNIROUTER, amountTokenDesired ); //allow pool to get tokens
        Uniswap(UNIROUTER).addLiquidityETH{ value: address(this).balance }(YIELD$Address, amountTokenDesired, 1, 1, address(this), INF);
    ...
    function withdrawRewardTokens(uint amount) public {
        ...
        IERC20(YIELD$Address).mint(msg.sender, amount);
    }
