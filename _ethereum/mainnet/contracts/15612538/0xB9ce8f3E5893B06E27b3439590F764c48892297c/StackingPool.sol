...
contract StakingPool is OwnableUpgradeable, ReentrancyGuard 
{
   ...
   // address private beneficiary;
    ...
    function claimAmount(uint256 poolId, address claimaddress) external {
        ...
        IERC20(ideaToken).safeTransferFrom(address(this),claimaddress, amount);
    }
    ...
    function stakingClaim(uint256 poolId, address claimaddress) external {
       ...
       // payable(claimaddress).IERC20(ideaToken).transfer(stakingAmount);
        IERC20(ideaToken).safeTransferFrom(address(this), claimaddress, stakingAmount);
