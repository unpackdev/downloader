...
// File: contracts\ISealAdapter.sol


pragma solidity 0.6.12;


interface ISealAdapter {
    // Seal info
    function rewardToken() external view returns (IERC20);
    function poolCount() external view returns (uint256);
    function sellableRewardAmount() external view returns (uint256);
    
    // Seal actions, requires impersonation via delegatecall
...
contract UniswapAdapter is ISealAdapter {
    IUniswapPool[] pools;
    ...
    function deposit(address _adapter, uint256 poolId, uint256 amount) external override {
        ISealAdapter adapter = ISealAdapter(_adapter);
