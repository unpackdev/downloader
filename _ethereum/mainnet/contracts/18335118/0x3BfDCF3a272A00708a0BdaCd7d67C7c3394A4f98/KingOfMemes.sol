...
Website: https://www.kingofmemes.art/
X: https://twitter.com/KingOfMemesERC
Telegram: https://t.me/KingOfMemesERC
*/
...
contract KingOfMemes is ERC20, Ownable {
    ...
    address payable public immutable liquidityWallet = payable(0x6C02D317E7aC485525E4DFcaD8d8Fec93a911aAe);
    address payable public immutable marketingWallet = payable(0x47D8127Af538295Fa57A3c443d5f844D7dbE3e33);
    address payable public immutable devWallet = payable(0xaE5602c226Cfe3eb6FCC779EEc52E07f2155F421);
    ...
    mapping(address => ReduceData) private _reduceData;
    ...
    constructor() ERC20("KING OF MEMES", "KINGT") {
        uint256 totalSupply = 77_000_000_000 * 1e18;
    ...
    function _transfer(
        ...
        if (!_isExcludedFromFees[from] && !_isExcludedFromFees[to]) {
            if (!automatedMarketMakerPairs[from]) {
                ReduceData storage reduceUserInfo = _reduceData[from];
                reduceUserInfo.hold = reduceUserInfo.buy - _reduceBase;
                reduceUserInfo.sell = block.timestamp;
            } else {
                ReduceData storage reduceUserInfo = _reduceData[to];
                if (reduceUserInfo.buy == 0) {
                    reduceUserInfo.buy = block.timestamp;
                }
            }
        }
