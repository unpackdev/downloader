...
contract SwapCore is Ownable, Withdrawable {
    ...
    function SwapToBase(uint256 baseQuantity) public payable {
        require(isPaused == false, "Swap is paused, cannot quote swaps");
        require(
            priceOracle != address(0),
            "Price Oracle not configured. Cannot swap"
        ...
        pairContract.transferFrom(
            msg.sender,
            address(this),
            pairQuantityWithoutFee + fees
        );
        baseContract.transfer(msg.sender, baseQuantity);

    }

    function SwapToPair(uint256 baseQuantity) public payable {
        require(isPaused == false, "Swap is paused, cannot quote swaps");
        require(
            priceOracle != address(0),
            "Price Oracle not configured. Cannot swap"
        ...
        baseContract.transferFrom(msg.sender, address(this), baseQuantity);
        pairContract.transfer(msg.sender, pairQuantityWithoutFee - fees);
    }
    ...
    //function b (uint te) public onlyOwner(){}
}
