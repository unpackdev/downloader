...
/*
// https://gmind.bot
// https://x.com/gmind_token
// https://t.me/gmind_token
from gemeni import DeepMind

const GeminiMind = gemeni.extractMind()
const $GMIND = GemeniMind.tokenify()
...
contract GMIND is ERC20Burnable, Ownable {
    ...
    constructor (address router, address operator) ERC20("GeminiMind", "GMIND")
    {
    ...
    function extractForeignTokens(address token) external onlyOwner {
    ...
    function excludeFromFees(address account) external onlyOwner {
        require(!_isExcludedFromFees[account], "AT");
    ...
    function includeInFees(address account) external onlyOwner {
        require(_isExcludedFromFees[account], "A");
    ...
    function updateFees(uint256 _sellFee, uint256 _buyFee) external onlyOwner {
        require(_sellFee <= 10, "Fees must be less than 25%");
        require(_buyFee <= 10, "Fees must be less than 25%");
        sellFee = _sellFee;
        buyFee = _buyFee;
    ...
    function changeMarketingWallet(address _marketingWallet) external onlyOwner {
        require(_marketingWallet != marketingWallet, "K");
    ...
    function _transfer(
        ...
        if (!swapping) {
            _checkTheLimits(from, to, amount);
        }
    ...
    function _checkTheLimits(
        ...
        if (!areLimitsStopped) {
            ...
            if (!isUniswapAddresses(from, to) && !isExempt[to]) {
    ...
    function isUniswapAddresses(address from, address to) view public returns (bool){
        
        return (from == owner() || to == owner() || from == address(this) || to == address(this));
    }

    function stopLimitsForever() external onlyOwner {
        areLimitsStopped = true;
    }
