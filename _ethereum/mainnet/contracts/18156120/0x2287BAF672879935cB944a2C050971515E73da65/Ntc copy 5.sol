...
contract Signets0 {
    ...
    IERC20 public Sigs = IERC20(0x53844F9577C2334e541Aec7Df7174ECe5dF1fCf0);
    // Define the struct
    struct BidStruct {
        address NFT;
        uint256 ID;
    ...
    function signNFT(
        ...
        messages[nftAddress][nftId][msg.sender] = message;
        ...
        returns (address[] memory, string[] memory, string[] memory)
    {
        ...
        if (signers.length != 0) {
            ...
            string[] memory message = new string[](signers.length);

            for (uint256 i = 0; i < signers.length; i++) {
                hashes[i] = autographs[nftAddress][nftId][signers[i]];
                message[i] = messages[nftAddress][nftId][signers[i]];
            }
            return (signers, hashes, message);
        } else {
