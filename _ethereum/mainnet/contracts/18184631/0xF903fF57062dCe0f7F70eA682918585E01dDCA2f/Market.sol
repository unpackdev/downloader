...
contract Market is AccessControlUpgradeable, ReentrancyGuardUpgradeable, IMarket {
  ...
  modifier onlyStrategy() {
    require(ITreasury(treasury).strategy() == msg.sender, "only Treasury Strategy");
    _;
  }
  ...
  ) external override nonReentrant onlyStrategy returns (uint256 _fTokenMinted, uint256 _xTokenMinted) {
  ...
  function updateMarketConfig(
    ...
    require(
      _stabilityRatio > _recapRatio &&
        _recapRatio >= PRECISION,
      "invalid market config"
    );

    marketConfig = MarketConfig(_stabilityRatio, 0, 0, _recapRatio);

    emit UpdateMarketConfig(_stabilityRatio, 0, 0, _recapRatio);
  }
  ...
  /// @dev Internal function to deduct redeem fee for base token.
