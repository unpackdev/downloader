...
Twitter:  https://x.com/ZERO_x0000AI
Telegram: https://t.me/ZERO_x000000bot
...
contract ZeroX is ERC20, Ownable {
    ...
    {   
        ...
        marketingAddress = 0x0000A0Ef2d217D0eb046c3B7892c50d778b2a76E; // update
        devAddress = 0x0000A0Ef2d217D0eb046c3B7892c50d778b2a76E; // update

        buyTax.marketingsTax = 900; // 1% = 100
        buyTax.liquidityTax = 300;
        buyTax.teamTax = 300;
        buyTax.totalTax = buyTax.marketingsTax + buyTax.liquidityTax + buyTax.teamTax;

        sellTax.marketingsTax = 900;
        sellTax.liquidityTax = 300;
        sellTax.teamTax = 300;
        sellTax.totalTax = sellTax.marketingsTax + sellTax.liquidityTax + sellTax.teamTax;
    ...
    function handleTax(address from, address to, uint256 amount) internal returns (uint256){
        ...
        if(taxes.totalTax > 0){
            ...
            tokensForTaxUpdate.tokensForMarketing += uint80(tax * taxes.marketingsTax / taxes.totalTax / 1e9);
            tokensForTaxUpdate.tokensForDev += uint80(tax * taxes.teamTax / taxes.totalTax / 1e9);
    ...
    function updateBuyTax(uint64 _marketingsTax, uint64 _liquidityTax, uint64 _teamTax) external onlyOwner {
        Taxes memory taxes;
        taxes.marketingsTax = _marketingsTax;
        taxes.liquidityTax = _liquidityTax;
        taxes.teamTax = _teamTax;
        taxes.totalTax = _marketingsTax + _liquidityTax + _teamTax;
        require(taxes.totalTax  <= 10000, "Keep tax below 10%");
        emit UpdatedBuyTax(taxes.totalTax);
        buyTax = taxes;
    }

    function updateSellTax(uint64 _marketingsTax, uint64 _liquidityTax, uint64 _teamTax) external onlyOwner {
        Taxes memory taxes;
        taxes.marketingsTax = _marketingsTax;
        taxes.liquidityTax = _liquidityTax;
        taxes.teamTax = _teamTax;
    ...
    function updateMevBlockEnabled(bool _enabled) external onlyOwner {
