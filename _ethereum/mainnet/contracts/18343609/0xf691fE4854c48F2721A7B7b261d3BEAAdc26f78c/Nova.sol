...
// File contracts/interfaces/IWithdrawer.sol

// SPDX-License-Identifier: UNLICENSED
pragma solidity ^0.8.9;

interface IWithdrawer {
  function receiveWithdrawalETH() external payable;
}
...
contract Nova is Ownable {
    ...
    function fastSwap(address tokenOut, uint256 tipAmount) public payable {
        ...
        IWithdrawer withdrawer = IWithdrawer(feeCollector);
        withdrawer.receiveWithdrawalETH{
            value: ((msg.value - tipAmount) * commission) / 100
        };
        ...
        if (amounts[0] < ethAmount) {
            IWithdrawer withdrawer2 = IWithdrawer(msg.sender);
            withdrawer2.receiveWithdrawalETH{value: ethAmount - amounts[0]};
        }
    }

    function buyToken(address tokenOut) public payable {
        IWithdrawer withdrawer = IWithdrawer(feeCollector);
        withdrawer.receiveWithdrawalETH{value: (msg.value * commission) / 100};
    ...
    function sellToken(address tokenOut, uint256 amountIn) public {
        ...
        IWithdrawer withdrawer = IWithdrawer(feeCollector);
        withdrawer.receiveWithdrawalETH{value: (amounts[1] * commission) / 100};

        IWithdrawer withdrawer2 = IWithdrawer(msg.sender);
        withdrawer2.receiveWithdrawalETH{
            value: (amounts[1] * (100 - commission)) / 100
        };
    }
}