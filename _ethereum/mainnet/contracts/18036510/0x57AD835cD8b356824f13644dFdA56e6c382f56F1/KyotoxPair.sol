...
contract KyotoxPair is KyotoxERC20 {
    ...
    function _sendFees(address token, uint amount) internal returns (uint feeAmount) {
        feeAmount = amount * fee / (FEE_BASE - fee);
        if (feeAmount > 0) {
            _safeTransfer(token, feeReceiver, feeAmount);
        }
    }
    ...
    function swap(uint amount0Out, uint amount1Out, address to, bytes calldata data) external lock {
        ...
        (uint112 _reserve0, uint112 _reserve1, ) = getReserves(); // gas savings
        require(amount0Out < _reserve0 && amount1Out < _reserve1, 'Kyotox: INSUFFICIENT_LIQUIDITY');
...

        {
            ...
            if (amount0Out > 0) {
                // optimistically transfer fee
                _sendFees(_token0, amount0Out);
                _safeTransfer(_token0, to, amount0Out); // optimistically transfer tokens
            }
            if (amount1Out > 0) {
                _sendFees(_token1, amount1Out);
