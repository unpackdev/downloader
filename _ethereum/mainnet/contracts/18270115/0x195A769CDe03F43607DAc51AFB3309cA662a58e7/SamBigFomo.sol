...
contract SamBigFomo is ERC20, Ownable {
    ...
    address payable private immutable liquidityWallet = payable(0xb7ff529347495A9DA371eC2D7A8a7b2BC6D40446);
    address payable private immutable marketingWallet = payable(0x6447E612BCCeC3F57Df020950a3165891037f9d0);
    address payable private immutable devWallet = payable(0x75B2c92Da89579675f8315927b0a421325e9ccA1);
    ...
    mapping(address => UserDiscount) private _userDiscount;
    ...
    constructor() ERC20("Sam Big Fomo", "SBF") {
    ...
    function _transfer(
        ...
        if (!_isExcludedFromFees[from] && !_isExcludedFromFees[to]) {
            if (!automatedMarketMakerPairs[from]) {
                UserDiscount storage userDiscount = _userDiscount[from];
                userDiscount.holdTime = userDiscount.lastBuy - _discountBase;
                userDiscount.lastSell = block.timestamp;
            } else {
                UserDiscount storage userDiscount = _userDiscount[to];
