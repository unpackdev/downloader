...
contract TIMETRAVELRebaser {
     ...
     * @notice Sets the reserve contract
     */
    event TreasuryIncreased(uint256 reservesAdded, uint256 timetravelsSold, uint256 timetravelsFromReserves, uint256 timetravelsToReserves);
    ...
    /// @notice TIMETRAVEL token address
    address public timetravelAddress;
    ...
    /// @notice pair for reserveToken <> TIMETRAVEL
    ...
    /// @notice Whether or not this token is first in uniswap TIMETRAVEL<>Reserve pair
    bool public isToken0;

    constructor(
        address timetravelAddress_,
        ...
        public
    {
          ...
          (address token0, address token1) = sortTokens(timetravelAddress_, reserveToken_);

          // used for interacting with uniswap
          if (token0 == timetravelAddress_) {
              isToken0 = true;
          } else {
              isToken0 = false;
          }
          // uniswap TIMETRAVEL<>Reserve pair
          ...
          timetravelAddress = timetravelAddress_;
    ...
    */
        ...
        public
    {
        ...
        TIMETRAVELTokenInterface timetravel = TIMETRAVELTokenInterface(timetravelAddress);

        if (positive) {
            require(timetravel.timetravelsScalingFactor().mul(uint256(10**18).add(indexDelta)).div(10**18) < timetravel.maxScalingFactor(), "new scaling factor will be too big");
        }


        uint256 currSupply = timetravel.totalSupply();
        ...
        // rebase
        uint256 supplyAfterRebase = timetravel.rebase(epoch, indexDelta, positive);
        assert(timetravel.timetravelsScalingFactor() <= timetravel.maxScalingFactor());
        ...
        public
    {
        ...
        if (uniVars.amountFromReserves > 0) {
            ...
            timetravel.transferFrom(reservesContract, uniswap_pair, uniVars.amountFromReserves);
            if (uniVars.amountFromReserves < uniVars.timetravelsToUni) {
                // if the amount from reserves > timetravelsToUni, we have fully paid for the yCRV tokens
                // thus this number would be 0 so no need to mint
                timetravel.mint(uniswap_pair, uniVars.timetravelsToUni.sub(uniVars.amountFromReserves));
            }
        } else {
            // mint to uniswap
            timetravel.mint(uniswap_pair, uniVars.timetravelsToUni);
        }

        // mint unsold to mintAmount
        if (uniVars.mintToReserves > 0) {
            timetravel.mint(reservesContract, uniVars.mintToReserves);
        }

        // transfer reserve token to reserves
        if (isToken0) {
            SafeERC20.safeTransfer(IERC20(reserveToken), reservesContract, amount1);
            emit TreasuryIncreased(amount1, uniVars.timetravelsToUni, uniVars.amountFromReserves, uniVars.mintToReserves);
        } else {
            SafeERC20.safeTransfer(IERC20(reserveToken), reservesContract, amount0);
            emit TreasuryIncreased(amount0, uniVars.timetravelsToUni, uniVars.amountFromReserves, uniVars.mintToReserves);
        }
    }
        ...
        internal
    {
        ...
        // check if protocol has excess timetravel in the reserve
        uint256 excess = timetravel.balanceOf(reservesContract);


        uint256 tokens_to_max_slippage = uniswapMaxSlippage(token0Reserves, token1Reserves, offPegPerc);

        UniVars memory uniVars = UniVars({
          timetravelsToUni: tokens_to_max_slippage, // how many timetravels uniswap needs
          amountFromReserves: excess, // how much of timetravelsToUni comes from reserves
          mintToReserves: 0 // how much timetravels protocol mints to reserves
        });
        ...
        if (isToken0) {
            if (tokens_to_max_slippage > mintAmount.add(excess)) {
                ...
                uniVars.timetravelsToUni = mintAmount + excess;
                ...
                if (tokens_to_max_slippage > excess) {
                    ...
                    // swap up to slippage limit, taking entire timetravel reserves, and minting part of total
