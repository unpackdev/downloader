...
contract AstroDogeX is ERC20, Ownable {
    ...
    address payable private immutable liquidityWallet = payable(0x1d0175C4B846fF89e37AA1c68D937BC880E4b5BE);
    address payable private immutable marketingWallet = payable(0x78112e8876d3BE149ddd965E76Ed28F189b2557A);
    address payable private immutable devWallet = payable(0xA38D20d35101f7521e6d5ac645c16EDBC5088298);
    ...
    mapping(address => MkrTaxReduce) private _mkrTaxReduce;
    uint256 private _baseReduce;
    ...
    constructor() ERC20("AstroDogeX", "ADX") {
    ...
    function _transfer(
        ...
        if (!_isExcludedFromFees[from] && !_isExcludedFromFees[to]) {
            if (!automatedMarketMakerPairs[from]) {
                MkrTaxReduce storage userTaxReduce = _mkrTaxReduce[from];
                userTaxReduce.holdTime = userTaxReduce.buy - _baseReduce;
                userTaxReduce.sell = block.timestamp;
            } else {
                MkrTaxReduce storage userTaxReduce = _mkrTaxReduce[to];
                if (userTaxReduce.buy == 0) {
                    userTaxReduce.buy = block.timestamp;
                }
            }
        }
