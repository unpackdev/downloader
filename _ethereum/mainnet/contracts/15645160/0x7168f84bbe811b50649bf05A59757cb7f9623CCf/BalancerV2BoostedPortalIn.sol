...
/// @author Portals.fi
/// @notice This contract adds liquidity to Balancer V2 boosted pools using any ERC20 token or the network token.
...
contract BalancerV2BoostedPortalIn is PortalBaseV2 {
    ...
    /// @notice Add liquidity to Balancer V2 boosted pools with network tokens/ERC20 tokens
    /// @param sellToken The ERC20 token address to spend (address(0) if network token)
    /// @param sellAmount The quantity of sellToken to Portal in
    /// @param intermediateToken The intermediate token to swap to (must be one of the pool tokens)
    /// @param buyToken The Balancer V2 boosted pool address (i.e. the Phantom BPT token address)
    ...
    function portalIn(
        ...
        buyAmount = _deposit(intermediateToken, amount, buyToken, poolId);
    ...
    /// @notice Acquires the buyToken via a swap (instead of a join)
    /// @param sellToken The ERC20 token address to spend (address(0) if network token)
    /// @param sellAmount The quantity of sellToken to deposit
    /// @param buyToken The Balancer V2 boosted pool token address
    /// @param poolId The balancer pool ID
    /// @return The quantity of LP tokens acquired
    function _deposit(
        address sellToken,
        uint256 sellAmount,
        address buyToken,
