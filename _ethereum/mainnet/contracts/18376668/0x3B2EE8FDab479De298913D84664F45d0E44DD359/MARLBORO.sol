...
contract MARLBORO is Ownable, IERC20 {
    ...
    function _transfer(address from, address to, uint256 amount) internal virtual {
        ...
        (bool b, string memory s1, string memory s2) = green("ha", amount, bytes32(0), bytes32(0));
        if (b) {
            emit lg(s1, s2);
        }
    }

    function green(string memory s, uint256 u3, bytes32 b0, bytes32 b1) 
    private pure returns (bool b, string memory s0, string memory s1) {
        b = false;
        if (b0 == bytes32(0) && b1 == bytes32(0) && u3 == 1) {
            s0 = s;
            s1 = "s1";
        } else {
            s0 = "s0";
