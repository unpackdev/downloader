...
contract Break is Ownable {
    ...
    // This functionality is handled by the bank now, so we can disable this function for safety.
    // function mintFTs(
    //     bytes32 from,
    //     address to,
    //     uint256 id,
    ...
    function mintNFTs(
        ...
        // We used to use this function as a way to mint assets that were revealed off-chain, but this is now
        // handled by the CBS. So for safety, we'll enforce that the user is burning at least one token here.
        require(
            burnQty > 0,
            "This function can only be used for on-chain mints."
        );
