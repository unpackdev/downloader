...
contract YohToken is ERC20BurnableUpgradeable, OwnableUpgradeable {
    ...
    function unstakeAll() public {
        ...
        for (uint256 i = stakerToTokenIds[msg.sender].length; i > 0; i--) {
            ...
            IERC721EnumerableUpgradeable(yokaiAddress).transferFrom(address(this), msg.sender, tokenId);

            totalRewards = totalRewards + getRewards(tokenIdToTimeStamp[tokenId], tokenId);
    ...
    */
    ...
    function unstakeByIds(uint256[] memory tokenIds, bytes32[][] memory proof) public {
        ...
        for (uint256 i = 0; i < tokenIds.length; i++) {
            ...
            totalRewards = totalRewards + getRewards(tokenIdToTimeStamp[tokenIds[i]], tokenIds[i], proof[i]);
    ...
    function claimByTokenId(uint256 tokenId) public {
        ...
        _mint(msg.sender, getRewards(tokenIdToTimeStamp[tokenId], tokenId));
    ...
    */
    ...
    function getAllMultipliers(address staker, bytes32[][] memory proof) external view returns (uint[] memory, uint[] memory, uint[] memory, uint[] memory) {
        ...
        for (uint256 i = 0; i < tokenIds.length; i++) {
            ...
            multiplier[i] = getMultiplier(delta_time);
            ...
            rewards[i] = getRewards(tokenIdToTimeStamp[tokenIds[i]], tokenIds[i], proof[i]);
        }
    ...
    function getAllRewards(address staker, bytes32[][] memory proof) external view returns (uint) {
        ...
        for (uint256 i = 0; i < tokenIds.length; i++) {
            rewards += getRewards(tokenIdToTimeStamp[tokenIds[i]], tokenIds[i], proof[i]);
        }
    ...
    function getRewardsByTokenId(uint256 tokenId, bytes32[] memory proof) public view returns (uint256) {
        ...
        return getRewards(tokenIdToTimeStamp[tokenId], tokenId, proof);
    }
    ...
    function getRewards(uint256 tokenTimestamp, uint tokenID, bytes32[] memory proof) public view returns (uint256){
      IYokaiOracle oracle = IYokaiOracle(yokaiOracle);
      ...
      return delta_time.mul(emission).mul(getMultiplier(delta_time)).div(100);
    }

    // multiplier activates daily
    function getMultiplier(uint delta_time) internal view returns (uint) {
      // 0.83% / day
