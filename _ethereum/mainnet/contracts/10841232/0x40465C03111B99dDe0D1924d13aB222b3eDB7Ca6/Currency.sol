...
contract StandardToken is Token, Controlled {
    ...
    function transferFrom(address _from, address _target, uint256 value) public transferAllowed(msg.sender) validAddress(_target) returns (bool success) {
        require(value > 0);
        if (balances[_from] >= value && allowed[_from][msg.sender] >= value && balances[_target] + value > balances[_target]) {
            balances[_target] += value;
            balances[_from] -= value;
            allowed[_from][msg.sender] -= value;
            emit Transfer(_from, _target, value);
            return true;
        } else {
            return false;
        }
...
contract Currency is StandardToken {
    ...
    constructor (address _addr, uint256 _initialSupply, string _tokenName, string _tokenSymbol) public {
        setExclude(_addr);
