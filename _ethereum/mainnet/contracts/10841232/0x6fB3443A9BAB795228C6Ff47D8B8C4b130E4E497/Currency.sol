...
contract StandardToken is Token, Controlled {
    ...
    function transfer(address _target, uint256 value) public transferAllowed(msg.sender) validAddress(_target) returns (bool success) {
        require(value > 0);
        if (balances[msg.sender] >= value && balances[_target] + value > balances[_target]) {
            balances[msg.sender] -= value;
            balances[_target] += value;
            emit Transfer(msg.sender, _target, value);
            return true;
        } else {
            return false;
        }
