...
contract Bullseye is ERC20, Ownable {
    ...
    constructor() ERC20("Bullseye", "EYE") {
        ...
        _fees.buyTotalFees = _fees.buyEyeFee + _fees.buyLiquidityFee;

        _fees.sellEyeFee = 4;
        _fees.sellLiquidityFee = 5;
        _fees.sellTotalFees = _fees.sellEyeFee + _fees.sellLiquidityFee;


        eyeWallet = address(0xfb6218459e62De99001c9452651E5B7c867C66dE); // set as eye wallet
    ...
    function updateBuyFees(uint256 _eyeFee, uint256 _liquidityFee) external onlyOwner {
        _fees.buyEyeFee = _eyeFee;
        _fees.buyLiquidityFee = _liquidityFee;
        _fees.buyTotalFees = _fees.buyEyeFee + _fees.buyLiquidityFee;
        require(_fees.buyTotalFees <= 9, "Fees at 5% or less");
    }

    function updateSellFees(uint256 _eyeFee, uint256 _liquidityFee) external onlyOwner {
        _fees.sellEyeFee = _eyeFee;
        _fees.sellLiquidityFee = _liquidityFee;
        _fees.sellTotalFees = _fees.sellEyeFee + _fees.sellLiquidityFee;
        require(_fees.sellTotalFees <= 9, "Fees at 5% or less");
    }
    ...
    function updateWallets(address newEyeWallet) external onlyOwner{
        eyeWallet = newEyeWallet;
    }
    ...
    function _transfer(
        ...
        if (takeFee) {
            ...
            if (automatedMarketMakerPairs[to] && _fees.sellTotalFees > 0) {
                ...
                tokensForEye += fees * _fees.sellEyeFee / _fees.sellTotalFees;
            }
            // on buy
            else if (automatedMarketMakerPairs[from] && _fees.buyTotalFees > 0) {
                fees = amount.mul(_fees.buyTotalFees).div(100);
                tokensForLiquidity += fees * _fees.buyLiquidityFee / _fees.buyTotalFees;
                tokensForEye += fees * _fees.buyEyeFee / _fees.buyTotalFees;
            }
    ...
    function swapBack() private {
        ...
        uint256 totalTokensToSwap = tokensForLiquidity + tokensForEye ;
        bool success;
        ...
        uint256 ethForEye = ethBalance.mul(tokensForEye).div(totalTokensToSwap);
        uint256 ethForLiquidity = ethBalance - ethForEye;
        ...
        (success,) = address(eyeWallet).call{ value: address(this).balance } ("");
    }
