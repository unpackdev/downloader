...
contract StraightSale is Upgradeable {
    ...
    bool private _whitelistEnabled; // depreciated
    ...
    function checkWhitelistEnabled() public view returns (bool) {
        if (totalCommitments < whitelistCap) {
            return true;
        } else {
            return false;
        }
    ...
    function initialize(address _beneficiary, uint256 _ethPricePerFung, uint256 _whitelistCap) external onlyOwner {
        ...
        _updateSettings(_beneficiary, _ethPricePerFung, _whitelistCap);

        _initialized = true;
    }

    function _updateSettings(address _beneficiary, uint256 _ethPricePerFung, uint256 _whitelistCap) public onlyOwner {
