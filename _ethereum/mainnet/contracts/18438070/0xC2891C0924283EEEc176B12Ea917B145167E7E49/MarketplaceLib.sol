...
library MarketplaceLib {
    ...
    event PurchaseEvent(uint40 indexed listingId, address buyer, uint24 count, uint256 amount);
    ...
    event ModifyListingType(uint40 indexed listingId, uint8 listingType);
    ...
    function constructListing(
        ...
        require(
            listingDetails.startTime == 0 || listingDetails.startTime > block.timestamp,
        ...
        if (listingDetails.type_ == ListingType.INDIVIDUAL_AUCTION) {
            ...
            require(
                listingDetails.initialAmount == 0 && listingDetails.startTime > block.timestamp,
    ...
    function modifyListing(
        ...
        require(
            startTime == 0 ||
                (startTime == listing.details.startTime && endTime > block.timestamp) ||
        ...
        require(
            !isFinalized(listing.flags) &&
                ((!isAuction(listing.details.type_) && listing.totalSold == 0) ||
                    (isAuction(listing.details.type_) && listing.bidCount == 0)),
