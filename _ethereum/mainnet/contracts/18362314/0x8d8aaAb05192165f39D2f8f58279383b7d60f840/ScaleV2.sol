...
/**
 * V1.1 is in order to account for the already minted tokens on base network
 * and contains changes to the addLiquidity function to account for that.
 * 
 * Since it's an upgrade, we can also omit the initialiser. 
 */

contract ScaleV2 is OFTCoreUpgradeable, IERC20Upgradeable {
    ...
    function _creditTo(
        ...
        emit Transfer(ZERO, _toAddress, _amount);
        return _amount;
    }
    ...
    function _burnLP() internal {
        ...
        if(tokensToBurn != 0) {
            ...
            _baseBalance[ZERO] = _baseBalance[ZERO] + reflectionBurnAmount;
