...
import "./Ownable.sol";
import "./IAdapter.sol";
import "./IAddressProviderV3.sol";
import "./IPriceOracleV3.sol";
...
contract GasPricer is IGasPricer, Ownable {
    ...
    constructor(address _IAddressProviderV3) {
        IAddressProviderV3 ap = IAddressProviderV3(_IAddressProviderV3);

        wethToken = ap.getAddressOrRevert(AP_WETH_TOKEN, NO_VERSION_CONTROL); // F:[CPF-1]
        priceOracle = IPriceOracleV3(
            ap.getAddressOrRevert(AP_PRICE_ORACLE, 3_00)
        ); // F:[CPF-1]
    }
