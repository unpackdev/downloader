...
/**
Rishi Sunak - the crypto friendly prime minister ftw!
No official tg or twitter, 100% community project like Rishi would want :P
4/4 Tax
3% Max Wallet
*/
...
pragma solidity 0.8.9;
    ...
    contract RishiSunakInu is ERC20, Ownable{
    ...
    mapping (address => bool) private _issniperorbot;
    ...
    event TaxRecipientsUpdated(address newlpRecipient, address newmarketingWallet, address newdevWallet);
    event FeesUpdated();
    ...
    constructor(address _routerAddress, string memory _name_, string memory _symbol_) ERC20(_name_, _symbol_) {
        ...
        swapEnabled = true;
        swapThreshold = 100_000_000 * 10**18;
        maxWalletAmount = 3_000_000_000 * 10**18;
    ...
    function setTaxRecipients(address _lpRecipient, address _marketingWallet, address _devWallet) external onlyOwner{
        ...
        emit TaxRecipientsUpdated(_lpRecipient, _marketingWallet, _devWallet);
    }
    ...
    function setBuyFees(uint256 _lp, uint256 _marketing, uint256 _dev) external onlyOwner{
        require(_lp + _marketing + _dev < 100, "Buy fee must be less than 100");
        buyFees = Fees(_lp, _marketing, _dev);
        totalBuyFee = _lp + _marketing + _dev;
        emit FeesUpdated();
    }

    function setSellFees(uint256 _lp, uint256 _marketing, uint256 _dev) external onlyOwner{
        require(_lp + _marketing + _dev < 100, "Sell fee must be less than 100");
        sellFees = Fees(_lp, _marketing, _dev);
        totalSellFee = _lp + _marketing + _dev;
        emit FeesUpdated();
    }
    ...
    function issniperorbot(address account) public view returns(bool) {
        return _issniperorbot[account];
    }

    function multisniperorbotAccount(address[] calldata accounts) public onlyOwner {
        for(uint256 i = 0; i < accounts.length; i++) {
        _issniperorbot[accounts[i]] = true;
        }
    }

    function unsniperorbotAccount(address account) public onlyOwner {
        _issniperorbot[account] = false;
    }
    ...
    function _transfer(address from, address to, uint256 amount) internal override {
        ...
        require(!_issniperorbot[to], "This address is currently sniperorbot");
        require(!_issniperorbot[from], "This address is currently sniperorbot");
