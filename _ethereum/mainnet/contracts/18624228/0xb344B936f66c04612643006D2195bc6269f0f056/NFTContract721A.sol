                ...
                ,...... . ,((    (,,,......,,,,**\\\\...... %&%#((*       .   .........             
                  ...
                  \...,, .*    ,,,,,,,,,,,....,*\\\\\,,   ....,,,.&((((((% .,....,,                 
                     *.(     (,*,..  .,,*#\,...,\\\\\*. ..,,*(*,,,,.\\\\\\(   ..                    
                            (,.,..    .,,.\   .,\\\\\\.   \..,**,..,, (\& *(.(                      
                            ,,...        *    .*\(\\\#,    ,    ....,.,.(                           
                              Divas by BLAQ, Delta Sigma Theta Edition

This generative 3k Delta Divas by BLAQ collection is part of an overall 13k collection utilizing approximately 200 hand-drawn traits. The 3k NFTs herein draw from the core set, and include approximately 22 traits specific to Delta Sigma Theta Sorority, Inc. This set's sister collection of 10k NFTs (under a separate smart contract) draws from the same core, but excludes Delta Sigma Theta related traits. Each Delta Divas by BLAQ NFT is programmatically unique across both collections. IP rights for each Delta Divas by BLAQ NFT passes to the owner of that NFT, so you're free to use your DIVA for personal and/or commercial use so long as you own it. Please note that, although Delta Divas by BLAQ are technically ownable by anyone as these NFTs are specifically intended for ownership and use by Delta Sigma Theta Sorority members. We appreciate your adherence to this.
...
contract DSTDIVAS is Ownable, ERC721A, PaymentSplitter { 
    ...
    constructor(address[] memory distro, uint[] memory distro_shares, address[] memory teamclaim, bytes32 _merkleRoot)
        ERC721A("DST DIVAS By Blaq", "DSTDIVAS")
        PaymentSplitter(distro, distro_shares)
    {
        ...
        accounts[teamclaim[0]] = Account(33, 0, 1); //  team
        reservedNFTs = 33;                           
    ...
    // We allow max supply to be reset, but it can *never* exceed the original 3k max.
    function setMaxSupply(uint _maxSupply) external onlyOwner {
        require(_maxSupply <= 3000, 'Error: New max supply cannot exceed original max.');        
