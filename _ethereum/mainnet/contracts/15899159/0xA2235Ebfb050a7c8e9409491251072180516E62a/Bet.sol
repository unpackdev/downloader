    ...
    nonReentrant notContract() {
        ...
        for(uint i = 0; i < EnteredEncierros[msg.sender].length; i++) {
            ...
            if(Encierros[sessionID].status == Status.Claimable && !HasClaimed[msg.sender][sessionID] && HasBet[msg.sender][sessionID]) {
                ...
                for (uint16 z = 0; z < BetAlphasPerEncierro[msg.sender][sessionID].length; z++) { // fetch their bet NFT ids for this encierro                    
                    // calculate winnings
    ...
    function getUnclaimedBetRewards(address account) external view returns (uint256){
        ...
        for(uint i = 0; i < EnteredEncierros[account].length; i++) {
            ...
            if(Encierros[sessionID].status == Status.Claimable && !HasClaimed[account][sessionID] && HasBet[account][sessionID]) {
                ...
                for (uint16 z = 0; z < BetAlphasPerEncierro[account][sessionID].length; z++) { // fetch their bet NFT ids for this encierro                    
                    // calculate winnings
