...
contract PriceCalculator is PriceCalculatorInterface {
  ...
  uint256 public constant TOLERANCE_RATE = 1030000000000000000; //= 103.1%
  uint256 public constant SECURE_RATE = 1100000000000000000; //110% max slippage for all orders
  ...
  function calculatePrice(
    ...
    uint256 lowPrice = (reserve1.divByRate(reserve0)).divByRate(TOLERANCE_RATE);
  ...
  function _calculatePriceAnd0_1RefundRate(
    ...
    if (executeAmount > AmountFLEX0_1) {
      ...
      if (nextLowPrice > price) {
        ...
        if (executeAmount < AmountFLEX0_1) {
          uint256 refundRate = (AmountFLEX0_1.sub(executeAmount)).divByRate(
            AmountFLEX0_1
          );
          return [nextLowPrice, 2, refundRate, executeAmount, all1_0Amount];
        }
      }
  ...
  function _calculatePriceAnd1_0RefundRate(
    ...
    if (executeAmount > AmountFLEX1_0) {
      ...
      if (nextHighPrice < price) {
        ...
        if (executeAmount < AmountFLEX1_0) {
          uint256 refundRate = (AmountFLEX1_0.sub(executeAmount)).divByRate(
            AmountFLEX1_0
          );
          return [nextHighPrice, 4, refundRate, all0_1Amount, executeAmount];
        }
      }
  ...
  function _calculateExecuteAmount0_1(
    ...
    uint256 possibleReserve = (opponentReserve.add(opppnentAmount)).divByRate(
      price
    );
  ...
  function _calculateExecuteAmount1_0(
    ...
    uint256 possibleReserve = (opponentReserve.add(opppnentAmount)).mulByRate(
      price
    );
