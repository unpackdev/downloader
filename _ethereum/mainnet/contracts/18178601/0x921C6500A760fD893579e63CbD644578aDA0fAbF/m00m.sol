...
contract m00mPhaseLimits is m00m {
    error PhaseLimitExceeded();

    struct PhaseLimit {
        uint128 limit;
        uint128 supply;
    }
    mapping(PHASE => PhaseLimit) public phaseLimits;

    function mint(PHASE phase, bytes32[] memory proof, uint256 quantity) public virtual override payable {
        PhaseConfig memory phaseConfig = phases[phase];
        PhaseLimit memory phaseLimit = phaseLimits[phase];

        if (phaseLimit.supply + quantity > phaseLimit.limit) revert PhaseLimitExceeded();
        if (totalSupply() + quantity > MAX_SUPPLY) revert MaxSupplyReached();
        if (msg.value * quantity < phaseConfig.price) revert InsufficientValue();
        if (
            phaseConfig.merkleRoot != bytes32(0)
        ...
        phaseLimits[phase].supply += uint128(quantity);
        numberMinted[phase][msg.sender] += quantity;

        _mint(msg.sender, quantity);
    }

    function setPhaseLimit(PHASE phase, uint128 phaseLimit) external onlyOwner {
        phaseLimits[phase].limit = phaseLimit;
    }
}

contract m00mV2 is m00mPhaseLimits {
    function mint(PHASE phase, bytes32[] memory proof, uint256 quantity) public virtual override payable {
        PhaseConfig memory phaseConfig = phases[phase];
        PhaseLimit memory phaseLimit = phaseLimits[phase];
