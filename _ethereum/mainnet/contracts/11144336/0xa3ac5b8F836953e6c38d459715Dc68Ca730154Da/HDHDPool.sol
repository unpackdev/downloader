...
contract HDTokenWrapper {
    ...
    //HD
    IERC20 public stakeToken = IERC20(0x6ce654aC973D326F89f0685E7459542641410eD9); 
 ...
 * HD-HD Pool
 */
contract HDHDPool is HDTokenWrapper, IRewardDistributionRecipient {
    ...
    function stake(uint256 amount) public checkOpen checkStart updateReward(msg.sender){ 
        require(amount > 0, "HD-HD-POOL: Cannot stake 0");
        super.stake(amount);
        emit Staked(msg.sender, amount);
    }

    function withdraw(uint256 amount) public checkStart updateReward(msg.sender){
        require(amount > 0, "HD-HD-POOL: Cannot withdraw 0");
    ...
    modifier checkStart(){
        require(block.timestamp > startTime,"HD-HD-POOL: Not start");
        _;
    }

    modifier checkOpen() {
        require(open, "HD-HD-POOL: Pool is closed");
        _;
    }
