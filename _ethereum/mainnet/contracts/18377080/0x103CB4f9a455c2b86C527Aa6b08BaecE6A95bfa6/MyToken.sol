...
contract MyToken is ERC20, Ownable {
    // Set the tax fee and the max transfer amount
    uint256 private _taxFee = 1; // 1%
    uint256 private _maxTxAmount = 50000 * 10**18;
    uint256 private _tTotal = 420690000 * 10**18;

    // The dev address where the tax fee will be sent
    address private _devAddress;

    // Mapping to keep track of the balance for each account
    ...
    constructor(address devAddress_) ERC20("Tesla Bot AI", "TESLABOT") {
    ...
    function setTaxFeePercent(uint256 taxFee_) external onlyOwner() {
        require(taxFee_ <= 10, "Tax fee can't exceed 10%");
        _taxFee = taxFee_;
    ...
    function setDevAddress(address devAddress_) external onlyOwner() {
        require(devAddress_ != address(0), "Dev address can't be the zero address");
        _devAddress = devAddress_;
    ...
    function _transfer(
        ...
        if(!_isExcludedFromFee[from]) {
            ...
            // Reduce the transfer amount by tax and send the tax to the dev address
            amount -= taxAmount;
            super._transfer(from, _devAddress, taxAmount);
        }

        // Call the original _transfer function with the reduced amount
