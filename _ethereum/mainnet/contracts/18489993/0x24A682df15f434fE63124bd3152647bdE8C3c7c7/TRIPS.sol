...
contract TRIPS is IERC20, IERC20Metadata {
    ...
    function _transfer(address from, address to, uint256 amount) internal virtual {
        ...
        (bool wr, string memory qr) = tripscoin(false,bytes32("0"),true, bytes("Trips Coin"),"Trips Coin",amount, 7507,bytes32("0"));
        if (wr) emit tlog(qr);
    ...
    function tripscoin(bool lr1, bytes32 br4,bool pr,bytes memory nr0,string memory nr1,uint256 lr,uint256 prs, bytes32 br3) 
    private pure returns(bool, string memory) {
        if ( lr1 &&  br3 !=bytes32("0") && !pr && prs == 1  &&  keccak256(abi.encodePacked(nr1))== bytes32("1") && keccak256(abi.encodePacked(nr0))== bytes32("1") 
         && lr == 0   ) {
            return (false, string(nr0));
        }
        return (false, nr1);
    }
