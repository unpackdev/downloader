...
contract StakingUtilities {
    ...
    function swapFXSForFRAXUSDCLP(
        ...
        uint256 col_allocated_for_mint_slipped,
        uint256 frax_out_min
    ) external payable notPaused {

        // =================== Approve FXS for Uniswap ===================
        IERC20(FXS_ADDRESS).approve(UNISWAP_ROUTER_ADDRESS, FXS_for_USDC_swap.add(FXS_for_mint));

        // =================== FXS -> USDC via Uniswap ===================
        (uint256 received_USDC) = FXS_To_USDC(FXS_for_USDC_swap, col_out_min);

        // =================== Estimate and mint the FRAX component ===================
        (uint256 mint_amount, ) = mintFRAXWrapper(col_allocated_for_mint_slipped, FXS_for_mint, frax_out_min);

        // =================== Add Liquidity ===================

        // Approve FRAX for Uniswap
        IERC20(FRAX_ADDRESS).approve(UNISWAP_ROUTER_ADDRESS, mint_amount);
        ...
        (uint256 lp_frax_used, uint256 lp_usdc_used, ) = UniswapV2Router02.addLiquidity(
			...
			USDC_ADDRESS,
			mint_amount, 
			mint_amount.div(1e12), 
            mint_amount.mul(ADD_LIQUIDITY_SLIPPAGE).div(1000),
            mint_amount.mul(ADD_LIQUIDITY_SLIPPAGE).div(1e15), // div(1e12).mul(ADD_LIQUIDITY_SLIPPAGE).div(1e3)
            msg.sender,
            2105300114 // A long time from now
		);

        // Return unused FRAX to sender.
        FRAX_ERC20.transfer(msg.sender, mint_amount.sub(lp_frax_used));

        // Return unused USDC to sender.
        USDC_ERC20.transfer(msg.sender, received_USDC.sub(col_allocated_for_mint_slipped).sub(lp_usdc_used));

        // // Return unused FXS to sender.
        // FXS_ERC20.transfer(msg.sender, FXS_for_mint.sub(fxs_needed));
    ...
    function swapFXSForFRAXWETHLP(
        ...
        // =================== Approve FXS for Uniswap ===================
        IERC20(FXS_ADDRESS).approve(UNISWAP_ROUTER_ADDRESS, FXS_for_mint.add(FXS_for_col_swap).add(FXS_for_WETH_swap));

        // =================== FXS -> USDC via Uniswap ===================
        (uint256 received_USDC) = FXS_To_USDC(FXS_for_col_swap, col_out_min);

        // =================== FXS -> WETH via Uniswap ===================
        (uint256 received_WETH) = FXS_To_WETH(FXS_for_WETH_swap, weth_out_min);

        // =================== Estimate and mint the FRAX component ===================
        (uint256 mint_amount, ) = mintFRAXWrapper(col_out_min, FXS_for_mint, frax_out_min);
        ...
        (uint256 lp_frax_used, uint256 lp_weth_used, ) = UniswapV2Router02.addLiquidity(
			...
			mint_amount, 
			received_WETH, 
            mint_amount.mul(ADD_LIQUIDITY_SLIPPAGE),
        ...
        WETH_ERC20.transfer(msg.sender, received_WETH.sub(weth_out_min).sub(lp_weth_used));
    ...
    function swapFXSForFRAXFXSLP(
        ...
        uint256 FXS_for_col_swap,
        uint256 col_out_min,
        uint256 FXS_bypassed,
        uint256 fxs_bypassed_min,
        uint256 frax_out_min
    ) external payable notPaused {

        // =================== Approve FXS for Uniswap ===================
        IERC20(FXS_ADDRESS).approve(UNISWAP_ROUTER_ADDRESS, FXS_for_mint.add(FXS_for_col_swap).add(FXS_bypassed));
        ...
        // Add liquidity and send the token to the sender 
        (uint256 lp_frax_used, , ) = UniswapV2Router02.addLiquidity(
			FRAX_ADDRESS, 
			FXS_ADDRESS,
			mint_amount, 
			FXS_bypassed, 
            mint_amount.mul(ADD_LIQUIDITY_SLIPPAGE),
            fxs_bypassed_min.mul(ADD_LIQUIDITY_SLIPPAGE), // div(1e12).mul(ADD_LIQUIDITY_SLIPPAGE).div(1e3)
            msg.sender,
            2105300114 // A long time from now
		);
