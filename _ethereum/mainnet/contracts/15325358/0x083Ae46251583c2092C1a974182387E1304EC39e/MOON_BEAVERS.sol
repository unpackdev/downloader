...
contract MOON_BEAVERS is Ownable, ERC721A, ReentrancyGuard {
    ...
    constructor() ERC721A("MOON BEAVERS", "MBEAVERS", 1000, maxToken) {
        setUriPrefix("ipfs://URI_Prefix/");
        setHiddenMetadataUri("ipfs://Hidden_DataURI/");
    }
    ...
    function mint(uint256 quantity) external payable mintCompliance(quantity) {
        if (whitelistSale) {
            require(
                numberMinted(msg.sender) + quantity <= whitelistedMintingLimit,
                "PER WALLET LIMIT REACHED"
            );
            require(
                totalSupply() + quantity <= wlSaleLimit,
                "CANNOT MINT MORE WL TOKENS"
            );
