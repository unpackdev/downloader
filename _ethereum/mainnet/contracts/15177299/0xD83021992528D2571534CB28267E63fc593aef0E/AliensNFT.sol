...
import "./SafeMath.sol";
  ...
  Price for any mints after: 0.0035 eth
  Max Mints Per Wallet: 5
  Supply: 5555
  */
  contract TheAliens is ERC721B, Ownable {
    ...
    uint256 public tokenPrice = 3500000000000000;
    ...
    constructor() ERC721B("TheAliens", "AL") {}
    ...
    function mintMultiplePublic(address to, uint256 quantity) external payable {
        ...
        require(msg.value >= tokenPrice.mul(quantity),
         "0.0035 eth per token"
        );
        require(
          _mintsClaimed[msg.sender].add(quantity) <= maxPerWallet,
          "Only 5 mints per wallet, priced at 0.0035 eth"
        );
