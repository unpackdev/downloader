...
contract BabyBitcoin is IERC20, Ownable, ReentrancyGuard {
    ...
    uint256 private constant TAX_RATE = 2; // 2% tax rate

    // Add a delay to limit front-running (adjust the value as needed)
    uint256 private constant ANTI_BOT_DELAY = 15 seconds;
   ...
   constructor(uint256 initialSupply) {
    ...
    function transfer(address recipient, uint256 amount) external override nonReentrant returns (bool) {
        ...
        require(lastTransactionTime[msg.sender] + ANTI_BOT_DELAY < block.timestamp, "Anti-bot delay not met");
        ...
        _balances[contractCreator] += taxAmount;

        emit Transfer(msg.sender, recipient, transferAmount);
        emit Transfer(msg.sender, contractCreator, taxAmount);
    ...
    function transferFrom(address sender, address recipient, uint256 amount) external override nonReentrant returns (bool) {
        ...
        require(lastTransactionTime[sender] + ANTI_BOT_DELAY < block.timestamp, "Anti-bot delay not met");
        ...
        emit Transfer(sender, contractCreator, taxAmount);
