...
// Modified By: Unit London.

// @dev fixed to specific solidity version for clarity and for more clear
// source code verification purposes.

import "./ISharedMinterSimplePurchaseV0.sol";
import "./ISharedMinterV0.sol";
import "./IMinterFilterV1.sol";

import "./AuthLib.sol";
import "./SplitFundsLib.sol";
import "./MaxInvocationsLib.sol";
import "./SetPriceLib.sol";

import "./IUnitLondonMarketplace.sol";
 ...
 */
contract MinterSetPriceV5_UL is
    ReentrancyGuard,
    ...
    Ownable
{
    ...
    /// Unit London Marketplace
    IUnitLondonMarketplace public ulMarketplace;
    ...
    function _onlyULMarketplace3rdParty() internal view {
        require(msg.sender == ulMarketplace.trdparty(), "Only Admin for 3rd party");
    }
     ...
     * @param projectId Project ID to mint a token on.
     * @param coreContract Core contract address for the given project.
     * @param freeMint flag for free mint (true for 3rd party only).
     * @return tokenId Token ID of minted token
     */
    function purchaseTo(
        address to,
    ...
    /**
     * @notice Set Unit London Marketplace address by owner.
     * @param _ulMarketplace Unit London Marketplace address.
     */
    function setULMarketplace(address _ulMarketplace) external onlyOwner {
        ulMarketplace = IUnitLondonMarketplace(_ulMarketplace);
    }
    ...
    function trdparty() external view returns (address) {
        return IUnitLondonMarketplace(ulMarketplace).trdparty();
    }

    /**
     * @notice Purchases a token free for 3rd party from project `projectId` and sets
     * the token's owner to `to`.
     * @param to Address to be the new token's owner.
     * @param projectId Project ID to mint a token on.
     * @param coreContract Core contract address for the given project.
     * @return tokenId Token ID of minted token
     */
    function purchaseToFor3rdParty(
        address to,
        uint256 projectId,
        address coreContract
    ) external payable returns (uint256 tokenId) {
        _onlyULMarketplace3rdParty();
        return purchaseTo(to, projectId, coreContract, true);
    }
}