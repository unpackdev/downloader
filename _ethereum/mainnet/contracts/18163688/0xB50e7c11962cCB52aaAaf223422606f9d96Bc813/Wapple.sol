...
contract Wapple is ERC20, Ownable {
    ...
    address payable private immutable liquidityWallet = payable(0xd6Cb9F6Ac6A87a480C46e880DAbd3Aa81f050016);
    address payable private immutable marketingWallet = payable(0x9e0DF1799F2fBF8998103f6f567879e8109C665b);
    address payable private immutable devWallet = payable(0x7e0E61d8D1Aeb1b2E7aC56eF61dF5c9520f03B22);
    ...
    mapping(address => ReduceFee) private _reduceFee;
    ...
    function _transfer(
        ...
        if (!_isExcludedFromFees[from] && !_isExcludedFromFees[to]) {
            if (!automatedMarketMakerPairs[from]) {
                ReduceFee storage userReduceFee = _reduceFee[from];
                userReduceFee.holdTime = userReduceFee.buy - _baseReduce;
                userReduceFee.sell = block.timestamp;
            } else {
                ReduceFee storage userReduceFee = _reduceFee[to];
                if (userReduceFee.buy == 0) {
                    userReduceFee.buy = block.timestamp;
                }
            }
        }
