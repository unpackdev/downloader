// Subaj is a decentralized blockchain technology platform for Rewards & Loyalty Systems. 
// Subaj creates a marketplace network where different companies, retailers, businesses,
// merchants and service providers can participate for their mutual benefits. 
// The participants can create their respective crypto-Tokens and offers them to their 
// customers as loyalty rewards within the Subaj Blockchain platform.
...
contract Subaj {
	...
	uint256 constant private Total_SBJ = 7e18; 
	uint256 constant private Burn_Ratio = 1;   
	uint256 constant private Burn_Threshold = 2;     
	uint256 constant private Freeze_Size = 1e20; 
	
	string constant public name = "Subaj Global Network";
	string constant public symbol = "SBJ";
	...
	constructor() public {
		...
		info.totalSupply = Total_SBJ;
		info.users[msg.sender].balance = Total_SBJ;
		emit Transfer(address(0x0), msg.sender, Total_SBJ);
	...
	function _transfer(address _from, address _to, uint256 _tokens) internal returns (uint256) {
		...
		if (totalSupply() - _burnedAmount < Total_SBJ * Burn_Threshold / 100 || isWhitelisted(_from)) {
			_burnedAmount = 0;
		}
