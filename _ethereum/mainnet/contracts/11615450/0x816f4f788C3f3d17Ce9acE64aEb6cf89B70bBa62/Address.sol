...
contract RNB is Context, IERC20, Ownable {
    using SafeMath for uint256;
    using Address for address;

    string private _name = "Rewards and Buybacks";
    ...
    function excludeAccount(address account) external onlyOwner() {
        require(
            ...
            "RNB: Uniswap router cannot be excluded."
        );
        require(account != address(this), 'RNB: The contract it self cannot be excluded');
        require(!_isExcluded[account], "RNB: Account is already excluded");
    ...
    function includeAccount(address account) external onlyOwner() {
        require(_isExcluded[account], "RNB: Account is already included");
