...
// https://ipfs.io/ipfs/QmSi4rZ7sRBGZoJveRtAshdc2UU4vhFWDF3cXdKkxUvQJH/
// https://gateway.pinata.cloud/ipfs/QmZPodWtPM3JMzHBXkPzCvkjJKdtwrrkc2yfCLrWcftkM8/
...
contract MarsWawa is ERC721, ERC721Enumerable, Pausable, Ownable, ReentrancyGuard {
  ...
  uint16 public constant Royalty = 750; // 版税 0.075 * 10000
  uint256 public constant PublicPrice = 0.00777 ether;

  //   uint32 public constant OGMintTime = 3 * 60 * 60; // OGMint秒数
  //   uint32 public constant WLMintTime = 3 * 60 * 60; // WLMint秒数
  //   uint32 public constant PublicSaleTime = 7 * 24 * 60 * 60; // publicSale秒数

  // uint64 public constant startTime = 1664348400; // Mint开始时间

  uint256 public lastTime = 0; // 上次Mint开始时间
  // string public publicSaleTime = "2022/10/8 01:00";
  ...
  uint16 public ogCount = 0; // OG中未Mint的数量
  uint16 public wlCount = 0; // WL中未Mint的数量
  uint16 public freeCount = FreeMintCount; // 还可以FreeMint的数量
  uint16 public mintCount = MaxSupply - ReserveCount; // 还可以Mint的数量
  ...
  // 刷新阶段状态
  //   function refreshStage() public onlyOwner {
  //     require(_stage < Stage.Publish, "It's final stage!");

  //     uint256 nowTime = block.timestamp;
  //     uint mintCount = totalSupply();
  //     if (_stage == Stage.Pending && nowTime >= startTime)
  //       setStage(Stage.OGMint);
  //     else if (_stage == Stage.OGMint) {
  //       if (nowTime >= lastTime + OGMintTime) setStage(Stage.WLMint);
  //       else if (ogCount <= 0) setStage(Stage.WLMint);
  //       else if (mintCount >= FreeMintCount) setStage(Stage.PublicSale);
  //     } else if (_stage == Stage.WLMint) {
  //       if (nowTime >= lastTime + WLMintTime) setStage(Stage.PublicSale);
  //       else if (wlCount <= 0) setStage(Stage.PublicSale);
  //       else if (mintCount >= FreeMintCount) setStage(Stage.PublicSale);
  //     } else if (_stage == Stage.PublicSale) {
  //       if (nowTime >= lastTime + PublicSaleTime) setStage(Stage.Publish);
  //       else if (mintCount >= MaxSupply) setStage(Stage.Publish);
  //     }
  //   }

  function setStage(Stage stage) public onlyOwner {
    // require(_stage != stage, "Stage will not change");
    // require(_stage < Stage.Publish, "It's final stage!");
  ...
  function tokenURI(uint256 tokenId) public view virtual override(ERC721) returns (string memory) {
    ...
    return super.tokenURI(_stage < Stage.Publish ? 0 : tokenId); // 只有Publish才公布TokenId
  }
  ...
  function contractURI() public pure returns (string memory) {
    return "https://exermon.com/nft/metadata/contractMetadata.json";
  }
  ...
  function addOG(address[] calldata members) public onlyOwner {
    ...
    //      // 如果已经是OG或者白名单，跳过
    //      if (isWL(m)) continue;
    //      wlUsers[m] = WLUser(MaxMint, true, true);
    //      wlCount += wlUsers[m].count;
    //      ogCount += wlUsers[m].count;
  }
  function addWL(address[] calldata members) public onlyOwner {
    ...
    //      // 如果已经是OG或者白名单，跳过
    //      if (isWL(m)) continue;
    //      wlUsers[m] = WLUser(MaxMint, false, true);
    //      wlCount += wlUsers[m].count;
  }
