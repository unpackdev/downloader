...
contract NexStream is ERC20, Ownable, Editor {
    using SafeMath for uint256;
	...
	uint256 public immutable taxFee = 500;
    ...
    constructor(address payable _taxAddress) ERC20("NexStream", "NXS") {
    ...
    function _setAutomatedMarketMakerPair(address pair, bool value) private {
        require(automatedMarketMakerPairs[pair] != value, "NXS: Automated market maker pair is already set to that value");
        automatedMarketMakerPairs[pair] = value;
        emit SetAutomatedMarketMakerPair(pair, value);
    }
		
    function excludeFromFees(address account, bool excluded) public OwnerOrEditor{
        require(_isExcludedFromFees[account] != excluded, "NXS: Account is already the value of 'excluded'");
	...
	function _transfer(address from, address to, uint256 amount) internal override {
		...
		if(automatedMarketMakerPairs[to])
		{
			...
			if(takeFee) 
			{
				uint256 vfees = amount.div(10000).mul(taxFee);
				if(vfees > 0) {
				   super._transfer(from, taxAddress, vfees);
				}
