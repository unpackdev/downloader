...
contract StakedUSDtoDSC is Ownable {
	using SafeMath for uint256;
    using SafeERC20 for IERC20;
	IERC20 public token;
	IERC20 public defiStableCash;
	uint256 public tokenDecimal;
	uint256 public duration = 7 * 24 * 60 * 60;
	uint256 public _PoolInterestRate = 12;
	uint256 public _PoolLimit = 10001000000000000000000;
	...
	constructor(
		...
		address _defiStableCash
	) public {
		token = IERC20(_token);
        tokenDecimal = _tokenDecimal;
		defiStableCash = IERC20(_defiStableCash);
	}
	...
	function rewardsOf(address stakeholder)
		public
		view
		returns (uint256 userRewardsDSC, uint256 totalRewardDSC)
	{
		uint256 totalRewardsDSC = 0;
		for (uint256 i = 0; i < _StakeSlots[stakeholder].length; i++) {
			...
			uint256 currentTime =
				infodsc.endedAt > block.timestamp
					? block.timestamp
					: infodsc.endedAt;
			uint256 spentTime = currentTime.sub(infodsc.startAt);

			uint256 newRewards =
				(infodsc.snapshotAmount * spentTime * _PoolInterestRate) /
					(duration * 10000);
			totalRewardsDSC = totalRewardsDSC.add(newRewards);
		}

		uint256 availableRewards =
			totalRewardsDSC - _PoolClaimedRewards[stakeholder];
		return (availableRewards, totalRewardsDSC);
	}
	...
	function Claimd() public {
		...
		uint256 available = _PoolAvailable.add(amountTransfer);
		_PoolAvailable = (_PoolLimit > available) ? available : _PoolLimit;
		uint256 availableTransfer =
			(_PoolLimit > available)
				? amountTransfer
				: (amountTransfer - available + _PoolLimit);
		defiStableCash.mint(msg.sender, availableTransfer);
