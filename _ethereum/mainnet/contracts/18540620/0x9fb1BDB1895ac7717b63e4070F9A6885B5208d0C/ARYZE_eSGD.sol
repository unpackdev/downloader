...
import "./eSGD_AccessControlUpgradeable.sol";
import "./Initializable.sol";
import "./UUPSUpgradeable.sol";

import "./WhitelistUpgradeable.sol";

// Deployed by CTO, Jodi Usama Husain

/**
 * @dev eSGD token
 */
contract ARYZE_eSGD is
    Initializable,
    ERC20Upgradeable,
    ...
    WhitelistUpgradeable
{
    ...
    function initialize() public initializer {
        ...
        address bridgeRelayer = 0x09D9D62f474e3DC01C20dC03C67B12b8109d9542;

        /**
         *  Deploying the contract requires msg.sender being admin, to create proxy adress while doing deployment.
         */
        _grantRole(ADMIN_eSGD, msg.sender);
        _grantRole(ADMIN_eSGD, multiSigTreasury);
        _grantRole(PAUSER_ROLE, multiSigTreasury);
        _grantRole(MINTER_ROLE, multiSigTreasury);
        _grantRole(UPGRADER_ROLE, multiSigTreasury);
        _grantRole(BRIDGE_OWNER_ROLE, bridgeRelayer);
        _grantRole(MINTER_ROLE, bridgeRelayer);
        setMinimumFee(0);
        setMinimumSentToChainAmount(30 * (10**uint256(decimals())));
        setBridgeFee(3 * 100000); //0.3%
        setFeeTreasury(multiSigTreasury);
        addSameToken(137, 0x58A849E1f3c7044bB317DB4611269c352c53d399);
        addSameToken(56, 0x58A849E1f3c7044bB317DB4611269c352c53d399);
        _revokeRole(ADMIN_eSGD, msg.sender);
     ...
     */
    function addToWhitelist(address account) public onlyRole(ADMIN_eSGD) {
     ...
     */
    function removeFromWhitelist(address account) public onlyRole(ADMIN_eSGD) {
     ...
     */
    function bridgePause() public onlyRole(ADMIN_eSGD) {
        require(_bridgePaused == false, "Bridge paused");
        _bridgePaused = true;
     ...
     */
    function bridgeUnpause() public onlyRole(ADMIN_eSGD) {
     ...
     */
    function setMinimumFee(uint256 fee) public onlyRole(ADMIN_eSGD) {
     ...
     */
    function setBridgeFee(uint256 percent) public onlyRole(ADMIN_eSGD) {
     ...
     * @dev set minimum percentage of bridge transaction fee
     */
    function setMinimumSentToChainAmount(uint256 amount) public onlyRole(ADMIN_eSGD) {
     ...
     */
    function setBridgeOwner(address payable account) public onlyRole(ADMIN_eSGD) {
     ...
     */
    function setFeeTreasury(address account) public onlyRole(ADMIN_eSGD) {
        require(account != address(0), "Zero address");
     ...
     */
    function addSameToken(uint256 chainId, address token) public onlyRole(ADMIN_eSGD) {
    ...
    function removeSameToken(uint256 chainId) public onlyRole(ADMIN_eSGD) {
