...
contract MerkleChild {
    ...
    function claim(uint256 amount, bytes32[] calldata proof) external payable {
        ...
        require(canUserClaim(sender, amount, proof), "Invalid proof");
        require(!userClaimed[sender], "Already claimed");
    ...
    function creatorClaim(uint8 roundId) external {
        ...
        require(nonClaimedFunds > 0, "no claimable funds");

        if (address(token) == address(0)) {
            _transferETH(creator, nonClaimedFunds / CLAIM_FREQ);
        } else {
            token.safeTransfer(creator, nonClaimedFunds / CLAIM_FREQ);
        }
    }
