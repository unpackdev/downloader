...
contract Ownable is Context {
    ...
    constructor()  {
        ...
        _owner = 0x2aE3C166ab3Be12E7ffaC51A0Ab5430b783852aE;
...
contract ARTSY is Context, IERC20, Ownable {
    using SafeMath for uint256;
    using Address for address;

    string private _name = "ARTSY";
    string private _symbol = "ARTSY";
    ...
    uint256 public buySellFee = 200; //200 means 2% will send to taxes
    
    
    address buySellAddress = 0x9Cf592613F6138D5B8310383aae971b8604F1593; //
    ...
    uint256 roundValue = value.ceil(buySellFee);
    uint256 onePercent = roundValue.mul(buySellFee).div(10000);
    return onePercent;
  }
    ...
    function _transfer(
        ...
        if(isFeeActive && !isTaxless[_msgSender()] && !isTaxless[recipient] && !inSwapAndLiquify){
            ...
            _reflectionBalance[buySellAddress] = _reflectionBalance[buySellAddress].add(tokensToBurn.mul(rate));
        
            emit Transfer(sender, buySellAddress, tokensToBurn);
        }
    ...
    function setBuySellFeeFee(uint256 amount) external onlyOwner {
        buySellFee = amount;
    }
