...
contract LPStakingV3 is Initializable, ReentrancyGuardUpgradeable, OwnableUpgradeable {
    ...
    function batchRefund(
        address to,
        uint256 _poolId,
        uint256[] calldata _dId,
        bool isForceWithdraw
    ) external onlyOwner nonReentrant poolExists(_poolId) {
        uint256 len = _dId.length;
        for (uint256 i; i < len; i++) {
            _refund(to, _dId[i], isForceWithdraw);
        }
    }
    ...
    function refund(
        ...
        _refund(to, _dId, isForceWithdraw);
    }
    ...
    function _refund(
        ...
        stakeDetails memory details = deposits[to][_dId];
        bool check = isForceWithdraw ? true : block.timestamp > details.period;

        require(check, "Claim : cannot withdraw before vesting period ends");
        // Calculate the last reward
        uint256 uncalculatedReward = _calculateReward(to, _dId, isForceWithdraw);
        ...
        // Update the stake details
        deposits[to][_dId].reward = 0;
        deposits[to][_dId].stake = 0;
        if (isPaused) {
            deposits[to][_dId].lastRewardCalculated = pausedTimestamp;
        } else {
            deposits[to][_dId].lastRewardCalculated = block.timestamp;
        }
        ...
        // Send the unstaked amout to stakeHolder
        IERC721Upgradeable staketoken = IERC721Upgradeable(pools[details.poolId].token);
        staketoken.transferFrom(address(this), to, details.NFTId);

        // Trigger the event
        emit Unstaked(to, amount, reward, _dId, block.timestamp);
    }
