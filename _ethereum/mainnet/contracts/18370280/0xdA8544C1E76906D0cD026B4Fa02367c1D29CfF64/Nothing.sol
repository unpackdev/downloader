...
contract Noththing is ERC721Enumerable, Ownable, ReentrancyGuard {
  using Strings for uint256;
  ...
  // Lists 
  uint256[606] public remainingIds;
  ...
  */
  ...
  function handleMintReward(uint256 _amount) private {
      ...
      uint256 mintShareS = _amount.div(mintReward); //10% Dev
      uint256 mintShareR = _amount.div(mintSplit); //10% reflections
      uint256 OwnerShare = _amount.sub(mintShareM + mintShareR + mintShareS);
      // Save dividend for community
      reflectDividend(mintShareR); //10%
      // Send owner/dev shares
      payable(owner()).transfer(OwnerShare); 
      payable(0x19eCb6Da52137979b06Cbc8809bDC8607c7050C3).transfer(mintShareS);
