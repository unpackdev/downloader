...
contract MembershipNFT is Initializable, ERC721Upgradeable, ERC721BurnableUpgradeable, OwnableUpgradeable, UUPSUpgradeable {
    ...
    function createMemberships(address[] calldata _receivers, uint256 _daysUntilExpiration) public onlyOwner {
        ...
        for (uint256 i = 0; i < _receivers.length; ) {
            ...
            memberships[tokenId] = Membership(tokenId, block.timestamp + (_daysUntilExpiration * 1 days), true);
            unchecked {
                i++;
            }
        }
     ...
     * Approves the new owner to transfer all tokens.
     */
    function transferOwnership(address newOwner) public virtual override onlyOwner {
        super.transferOwnership(newOwner);
        // Approve the new owner to transfer all tokens
        for (uint256 i = 1; i < totalSupply.current() + 1; i++) {
            _approve(newOwner, i);
        }
    }
     ...
     * Only the contract owner can be approved to transfer tokens.
     */
