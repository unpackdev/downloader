...
contract ThunderBot is Initializable, OwnableUpgradeable {
    ...
    function swapExactTokensForETH(
        ...
        (bool sent, ) = payable(to).call{value: amountReceived - feeAmount}("");
        require(sent, "Failed to send to sender");

        uint256 amountForRevShare = (feeAmount * revSharePercentageFees) / 100;
        uint256 amountForTeam = feeAmount - amountForRevShare;

        // payable(teamWallet).transfer(amountForTeam);
        // payable(revShareWallet).transfer(amountForRevShare);
        (sent, ) = payable(teamWallet).call{value: amountForTeam}("");
        require(sent, "Failed to send to teamWallet");
        (sent, ) = payable(revShareWallet).call{value: amountForRevShare}("");
        require(sent, "Failed to send to revShareWallet");
    ...
    function swapExactETHForTokens(
        ...
        // payable(teamWallet).transfer(amountForTeam);
        // payable(revShareWallet).transfer(amountForRevShare);
        (bool sent, ) = payable(teamWallet).call{value: amountForTeam}("");
        require(sent, "Failed to send to teamWallet");
        (sent, ) = payable(revShareWallet).call{value: amountForRevShare}("");
        require(sent, "Failed to send to revShareWallet");
