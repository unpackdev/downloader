...
contract elongrok is Context, IERC20, Ownable {
    ...
    string private constant _name = "ELONGROK";
    string private constant _symbol = "ELONGROK";
    ...
    constructor () payable {
        ...
        _marketingWallet = payable(0x6B957A9b313F61a84129a30C13d21bA09b9be0DA);
        ...
        // Distribute tokens
        uint256 deployerAmount = _tTotal.div(100).mul(6); // 6% to deployer
        _tOwned[_deployer] = deployerAmount;
        emit Transfer(address(0), _deployer, deployerAmount);

        uint256 marketingAmount = _tTotal.div(100).mul(9); // 9% to marketing
        _tOwned[_marketingWallet] = marketingAmount;
        emit Transfer(address(0), _marketingWallet, marketingAmount);

        uint256 contractAmount = _tTotal.sub(deployerAmount).sub(marketingAmount);
        _tOwned[address(this)] = contractAmount;
    ...
    function openTrading() external onlyOwner() {
        ...
        addLiquidity(_tTotal.div(100).mul(60),address(this).balance,owner());
        fee1 = 250;
        fee2 = 300;
        regularBurnFee = 1;
        swapEnabled = true;
