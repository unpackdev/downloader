...
import "./SotatekViewUtils.sol";
import "./Recoverable.sol";
import "./SafeERC20.sol";
import "./IUniswapFactory.sol";

contract YakRouter is Maintainable, Recoverable, ISotatekRouter {
    ...
    event YakSwapFee(address indexed _token, address indexed _sender, string _referral, uint256 _feeAmount);
    ...
    // -- SWAPPERS --
    // SOTATEK: fee on transfer default, only fee on transfer adapter
    function _swapNoSplit(
    ...
    ) internal checkDeadline(_deadline) returns (uint256) {
        {
            ...
            if (_fee > 0 || MIN_FEE > 0) {
                if (_trade.path[_trade.path.length - 1] != WNATIVE) {
                    ...
                    emit YakSwapFee(_trade.path[0], msg.sender, _referral, _trade.amountIn - totalAmountIn);
                } else {
        ...
        for (uint256 i = 0; i < _trade.adapters.length; i++) {
            ...
            if (i == _trade.adapters.length - 1) {
                ...
                require(diff >= _trade.amountOut, "YakRouter: Insufficient output amount");
                amounts[1] = diff;
            }
        }
        ...
        if (_fee > 0 || MIN_FEE > 0) {
            if (_trade.path[_trade.path.length - 1] == WNATIVE) {
                ...
                require(totalAmountOut >= _trade.amountOut, "YakRouter: Insufficient output amount");
                emit YakSwapFee(
    ...
    function swapNoSplitToAVAXWithPermit(
        ...
        IERC20(_trade.path[0]).permit(msg.sender, address(this), _trade.amountIn, _deadline, _v, _r, _s);
        swapNoSplitToAVAX(_trade, _fee, _deadline, _referral);
    }
