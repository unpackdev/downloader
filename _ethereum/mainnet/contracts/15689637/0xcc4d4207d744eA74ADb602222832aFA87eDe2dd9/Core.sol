...
contract Core is Initializable, UUPSUpgradeable, OwnableUpgradeable {
    ...
    event ProtocolSupportAdded(string protocol);

    // The comment below allows the constructor to be Add.
    ...
    function addProtocol(string memory _protocol) external onlyOwner {
        ...
        emit ProtocolSupportAdded(_protocol);
    }
    ...
    function registerNewStream(
        ...
        // check and revert if stream key is invalid
        require(streamKey != bytes32(0), "ERR_INVALID_STREAM_KEY_ZERO");
    ...
    function createNewEpoch(
        ...
        // check if constructor params of the newly created future is same as provided
        if(IFuture(newEpochAddr).owner() != address(this) ||
           ...
           IFuture(newEpochAddr).treasury() != _treasuryAddress) revert("ERR_INVALID_EPOCH");
    ...
    */
    ...
    function deposit(bytes32 _streamKey, uint256 _amountUnderlying) external returns (uint256, uint256) {
        require(_amountUnderlying != 0, "ERR_INVALID_ZERO_AMOUNT");
    ...
    function redeemYield(bytes32 _streamKey, uint256 _epoch) external isEpochExpired(_streamKey, _epoch) {
        ...
        // check if user has YT to redeem
        require(IERC20Upgradeable(epochInstance.getYT()).balanceOf(msg.sender) > 0, "ERR_INSUFFICIENT_BALANCE");
