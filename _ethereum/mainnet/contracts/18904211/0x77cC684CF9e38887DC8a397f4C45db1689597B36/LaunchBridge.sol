    ...
    PausableUpgradeable
{
     ...
     */
    function getMainnetBridge()
        public
        view
        returns (IMainnetBridge mainnetBridge)
    {
    ...
    function totalUSDBalance() public returns (uint256) {
        return
            DAI.balanceOf(address(this)) +
            DSR_MANAGER.daiBalance(address(this));
    }
     ...
     * @param daiAmount Amount of DAI obtained after conversion (wad)
     */
    function _recordDepositUSDAfterTransfer(
        uint256 depositedAmount,
        uint256 daiAmount
    ) internal {
        ...
        if (userETHShares > 0) {
            ...
            */
            ...
            if (ethAmountToMove > 0) {
                mainnetBridge.depositETHTo{value: ethAmountToMove}(
                    user,
                    0,
                    bytes("")
                );
        ...
        if (userUSDShares > 0) {
            ...
            */
            ...
            if (daiAmountToMove > contractDAIBalance) {
                DSR_MANAGER.exit(
                    address(this),
                    daiAmountToMove - contractDAIBalance
                );
            }

            getMainnetBridge().depositERC20To(
                address(DAI),
                address(DAI),
                user,
                daiAmountToMove,
