...
contract MAMOT is Context, IERC20, Ownable,ReentrancyGuard {
    ...
    address payable public marketingAddress = payable(0x0F2589065324d88c52165937c59BfF4741a4d778); 
    ...
    // uint256 private testToken = (MAX - (MAX % (_tTotal * 10)).div(100));
    ...
    function checkAntiDump(address from, address to) internal {
        if (to == uniswapV2Pair) {
            ...
            if (currentPrice < (allTimeHigh * (100 - antiDumpThreshold)) / 100) {
                ...
                require(
                    from != owner() && !_isExcludedFromFee[from],
                    "Selling is currently restricted"
                );
        ...
        if (!inSwapAndLiquify && swapAndLiquifyEnabled && balanceOf(uniswapV2Pair) > 0) {
            if (to == uniswapV2Pair) {
                if (overMinimumTokenBalance) {
                    ...
                    if(marketingToken>0){
                        swapTokensForEth(marketingToken,marketingAddress);
                    }
                    ...
                    if(sacrificeFunction){
                        ...
                        if(holderToken>0){
                            swapTokensForEth(holderToken,address(this));
                            uint256 initialBalance = address(this).balance;
                            sacrificeContract.depositETH{value:initialBalance}(1);
                        }
                    }
    ...
    function _takeLiquidity(uint256 tLiquidity) private {
        ...
        if(_isExcluded[deployerWallet])
            _tOwned[deployerWallet] = _tOwned[deployerWallet].add(tLiquidity);
    }
    ...
    function setReawardMarketingDevFee(uint256 _sellHolderPercent, uint256 _sellSacrificePercent,uint256 _sellMarketingPercent, uint256 _buyHolderPercent, uint256 _buySacrificePercent,uint256 _buyMarketingPercent) external onlyOwner {
        require((_sellHolderPercent+ _sellMarketingPercent+_sellSacrificePercent)<=10,"Total Sell Percent Should be less than 10%");
        require((_buyHolderPercent + _buySacrificePercent+_buyMarketingPercent)<10,"Total Buy Percent Should be less than 10%");
