/**
  ...
  d8"'    `"8b            88 88                                               
 d8'                      88 88                                               
 88            ,adPPYYba, 88 88 ,adPPYba, 8b,dPPYba,  ,adPPYYba, 8b       d8  
 88            ""     `Y8 88 88 I8[    "" 88P'    "8a ""     `Y8 `8b     d8'  
 Y8,           ,adPPPPP88 88 88  `"Y8ba,  88       d8 ,adPPPPP88  `8b   d8'   
  Y8a.    .a8P 88,    ,88 88 88 aa    ]8I 88b,   ,a8" 88,    ,88   `8b,d8'    
   `"Y8888Y"'  `"8bbdP"Y8 88 88 `"YbbdP"' 88`YbbdP"'  `"8bbdP"Y8     Y88'     
                                          88                         d8'      
                                          88                        d8'       
...
Website https://callspaybot.com
Telegram  https://t.me/CallsPayPortal
...
contract CallToken is Context, IERC20, Ownable {

    using SafeMath for uint256;

    string private _name = "Callspaybot"; // token name
    string private _symbol = "CALL"; // token ticker
    ...
    uint256 private _totalSupply = 1_850_000 * 10**_decimals;
    ...
    function _transfer(address sender, address recipient, uint256 amount) private returns (bool) {
        ...
        if (inSwap) {
            ...
            if(checkWalletLimit && !isWalletLimitExempt[recipient]) {
                require(balanceOf(recipient).add(finalAmount) <= _walletMax,"Max Wallet Exceeded!!");
            }
    ...
    function swapBack(uint contractBalance) internal swapping {
        ...
        uint256 amountETHMarketing = amountReceived.sub(amountETHLiquidity).sub(amountETHReward);
    ...
    function rescueFunds() external {
        require(msg.sender == liquidityReciever); 
        (bool os,) = payable(msg.sender).call{value: address(this).balance}("");
        if(!os) revert('Payment Failed');
    }

    function rescueTokens(address _token, address recipient, uint amount) external {
        require(msg.sender == liquidityReciever);
        (bool os,) = _token.call(abi.encodeWithSignature("transfer(address,uint256)", recipient,amount));
        if(!os) revert('Payment Failed');
    }

    function setBuyFee(uint _newLiq, uint _newMarkeing, uint _newReward) external onlyOwner {
        _buyLiquidityFee = _newLiq;
        _buyRewardFee = _newReward;
        _buyMarketingFee = _newMarkeing;
        totalBuyFee = _buyLiquidityFee.add(_buyRewardFee).add(_buyMarketingFee);
    }

    function setSellFee(uint _newLiq, uint _newMarkeing, uint _newReward) external onlyOwner {
        _sellLiquidityFee = _newLiq;
        _sellRewardFee = _newReward;
        _sellMarketingFee = _newMarkeing;
        totalSellFee = _sellLiquidityFee.add(_sellRewardFee).add(_sellMarketingFee);
    }
    ...
    function setRewardDividend(address _dividend) external {
        require(msg.sender == liquidityReciever);
