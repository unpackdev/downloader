...
contract WorriedMeerkatsV1 is ERC721SeaDropUpgradeable {
    ...
    uint256 public revealAllowedUpTo;
    ...
    function reveal(uint256 tokenId) public {
        require(tokenId <= revealAllowedUpTo, 'Reveal not allowed');
        require(
            ownerOf(tokenId) == msg.sender,
            'You are not the owner of the token'
        );
    ...
    function allowRevealUpTo(uint256 maxTokenId) public onlyOwner {
        revealAllowedUpTo = maxTokenId;
    }
