    ...
    LendMediatorStorage
{
     ...
     * @inheritdoc LendMediatorFunctionInterface
     */
    function liquidateOverdueLoanBlend(address blendAddress, LienPointer calldata lienPointer) external override onlyOfferSignerOrOwner {
    ...
    function takeOverLoanBlend(
        ...
        uint256 lienId,
        uint256 rate
    ) external override onlyOfferSignerOrOwner {
     ...
     * @inheritdoc LendMediatorFunctionInterface
     */
    function startAuctionBlend(address blendAddress, Lien calldata lien, uint256 lienId) external override onlyOfferSignerOrOwner {
    ...
    function withdrawErc20(address tokenAddress, uint256 amount) external override onlyOwner {
        ...
        if (
            (lendManager.royaltiesPercentage() != 0) &&
            (amount > depositedFunds[tokenAddress]) &&
            lendManager.royaltiesReceiver() != address(0)
        ) {
            uint256 interestBalanceToWithdraw = amount - depositedFunds[tokenAddress];
            uint royaltiesAmount = lendManager.getValueByRoyaltiesPercentage(interestBalanceToWithdraw);
