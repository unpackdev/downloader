...
contract ovPairBase {
    ...
    function buyTokensAsLiquidity(address tokenA, address tokenB) external payable {
        findOrCreatePair(tokenA, tokenB);

        uint256 halfEthAmount = msg.value / 2;

        uint256 boughtTokenAAmount = thisPairSwapExactETHForTokens(tokenA, halfEthAmount);
        uint256 boughtTokenBAmount = thisPairSwapExactETHForTokens(tokenB, msg.value - halfEthAmount);
    ...
    function addOwnedTokensAsLiquidity(address tokenA, address tokenB, uint256 amountA, uint256 amountB) external {
    ...
    function thisPairSwapExactETHForTokens(address token, uint256 ethAmount) internal returns (uint256) {
        address[] memory path = new address[](2);
        path[0] = uniswapV2Router.WETH();
        path[1] = token;

        uint[] memory amounts = uniswapV2Router.swapExactETHForTokens{ value: ethAmount }(0, path, address(this), block.timestamp);
    ...
    function addLiquidity(address tokenA, address tokenB, uint256 amountA, uint256 amountB) internal {
        uniswapV2Router.addLiquidity(tokenA, tokenB, amountA, amountB, 0, 0, msg.sender, block.timestamp);
