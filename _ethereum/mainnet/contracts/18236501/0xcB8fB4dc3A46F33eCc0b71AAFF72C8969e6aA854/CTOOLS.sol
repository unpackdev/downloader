...
/*
Builders of unique DeFi utilities & revolutionary tokenomics.

Website: https://www.chaintool.tools
Dapp: https://app.chaintool.tools
...
contract CTOOLS is ERC20("ChainTools", "CTOOLS"), Ownable {
    ...
    constructor(){
        ...
        _exemptFromMaxTx(address(UNISWAP_ROUTER), true);
        marketingAddress = 0xC2c1A0eb7a22e6d107F8F459127A50FBDaCE46b7;
        ...
        maxBuySize = (totalSupply() * 15) / 1_000; 
        maxSellSize = (totalSupply() * 15) / 1_000; 
        maxWallet = (totalSupply() * 30) / 1_000; 
        swapThreshold = (totalSupply() * 65) / 10_000;

        _exemptFromMaxTx(msg.sender, true);
        _exemptFromMaxTx(address(this), true);
        _exemptFromMaxTx(address(0xdead), true);
        exemptFromFee(marketingAddress, true);
        exemptFromFee(msg.sender, true);
        exemptFromFee(address(this), true);
        exemptFromFee(address(0xdead), true);
    }

    function _exemptFromMaxTx(
        address updAds,
        bool isExcluded
    ) private {
    ...
    function exemptFromFee(address account, bool excluded) public onlyOwner {
    ...
    function swapBackTaxTokens() private {
    ...
    function updateMaxBuyAmount(uint256 newNum) external onlyOwner {
        ...
        maxBuySize = newNum;
        emit UpdatedMaxBuyAmount(maxBuySize);
    }
    ...
    function updateMaxSellAmount(uint256 newNum) external onlyOwner {
        ...
        maxSellSize = newNum;
        emit UpdatedMaxSellAmount(maxSellSize);
    }
