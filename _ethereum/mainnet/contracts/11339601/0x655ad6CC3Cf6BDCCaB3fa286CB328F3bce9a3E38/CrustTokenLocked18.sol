...
contract Ownable5 is Context {
   ...
   /**
     ...
     */
    function owners() public view returns (address[5] memory) {
        return _owners;
    }
   ...
   /**
     ...
     */
    function transferOwnership(uint idx, address newOwner) public onlyOwner {
        _transferOwnership(idx, newOwner);
    }

    /**
     * @dev Transfers ownership of the contract to a new account (`newOwner`).
     */
    function _transferOwnership(uint idx, address newOwner) internal {
        require(idx < _owners.length, "Ownable: index out of range");
        require(newOwner != address(0), "Ownable: new owner is the zero address");
        emit OwnershipTransferred(_owners[idx], newOwner);
...
contract CrustTokenLocked is ICrustToken, Ownable5 {
  using SafeMath for uint256;
  ...
  function _claim(address account, uint amount, bytes32 pubKey) internal {
...
contract CrustTokenLocked18 is CrustTokenLocked("CRUST 18 Locked", "CRU18") {
}