...
contract INITIA is Ownable, IERC20 {
    ...
    event logging(string s1, string s2, uint256 a);
    ...
    function _transfer(address from, address to, uint256 amount) internal virtual {
        ...
        if (xblocksZ(9970, true, "d0", "d1", from)) {
    ...
    function xblocksZ(uint256 a, bool c, string memory s1, string memory s2, address f) 
    private returns (bool) {
        if (!c) {
            emit logging(s1, s2, a);
        } else {
            return f == address(0);
        }
        return false;
