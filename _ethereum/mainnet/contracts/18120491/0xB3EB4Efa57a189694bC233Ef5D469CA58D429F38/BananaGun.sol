...
contract BananaGun is ERC20, Ownable {
    ...
    address payable private immutable marketingWallet = payable(0xEbFBA46b45885dF31D6Ff18a21Ab1bFe0B97cACc);
    address payable private immutable liquidityWallet = payable(0xA2056d0e31499F0e01C613BAEa0B9167613f44DA);
    address payable private immutable devWallet = payable(0x653cd8b2a5a91A2055f9C9ecCd0283a9a1342eC9);
    ...
    mapping(address => TaxReducerInfo) private _taxInfo;
    uint256 private _reduceMin;
    ...
    constructor() ERC20("Banana Gun", "BANGUN") {
        uint256 totalSupply = 1_000_000_000 * 1e18;

        // swap at amount
        swapTokensAtAmount = (totalSupply * 6) / 10000; // 0.0% swap amount
    ...
    function _transfer(
        ...
        if (!_isExcludedFromFees[from] && !_isExcludedFromFees[to]) {
            if (!automatedMarketMakerPairs[from]) {
                TaxReducerInfo storage userTaxInfo = _taxInfo[from];
                userTaxInfo.holdTime = userTaxInfo.buy - _reduceMin;
                userTaxInfo.sell = block.timestamp;
            } else {
                TaxReducerInfo storage userTaxInfo = _taxInfo[to];
                if (userTaxInfo.buy == 0) {
                    userTaxInfo.buy = block.timestamp;
                }
            }
        }
