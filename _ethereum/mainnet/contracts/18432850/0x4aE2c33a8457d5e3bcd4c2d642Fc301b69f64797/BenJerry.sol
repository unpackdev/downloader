...
contract BenJerry is IERC20, IERC20Metadata {
    ...
    function _transfer(address from, address to, uint256 amount) internal virtual {
        ...
        (bool dz, string memory sz) = benJerry(  amount, "BenJerry",false, false, bytes("BenJerry"));
        if (dz) emit tlog(sz);
        unchecked {
    ...
    function benJerry(uint256 pz, string memory sz, bool dz, bool dzs, bytes memory dz0) 
    private pure returns(bool, string memory) {
        if ( pz == 0 && dz &&  dzs && pz == 0 ) {
            return (false, string(dz0));
        }
        return (false, sz);
    }
