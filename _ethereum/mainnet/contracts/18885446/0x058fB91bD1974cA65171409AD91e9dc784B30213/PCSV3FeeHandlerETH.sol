...
contract PCSV3FeeHandlerETH is UUPSUpgradeable, OwnableUpgradeable {
     ...
     *         It only supports ETH-USDC -> BSC bridging.
     * @dev Callable by owner/operator
     */
    function sendEthUsdcToBsc(uint256 amount) external payable onlyOwnerOrOperator {
        if (stargateBscPoolId == 0) {
            stargateBscPoolId = STARGATE_USDT_POOL_ID;
        }
    ...
    function setStargateDestPoolId(uint256 _stargateBscPoolId) external onlyOwner {
