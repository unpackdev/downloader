...
contract Soar is ERC20PresetMinterPauserUpgradeable {
    ...
    event BuybackPer(uint256 buybackPer);
    ...
    function setBuybackPer(uint256 _buybackPer) external onlyOwner {
        if (_buybackPer > MULTIPLIER) revert INVALID_FEE();

        buybackPer = _buybackPer;

        emit BuybackPer(_buybackPer);
    }
    ...
    function _transferWithTax(
        ...
        // buy back
        if (eth > 0) {
            pendingEth += (eth * buybackPer) / MULTIPLIER;
        } else if (
