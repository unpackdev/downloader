...
contract TrustedNodes is PolicedUtils, TimeUtils {
    ...
    // last generation in which each trustee redeemed rewards
    // this is here bc this contract is proxied, cant be inserting stuff in the middle of storage
    mapping(address => uint256) public lastRedeemedGeneration;

    // any address that has ever been a trustee
    address[] public everTrustee;
    ...
    function redeemVoteRewards() external {
        ...
        // calculate redeemable rewards from previous year
        uint256 currentGeneration = IGeneration(policyFor(ID_TIMED_POLICIES))
            .generation();
        uint256 yearGenerationCount = currentGeneration - yearStartGen;
        uint256 generationsSinceLastWithdraw = currentGeneration -
            lastRedeemedGeneration[msg.sender];
        uint256 redeemableGenerations = yearGenerationCount >
            generationsSinceLastWithdraw
            ? generationsSinceLastWithdraw
            : yearGenerationCount;
        uint256 rewardsToRedeem = (
            record > redeemableGenerations ? redeemableGenerations : record
        );
        lastYearVotingRecord[msg.sender] = record - rewardsToRedeem;
        lastRedeemedGeneration[msg.sender] = currentGeneration;
        ...
        require(rewardsToRedeem > 0, "No vested rewards to redeem");
     ...
     *
     * Makes the assumption that there will be a trustee election resulting in a new cohort
     * once and only once per year, directly following the call to annualUpdate.
     */
