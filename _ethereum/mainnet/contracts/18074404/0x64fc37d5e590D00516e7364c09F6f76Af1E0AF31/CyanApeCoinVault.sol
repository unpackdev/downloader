        ...
        onlyRole(CYAN_APE_PLAN_ROLE)
    {
        ...
        for (uint256 ind; ind < deposits.length; ) {
            ...
            if (amount > 0) {
                ...
                uint256 cyanServiceFee = (amount * serviceFeePercent) / 10000;
                uint256 userDepositedAmount = amount - cyanServiceFee;
                uint256 mintAmount = (userDepositedAmount * totalToken) / totalCurrency;

                totalAmount += amount;
                totalUserDeposits += userDepositedAmount;
        ...
        amounts.collectedServiceFeeAmount += (totalAmount - totalUserDeposits);
        claimRewardsAndStake(totalUserDeposits);
