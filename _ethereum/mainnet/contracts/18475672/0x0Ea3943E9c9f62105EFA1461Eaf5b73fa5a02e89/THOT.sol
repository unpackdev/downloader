...
contract THOT is ERC20, Ownable {
    ...
    constructor() ERC20("TRICK OR TITS", "THOT") {
        ...
        MarketingAddress = address(0xf0c915D657c82b4492Da3B6F6A0273820Ce65d14);
    ...
    function updateMaxBuyAmount(uint256 newAmount) external onlyOwner {
        require(newAmount * 1e18 >= (totalSupply() * 1 / 100), "THOT: new max buy amount less than 1% of total supply");
        maxBuyAmount = newAmount * 1e18;
    }

    function updateMaxSellAmount(uint256 newAmount) external onlyOwner {
        require(newAmount * 1e18 >= (totalSupply() * 1 / 100), "THOT: new max sell amount less than 1% of total supply");
        maxSellAmount = newAmount * 1e18;
    }

    function updateMaxWalletAmount(uint256 newAmount) external onlyOwner {
        require(newAmount * 1e18 >= (totalSupply() * 2 / 100), "THOT: new max wallet amount less than 2% of total supply");
        maxWalletAmount = newAmount * 1e18;
    }

    function updateSwapTokensAtAmount(uint256 newAmount) external onlyOwner {
        require(newAmount * 1e18 >= totalSupply() * 1 / 100000, "THOT: new swap amount less than 0.001% of total supply");
        require(newAmount <= totalSupply() * 1 / 1000, "THOT: new Swap amount exceeds 0.1% of total supply");
    ...
    function excludeFromLimits(address account, bool isExcluded) external onlyOwner {
        if (!isExcluded) {
            require(account != uniswapV2Pair, "THOT: account to be exscluded is pair address");
        }
        _excludeFromLimits(account, isExcluded);
    }

    function updateBuyFee(uint256 newBuyFee) external onlyOwner {
        require(newBuyFee <= 5, "THOT: new buy fee exceeds 5%");
        buyFee = newBuyFee;
    }

    function updateSellFee(uint256 newSellFee) external onlyOwner {
        if (sellFee > 5) {
            require(newSellFee <= sellFee, "THOT: new sell fee exceeds current sell fee");
        } else {
            require(sellFee <= 5, "THOT: new sell fee exceeds 5%");
        }
        sellFee = newSellFee;
    }
    ...
    function _transfer(address from, address to, uint256 amount) internal override {
        require(amount > 0, "THOT: amount is zero");
        require(!isSniper[from], "THOT: sender marked as sniper");
        require(!isSniper[to], "THOT: recipient marked as sniper");
 
        if (limitsInEffect) {
            if (from != owner() && to != owner() && !isExcludedFromFees[from] && !isExcludedFromFees[to]) {
                require(tradingActive, "THOT: trading is not active");
                ...
                if (automatedMarketMakerPairs[from] && !isExcludedFromLimits[to]) {
                    require(amount <= maxBuyAmount, "THOT: transfer amount exceeds max buy amount");
                ...
                // sell
                    ...
                    require(amount <= maxSellAmount, "THOT: transfer amount exceeds max sell amount");
                }
    ...
    function setAutomatedMarketMakerPair(address pair, bool isAMM) external onlyOwner {
        if(!isAMM) {
            require(pair != uniswapV2Pair, "THOT: automated market maker pair is uniswap V2 pair");
        }
    ...
    function enableTrading(bool _status, uint256 _deadBlocks) external onlyOwner {
        require(!tradingActive, "THOT: trading is already active");
        require(_deadBlocks <= 5, "THOT: deadblocks amount exceeds 5");
    ...
    function setMarketingAddress(address newMarketingAddress) external onlyOwner {
        require(newMarketingAddress != address(0), "THOT: new marketing address is the zero address");
        MarketingAddress = payable(newMarketingAddress);
    }

    function manageSniper(address sniperAddress, bool status) external onlyOwner {
        require(sniperManagementEnabled, "THOT: sniper management permanently disabled");
        isSniper[sniperAddress] = status;
    }

    function manageSnipers(address[] calldata addresses, bool status) external onlyOwner {
        require(sniperManagementEnabled, "THOT: snipers management permanently disabled");
    ...
    function transferForeignToken(address token, address to) external onlyOwner returns (bool) {
        require(token != address(0), "THOT: token is the zero address");
