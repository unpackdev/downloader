...
contract PolitizooThirdParty is ERC721A, IERC2981 {

    /// @notice 0.25 ether OR 250000000000000000 wei
    uint256 public constant MINT_COST = 0.25 ether;
    uint256 private constant MAX_NFTS_FOR_SALE = 970;
    /// @notice a wallet can mint no more than 10 nfts
    uint256 public constant MAX_MINTS = 10;
    /// @notice only allows to mint 3 max nfts per transaction
    uint256 public constant MAX_MINT_PER_TRANSACTION = 3;
    uint256 private constant AUCTION_INTERVAL = 30 days;
    uint256 private LAST_AUCTION = 0;
    uint256 public MAX_AUCTIONS = 30;
    uint256 public TOTAL_AUCTIONS = 0;

    mapping(address => uint256) private addressMintedBalance;

    // Address of the royalties recipient
    address private _royaltiesReceiver = 0x95D5FB927dcABDf392b535bDcB7611bC48eD2657;
    ...
    uint256 private developerFee = 5; //5%
    // Address of the DAO wallet
    address payable private daoWallet;
    uint256 public daoWalletShare = 75;
    

    constructor(address payable _daoWallet) ERC721A("Politizoo 3rd Party", "POZO", MAX_MINT_PER_TRANSACTION) {
    ...
    /// @notice Can mint max 3 at a time and 10 NFTs in total
    function mint(uint256 quantity) external payable {
        ...
        require(msg.value >= quantity*MINT_COST, "More ether required!");
        addressMintedBalance[msg.sender] += quantity; //tracking minted 
        _safeMint(msg.sender, quantity);
        payable(developerWallet).transfer(msg.value*developerFee/100);
        payable(daoWallet).transfer(msg.value*daoWalletShare/100);
    }
    ...
    /// @notice allows owner to auction 30 NFTs each month from a reserved pool of 30 NFTs
    function monthlyAuction() external onlyOwner {
        require(block.timestamp > LAST_AUCTION + AUCTION_INTERVAL, "Need to wait 30 days for next auction");
        require(TOTAL_AUCTIONS < MAX_AUCTIONS, "Cannot auction more than 30 NFTs");
        LAST_AUCTION = block.timestamp;
        TOTAL_AUCTIONS += 1;
    ...
    /// @notice allows contract owner to withdraw funds
