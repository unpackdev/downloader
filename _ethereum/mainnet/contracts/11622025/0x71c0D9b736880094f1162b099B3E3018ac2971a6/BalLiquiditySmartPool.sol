...
abstract contract BasicSmartPool is KToken,ReentryProtection{
  ...
  function _joinMint(uint256 amount)internal returns(uint256){
  ...
  function _exitBurn(uint256 amount)internal returns(uint256){
    uint256 exitAmount=_calcExitAmount(amount);
  ...
  function _calcExitAmount(uint256 amount)internal view returns(uint256){
    uint256 amountRatio=amount.div(_exitFeeRatio.denominator);
    return amountRatio.mul(_exitFeeRatio.denominator-_exitFeeRatio.ratio);
  }
...
contract BalLiquiditySmartPool is BasicSmartPool,IFundPool{
  ...
  function exitPool(address from,uint256 amount) external override ready denyReentry{
    ...
    uint256 poolTotal = totalSupply();
    uint256 exitAmount=_exitBurn(amount);
