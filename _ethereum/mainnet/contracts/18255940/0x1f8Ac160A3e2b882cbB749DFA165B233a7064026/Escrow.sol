...
contract Escrow is Ownable, ReentrancyGuard {
  ...
  event FeeWalletChanged(address indexed old_addr, address indexed new_addr);
  ...
  function admin_allow_transaction(bytes32 escrowID, bool is_allow) external onlyOwner {
    require(escrowTransactions[escrowID].buyer != address(0x0), "Transaction is not completed one");
    require(escrowTransactions[escrowID].seller != address(0x0), "Transaction is not completed one");
    require(escrowTransactions[escrowID].status != EscrowStatus.Released, "Even admin can't control the released transaction");
    escrowTransactions[escrowID].is_admin_allow = is_allow;
  }

  function updateFeeWalletAddr(address new_fee_wallet) external onlyOwner {
    emit FeeWalletChanged(fee_wallet_addr, new_fee_wallet);
