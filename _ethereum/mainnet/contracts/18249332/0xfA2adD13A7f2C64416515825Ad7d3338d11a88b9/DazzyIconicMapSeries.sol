...
contract DazzyIconicMapSeries is DefaultOperatorFilterer, ERC2981, ERC721AQueryable, Ownable {
    using MerkleProof for bytes32[];
    bytes32 public whitelistMerkleRoot;
    bool public presaleActive;
    bool public publicsaleActive;
    bool public openseaEnabled;
    uint256 public constant SUPPLY = 10000;
    uint256 public constant PRESALE_SUPPLY = 2000;
    uint256 public PUBLIC_SALE_PRICE = 300000000000000000;
    uint256 public PRIVATE_SALE_PRICE = 300000000000000000;
    uint256 public dazzyPerTransaction = 50;
    ...
    constructor() ERC721A("Dazzables POP: Iconic Maps Series - RUST feat. GHOST", "DAZZYXCOD#1") { }

    /* ADMIN FUNCTIONS */
    function toggleOpensea() external onlyOwner {
        openseaEnabled = !openseaEnabled;
    }

    function setDazzyPerTransaction(uint256 count) external onlyOwner {
        dazzyPerTransaction = count;
    }
    ...
    function presaleMint(uint256 count, bytes32[] memory proof) payable public {
        ...
        if (PRESALE_SUPPLY < (totalSupply() + count)) revert SoldOut();
        if (count > dazzyPerTransaction) revert MaxPerTransactionExceeded();
        if (msg.value < (PRIVATE_SALE_PRICE*count)) revert InsufficientBalance();
    ...
    function mint(uint256 count) payable public {
        ...
        if (SUPPLY < (totalSupply() + count)) revert SoldOut();
        if((msg.value*count) < (PUBLIC_SALE_PRICE*count)) revert InsufficientBalance();
    ...
    {
        if(!openseaEnabled) _requireNotOpenSea(operator);
