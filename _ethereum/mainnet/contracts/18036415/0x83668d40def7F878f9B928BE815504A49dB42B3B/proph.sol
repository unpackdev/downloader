...
contract prophet is ERC20, Ownable {
    ...
    event FeesUpdated(uint8 buyFee, uint8 sellFee, uint8 prophetPercent, uint8 liquidityPercent, uint8 teamPercent);
    ...
    event prophetWalletUpdated(address indexed newWallet, address indexed oldWallet);
    ...
    error prophet__BlacklistModificationDisabled();
    error prophet__BuyAmountGreaterThanMax();
    error prophet__CannotBlacklistLPPair();
    error prophet__CannotBlacklistRouter();
    error prophet__CannotRemovePairFromAMMs();
    error prophet__CannotSetWalletToAddressZero();
    error prophet__CannotTransferFromAddressZero();
    error prophet__CannotTransferToAddressZero();
    error prophet__ErrorWithdrawingEth();
    error prophet__FeeChangeRenounced();
    error prophet__MaxFeeFivePercent();
    error prophet__MaxTransactionTooLow();
    error prophet__MaxWalletAmountExceeded();
    error prophet__MaxWalletAmountTooLow();
    error prophet__OnlyOwner();
    error prophet__ReceiverBlacklisted();
    error prophet__ReceiverCannotBeAddressZero();
    error prophet__SellAmountGreaterThanMax();
    error prophet__SenderBlacklisted();
    error prophet__StuckEthWithdrawError();
    error prophet__SwapAmountGreaterThanMaximum();
    error prophet__SwapAmountLowerThanMinimum();
    error prophet__TokenAddressCannotBeAddressZero();
    error prophet__TradingNotActive();

    constructor(address ownerWallet, address teamWallet_, address prophetWallet_, address routerAddress) {
        ...
        feeAmounts = Fees({buy: 15, sell: 45, prophet: 0, liquidity: 0, team: 100});

        prophetWallet = prophetWallet_;
    ...
    function updateSwapTokensAtAmount(uint256 newAmount) external {
        ...
        if (newAmount < MIN_SWAP_AMOUNT) {
            revert prophet__SwapAmountLowerThanMinimum();
        }
        if (newAmount > MAX_SWAP_AMOUNT) {
            revert prophet__SwapAmountGreaterThanMaximum();
        }
    ...
    function updateMaxTransactionAmount(uint256 newAmount) external {
        ...
        if (newAmount < MAX_SUPPLY * 5 / 1000) {
            revert prophet__MaxTransactionTooLow();
        }
    ...
    function updateMaxWalletAmount(uint256 newNum) external {
        ...
        if (newNum < MAX_SUPPLY / 100) {
            revert prophet__MaxWalletAmountTooLow();
        }
    ...
    function updateBuyFees(uint8 prophetFee, uint8 liquidityFee, uint8 teamFee) external {
        _requireIsOwner();

        if (settings.feeChangeRenounced) {
            revert prophet__FeeChangeRenounced();
        }

        uint8 totalFees = prophetFee + liquidityFee + teamFee;
        if (totalFees > 99) {
            revert prophet__MaxFeeFivePercent();
        }

        uint8 sellFee = feeAmounts.sell;
        uint8 revPercent = prophetFee * 100 / totalFees;
        uint8 liqPercent = liquidityFee * 100 / totalFees;
        uint8 teamPercent = 100 - revPercent - liqPercent;

        feeAmounts =
            Fees({buy: totalFees, sell: sellFee, prophet: revPercent, liquidity: liqPercent, team: teamPercent});
        emit FeesUpdated(totalFees, sellFee, revPercent, liqPercent, teamPercent);
    }

    function updateSellFees(uint8 prophetFee, uint8 liquidityFee, uint8 teamFee) external {
        _requireIsOwner();
        ...
        feeAmounts =
            Fees({buy: buyFee, sell: totalFees, prophet: revPercent, liquidity: liqPercent, team: teamPercent});
    ...
    function setAutomatedMarketMakerPair(address pair, bool value) external {
        ...
        if (pair == uniswapV2Pair) {
            revert prophet__CannotRemovePairFromAMMs();
        }
    ...
    function updateprophetWallet(address newWallet) external {
        _requireIsOwner();
        if (newWallet == address(0)) {
            revert prophet__CannotSetWalletToAddressZero();
        }
        address oldWallet = prophetWallet;
        prophetWallet = newWallet;
        emit prophetWalletUpdated(newWallet, oldWallet);
    }
    ...
    function withdrawStuckprophet(uint256 amount) external {
        _requireIsOwner();
    ...
    function withdrawStuckToken(address _token) external {
        _requireIsOwner();
        if (_token == address(0)) {
            revert prophet__TokenAddressCannotBeAddressZero();
        }
    ...
    function withdrawStuckEth() external {
        ...
        if (!success) {
            revert prophet__ErrorWithdrawingEth();
        }
    }
    ...
    function blacklist(address account) external {
        ...
        if (settings.blacklistRenounced) {
            revert prophet__BlacklistModificationDisabled();
        }
        if (account == uniswapV2Pair) {
            revert prophet__CannotBlacklistLPPair();
        }
        if (account == address(uniswapV2Router)) {
            revert prophet__CannotBlacklistRouter();
        }
    ...
    function _transfer(address from, address to, uint256 amount) internal override {
        ...
        if (from == address(0)) {
            revert prophet__CannotTransferFromAddressZero();
        }
        if (to == address(0)) {
            revert prophet__CannotTransferToAddressZero();
        }
        ...
        if (!settingCache.tradingActive) {
            if (!fromData.isExcludedFromFees) {
                if (!toData.isExcludedFromFees) {
                    revert prophet__TradingNotActive();
                }
            }
        }
        ...
        if (fromData.isBlacklisted) {
            revert prophet__SenderBlacklisted();
        }
        if (toData.isBlacklisted) {
            revert prophet__ReceiverBlacklisted();
        }
            ...
            if (settingCache.limitsInEffect) {
                ...
                if (txType == 1 && !toData.isExcludedFromMaxTransactionAmount) {
                    if (amount > maxTransactionAmount) {
                        revert prophet__BuyAmountGreaterThanMax();
                    }
                    if (amount + balanceOf(to) > maxWallet) {
                        revert prophet__MaxWalletAmountExceeded();
                    }
                }
                //when sell
                else if (txType == 2 && !fromData.isExcludedFromMaxTransactionAmount) {
                    if (amount > maxTransactionAmount) {
                        revert prophet__SellAmountGreaterThanMax();
                    }
    ...
    function _swapBack() internal {
        ...
        uint256 ethForprophet = ethBalance * feeCache.prophet / 100;
        uint256 ethForTeam = ethBalance * feeCache.team / 100;
        uint256 ethForLiquidity = ethBalance - ethForprophet - ethForTeam;
        ...
        if (ethForprophet > 0) {
            (success,) = address(prophetWallet).call{value: ethForprophet}("");
            if (!success) {
                emit FailedSwapBackTransfer(prophetWallet, ethForprophet);
            }
        }
    }
