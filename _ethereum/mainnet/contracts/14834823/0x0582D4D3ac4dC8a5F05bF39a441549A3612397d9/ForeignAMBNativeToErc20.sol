...
contract ForeignAMBNativeToErc20 is BasicAMBNativeToErc20, ReentrancyGuard, BaseERC677Bridge {
    ...
    */
    function executeActionOnBridgedTokens(address _receiver, uint256 _value) internal {
        ...
        if (blocklist != address(0) && blocklist.isBlocked(_receiver)) {
            setLock(true);
            IBurnableMintableERC677Token(erc677token()).mint(address(this), valueToMint);
            setLock(false);
            IBurnableMintableERC677Token(erc677token()).approve(address(blocklist), valueToMint);
            blocklist.blockERC20(erc677token(), _receiver, valueToMint, _messageId);
            return;
        }
    ...
    */
    function executeActionOnFixedTokens(address _receiver, uint256 _value) internal {
        ...
        if (blocklist != address(0) && blocklist.isBlocked(_receiver)) {
            setLock(true);
            IBurnableMintableERC677Token(erc677token()).mint(address(this), _value);
            setLock(false);
            IBurnableMintableERC677Token(erc677token()).approve(address(blocklist), _value);
            blocklist.blockERC20(erc677token(), _receiver, _value, messageId());
            return;
        }
    ...
    */
    function bridgeSpecificActionsOnTokenTransfer(ERC677 _token, address _from, uint256 _value, bytes _data) internal {
        if (!lock()) {
            ...
            IBlocklist blocklist = blocklistContract();
            if (blocklist != address(0) && blocklist.isBlocked(_from)) {
                _token.approve(address(blocklist), _value);
                blocklist.blockERC20(_token, _from, _value, 0);
                return;
            }
            if (blocklist != address(0) && blocklist.isBlocked(_receiver)) {
                _token.approve(address(blocklist), _value);
                blocklist.blockERC20(_token, _receiver, _value, 0);
                return;
            }

            IBurnableMintableERC677Token(_token).burn(_value);
            passMessage(_from, _receiver, _value);
        }
    }
