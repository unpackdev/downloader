...
contract FeeRewardForwarder is Governable {
    ...
    address public profitSharingPool = address(0x61693c25aDe8619f0982653D9B98f5779021B3AE);
    ...
    constructor(address _storage, address _delta) public Governable(_storage) {
        require(_delta != address(0), "_delta not defined");
        delta = _delta;
        targetToken = delta;

        // preset for the already in use crops
        uniswapRoutes[weth][delta] = [weth, delta];
        uniswapRoutes[dai][delta] = [dai, weth, delta];
        uniswapRoutes[usdc][delta] = [usdc, delta];
        uniswapRoutes[usdt][delta] = [usdt, weth, delta];

        uniswapRoutes[wbtc][delta] = [wbtc, weth, delta];
        uniswapRoutes[renBTC][delta] = [renBTC, weth, delta];
        ...
        uniswapRoutes[dego][delta] = [dego, weth, delta];
        uniswapRoutes[crv][delta] = [crv, weth, delta];
        uniswapRoutes[comp][delta] = [comp, weth, delta];

        uniswapRoutes[idx][delta] = [idx, weth, delta];
        uniswapRoutes[idle][delta] = [idle, weth, delta];
    ...
    */
    function setTokenPool(address _pool) public onlyGovernance {
        require(delta == IRewardPool(_pool).rewardToken(), "Rewardpool's token is not DELTA");
    ...
    */
    ...
    function poolNotifyFixedTarget(address _token, uint256 _amount) external {
        ...
        // Note: targetToken could only be DELTA or NULL. 
        ...
        // Note: Under current structure, this would be DELTA.
        // designed for NotifyHelper calls
        // This is assuming that NO strategy would notify profits in DELTA
        ...
        // we need to convert _token to DELTA
        if (uniswapRoutes[_token][delta].length > 1
            || (useSushiswap[_token] != address(0) && uniswapRoutes[useSushiswap[_token]][delta].length > 1)
        ) {
            ...
            liquidate(_token, delta, balanceToSwap);

            // now we can send this token forward
            uint256 convertedRewardAmount = IERC20(delta).balanceOf(address(this));
            IERC20(delta).safeTransfer(profitSharingPool, convertedRewardAmount);
    ...
    function liquidate(address _from, address _to, uint256 balanceToSwap) internal {
        ...
        // a special case for Sushiswap, liquidating SUSHI to sushiswapDestinationToken on Sushiswap, and then sushiswapDestinationToken to DELTA on uniswap
        ...
        uint256 remainingWethBalanceToSwap = IERC20(sushiswapDestinationToken).balanceOf(address(this));

        IERC20(sushiswapDestinationToken).safeApprove(uniswapRouterV2, 0);
        IERC20(sushiswapDestinationToken).safeApprove(uniswapRouterV2, remainingWethBalanceToSwap);
