...
contract AVRA is
    ...
    Freezable
{
    string constant private _name = "AVRA";
    string constant private _symbol = "AVRA";
    uint8 constant private _decimals = 18;
    uint256 constant private _initial_supply = 3_100_000_000;
        ...
        returns (bool success)
    {
        require(
            to != address(0),
            "AVRA/transfer : Should not send to zero address"
        );
        ...
        returns (bool success)
    {
        require(
            to != address(0),
            "AVRA/transferFrom : Should not send to zero address"
        );
        ...
        _approve(
            ...
            _allowances[from][msg.sender].sub(
                amount,
                "AVRA/transferFrom : Cannot send more than allowance"
            )
        );
        success = true;
    }
        ...
        returns (bool success)
    {
        require(
            spender != address(0),
            "AVRA/approve : Should not approve zero address"
        );
