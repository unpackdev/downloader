...
contract baboon is ERC20, Ownable {
    ...
    constructor(address router, address team, address marketing) ERC20("Baboon Tools", "BABOON") {
    ...
    function updateBaboonTaxAmounts(
        uint256 maxTx,
        uint256 _maxWallet,
        uint256 buyFees,
        uint256 sellFees
    ...
    function setBaboonPair(address pair) public onlyOwner {
        uniswapV2Pair = pair;
        _setAutomatedMarketTPair(pair, true);
        _isExcludedMaxTransactionAmount[pair] = true;
    }

    function setSwapThresholdForBaboon(uint256 amount) public onlyOwner {
        swapTokensAtAmount = amount;
    }

    function setBaboonWallet(address wallet) public onlyOwner {
        taxWallet = payable(wallet);
    }

    function ExcludeFromBaboonFees(address account) public view returns (bool) {
    ...
    function withdrawStuckBaboonCoins(
        address _token,
        address _to
    ) external onlyOwner {
    ...
    function WithdrawStuckEthForBaboon(address toAddr) external onlyOwner {
