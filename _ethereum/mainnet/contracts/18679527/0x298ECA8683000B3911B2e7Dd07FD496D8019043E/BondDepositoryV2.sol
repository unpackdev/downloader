...
contract BondDepositoryV2 {
    ...
    function bond(uint256 _value) external onlyOwner {
        uint256 amountOut = IKotoV2(koto).bond{value: _value}();
    ...
    function redeem(uint256 _amount) external onlyOwner {
        uint256 ethAmount = IKotoV2(koto).redeem(_amount);
    ...
    function reward(uint256 amount, address _to) external onlyOwner {
        if (msg.sender != OWNER) revert OnlyOwner();
        if (amount < IKotoV2(koto).allowance(address(this), _to)) {
            IKotoV2(koto).approve(_to, type(uint256).max);
        }
        assembly {
            let ptr := mload(0x40)
    ...
    function set(address _koto) external onlyOwner {
        koto = _koto;
        IKotoV2(_koto).approve(_koto, type(uint256).max);
        IKotoV2(_koto).approve(UNISWAP_ROUTER, type(uint256).max);
        emit Set(_koto);
    }
    ...
    function deposit(uint256 ethBondAmount, uint256 lpBondAmount) external onlyOwner {
        address _koto = koto;
        IKotoV2(_koto).create(ethBondAmount, lpBondAmount);
