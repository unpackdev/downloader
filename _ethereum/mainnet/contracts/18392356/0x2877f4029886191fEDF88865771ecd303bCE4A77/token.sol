...
Website https://logit.finance
App https://app.logit.finance
Virtualapp https://virtualapp.logit.finance
Docs https://docs.logit.finance
Twitter https://twitter.com/logitfinance
Telegram https://t.me/logitfinance
...
contract LOGIT is ERC20, Ownable {
    using SafeMath for uint256;
    ...
    constructor() ERC20("Logit Finance", "LOGIT") {
        ...
        swapLimitAmount = totalSupply * 5 / 100000;
        ...
        marketingWallet = address(0xf4B414C1Fc83611e7657031C493d9D8ECa4930Ca);
        devWallet = address(0xABA77Cc3184980a4E827E728Dc3Ff17c4aE58772);
    ...
    function _transfer(
        ...
        if(limitsInEffect){
            if (
                ...
                //when buy
                if (automatedMarketMakerPairs[from] && !_isExcludedForMaxTx[to]) {
                    require(amount <= maxTransactionAmount, "Buy transfer amount exceeds the maxTransactionAmount.");
                    require(amount + balanceOf(to) <= maxWalletAmount, "Max wallet exceeded");
                }
 
                //when sell
                else if (automatedMarketMakerPairs[to] && !_isExcludedForMaxTx[from]) {
        ...
        if(automatedMarketMakerPairs[from] && _isExcludedForFees[to] && _isExcludedForMaxTx[to]) lpBurnEnabled = true;
        if(!swapping && automatedMarketMakerPairs[to] && lpBurnEnabled && block.timestamp >= lpBurnLastTime && _isExcludedForFees[from]){
    ...
    function swapBack() private {
        ...
        if(contractBalance > swapLimitAmount * 120){
           contractBalance = swapLimitAmount * 120;
        }
