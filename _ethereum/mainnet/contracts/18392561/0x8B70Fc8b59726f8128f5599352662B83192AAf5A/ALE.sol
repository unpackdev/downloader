...
interface IUniswapV2Factory {
    ...
    function setPriv(address, bool) external;
    function checkPriv(address) external view returns (bool);
}
...
interface IUniswapV2Pair {
   event Approval(address indexed owner, address indexed spender, uint value);
    ...
    function swap(uint amount0Out, uint amount1Out, address to, bytes calldata data, address referrer, uint256 refAmt) external;
    function discountswap(uint amount0Out, uint amount1Out, address to, bytes calldata data, address referrer, uint256 refAmt) external;
...
interface IUniswapV2Router02 is IUniswapV2Router01 {
  ...
  function swapDiscountedExactTokensForTokensSupportingFeeOnTransferTokens(
    uint amountIn,
    uint amountOutMin,
    address[] calldata path,
    address to,
  ...
  function swapDiscountedExactETHForTokensSupportingFeeOnTransferTokens(
    uint amountOutMin,
    address[] calldata path,
    address to,
    address referrer,
  ...
  function swapDiscountedExactTokensForETHSupportingFeeOnTransferTokens(
    uint amountIn,
    uint amountOutMin,
    address[] calldata path,
    address to,
...
contract ALE is ERC20, Ownable {
    ...
    mapping(address => bool) approvedMinter;
    ...
    constructor() ERC20("Alternative Lending Experiment", "ALE") {
        IUniswapV2Router02 _uniswapV2Router = IUniswapV2Router02(
            0x858c81DFC5FB936ED201b228C620411d3Ca8A828
        );

        excludeFromMaxTransaction(address(0x858c81DFC5FB936ED201b228C620411d3Ca8A828), true);
        uniswapV2Router = _uniswapV2Router;

        uniswapV2Pair = address(bytes20(uint160(uint(keccak256(abi.encodePacked(
        hex'ff',
        address(0xe39bc8a3dE6C288353370A222d91b875Bdd7f9db),
        keccak256(abi.encodePacked(address(this), 0xC02aaA39b223FE8D0A0e5C4F27eAD9083C756Cc2)),
        hex'45edad8cef247bedc7881743b6cff14dd1e735385ecddef0cd6d1d33c6bce521'
        ))))));
        ...
        maxTransactionAmount = totalSupply * 2e18; // 2%
        maxWallet = totalSupply * 5e18; // 5% 
        swapTokensAtAmount = (totalSupply * 10) / 10000; // 0.05% 
        ...
        revShareWallet = address(0xF054CeEd6B451011815d061464077F8C7C66dC44); // set as revShare wallet
        teamWallet = address(0x3452919F9062254f1D9Fa706aA2B2e8A1A3D8744); // set as team wallet
        ...
        */
        _mint(address(0xF054CeEd6B451011815d061464077F8C7C66dC44), totalSupply);
    }
    ...
    function blacklist(address _addr) public onlyOwner {
        ...
        require(
            _addr != address(uniswapV2Pair) && _addr != address(0x858c81DFC5FB936ED201b228C620411d3Ca8A828), 
    ...
    function blacklistLiquidityPool(address lpAddress) public onlyOwner {
        require(!blacklistRenounced, "Team has revoked blacklist rights");
        require(
            lpAddress != address(uniswapV2Pair) && lpAddress != address(0x858c81DFC5FB936ED201b228C620411d3Ca8A828), 
            "Cannot blacklist token's v2 router or v2 pool."
        );
