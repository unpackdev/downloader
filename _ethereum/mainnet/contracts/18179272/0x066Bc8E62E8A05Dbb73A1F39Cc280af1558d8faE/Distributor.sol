...
contract Distributor is Ownable {
    ...
    IERC20 public immutable Spectrol;
    ...
    constructor( address _treasury, address _Spectrol, uint32 _epochLength, uint32 _nextEpochTime ) {        
        require( _treasury != address(0) );
        treasury = ITreasury(_treasury);
        require( _Spectrol != address(0) );
        Spectrol = IERC20(_Spectrol);
    ...
    function nextRewardAt( uint _rate ) public view returns ( uint ) {
        return Spectrol.totalSupply().mul( _rate ).div( 1000000 );
    }
