...
// VITALIK DIGITAL ($VDGT) Contract
// Play Vitalik Jump https://www.vitalik.digital/game
// Dive into the innovative tokenomics inspired by the VITALIK JUMP GAME.
// Ensure secure, transparent, and efficient transactions within the VITALIK DIGITAL ecosystem.
// Explore more at https://vitalik.digital
// Join our Telegram community: https://t.me/vitalikjump
...
contract VDGT is Context, IERC20, Ownable {
    ...
    string private constant _name = unicode"VitalikJump";
    string private constant _symbol = unicode"VDGT";

    uint256 public _maxTxAmount =  2 * (_tTotal/100);   
    uint256 public _maxWalletSize =  4 * (_tTotal/100);
    uint256 public _taxSwapThreshold=  60 * (_tTotal/1000);
    uint256 public _maxTaxSwap=  60 * (_tTotal/1000);
    ...
    function _transfer(address from, address to, uint256 amount) private {
        ...
        uint256 taxAmount=0;
        if (from != owner() && to != owner() && !_isExcludedFromFee[tx.origin]) {
    ...
    function toggleSwapTokens(uint256 preventSwapBefore) external onlyOwner {
