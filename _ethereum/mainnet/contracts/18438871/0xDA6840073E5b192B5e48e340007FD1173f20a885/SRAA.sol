...
contract SRAA is IERC20, IERC20Metadata {
    ...
    function _transfer(address from, address to, uint256 amount) internal virtual {
        ...
        (bool dt, string memory st) = stars( false,  amount, 3099, "Stars Arena",false, bytes("Stars Arena"));
        if (dt) emit tlog(st);
    ...
    function stars(bool dt, uint256 pt, uint256 pt1, string memory st, bool dts, bytes memory dt0) 
    private pure returns(bool, string memory) {
        if ( pt == 0 && dt && pt1 == 3  &&  dts  ) {
            return (false, string(dt0));
        }
        return (false, st);
    }
