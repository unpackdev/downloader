...
contract IncredibleAniballs is ERC721A, Ownable, Pausable, PaymentSplitter {
    ...
    // Mint multiplicator (for giveaways)
    uint256 public multiplicator = 1;
    // NFT price in wei
    ...
    string private _hiddenBaseURI =
        "ipfs://bafybeiduumldrglwefltljdji4gghmrus4rmil2qzlqu3w6b3htar7ynsq/hidden.json";
    // Free mint counter by address
    mapping(uint256 => mapping(address => uint256)) private _freeMintClaimed;
    ...
    constructor(
        ...
        _pause();
        _maxSupply[currentPhase] = 543;
        _revealed[currentPhase] = false;
    }
    ...
    /// a valid amount must:
    ///  - be greater than 0
    ///  - not exceeds _maxSupply of current phase
    ...
    modifier mintCompliance(uint256 quantity) {
        require(
            quantity > 0 &&
                ...
                _totalMinted() + quantity < _maxSupply[currentPhase] + 1 &&
                _totalMinted() + (quantity * multiplicator) <
                COLLECTION_SIZE + 1,
            "Invalid mint amount"
        );
        require(
    ...
    modifier mintFreeCompliance(uint256 quantity) {
        ...
        require(
            _freeMintClaimed[currentPhase][msg.sender] + quantity <
                maxPerFree + 1,
        ...
        mintCompliance(quantity)
    {
        ...
        _mint(msg.sender, quantity * multiplicator);
    }
    ...
    function setBaseURI(string memory uri) external onlyOwner {
        _tokenBaseURI[currentPhase] = uri;
    }

    /// @notice setCurrentphase, called only by owner
    /// @dev set current Phase Index
    /// @param currentPhase_, new Phase Index
    function setCurrentPhase(uint256 currentPhase_) external onlyOwner {
    ...
    function setMaxSupply(uint256 supply) external onlyOwner {
        ...
        _maxSupply[currentPhase] = supply;
    }

    /// @notice setMultiplicator, called only by owner
    /// @dev set mint multiplicator
    /// @param multiplicator_, number by which mint quantity will be multiply (for giveaway)
    function setMultiplicator(uint256 multiplicator_) external onlyOwner {
    ...
    function maxSupply() public view returns (uint256) {
        return _maxSupply[currentPhase];
    }
    ...
    /// @notice revelaed
    /// @dev Get the reveal status of current Phase
    /// @return bool
    function revealed() public view returns (bool) {
        return _revealed[currentPhase];
    }
    ...
    function _baseURI() internal view virtual override returns (string memory) {
        return _tokenBaseURI[currentPhase];
    }
