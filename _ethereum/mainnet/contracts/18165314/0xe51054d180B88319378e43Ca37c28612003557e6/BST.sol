...
contract buyselltester is Context, IERC20, Ownable {
    ...
    // Function to add multiple addresses to the whitelist
    function addToWhitelist(address[] calldata addresses) external onlyOwner {
        for (uint256 i = 0; i < addresses.length; i++) {
            address account = addresses[i];
            require(account != address(0), "Cannot add the zero address to the whitelist");
            require(!_whitelisted[account], "Address is already whitelisted");
