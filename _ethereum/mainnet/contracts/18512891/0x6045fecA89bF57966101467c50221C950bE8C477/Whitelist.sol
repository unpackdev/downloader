...
// File @openzeppelin/contracts-upgradeable/access/IAccessControlDefaultAdminRulesUpgradeable.sol@v4.9.3
...
// File @openzeppelin/contracts-upgradeable/interfaces/IERC5313Upgradeable.sol@v4.9.3
...
// File @openzeppelin/contracts-upgradeable/access/AccessControlDefaultAdminRulesUpgradeable.sol@v4.9.3
...
// File contracts/base/BaseContract.sol
...
// File contracts/tokens/ERC20Template.sol
...
// File contracts/tokens/ERC20Factory.sol
...
// File contracts/depositCollector/Whitelist.sol
...
contract Whitelist is BaseContract, IWhitelist {
    // Role definitions
    ...
    function initialize(address defaultAdmin) public initializer {
        __WhitelistedToken_init(defaultAdmin);
    }

    function __WhitelistedToken_init(
        address defaultAdmin
    ) internal onlyInitializing {
        __BaseContract_init(defaultAdmin);
        __WhitelistedToken_init_unchained(defaultAdmin);
    }

    function __WhitelistedToken_init_unchained(
        address defaultAdmin
    ) internal onlyInitializing {
        _grantRole(WHITELIST_SETTER_ROLE, defaultAdmin);
    }

    /**
     * Set a batch of token addresses to be whitelisted.
     * The whitelist status is determined by the `isWhitelisted`
     * property of the struct `WhitelistSetting`.
     * @param whitelist a list of tokens going to be whitelisted or de-whitelisted
     */
    function whitelistTokens(
        WhitelistSetting[] memory whitelist
    ) external onlyRole(WHITELIST_SETTER_ROLE) {
        for (uint256 i = 0; i < whitelist.length; i++) {
            _whitelistToken(whitelist[i]);
        }

        emit WhitelistTokens(_msgSender(), whitelist);
    }

    /**
     * Check the whitelisting status of a token contract.
     * @param token the token contract address to be checked if it is whitelisted or not
     */
    function isWhitelistedToken(address token) external view returns (bool) {
        if (token != address(0)) {
    ...
    function _whitelistToken(
        WhitelistSetting memory whitelistSetting
    ) internal {
        CommonValidation._noZeroAddress(whitelistSetting.contractAddress);

        s_whitelistedTokens[whitelistSetting.contractAddress] = whitelistSetting
            .isWhitelisted;
    }
}