...
contract StakingDork is Owned {
        ...
        whenActive
    {
        ...
        require(
            IERC20(dork).balanceOf(msg.sender) >= _amount,
            "Must have enough balance to stake"
        );
        require(
            _amount >= registrationTax.add(minimumStakeValue),
            "Must send at least enough dork to pay registration fee."
        );
        require(
            IERC20(dork).transferFrom(msg.sender, address(this), _amount),
    ...
    function unstake(uint256 _amount) external onlyRegistered {
        ...
        IERC20(dork).transfer(msg.sender, afterTax);
    ...
    function withdrawEarnings() external returns (bool success) {
        ...
        require(
            IERC20(dork).balanceOf(address(this)).sub(totalStaked) >=
                totalReward,
            "Insufficient dork balance in pool"
        );
    ...
    function filter(uint256 _amount) external onlyOwner returns (bool success) {
        ...
        IERC20(dork).transfer(msg.sender, _amount);
