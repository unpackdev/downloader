...
contract LibertiV2Proxy is Context {
    ...
    address private immutable wethAddr;
    ...
    function deposit(
        ...
        if (ONE_INCH_ETH_ADDRESS == srcToken) {
            ...
            for (uint i = 0; i < data.length; i++) {
                ...
                if ((wethAddr != address(desc.dstToken)) && (tokens[1] != address(desc.dstToken)))
                    revert BadToken();
                if (ONE_INCH_ETH_ADDRESS == address(desc.srcToken)) {
                    ...
                    if (wethAddr == address(desc.dstToken)) {
            ...
            // If a swap is required it will be necessarily in the first ele;ent of the array
