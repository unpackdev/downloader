...
contract Angelus_RevealVerifier {
    // Omega
    uint256 constant w1 =
    ...
    uint16 constant nPublic = 35;
    uint16 constant nLagrange = 35;

    uint256 constant Qmx =
        4783095396951306820300736261267016194303883792756596487872052163028057882406;
    uint256 constant Qmy =
        5087239812741920195180515874660433183332604359616615281271160839435216986595;
    uint256 constant Qlx =
        8586733084738978121604484736602861718805439225204398389530035155037086934398;
    uint256 constant Qly =
        13281527449033201572255655137879731976855529891076439791955357487895119616942;
    uint256 constant Qrx =
        19863611030854711815693541197937534715951122345284322175532753903622269197804;
    uint256 constant Qry =
        2243236748262097773248578845785735563798146367015429276244101638103541773312;
    uint256 constant Qox =
        18753434914798569196428686628499199931075833046499510270464440269787513604710;
    uint256 constant Qoy =
        21107558925285119634135675950365379819248176292741712924288257506825598011512;
    uint256 constant Qcx =
        4380899062405711593491374198781167309599089046088543621059584072725608326481;
    uint256 constant Qcy =
        18969242712067125064448814816399268625044767994365748717819354641430007232938;
    uint256 constant S1x =
        12095515471661952360576561153749698460449553362746536819950308500548916329794;
    uint256 constant S1y =
        19539214990639093340120290157214206406577248732254076506420403898819966467851;
    uint256 constant S2x =
        19222532931008561483185558736531593443816664589370775388213761762534248122836;
    uint256 constant S2y =
        7522079400376446552496907606983812416612201169186782218879228033551487589525;
    uint256 constant S3x =
        18650154143115374389001237245480796679395535070908826735250799175118494999122;
    uint256 constant S3y =
        11915490681273238207244450853609585364576732833595020707740828257723321892146;
    ...
    function verifyProof(
        ...
        assembly {
            ...
            function calculateChallenges(pMem, pPublic) {
                ...
                mstore(add(mIn, 1632), calldataload(pA))
                mstore(add(mIn, 1664), calldataload(add(pA, 32)))
                mstore(add(mIn, 1696), calldataload(pB))
                mstore(add(mIn, 1728), calldataload(add(pB, 32)))
                mstore(add(mIn, 1760), calldataload(pC))
                mstore(add(mIn, 1792), calldataload(add(pC, 32)))

                beta := mod(keccak256(mIn, 1824), q)
            ...
            function calculateLagrange(pMem) {
                ...
                inverseArray(add(pMem, pZhInv), 36)
