...
contract FuturamaPunks is ERC721A, Ownable {
    ...
    constructor(string memory _name, string memory _symbol) ERC721A(_name, _symbol) {
        ...
        setBaseURI(""); //put your baseURI here
    }
    ...
    function _mint(uint256 count) external payable {
        ...
        require(msg.value >= count * price, "Not enough ether sent");
        require(totalSupply() + count <= maxSupply, "Not enough tokens left");
        require(mintEnabled, "Mint is not live yet.");
        require(count + _numberMinted(msg.sender) <= maxPerWallet, "Exceeded the limit");
        require(tx.origin == msg.sender, "Contract minting is not allowed");

        _safeMint(msg.sender, count);
    }

    function _freeMint() external {
        
        require(!freeMinted[msg.sender], "Already Minted");
        require(totalFreeMinted + 1 <= maxFreeSupply, "Free mint quota reached");
        require(totalSupply() + 1 <= maxSupply, "Not enough tokens left");
        require(freeMintEnabled, "Free Mint is not live yet.");
        require(1 + _numberMinted(msg.sender) <= maxPerWallet, "Exceeded the limit");
        require(tx.origin == msg.sender, "Contract minting is not allowed");
        freeMinted[msg.sender] = true;
        _safeMint(msg.sender, 1);
        totalFreeMinted = totalFreeMinted + 1;
    }
    ...
    function flipFreeSale() external onlyOwner {
