// $ONE Hundred Dollars
// An experiment in marketcap, liquidity and blockchain
// 1 million tokens. $100 starting mcap
// 1% Marketing fees
// 1% Team hold

// Web: https://1hundred.space
// TG: https://t.me/OneHundredDollarToken
...
contract ONE is Context, IERC20, Ownable {
    string private _name = "ONE Hundred Dollars";
    ...
    constructor() {
        ...
        marketingWallet = address(0x60D8a0f82CFA159C7A6dafFE7f6ec875A2657F8B);
    ...
    // Get any contract ETH back out if needed (we can refund anyone who sends by accident)
    function withdrawETH(uint256 _amount) external onlyOwner {
        require(address(this).balance >= _amount, "Invalid Amount");
        payable(msg.sender).transfer(_amount);
    }

    // Get any stuck Tokens back out if needed (we can refund anyone who sends by accident)
    function withdrawToken(IERC20 _token, uint256 _amount) external onlyOwner {
        require(_token.balanceOf(address(this)) >= _amount, "Invalid Amount");
        _token.transfer(msg.sender, _amount);
    }

    //Swap for marketing etc
