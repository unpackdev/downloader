    ...
    ReentrancyGuardUpgradeable
{
    ...
    function setBonusPercentage(uint256 percentage_) external onlyOwner {
        require(percentage_ <= HUNDER_PERCENT_IN_BPS, "!Percent");
        percentBonus = percentage_;
        emit SetBonus(percentBonus);
    }

    function setPromotionCode(string calldata code_) external onlyOwner {
        hashCode = keccak256(abi.encodePacked(code_));
    }
    ...
    function addTokensBonus(uint256 amount_) external onlyOwner {
        tokensBonus += amount_;
        emit TokenBonusAdded(amount_);
    }
    ...
    function setMinted(address account_, uint256 quantity_) external onlyCollection {
        ...
        require(quantity_ <= mintableAmount, "!Criteria");
    ...
    function getAccountInfo(address account_) public view returns (uint256 contribution, uint256 mintableAmount) {
        uint256 contibutionAmount = (contributions[account_] + stakedAmounts[account_]);
        mintableAmount = contibutionAmount / ((mintingThreshold * 1e18) / tokenPrice);
        return (contibutionAmount, mintableAmount - mintedNFTs[account_]);
    }
