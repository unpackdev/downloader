...
contract FDA is ERC721, ERC721Burnable, Ownable {
    using Strings for uint256;
    string private _baseUrl;
    uint256 private _totalTokens =1; // Total number of tokens minted + 1
    uint256 private _max_supply = type(uint256).max;
    ...
    function batchMint(uint256 count) public onlyOwner returns (uint256) {
        uint256 _tokenStartValue = _totalTokens;
        require(_totalTokens + count - 1 <= _max_supply, "Max supply reached");
        for (uint256 i = _tokenStartValue; i < _tokenStartValue + count; i++) {
            _mint(msg.sender, i );
        }
        _totalTokens = _totalTokens + count;
        return _tokenStartValue;
    }
    ...
    function contractURI() public view returns (string memory) {
        return string(abi.encodePacked(_baseUrl, "metadata.json"));
    }
    ...
    function tokenURI(uint256 tokenId) public view virtual override returns (string memory) {
        ...
        if (bytes(_baseUrl).length > 0 ){
            string memory part_1 = string(abi.encodePacked(_baseUrl, tokenId.toString()));
            return string(abi.encodePacked(part_1, ".json"));
        } else {
            return "";
        }
    }
