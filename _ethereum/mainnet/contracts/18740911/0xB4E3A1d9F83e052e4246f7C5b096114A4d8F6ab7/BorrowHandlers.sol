...
abstract contract BorrowHandlers is IBorrowHandlers, BorrowCheckers, SafeMint {
    ...
    function useCollateral(OfferArg[] memory args, address from, NFToken memory nft) internal returns (uint256 loanId) {
        ...
        Ray feePercentage = feeRatePerAssetStorage().feeRatePerAsset[collatState.assetLent];
        // save gas if fee is deactivated
        if (Ray.unwrap(feePercentage) != 0) {
            address admin = IOwnershipFacet(address(this)).owner();
            feeAmount = RayMath.mul(lent, feePercentage);
            collatState.assetLent.checkedTransferFrom(signer, admin, feeAmount);
        }

        collatState.assetLent.checkedTransferFrom(signer, collatState.from, lent - feeAmount);
