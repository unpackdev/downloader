...
contract AdminFacet is IAdminFacet {
    ...
    /// @notice updates the fee rate for an asset
    /// @param asset the erc20 on which a new fee rate is taking effect
    /// @param newFeeRate the new fee rate
    function setFeeRateForAsset(IERC20 asset, Ray newFeeRate) external onlyOwner {
        feeRatePerAssetStorage().feeRatePerAsset[asset] = newFeeRate;
        emit NewFeeRatePerAsset(asset, newFeeRate);
    }
}
