...
contract ProtocolFacet is IProtocolFacet {
    ...
    /// @notice get the fee rate that will be charged for an asset
    /// @param asset the erc20 on which the fee applies
    /// @return feeRate fee as a percentage
    function getFeeRateForAsset(IERC20 asset) external view returns (Ray feeRate) {
        return feeRatePerAssetStorage().feeRatePerAsset[asset];
    }
}
