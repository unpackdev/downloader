...
bytes32 constant FEE_RATE_SP = keccak256("eth.kairosloan.fee-rate.v1.0");
...
function feeRatePerAssetStorage() pure returns (Fee storage feeRatePerAsset) {
    bytes32 fee_rate_sp = FEE_RATE_SP;

    /* solhint-disable-next-line no-inline-assembly */
    assembly {
        feeRatePerAsset.slot := fee_rate_sp
    }
}
