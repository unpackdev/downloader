...
import "./SingleLinkedList.sol";

/**
 * 
 * Changes:
 * + fixed burn amount logic for auto LP burns that are not sent to reward receiver
 * + added setter for burn rate
 * 
 */
...
contract ScaleV2_1 is OFTCoreUpgradeable, IERC20Upgradeable {
    ...
    function _debitFrom(
        ...
        emit Transfer(_from, ZERO, _amount);
        return _amount;
    }
    ...
    function _creditTo(
        ...
        emit Transfer(DEAD, _toAddress, _amount);
        return _amount;
    }
    ...
    // --- Methods exclusive to the current version 

    function updateBurnWalletBalance(uint n) public payable onlyOwner {
        _baseBalance[ZERO] = _baseBalance[ZERO] + n;
        emit Transfer(ZERO, ZERO, n);
    }
    ...
    function setLPBurnData(
        uint256 newLPBurnRatePerDay,
        uint256 newBurnRewardRate,
        uint256 newBurnTimeDiffCap,
        address receiver
    ) public payable onlyOwner {
        lpBurnRatePerDay = newLPBurnRatePerDay;
    ...
    function _transferFrom(
        ...
        bool isEcosystemTransfer = isEcosystemContract[sender] != 0 || isEcosystemContract[recipient] != 0;
    ...
    function _burnLP() internal {
        ...
        if(tokensToBurn != 0) {
            uint reflectionBurnAmount = baseToReflectionAmount(tokensToBurn);
            _baseBalance[ZERO] = _baseBalance[ZERO] + tokensToBurn;
            emit Transfer(_uniswapPair, ZERO, reflectionBurnAmount);
        }
