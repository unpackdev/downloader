...
interface IPaymentPlan {
    ...
    // Errors
    error NotSubscribed(string);
    error AlreadySubscribed(string);
    ...
    event SubscriptionExtended(string invoice, string indexed purchaser, uint128 indexed expiry);
    event SubscriptionCreated(string invoice, string code, string indexed purchaser, uint128 indexed expiry);
    ...
    /// @param userId The invoice reference
    ...
    function subscribe(string calldata userId, string calldata invoice, string calldata planId, address token, string calldata code) external payable;

    /// @notice Renew a subscription
    /// @param userId The user id to map the sub to
    /// @param invoice The invoice reference
    /// @param planId The plan ID to renew
    /// @param token The token used for payment
    function renewSubscription(string calldata userId, string calldata invoice, string calldata planId, address token) external payable;
    ...
    function updateUserSubscription(string calldata userId, Subscription calldata newSubscription) external;
