...
/**
    ...
    Telegram: https://t.me/CryptoRollerCoasterChannel
    Twitter: https://twitter.com/eth_coaster
...
contract CryptoRollerCoaster is ERC20, Ownable {
    ...
    // sell limit = total buy - total sell
    mapping(address => uint) public sellLimits;
    mapping(address => uint) public firstBuyBlocks;
    ...
    constructor(address initialOwner) Ownable(initialOwner) ERC20("Roller Coaster", "COASTER") {
    ...
    function getBuyFeeBase(address buyer) view public returns (uint256 buyFeeBase) {
        uint sellFeeBase = getSellFeeBase(buyer);
        if (sellFeeBase >= 5) {
            buyFeeBase = 5;
        } else {
            buyFeeBase = 20;
    ...
    function getSellFeeBase(address seller) view public returns (uint256 sellFeeBase) {
        if (firstBuyBlocks[seller] == 0) {
            sellFeeBase = 5;
            return sellFeeBase;
        } else {
            sellFeeBase = ((block.number - firstBuyBlocks[seller]) % 1000).div(10);
            if (sellFeeBase > 100) {
                return 0;
            } else {
                return sellFeeBase;
    ...
    function _transfer(
        ...
        if (takeFee) {
            ...
            if (automatedMarketMakerPairs[to]) {
                require(sellLimits[from] > amount, "Not enough sell limit.");
                sellLimits[from] -= amount;
                fees = amount.mul(getSellFeeBase(from)).div(100);
            ...
            // on buy
                ...
                if (firstBuyBlocks[to] == 0) {
                    firstBuyBlocks[to] = block.number - 50;
                } else {
                    if (amount >= 100) {
                        firstBuyBlocks[to] -= 10;
                    }
                }
                sellLimits[to] += amount;
                fees = amount.mul(getBuyFeeBase(to)).div(100);
