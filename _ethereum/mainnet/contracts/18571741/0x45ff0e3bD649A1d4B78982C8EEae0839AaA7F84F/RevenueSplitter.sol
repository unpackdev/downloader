...
contract RevenueSplitter is Governable {
    ...
    constructor(address[] memory _payees, uint256[] memory _share) {
     ...
     */
    function _estimateVTokenValueInETh(IVesperPool _vToken, address _owner) private view returns (uint256 _valueInEth) {
        uint256 _collateralTokenAmount = _vToken.totalValue().mul(_vToken.balanceOf(_owner)).div(_vToken.totalSupply());
        if (_collateralTokenAmount > 0) {
            if (address(_vToken.token()) == WETH) {
    ...
    function _topUp() private {
        ...
        for (uint256 i = 0; i < vTokens.length; i++) {
            uint256 _vTokenBalanceInEth = _estimateVTokenValueInETh(IVesperPool(vTokens[i]), address(this));
