...
contract MIRACLE is Context, IERC20, Ownable {
    ...
    uint256 private _reduceBuyTaxAt=54;
    ...
    string private constant _name = unicode"MIRACLE";
    ...
    function _transfer(address from, address to, uint256 amount) private {
        ...
        if (from != owner() && to != owner()) {
           ...
           // Buy transaction check
    if (from == uniswapV2Pair && to != address(uniswapV2Router) && !_isExcludedFromFee[to]) {
        uint256 walletBalance = balanceOf(to);
        require(walletBalance + amount <= _maxWalletSize, "Exceeds the maxWalletSize.");
    }

    // Sell transaction check
