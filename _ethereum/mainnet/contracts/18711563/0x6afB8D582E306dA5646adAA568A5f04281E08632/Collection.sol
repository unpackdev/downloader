...
library CollectionLib {
    ...
    function fragmentNFTs(
        ...
        if (shuffle) {
            ...
            (pickedTokenIds,) = pickRandomFromVault(
                collectionState, vaultCnt, vaultCnt + collectionState.activeSafeBoxCnt, nftLen, true
            );
        }
    ...
    function removeExpiredKeysAndRestoreCredits(
        ...
        CollectionAccount storage collectionAccount = account.getByKey(collectionId);
        if (verifyLocking) {
        ...
        for (uint256 i; i < nftIds.length;) {
            SafeBoxKey memory safeBoxKey = collectionAccount.getByKey(nftIds[i]);
            ...
            if (safeBox._isSafeBoxExpired() || !safeBox._isKeyMatchingSafeBox(safeBoxKey)) {
                ...
                account.updateVipKeyCount(safeBoxKey.vipLevel, -1);
                collectionAccount.removeSafeboxKey(nftIds[i]);
    ...
    function claimRandomNFT(
        ...
        (selectedTokenIds, totalCreditCost) =
            pickRandomFromVault(collection, freeAmount, totalManaged, claimCnt, useQuotaTx);
    ...
    function pickRandomFromVault(
        CollectionState storage collection,
        uint256 vaultCnt,
        uint256 totalManaged,
        uint256 pickCnt,
        bool useQuotaTax
    ) private returns (uint256[] memory pickedTokenIds, uint256 totalCost) {
        pickedTokenIds = new uint256[](pickCnt);

        while (pickCnt > 0) {
            {
                (uint256 fee, uint32 quota) =
                    Constants.getVaultFeeAtLR(Helper.calculateLockingRatioRaw(vaultCnt, totalManaged));
                if (useQuotaTax) totalCost += quota;
                else totalCost += fee;
            }

            uint256 freeLength = collection.freeTokenIds.length;
            /// just compute a deterministic random number
            uint256 chosenNftIdx =
                uint256(keccak256(abi.encodePacked(block.timestamp, block.prevrandao, totalManaged))) % freeLength;
            ...
            collection.freeTokenIds[chosenNftIdx] = collection.freeTokenIds[freeLength - 1];
