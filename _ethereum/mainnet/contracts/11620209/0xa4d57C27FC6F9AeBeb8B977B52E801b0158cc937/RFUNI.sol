...
Only uniswap router is whitelisted. Owner address is whitelisted to facilitate pool creation and closing and to avoid double burn. 
...
Game guide:
- Best time to buy is after bots sell and before liquidity lock. And liquidity will be locked as usual.
- Once price 2x your entry, take out your initial investment and leave the rest for more gains (moonbag)
- Sell at the highest possible price
- Use the least amount of slippage, but higher gas to avoid frontrunner bots that end up costing you more
- Don't bet the farm! Invest what you can risk. This is degen, it's a chance to win big and fast, but also to lose fast, so don't risk too much. 
...
Usecase: 
Contrary to what most of you do in crypto: Buy low sell high. 
...
contract RFUNI is ERC20Detailed {
  ...
  string constant tokenName = "RFUNI";
  string constant tokenSymbol = "RFUNI Token";
  uint8  constant tokenDecimals = 18;
  
  uint256 constant easyDecimals = 1000000000000000000;
  // yes you can use an exponent instead if you want
  uint256 _totalSupply = 100000 * easyDecimals;
  
  
  //% 
  uint256 public burnPercentage = 1500;
