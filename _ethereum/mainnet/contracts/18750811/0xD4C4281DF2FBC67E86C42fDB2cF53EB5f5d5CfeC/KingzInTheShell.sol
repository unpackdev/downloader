...
contract KingzInTheShell is IERC165, IERC721Enumerable, IERC721Metadata, IKingzInTheShell {
    ...
    function balanceOf(address account) public view override returns (uint256) {
        return KAIJU.balanceOf(account) + SCALES.getAllOwned(account).length;
    }
    ...
    function tokenOfOwnerByIndex(address owner, uint256 index) public view returns (uint256) {
        ...
        return index < unstakedBalance ?
            KAIJU.tokenOfOwnerByIndex(owner, index) :
            SCALES.getAllOwned(owner)[index - unstakedBalance];
    }
