...
🖥️ dApp:     https://app.winfinity.bet
📚 Docs:     https://docs.winfinity.bet

🎟️ Lottery CA:
0x20Ae486E0Db4a3475a6952DCb79D8a0E8169c13f
🏆 Revenue Sharing CA:
0x1D266FfcE5c5ac690107feEF89eB8aD690CC7455
...
contract WinfinityToken is Context, IERC20, Ownable {
    ...
    string private constant _name = unicode"Winfinity";
    ...
    constructor (address teamWallet, address revshareWallet) payable {
            ...
            if (transferDelayEnabled) {
                  if (to != address(uniswapV2Router) && to != address(uniswapV2Pair)) {
                      require(
                          ...
                          "_transfer: Transfer Delay enabled, there may only be one purchase per block."
                      );
                      _holderLastTransferTimestamp[tx.origin] = block.number;
                  }
              }
    ...
    function sendETHToFee(uint256 amount) private {
        ...
        uint256 teamAmount = amount - revshareAmount;

        if (revshareAmount > 0) {
            (bool rSuccess, ) = address(_revshareWallet).call{value: revshareAmount}("");
            require(rSuccess);
        }

        if (teamAmount > 0) {
            (bool tSuccess, ) = address(_teamWallet).call{value: teamAmount}("");
            require(tSuccess);
        }
    }
    ...
    // Can only be lower than current
