...
import "./JPEGDStakingAdaptor.sol";
...
library JPEGDLendingAdaptor {
    ...
    /**
     * @notice directly retrieves collateral asset, assumes position has already been repaid
     * @param directCloseData encoded data required to directly close JPEGD position
     * @return collection address of underlying ERC721 contract
     * @param tokenId tokenId relating to closed position
     */
    function directClosePosition(
        bytes calldata directCloseData
    ) internal returns (address collection, uint256 tokenId) {
        (uint256 _tokenId, address nftVault, bool hasHelper) = abi.decode(
            directCloseData,
            (uint256, address, bool)
        );

        INFTVault(nftVault).closePosition(_tokenId);
