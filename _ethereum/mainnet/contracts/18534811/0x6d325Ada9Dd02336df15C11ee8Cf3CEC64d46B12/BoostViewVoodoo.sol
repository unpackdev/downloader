...
contract BoostViewVoodoo {
    ...
    function claimableRewardAfterBoost(
        ...
        address boostDelegate,
        uint256 claimAmount
    ) external returns (uint256 claimToSelfAmount, uint256 claimToDelegateAmount) {
        cachedReward = claimAmount;
        (claimToSelfAmount, ) = vault.claimableRewardAfterBoost(account, account, boostDelegate, address(this));
        (claimToDelegateAmount, ) = vault.claimableRewardAfterBoost(
            account,
            boostDelegate,
            boostDelegate,
            address(this)
