...
contract ConnectorV2Dsa {
    ...
    function delegateDoughCall(uint256 _actionId, address _token1, address _token2, uint256 _amount, bool _opt) external payable {
        ...
        if (_token1 == address(0)) revert CustomError("ConnectorV2Dsa: invalid token1");
        if (_token1 != _token2) revert CustomError("ConnectorV2Dsa: invalid token2");
        if (_opt) revert CustomError("ConnectorV2Dsa: invalid option");

        if (_actionId == 0) {
            // withdrawEthFromDsa
            if (_token1 != _ETH) revert CustomError("ConnectorV2Dsa: withdrawEthFromDsa: invalid token");
            ...
            if (_amount > IERC20(_token1).balanceOf(address(this))) revert CustomError("ConnectorV2Dsa: withdrawTokenFromDsa: invalid amount");
            if (_token1 == _WETH) {
                IWETH(_WETH).withdraw(_amount);
                payable(owner).transfer(_amount);
            } else {
                IERC20(_token1).safeTransfer(owner, _amount);
            }
        }
    }
}
