...
contract PerfectPunksRenderer is Ownable {
    IStringUtilsV3 stringUtils;
    ICryptoPunksData cryptoPunksData;

    bool public showText = true;

    function setShowText(bool _showText) external onlyOwner {
        showText = _showText;
    }
...
'<svg width="640" height="640" version="1.1" viewBox="0 0 640 640" xmlns="http://www.w3.org/2000/svg">'
  ...
  '<rect y="0" height="640" x="0" width="640" fill="url(#bg)"/>'
   ...
   punkSvg,
   showText ? string(abi.encodePacked('<rect y="590" height="40" fill="white" opacity="0.5" stroke="black" x="100" width="460"/><text x="50%" text-anchor="middle" y="620">Perfect Punk #', strPunkId, ' V1+V2</text>')) : '',
'</svg>'
        );
    }

    function getTraitsJsonValue(uint256 tokenId) internal view returns(string memory) {
    ...
    function getTokenURI(uint256 punkId) public view returns (string memory) {
        ...
        string memory traits = getTraitsJsonValue(punkId);
    ...
    function getContractURI(address) public view returns(string memory) {
        ...
        '{'
            ...
            '"external_link": "https://perfectpunks.cyberpnk.win"'
        '}'));
    }
}
