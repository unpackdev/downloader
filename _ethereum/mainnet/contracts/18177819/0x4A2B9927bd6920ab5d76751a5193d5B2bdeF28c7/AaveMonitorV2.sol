...
contract AaveMonitorV2 is AdminAuth, DSMath, AaveSafetyRatioV2 {
    ...
    // New strategy aave sub proxy
    IAaveSubProxy public aaveSubProxy = IAaveSubProxy(0xf5b40E26cEE37650f5B93946Bd4AC3e39CbcE76b);
    ...
    /// @dev One time migrate method to move all aaveV2 positions to the new dfs strategy system
    function migrate(address[] memory proxies) external onlyApproved {
        for (uint256 i  = 0; i < proxies.length; ++i) {
            address userProxy = proxies[i];

            AaveSubscriptionsV2.AaveHolder memory oldAaveSub = subscriptionsContract.getHolder(userProxy);

            if (subscriptionsContract.isSubscribed(userProxy)) {
                // format for new data sub
                IAaveSubProxy.AaveSubData memory newSubInfo = IAaveSubProxy.AaveSubData({
                    minRatio: oldAaveSub.minRatio,
                    maxRatio: oldAaveSub.maxRatio,
                    targetRatioBoost: oldAaveSub.optimalRatioBoost,
                    targetRatioRepay: oldAaveSub.optimalRatioRepay,
                    boostEnabled: oldAaveSub.boostEnabled
                });

                // call aaveMonitorProxy which will call users DSProxy to call aaveSubProxy and subscribe to strategy system with same data
                // call to aaveSubProxy will also unsub the user from this automation
                aaveMonitorProxy.callExecute(
                    userProxy,
                    address(aaveSubProxy),
                    abi.encodeWithSelector(IAaveSubProxy.subToAaveAutomation.selector, newSubInfo)
                );
            }
        }
    }
