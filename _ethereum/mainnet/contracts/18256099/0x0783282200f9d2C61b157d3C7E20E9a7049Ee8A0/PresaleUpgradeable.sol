    ...
    ReentrancyGuardUpgradeable
{
    ...
    mapping(address => uint256) public mintedNFTs; // numbers  of minted NFTs
    mapping(address => uint256) public stakedAmounts; // amounts of token has been staked

    modifier onlyFinalized() {
        require(isFinalized == true, "!Finalize");
        _;
    }

    modifier onlyCollection() {
        require(_msgSender() == collection, "!Auth");
        _;
    }
    ...
    function initialize(
        ...
        uint256 mintingThreshold_
    ) external initializer {
        __Ownable_init_unchained();
        __UUPSUpgradeable_init_unchained();
        __ReentrancyGuard_init_unchained();
        __init(presaleToken_, beneficiary_, tokenPrice_, minPurchase_, mintingThreshold_);
    }
    ...
    function addTokensPresale(uint256 amount_) external onlyOwner {
        tokensTotal += amount_;
        emit TokenSaleAdded(amount_);
    }
    ...
    function setStakingManager(address stakingManager_) external onlyOwner {
    ...
    function setMintingCollection(address collection_) external onlyOwner {
        collection = collection_;
    }

    // MINT
    function setMinted(address account_, uint256 quantity_) external onlyCollection {
        (, uint256 mintableAmount) = getAccountInfo(account_);
        require(quantity_ <= mintableAmount && ICollection(collection).totalMinted() + quantity_ < 10001, "!Criteria");
        mintedNFTs[account_] += quantity_;
    }

    function updateFromOrtherNetwork(
        address[] calldata users_,
        uint256[] calldata userDeposits_,
        bool isStaking_
    ) external onlyOwner {
        uint256 length = users_.length;
        if (userDeposits_.length != length) revert LengthMisMatch();
        for (uint256 i = 0; i < length; ) {
            _updateTotal(userDeposits_[i]);
            _updateContribution(users_[i], userDeposits_[i], isStaking_);
            unchecked {
                ++i;
            }
        }
    ...
    function claim(bool isStaking_) external nonReentrant onlyFinalized {
        address sender = _msgSender();

        require(!hasClaimed[sender], "Claimed");
        hasClaimed[sender] = true;

        uint256 contributionAmount = contributions[sender];
        contributions[sender] = 0;
        if (isStaking_) {
            stakedAmounts[sender] += contributionAmount;
            StakingManager(stakingManager).depositByPresale(sender, contributionAmount);
        } else {
            SafeTransferLib.safeTransfer(presaleToken, sender, contributionAmount);
        }

        emit Released(sender, contributionAmount);
    }

    function distribute(address[] calldata beneficiaries_) external nonReentrant onlyFinalized onlyOwner {
        uint256 length = beneficiaries_.length;
        address recipient;
        uint256 contributionAmount;

        for (uint256 i = 0; i < length; ) {
            recipient = beneficiaries_[i];

            require(!hasClaimed[recipient], "Claimed");
            hasClaimed[recipient] = true;

            contributionAmount = contributions[recipient];
            contributions[recipient] = 0;

            SafeTransferLib.safeTransfer(presaleToken, recipient, contributionAmount);

            unchecked {
                ++i;
            }

            emit Released(beneficiaries_[i], contributionAmount);
        }
    }

    // BUY PROCESSING
    function buy(address paymentToken_, uint256 paymentAmount_, bool isStaking_) external payable nonReentrant {
        address sender = _msgSender();
        ...
        _updateTotal(tokenAmount);
        _updateContribution(sender, tokenAmount, isStaking_);
    ...
    function buyExactTokens(
        ...
        _updateTotal(tokenAmount_);
        _updateContribution(sender, tokenAmount_, isStaking_);
    ...
    function deposit(uint256 amount) external nonReentrant {
        ...
        stakedAmounts[contributor] += amount;
        StakingManager(stakingManager).depositByPresale(contributor, amount);
    }
    ...
    function __init(
        ...
        minPurchase = minPurchase_;
        mintingThreshold = mintingThreshold_;
    }
    ...
    function _updateContribution(address contributor_, uint256 amount_, bool isStaking_) internal {
        if (isStaking_) {
            stakedAmounts[contributor_] += amount_;
            StakingManager(stakingManager).depositByPresale(contributor_, amount_);
        } else {
            contributions[contributor_] += amount_;
        }
    }

    function _updateTotal(uint256 tokenAmount_) internal {
        tokensSold += tokenAmount_;
        require(tokensTotal >= tokensSold, "Exceed");
    }
    ...
    {
        raisedUsd = (tokensSold * tokenPrice) / 1 ether;
        totalRaisedUsd = (tokensTotal * tokenPrice) / 1 ether;
        return (tokenPrice, minPurchase, mintingThreshold, tokensSold, tokensTotal, raisedUsd, totalRaisedUsd);
    }

    function getAccountInfo(address account_) public view returns (uint256 contribution, uint256 mintableAmount) {
        mintableAmount = (contributions[account_] + stakedAmounts[account_]) / ((mintingThreshold * 1e18) / tokenPrice);
        return (contributions[account_], mintableAmount - mintedNFTs[account_]);
    }
