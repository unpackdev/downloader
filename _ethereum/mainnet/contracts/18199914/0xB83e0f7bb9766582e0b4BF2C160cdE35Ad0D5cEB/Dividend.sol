...
contract Dividend is Ownable {
    ...
    function whitelistForDividend(address owner_, bool _isWhitelist) external onlyOwner {
      _whitelists[owner_] = _isWhitelist;
    }

    function setPairForDividend(address _pair) external onlyOwner {
      pair = _pair;
      isDividendFinished = false;
      claimTime = 0;
    }

    function setTokenForDividend(address _token) external onlyOwner {
      token = _token;
    }
