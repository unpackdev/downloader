...
import "./Utils.sol";
import "./AllowanceSetter.sol";
import "./ExchangeHandler.sol";
import "./TotleControl.sol";
import "./WETH.sol";
        ...
        returns (uint256 amountSpentOnOrder, uint256 amountReceivedFromOrder)
    {
        ...
        if (amountSpentOnOrder > 0) {
            amountReceivedFromOrder = swap(
                amountSpentOnOrder,
                data.sourceAsset == Utils.eth_address() ? address(weth) : data.sourceAsset,
                data.destinationAsset == Utils.eth_address() ? address(weth) : data.destinationAsset
            );
        }
