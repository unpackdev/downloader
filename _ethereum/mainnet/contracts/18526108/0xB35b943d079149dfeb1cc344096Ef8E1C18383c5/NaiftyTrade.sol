...
contract NaiftyTrade is Initializable, OwnableUpgradeable {
    ...
    function calc(
        ...
        returns (
            ...
            uint256        )
    {
        uint256 fee = pERCent(amount, serviceValue);
        uint256 roy = pERCent(amount, royal);
        uint256 netamount = 0;
        if (sellervalue != 0) {
            uint256 fee1 = pERCent(amount, sellervalue);
            fee = fee.add(fee1);
            netamount = amount.sub(fee1.add(roy));
        } else {
            netamount = amount.sub(roy);
        }
        return (rewardPer != 0 ? fee.sub(pERCent(fee, rewardPer).add(pERCent(fee, buyerRewardPer))) : fee, roy, netamount, pERCent(fee, rewardPer), pERCent(fee, buyerRewardPer));
    }
    ...
    function saleToken(
        ...
        if (ids[3] == 721) {
            IERC721Upgradeable(_conAddr[0]).safeTransferFrom(
                from,
                msg.sender,
                ids[0]
            );
            ...
            if (IERC1155Upgradeable(_conAddr[0]).balanceOf(from, ids[0]).sub(ids[2]) == 0) {
                if (order_place[from][ids[0]].price > 0) {
                    delete order_place[from][ids[0]];
                }
            }
            IERC1155Upgradeable(_conAddr[0]).safeTransferFrom(
                from,
                msg.sender,
                ids[0],
                ids[2],
    ...
    function acceptBId(
        ...
        if (ids[3] == 721) {
            ...
            if (order_place[msg.sender][ids[0]].price > 0) {
             ...
             if (
                IERC1155Upgradeable(_conAddr[0]).balanceOf(msg.sender, ids[0]).sub(ids[2]) == 0
            ) {
                if (order_place[msg.sender][ids[0]].price > 0) {
                    delete order_place[msg.sender][ids[0]];
                }
