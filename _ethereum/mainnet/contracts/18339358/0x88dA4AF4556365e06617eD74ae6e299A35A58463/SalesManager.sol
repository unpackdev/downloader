...
contract SalesManager is ISalesManager, PausableUpgradeable, UUPSUpgradeable {
  ...
  function bulkPurchaseAndRetire(
    ...
    uint256[] memory tokenIds = new uint256[](_authorisationParams.length);
    uint256[] memory amounts = new uint256[](_authorisationParams.length);

    // verify data and rip out to our data structures
    for (uint256 i; i < _authorisationParams.length; ) {
      _verifyPurchaseSignature(_authorisationParams[i]);

      // if transfer amount is 0 - skip
      if (_authorisationParams[i].sellAmount != 0) {
        IMPTAddress.safeTransferFrom(
          msg.sender,
          IMPTTreasuryAddress,
          _authorisationParams[i].sellAmount
        );
      }

      tokenIds[i] = _authorisationParams[i].tokenId;
      amounts[i] = _authorisationParams[i].amount;
    ...
    for (uint256 i; i < _authorisationParams.length; ) {
      // emit events
      emit PurchaseCompleted(
        _authorisationParams[i].requestId,
        _authorisationParams[i].sellAmount
      );

      // burn the tokens from this address
      CarbonCreditNFTContract.retire(
        _authorisationParams[i].tokenId,
        _authorisationParams[i].amount
      );
