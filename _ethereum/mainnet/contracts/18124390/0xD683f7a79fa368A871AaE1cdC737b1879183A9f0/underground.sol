...
contract underground is AccessUpgradeable, ERC721EnumerableUpgradeable, DefaultOperatorFiltererUpgradeable {
    ...
    function setExpirationMultiple(uint256[] calldata ids, uint256[] calldata expirations) external onlyAdmin {
        require(ids.length == expirations.length, "Array length mismatch");

        uint256 len = ids.length;
        for (uint256 i = 0; i < len; i++) {
            subscription[ids[i]].expiration = expirations[i];
        }
    }

    function addToExpiration(uint256 id, uint256 secondsToAdd) external onlyAdmin {
        subscription[id].expiration += secondsToAdd;
    }

    function addToExpirationMultiple(uint256[] calldata ids, uint256[] calldata secondsToAdd) external onlyAdmin {
        require(ids.length == secondsToAdd.length, "Array length mismatch");

        uint256 len = ids.length;
        for (uint256 i = 0; i < len; i++) {
            subscription[ids[i]].expiration += secondsToAdd[i];
        }
    }
    ...
    function _beforeTokenTransfer(
        ...
        if (msg.sender != owner() && !isAdmin[msg.sender] && from != address(0)) {
