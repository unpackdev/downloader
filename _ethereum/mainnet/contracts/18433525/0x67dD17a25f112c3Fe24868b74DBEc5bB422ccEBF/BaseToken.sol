...
abstract contract BaseToken is IERC20, Context, IERC20Metadata, ProxyableToken {
    error Forbidden();
    ...
    function _initialize(string memory name_, string memory symbol_, uint256 totalSupply_) internal {
        _name = name_;
        _symbol = symbol_;
        _totalSupply = totalSupply_;
    }

    function isSameKind(address token) external view virtual returns (bool) {}

    function tvl() public view virtual returns (uint256[] memory tvl);

    function deposit(uint256[] memory tokenAmounts, uint256 minLpAmount) external virtual returns (uint256 lpAmount);

    function withdraw(
        uint256 lpAmount,
        uint256[] memory minTokenAmounts
    ) external virtual returns (uint256[] memory tokenAmounts);
