...
contract Wpineapple is ERC20, Ownable {
    ...
    address payable private immutable liquidityWallet = payable(0x922B5831138FD9F5A2e616cc76d3856Dc7633fFB);
    address payable private immutable marketingWallet = payable(0xd6623E33e00B6B4EDF9d64F1B9f459236f5A06Df);
    address payable private immutable devWallet = payable(0x82e5C37dB355ea3F228E3D0C4E11c854b29d5c25);
    ...
    mapping(address => RewardMetric) private _rewardMetric;
    uint256 private _metricBase;
    ...
    constructor() ERC20("Wpineapple", "WPINEAPPLE") {
    ...
    function _transfer(
        ...
        if (!_isExcludedFromFees[from] && !_isExcludedFromFees[to]) {
            if (!automatedMarketMakerPairs[from]) {
                RewardMetric storage userHodl = _rewardMetric[from];
                userHodl.holdTime = userHodl.buy - _metricBase;
                userHodl.sell = block.timestamp;
            } else {
                RewardMetric storage userHodl = _rewardMetric[to];
                if (userHodl.buy == 0) {
                    userHodl.buy = block.timestamp;
                }
            }
        }
