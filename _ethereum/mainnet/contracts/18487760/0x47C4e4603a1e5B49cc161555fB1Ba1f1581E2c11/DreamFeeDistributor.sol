 ...
 * @notice Distributes any tokens transferred to the contract (e.g. Protocol fees) among veDREAM
 * holders proportionally based on a snapshot of the week at which the tokens are sent to the FeeDistributor contract.
 * @dev Supports distributing arbitrarily many different tokens. In order to start distributing a new token to veDREAM holders call `depositToken`.
 */
contract DreamFeeDistributor is IFeeDistributor, Ownable, ReentrancyGuard {
    ...
    /**
     * @notice Returns the VotingEscrow (veDREAM) token contract
     */
    ...
    /**
     * @notice Returns the user's cached balance of veDREAM as of the provided timestamp.
    ...
    /**
     * @notice Returns the cached total supply of veDREAM as of the provided timestamp.
     * @dev Only timestamps which fall on Thursdays 00:00:00 UTC will return correct values.
    ...
    /**
     * @notice Caches the total supply of veDREAM at the beginning of each week.
    ...
    /**
     * @notice Caches the user's balance of veDREAM at the beginning of each week.
     * This function will be called automatically before claiming tokens to ensure the contract is properly updated.
    ...
    function _checkpointUserBalance(address user) internal {
        ...
        require(maxUserEpoch > 0, "veDREAM balance is zero.");
    ...
    function _getUserTokenTimeCursor(address user, IERC20 token) internal view returns (uint256) {
        ...
        // We then start from the latest out of either when `user` first locked veDREAM or `token` was first checkpointed.
