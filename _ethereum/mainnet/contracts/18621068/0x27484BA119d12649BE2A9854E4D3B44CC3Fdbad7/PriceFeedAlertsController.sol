...
import "./IMigrationDataReceiver.sol";
  ...
  TypeAndVersionInterface
{
  ...
  /// @notice This error is thrown when the given operator is invalid.
  ...
  /// @param feed The address of the feed an alert was raised for
  ...
  event AlertRaised(
    address indexed feed, address indexed alerter, uint256 indexed roundId, uint96 rewardAmount
  );
  ...
  struct ConstructorFeedConfigParams {
    ...
    /// max value of uint96 is greater than total supply of LINK
    uint96 slashableAmount;
  ...
  struct SetFeedConfigParams {
    ...
    /// max value of uint96 is greater than total supply of LINK
    uint96 alerterRewardAmount;
    /// @notice The operator addresses that will get slashed if an alert is raised
    /// for the feed
    address[] slashableOperators;
  }
  ...
  /// @notice This struct defines a feed that alerters can raise alerts for.
  struct PriceFeed {
    /// @notice The config for the feed
    FeedConfig config;
    /// @notice The slashable operators on a feed
    address[] slashableOperators;
    /// @notice The last alerted round ID of the feed
    uint256 lastAlertedRoundId;
  }
  ...
  /// @notice Mapping of the price feeds that alerters can raise alerts for to the feed's configs,
  /// slashable operators, and the round ID of the last feed round an alert was raised.
  mapping(address feed => PriceFeed) private s_feeds;
  /// @notice The set of price feed addresses
  EnumerableSet.AddressSet private s_priceFeedAddresses;
    ...
    PausableWithAccessControl(params.adminRoleTransferDelay, msg.sender)
  {
    ...
    for (uint256 i; i < params.feedConfigs.length; ++i) {
      ...
      setFeedConfigParams[0] = SetFeedConfigParams({
        ...
        slashableOperators: config.slashableOperators
      });
    ...
    onlyRole(DEFAULT_ADMIN_ROLE)
  {
    ...
    if (newCommunityStakingPool == oldCommunityStakingPool) return;
    ...
    onlyRole(DEFAULT_ADMIN_ROLE)
  {
    ...
    if (newOperatorStakingPool == oldOperatorStakingPool) return;
  ...
  function getFeedConfig(address feed) external view returns (FeedConfig memory) {
    _validateFeed(feed);
    return s_feeds[feed].config;
  }

  /// @notice Returns the addresses of the alertable feeds
  /// @return The list of alertable feed addresses
  function getFeeds() external view returns (address[] memory) {
    return s_priceFeedAddresses.values();
  }
  ...
  function raiseAlert(address feed) external whenNotPaused {
    ...
    emit AlertRaised(
      feed, msg.sender, returnValues.roundId, returnValues.feedConfig.alerterRewardAmount
    );
  }
  ...
  function canAlert(address alerter, address feed) external view returns (bool) {
    ...
    return returnValues.canAlert && !paused() && s_operatorStakingPool.isActive();
  }
  ...
  function getSlashableOperators(address feed) external view returns (address[] memory) {
    _validateFeed(feed);
    return s_feeds[feed].slashableOperators;
  }
    ...
    validateMigrationTargetSet
  {
    ...
    for (uint256 i; i < feeds.length; ++i) {
      ...
      lastAlertedRoundIds[i] =
        LastAlertedRoundId({feed: feed, roundId: s_feeds[feed].lastAlertedRoundId});
  ...
  function _validateMigrationTarget(address newMigrationTarget) internal override {
  ...
  function _setFeedConfigs(SetFeedConfigParams[] memory configs) private {
    ...
    for (uint256 i; i < configs.length; ++i) {
      ...
      FeedConfig storage config = s_feeds[configParams.feed].config;
      ...
      _setSlashableOperators(configParams.feed, configParams.slashableOperators);

      s_priceFeedAddresses.add(configParams.feed);
      emit FeedConfigSet(
        configParams.feed,
  ...
  function _canAlert(
    ...
    if (!s_priceFeedAddresses.contains(feed)) return returnValues;

    FeedConfig memory feedConfig = s_feeds[feed].config;
    (uint256 roundId,,, uint256 updatedAt,) = AggregatorV3Interface(feed).latestRoundData();
    returnValues.roundId = roundId;
    returnValues.feedConfig = feedConfig;

    if (
      !_hasSlasherRole() || feedConfig.priorityPeriodThreshold == 0
        || s_feeds[feed].lastAlertedRoundId >= roundId
    ) {
      return returnValues;
    }

    // must be staking to alert
    if (s_operatorStakingPool.getStakerPrincipal(alerter) == 0) {
      if (s_communityStakingPool.getStakerPrincipal(alerter) == 0) return returnValues;
    }
  ...
  /// @return True if this contract has the slasher role, false otherwise
  ...
  function _setSlashableOperators(address feed, address[] memory operators) private {
    if (operators.length == 0) revert InvalidOperatorList();
    ...
    for (uint256 i; i < operators.length; ++i) {
      ...
      if (!s_operatorStakingPool.isOperator(operator)) revert InvalidOperator(operator);
    }

    s_feeds[feed].slashableOperators = operators;
    emit SlashableOperatorsSet(feed, operators);
  }

  /// @notice Returns whether or not a feed is invalid
  /// @param feed The address of the feed to validate
  function _validateFeed(address feed) private view {
    if (!s_priceFeedAddresses.contains(feed)) revert FeedDoesNotExist();
  }

  /// @notice Helper function for removing a feed config
  /// @param feed The address of the feed to remove
  function _removeFeedConfig(address feed) private {
    _validateFeed(feed);
    delete s_feeds[feed];
    s_priceFeedAddresses.remove(feed);
