 ...
 */
contract Timelock is AccessControlEnumerable, IERC721Receiver, IERC1155Receiver {
  /**
  ...
  function getMinDelay(Call[] calldata calls) public view returns (uint256) {
    ...
    uint256 largestDelay = callsLength > 0 ? s_minDelay : 0;
  ...
  function _setDelay(address target, bytes4 selector, uint256 newDelay) internal {
    ...
    if (s_delays[target][selector] == newDelay) return;
