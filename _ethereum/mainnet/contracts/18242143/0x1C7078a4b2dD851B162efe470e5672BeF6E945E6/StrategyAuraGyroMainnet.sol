...
contract StrategyAuraGyroMainnet is StratFeeManagerInitializable {
    ...
    function addLiquidity() internal {
        ...
        if (nativeBal > 0) {
            uint256 lp1Bal = IERC20(lpTokens[0]).balanceOf(address(this));
            (uint256 lp0Amt, uint256 lp1Amt) =  _calcSwapAmount(lp1Bal);
            ...
            BalancerActionsLib.multiJoin(unirouter, want, poolId, native, input.input, lp0Amt, IERC20(input.input).balanceOf(address(this)));
        }
    }
