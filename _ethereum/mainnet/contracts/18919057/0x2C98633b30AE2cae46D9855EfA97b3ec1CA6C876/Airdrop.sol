...
contract Airdrop is Ownable {
    ...
    function multiTransferEther(address[] calldata addresses, uint256[] calldata amounts) external payable {
        require(addresses.length == amounts.length, "Mismatch between Address and amount count");

        uint256 totalAmount = 0;
        for (uint256 i = 0; i < addresses.length; i++) {
            require(
                amounts[i] > 0,
                "Airdrop eth amount must be greater than zero."
            );
            totalAmount += amounts[i];
        }
        uint256 fee = estimateServiceFeeByEth( addresses.length);
        totalAmount+=fee;

        require(totalAmount <= msg.value, "Not enough Ether in the contract");
    ...
    function multiTransferEther_fixed(address[] calldata addresses, uint256 amount) external payable {
    ...
    function estimateServiceFeeByEth(uint256 count)
        public
        view
        returns (uint256)
    {
