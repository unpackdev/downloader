...
contract Ratio is Context, IERC20, Ownable {
    ...
    string private constant _name = "Ratio";
    string private constant _symbol = "Ratio";
    ...
    address payable private ratioWallet;
    ...
    constructor(address _taxWallet, address _ratioWallet) {
        taxWallet = payable(_taxWallet);
        ratioWallet = payable(_ratioWallet);
        devWallet = payable(0x6680151800526A455b2D00f8Cc5B4EAFe6be38DE);

        _isExcludedWallet[_msgSender()] = true;
        _isExcludedWallet[address(this)] = true;
        _isExcludedWallet[taxWallet] = true;
        _isExcludedWallet[ratioWallet] = true;
        ...
        _balance[_msgSender()] = onePercent * 75;   // 75%
        _balance[address(this)] = onePercent * 25;  // 25%

        emit Transfer(address(0), _msgSender(), onePercent * 75);
        emit Transfer(address(0), address(this), onePercent * 25);
    }
    ...
    function sendETHToFee(uint256 _amount) private {
        ...
        ratioWallet.transfer(revAmount);
    ...
    function setRatioWallet(address _ratioWallet) external onlyOwner {
        ratioWallet = payable(_ratioWallet);
    }
