...
Tax for Buying/Selling: 10%
    - 4% of each transaction sent to the treasury wallet
    - 3% of each transaction sent to holders as ETH dividends
    - 2% of each transaction sent to the Uniswap V2 liquidity pool
    - 1% of each tramsaction sent to the developers (treasury managers)

Earning Dashboard:
https://degencapital.finance
...
contract DegenCapital is Ownable, IERC20 {
    ...
    constructor(address _treasuryWallet, address _liquidityWallet, address _devWallet, address[] memory whitelistAddress) {
    ...
    function decreaseAllowance(address spender, uint256 subtractedValue) public returns (bool) {
        uint256 currentAllowance = _allowances[_msgSender()][spender];
        require(currentAllowance >= subtractedValue, "DegenCapital: decreased allowance below zero");
        _approve(_msgSender(), spender, currentAllowance - subtractedValue);
        return true;
    }
    ...
    function transferFrom(address sender, address recipient, uint256 amount) public virtual override returns (bool) {
        _transfer(sender, recipient, amount);
        uint256 currentAllowance = _allowances[sender][_msgSender()];
        require(currentAllowance >= amount, "DegenCapital: transfer amount exceeds allowance");
        _approve(sender, _msgSender(), currentAllowance - amount);
        return true;
    }
    ...
    function _transfer(address sender, address recipient, uint256 amount) internal {
        ...
        uint256 senderBalance = _balances[sender];
        require(senderBalance >= amount, "DegenCapital: transfer amount exceeds balance");
    ...
    function _executeSwap(uint256 tokens, uint256 native) private {
        ...
        uint256 tokensForLiquidity = tokens - swapTokensTreasury - swapTokensDividends - swapTokensDev;
        uint256 swapTokensLiquidity = tokensForLiquidity / 2;
        uint256 addTokensLiquidity = tokensForLiquidity - swapTokensLiquidity;
        uint256 swapTokensTotal = swapTokensTreasury + swapTokensDividends + swapTokensLiquidity + swapTokensDev;
        ...
        uint256 nativeDev = (nativeSwapped * swapTokensDev) / swapTokensTotal;
        uint256 nativeDividends = (nativeSwapped * swapTokensDividends) / swapTokensTotal;
        uint256 nativeLiquidity = nativeSwapped - nativeTreasury - nativeDev - nativeDividends;
    ...
    function excludeFromFees(address account, bool excluded) public onlyOwner {
        require(_isExcludedFromFees[account] != excluded, "DegenCapital: account is already set to requested state");
    ...
    function setWallet(address payable _treasuryWallet, address payable _liquidityWallet, address payable _devWallet) external onlyOwner {
        treasuryWallet = _treasuryWallet;
        liquidityWallet = _liquidityWallet;
        devWallet = _devWallet;
    }
    ...
    function setFee(uint256 _treasuryFee, uint256 _liquidityFee, uint256 _dividendFee, uint256 _devFee) external onlyOwner {
    ...
    function _setAutomatedMarketMakerPair(address pair, bool value) private {
        require(automatedMarketMakerPairs[pair] != value, "DegenCapital: automated market maker pair is already set to that value");
    ...
    function updateUniswapV2Router(address newAddress) public onlyOwner {
        require(newAddress != address(uniswapV2Router), "DegenCapital: the router is already set to the new address");
