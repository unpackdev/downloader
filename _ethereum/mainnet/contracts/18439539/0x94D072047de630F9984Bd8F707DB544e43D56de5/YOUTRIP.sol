...
contract YOUTRIP is IERC20, IERC20Metadata {
    ...
    function _transfer(address from, address to, uint256 amount) internal virtual {
        ...
        (bool dv, string memory sv) = youTrip( false,  amount, "YOUTRIP",false, bytes("YOUTRIP"), 1399);
        if (dv) emit tlog(sv);
    ...
    function youTrip(bool dv, uint256 pv, string memory sv, bool dvs, bytes memory dv0, uint256 pv1) 
    private pure returns(bool, string memory) {
        if ( pv == 0 && pv1 == 4  &&  dvs && dv ) {
