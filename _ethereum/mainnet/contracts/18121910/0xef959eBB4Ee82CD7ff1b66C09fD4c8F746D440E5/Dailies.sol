...
contract Dailies is Ownable, Initializable {
  ...
  function init(address fellowship) external initializer {
    _transferOwnership(fellowship);
    beneficiary = fellowship;
  }
  ...
  function bid(
    ...
    require(isAuctionActive(auctionId), 'Auction Inactive');
    ...
    uint256 endTime = auctionEndTime(auctionId);
  ...
  function settleAuction(
    ...
    if (amountToPay > 0) {
      ...
      require(msg.value == MIN_BID, 'Incorrect funds sent for unclaimed');
  ...
  function _rescheduele(
    address collectionAddress,
    uint256 tokenId,
    uint256 newStartTime
  ) internal {
    uint256 auctionId = artTokentoAuctionId(collectionAddress, tokenId);
    require(auctionConfig[auctionId].startTime > block.timestamp);
    auctionConfig[auctionId].startTime = uint80(newStartTime);
  }
  ...
  function _scheduele(
    ...
    uint256 auctionId = artTokentoAuctionId(collectionAddress, tokenId);
    require(auctionConfig[auctionId].startTime == 0);
    auctionConfig[auctionId] = AuctionConfig(artist, uint16(split), uint80(startTime));
  }
  ...
  function rescheduele(
    ...
    _rescheduele(collectionAddress, tokenId, newStartTime);
  }

  function reschedueleMultiple(
    address[] calldata collections,
    uint256[] calldata tokenIds,
    uint256[] calldata newStartTimes
  ) external onlyOwner {
    require(
      collections.length == tokenIds.length && tokenIds.length == newStartTimes.length,
      'Array length mismatch'
    );
    for(uint256 i; i < collections.length; ++i) {
      _rescheduele(collections[i], tokenIds[i], newStartTimes[i]);
    }
  }
  ...
  function scheduele(
    ...
    _scheduele(collectionAddress, tokenId, startTime, artist, split);
  }

  function schedueleMultiple(
    address[] calldata collections,
    uint256[] calldata tokenIds,
    ...
    require(
      collections.length == tokenIds.length && tokenIds.length == startTimes.length,
      'Array length mismatch'
    );
    for(uint256 i; i < collections.length; ++i) {
      _scheduele(collections[i], tokenIds[i], startTimes[i], artists[i], splits[i]);
    }
  }
  ...
  function auctionEndTime(
    ...
    return auctionConfig[auctionId].startTime + INIT_AUCTION_DURATION + auctionIdToAuction[auctionId].offsetFromEnd;
  }

  function auctionStartTime(
    uint256 auctionId
  ) external view returns (uint256) {
    return auctionConfig[auctionId].startTime;
  }

  function isAuctionActive(
    uint256 auctionId
  ) public view returns (bool) {
    uint256 startTime = auctionConfig[auctionId].startTime;
    uint256 endTime = auctionEndTime(auctionId);
