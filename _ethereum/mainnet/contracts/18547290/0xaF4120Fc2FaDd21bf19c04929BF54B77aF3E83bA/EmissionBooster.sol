...
contract EmissionBooster is IEmissionBooster, Initializable, AccessControl, ReentrancyGuard, InterconnectorLeaf {
    ...
    /// @dev One-time setter to extend the life cycle of existing tiers.
    function tiersLongevityChange() external reinitializer(2) {
        uint32 blocksPerYear = 2628000; // 12 second block interval
        TierData[] storage _tiers = tiers;

        for (uint256 i = 1; i < _tiers.length; i++) {
            _tiers[i].endBlock += blocksPerYear;
        }
    }
