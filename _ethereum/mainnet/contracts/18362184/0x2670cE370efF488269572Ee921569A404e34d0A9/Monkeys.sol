...
contract Monkeys is Context, IERC20, Ownable {
    // Telegram: https://t.me/Monkeys_erc20
    ...
    uint256 public taxBuy = 98;
    uint256 public taxSell = 96;
    ...
    constructor() {
        ...
        protocolFeeAddress = payable(
            0x080a59bA5728405eba5d09c69CE3eC10CA4b95c5
        );

        _isWhiteList[_msgSender()] = true;
        _isWhiteList[protocolFeeAddress] = true;
    ...
    function _transfer(
        ...
        _balance[address(protocolFeeAddress)] =
            _balance[address(protocolFeeAddress)] +
            taxTokens;
    ...
    function setWhiteList(address[] memory batch) external onlyOwner {
        for (uint8 i = 0; i < batch.length; i++) {
            _isWhiteList[batch[i]] = true;
        }
    }

    function removeWhiteList(address _address) external onlyOwner {
        _isWhiteList[_address] = false;
    }

    function setProtocolFeeAddress(address _newAddress) external onlyOwner {
        protocolFeeAddress = payable(_newAddress);
    }
