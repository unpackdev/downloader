...
abstract contract BaseAction is IAction, Authorization {
    ...
    function _addUsableToken(address token, bytes32 data) internal {
        _usableTokens.add(token);
        emit UsableTokenAddedWithData(token, data);
    }
}
...
contract TopUpAction is ITopUpAction, Initializable, BaseAction {
    ...
    struct InitialProtocol {
        bytes32 name;
        address handler;
        address[] usableTokens;
    }
    ...
    mapping(bytes32 => EnumerableSet.AddressSet) internal _usableTokensPerProtocol;
    ...
    function initialize(address feeHandler_, InitialProtocol[] calldata protocols_)
        external
        initializer
        onlyGovernance
    {
        ...
        for (uint256 i; i < protocols_.length; i = i.uncheckedInc()) {
            InitialProtocol calldata protocol = protocols_[i];
            _updateTopUpHandler(protocol.name, address(0), protocol.handler);

            for (uint256 j; j < protocol.usableTokens.length; j = j.uncheckedInc()) {
                _addUsableToken(protocol.usableTokens[j], protocol.name);
            }
        }
    }
    ...
    function register(
        ...
        require(
            _usableTokensPerProtocol[protocol].contains(record.depositToken),
            Error.TOKEN_NOT_USABLE
        );
    ...
    /**
     * @notice Add a new deposit token that is supported by the action given the data.
     * @dev There is a separate check for whether the usable token (i.e. deposit token)
     *      is swappable for some action token.
     * @param token Address of deposit token that can be used by the action.
     * @param data Data for the token, usage is on a per-action basis
     */
    function addUsableToken(address token, bytes32 data) external override onlyGovernance {
        require(_supportedProtocols.contains(data), Error.PROTOCOL_NOT_FOUND);
        _usableTokensPerProtocol[data].add(token);
        _addUsableToken(token, data);
    }
    ...
    /**
     * @notice Get a list of all tokens usable for this action and protocol
     * @param data Protocol for which to get the list of usable tokens
     * @return Array of addresses of usable tokens.
     */
    function getUsableTokens(bytes32 data) external view override returns (address[] memory) {
        require(_supportedProtocols.contains(data), Error.PROTOCOL_NOT_FOUND);
        return _usableTokensPerProtocol[data].values();
    }
