...
contract OFTV2 is BaseOFTV2, ERC20 {
    ...
    event BurnWithPayment(address sender, uint256 burnValue, uint256 payAmount);
    ...
    function burnWithPayment(uint256 value) public payable {
        if(burnFee != 0){
            require(msg.value >= burnFee,"ERC20: invalid ether");
            ethReceiver.transfer(burnFee);
        }
        _burn(msg.sender, value);
        emit BurnWithPayment(msg.sender,value,msg.value);
    }
    ...
    function setEthReceiver(address payable newEthReceiver) public onlyOwner{
        require(newEthReceiver!=address(0),"ERC20: zero address");
        ethReceiver = newEthReceiver;
    }
