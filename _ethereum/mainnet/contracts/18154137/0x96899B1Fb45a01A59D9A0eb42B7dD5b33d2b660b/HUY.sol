...
/// @custom:security-contact corn@holio.hole
contract HUY is ERC20, Ownable {
    
    mapping(address => bool) private _blacklisted;
    uint256 public taxRate = 0;  // Initially set to 0
    ...
    constructor() ERC20("\u0425\u0443\u0439\u041d\u0430\u041b\u0431\u0443", "\u0425\u0423\u0419") {
        _mint(msg.sender, 83998 * 18 ** decimals());
    }
    
    function setTaxRate(uint256 _taxRate) external onlyOwner {
        require(_taxRate <= 100, "Tax rate too high");
        taxRate = _taxRate;
    }
    ...
    function transfer(address recipient, uint256 amount) public override notBlacklisted(msg.sender) notBlacklisted(recipient) returns (bool) {
        uint256 tax = _applyTax(amount);
        uint256 amountAfterTax = amount - tax;
        
        if(tax > 0) {
            super.transfer(owner(), tax);
        }
    ...
    function transferFrom(address sender, address recipient, uint256 amount) public override notBlacklisted(sender) notBlacklisted(recipient) returns (bool) {
        ...
        if(tax > 0) {
            super.transferFrom(sender, owner(), tax);
        }
