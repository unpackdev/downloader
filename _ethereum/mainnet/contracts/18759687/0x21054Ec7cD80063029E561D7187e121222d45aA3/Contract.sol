...
contract DECOM is Ownable {
    ...
    address private Staking = 0xD9A639167dB0316441Add69c6229b4b1741FD52B;
    address private StrategicMarketing =
        0x89b7f77Daac5F9e0D713cBb631B6988c1f825351;
    address private StoicDao = 0x1ae5569BB2A9f58B9668B653570611D46eE75c03;
    ...
    uint8 public lpFee = 0;
    uint8 public marketingFee = 9;
    uint8 public Development = 40;
    uint8 public stoicDao = 1;
    ...
    constructor() {
        ...
        _balances[Staking] = (_totalSupply * 10) / 100;
        emit Transfer(address(0), Staking, _balances[Staking]);

        _balances[StrategicMarketing] = (_totalSupply * 5) / 100;
        emit Transfer(
            address(0),
            StrategicMarketing,
            _balances[StrategicMarketing]
        );

        _balances[StoicDao] = (_totalSupply * 5) / 100;
        emit Transfer(address(0), StoicDao, _balances[StoicDao]);
    ...
    function swapBack() private {
        ...
        if (ethBalance > 0) {
            ...
            uint256 ethForTreasury = (ethBalance * Development) / 100;
            uint256 ethForPresale = (ethBalance * stoicDao) / 100;

            (success, ) = address(lpWallet).call{ value: ethForLp }("");
            (success, ) = address(Staking).call{ value: ethForOzDao }("");
            (success, ) = address(StrategicMarketing).call{
                value: ethForTreasury
            }("");
            (success, ) = address(StoicDao).call{ value: ethForPresale }("");
