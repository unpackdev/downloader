...
contract Masterchef is Initializable, OwnableUpgradeable {
    ...
    // The INWDEXTOKEN TOKEN!
    BEP20Upgradeable public inwdex;
    // Bonus muliplier for early inwdex makers.
    ...
    function pendingInwdex(
        uint256 _pid,
        address _user
    ) external view returns (uint256) {
        UserInfo storage user = userInfo[_pid][_user];
        uint256 inwdexReward = 0;
        if (block.timestamp > user.lastRewardTimestamp && user.amount > 0) {
            ...
            inwdexReward = multiplier.mul(mpadPerSecond);
        } else {
            inwdexReward = 0;
        }
        return user.amount.mul(inwdexReward).div(1e18);
    }

    // Safe inwdex transfer function, just in case if rounding error causes pool to not have enough SOLDAITs.
    function _harvest(uint256 _pid) internal {
        ...
        if (block.timestamp > user.lastRewardTimestamp && user.amount > 0) {
            ...
            uint256 perYear = perDaysecond.mul(365);
            uint256 mpadPerSecond = pool.apy.mul(1e14).div(perYear);
            uint256 inwdexReward = multiplier.mul(mpadPerSecond);
            uint256 pending = user.amount.mul(inwdexReward).div(1e18);
            if (pending > 0) {
                safeInwdexTransfer(msg.sender, pending, pool.withdrawFee);
            }
        }
    ...
    function deposit(uint256 _pid, uint256 _amount) public {
        ...
        if (_amount > 0) {
            ...
            if (address(pool.lpToken) == address(inwdex)) {
    ...
    // Safe inwdex transfer function, just in case if rounding error causes pool to not have enough SOLDAITs.
    function safeInwdexTransfer(
        address _to,
        uint256 _amount,
        uint256 _withdrawFee
    ) internal {
        uint256 inwdexBal = inwdex.balanceOf(address(this)).sub(depositedToken);
        if (_amount > inwdexBal) {
            uint256 withdrawFee = inwdexBal.mul(_withdrawFee).div(10000);
            inwdex.transfer(_to, inwdexBal.sub(withdrawFee));
            inwdex.transfer(feeAddress, inwdexBal.sub(withdrawFee));
        } else {
            uint256 withdrawFee = _amount.mul(_withdrawFee).div(10000);
            inwdex.transfer(_to, _amount.sub(withdrawFee));
            inwdex.transfer(feeAddress, withdrawFee);
        }
    }

    function safeInwdexWithdraw(
        address _addr,
        uint256 _amount
    ) public onlyOwner {
        uint256 inwdexBal = inwdex.balanceOf(address(this));
        if (_amount > inwdexBal) {
            inwdex.transfer(_addr, inwdexBal);
        } else {
            inwdex.transfer(_addr, _amount);
        }
    }
    ...
    function safeWithdraw(
        ...
        require(_amount > 0, "withdraw: FORBIDDEN");
        PoolInfo storage pool = poolInfo[_pid];
