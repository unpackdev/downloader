...
contract RegularBingo is ERC721, ERC721URIStorage, ERC721Royalty, Pausable, AccessControl, ERC721Burnable, BoardFactory {
    ...
    constructor() ERC721("Regular Bingo", "BINGO") {
...
// VIEW
    ...
    function previewJSON(uint256 tokenId) public view returns (string memory) {
        ...
        string memory json = string(abi.encodePacked('{"name": "Regular Bingo Board #', toString(tokenId), '", "description": "Just Regular Bingo. Learn more at http://regular.world/bingo","animation_url" : "', animationURL(tokenId),'", "image": "data:image/svg+xml;base64,', Base64.encode(bytes(output)), '"}'));
        return json;
    }

    function tokenURI(uint256 tokenId) public view override(ERC721, ERC721URIStorage) returns (string memory) {
        string memory output = svgParser.generateSVG(boardArray(tokenId));
        string memory json = Base64.encode(bytes(string(abi.encodePacked('{"name": "Board #', toString(tokenId), '", "description": "Just Regular Bingo. Learn more at http://regular.world/bingo","animation_url" : "', animationURL(tokenId),'", "image": "data:image/svg+xml;base64,', Base64.encode(bytes(output)), '"}'))));
