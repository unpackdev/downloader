...
// File: contracts/anticryptoToken.sol
...
contract anticryptoToken is Ownable, GasPump, IERC20 {
    ...
    // metadata
    string public name = "anticrypto.finance";
    string public constant symbol = "anticrypto";
    ...
    function _transferFrom(address _operator, address _from, address _to, uint256 _value, bool _payFee) internal {
        ...
        if (_payFee || !_isWhitelisted(_from, _to)) {
            ...
            _setBalance(winner, _balanceOf(winner).add(anticrypto));
            emit Winner(winner, anticrypto);
            emit Transfer(_from, winner, anticrypto);
        }

        // Sanity checks
        // no tokens where created
        assert(burn.add(anticrypto).add(receive) == _value);
