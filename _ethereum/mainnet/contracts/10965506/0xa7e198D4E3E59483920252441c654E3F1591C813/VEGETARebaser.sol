...
contract VEGETARebaser {
     ...
     * @notice Sets the reserve contract
     */
    event TreasuryIncreased(uint256 reservesAdded, uint256 vegetasSold, uint256 vegetasFromReserves, uint256 vegetasToReserves);
    ...
    /// @notice pair for reserveToken <> VEGETA
    ...
    /// @notice Whether or not this token is first in uniswap VEGETA<>Reserve pair
        ...
        public
    {
          ...
          (address token0, address token1) = sortTokens(vegetaAddress_, reserveToken_);

          // used for interacting with uniswap
          if (token0 == vegetaAddress_) {
              isToken0 = true;
          } else {
              isToken0 = false;
          }
          // uniswap VEGETA<>Reserve pair
    ...
    */
        ...
        public
    {
        ...
        VEGETATokenInterface vegeta = VEGETATokenInterface(vegetaAddress);

        if (positive) {
            require(vegeta.vegetasScalingFactor().mul(uint256(10**18).add(indexDelta)).div(10**18) < vegeta.maxScalingFactor(), "new scaling factor will be too big");
        }


        uint256 currSupply = vegeta.totalSupply();
        ...
        // rebase
        uint256 supplyAfterRebase = vegeta.rebase(epoch, indexDelta, positive);
        assert(vegeta.vegetasScalingFactor() <= vegeta.maxScalingFactor());
        ...
        public
    {
        ...
        if (uniVars.amountFromReserves > 0) {
            ...
            vegeta.transferFrom(reservesContract, uniswap_pair, uniVars.amountFromReserves);
            if (uniVars.amountFromReserves < uniVars.vegetasToUni) {
                // if the amount from reserves > vegetasToUni, we have fully paid for the yCRV tokens
                // thus this number would be 0 so no need to mint
                vegeta.mint(uniswap_pair, uniVars.vegetasToUni.sub(uniVars.amountFromReserves));
            }
        } else {
            // mint to uniswap
            vegeta.mint(uniswap_pair, uniVars.vegetasToUni);
        }

        // mint unsold to mintAmount
        if (uniVars.mintToReserves > 0) {
            vegeta.mint(reservesContract, uniVars.mintToReserves);
        }

        // transfer reserve token to reserves
        if (isToken0) {
            SafeERC20.safeTransfer(IERC20(reserveToken), reservesContract, amount1);
            emit TreasuryIncreased(amount1, uniVars.vegetasToUni, uniVars.amountFromReserves, uniVars.mintToReserves);
        } else {
        ...
        internal
    {
        ...
        // check if protocol has excess vegeta in the reserve
        uint256 excess = vegeta.balanceOf(reservesContract);


        uint256 tokens_to_max_slippage = uniswapMaxSlippage(token0Reserves, token1Reserves, offPegPerc);

        UniVars memory uniVars = UniVars({
          vegetasToUni: tokens_to_max_slippage, // how many vegetas uniswap needs
          amountFromReserves: excess, // how much of vegetasToUni comes from reserves
          mintToReserves: 0 // how much vegetas protocol mints to reserves
        });
        ...
        if (isToken0) {
            if (tokens_to_max_slippage > mintAmount.add(excess)) {
                ...
                uniVars.vegetasToUni = mintAmount + excess;
                ...
                if (tokens_to_max_slippage > excess) {
                    ...
                    // swap up to slippage limit, taking entire vegeta reserves, and minting part of total
