...
library RaffleLib {
    ...
    function ownerInitRaffles(
        ...
        Fees memory fees = Fees({
            royalty: FeeRate({receipt: feeConf.royalty.receipt, rateBips: feeConf.royalty.raffle}),
            protocol: FeeRate({receipt: feeConf.safeboxFee.receipt, rateBips: feeConf.safeboxFee.raffle})
        });
        (uint64[] memory raffleActivityIds, uint48 raffleEndTime, uint192 safeBoxExpiryTs) =
            _ownerInitRaffles(collection, userAccount.getByKey(param.collection), param, fees);
    ...
    function prepareSettleRaffles(CollectionState storage collection, uint256[] calldata nftIds)
        public
        returns (bytes memory compactedNftIds, uint256 nftLen)
    {
    ...
    function settleRaffles(
        ...
        for (uint256 i; i < nftIds.length;) {
            ...
            distributeFunds(userAccounts, raffle.fees, raffle.owner, raffle.token, raffle.collectedFund);
    ...
    function distributeFunds(
        ...
        (uint256 priceWithoutFee, uint256 protocolFee, uint256 royalty) = Helper.computeFees(price, fees);
