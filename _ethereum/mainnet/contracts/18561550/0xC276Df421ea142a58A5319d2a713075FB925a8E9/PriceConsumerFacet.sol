...
contract PriceConsumerFacet is Modifiers {
    ...
    function getTokenPriceFromDex(
        ...
        address pair = LibPriceConsumer.getPair(_tokenIn, _tokenOut);

        if (pair == address(0)) {
            pair = LibPriceConsumer.getPair(_tokenIn, wethAddress());

            address pairWethTokenOut = LibPriceConsumer.getPair(
                wethAddress(),
                _tokenOut
            );
            if (pair == address(0) && pairWethTokenOut == address(0)) return 0;
