...
contract sdCurveCDP is Initializable, ReentrancyGuardUpgradeable {
    ...
    //Fees distribution
    mapping(address => uint256) public onRewardRestakeFee; //ChangeOnR

    uint256 public onRewardRestakePercentage; //ChangeOnR
    ...
    //ChangeVT
    mapping(address => uint256) public sdtLocked;
    uint256 public totalUnlocked;
    address public lockToken;
    //ChangeFRAX
    address public sdFraxClaimContract;
    address public sdFRAX;
    address public frax;
    ...
    address public swapContract; //ChangeSW
    address public veSdtContract;
    address public multiMerkleStashContract;
    ...
    function initialize(
        ...
        address _swapContract, //ChangeSW
        address _veSdtContract,
        address _multiMerkleStashContract
        ...
        totalSupply = 0;
        harvesterFee = 100;
        onRewardRestakePercentage = 5000; //ChangeOnR
        masterClaimPercentage = 2500;
        adminPercentage = 2500;
        swapContract = _swapContract;
        veSdtContract = _veSdtContract;
        multiMerkleStashContract = _multiMerkleStashContract;
    }

            //ChangeFRAX
    function setsdFRAX(address _contract) public onlyAdmin { //ChangeFRAX
    sdFRAX = _contract;
}
   function setsdFraxClaimContract(address _contract) public onlyAdmin { //ChangeFRAX
    sdFraxClaimContract = _contract;
}
   function setfrax(address _contract) public onlyAdmin { //ChangeFRAX
    frax = _contract;
}
  function setVeSDtContract(address _contract) public onlyAdmin { 
    veSdtContract = _contract;
}
  function setmultiMerkleStashContract(address _contract) public onlyAdmin { //Change
    multiMerkleStashContract = _contract;
}

    function setlockToken(address _lockToken) public onlyAdmin {
        //ChangeVT
        lockToken = _lockToken;
    }

    function setSwapContract(address _swapContract) public onlyAdmin {
        //ChangeSW
        swapContract = _swapContract;
    }
    ...
    //Fee Distribution
    function setOnRewardRestakePercentage(uint256 _fee) external onlyAdmin { //ChangeOnR
        onRewardRestakePercentage = _fee; 
    }
    ...
    function masterClaim() external nonReentrant {
        ...
        for (uint256 i = 0; i < feePoolsCount; ++i) {
            ...
            uint256 newRewards = IERC20(feePool.feeToken).balanceOf(
                address(this)
            ) - (feePool.totalRsrv + totalUnlocked); //ChangeVT
            ...
            if (newRewards > 0) {
                ...
                uint256 adminReward = (newRewardsFees * adminPercentage) /
                    10000;
            uint256 onRewardRestakeReward = (newRewardsFees * onRewardRestakePercentage) / //ChangeOnR
                10000;

            onRewardRestakeFee[feePool.feeToken] += onRewardRestakeReward;

                if (masterClaimReward > 0) {
                    feePool.feeReserves +=
                        newRewardsFees -
                        (masterClaimReward + adminReward); //ChangeSW
                    feePool.totalRsrv +=
                        newRewardsFees -
                        (masterClaimReward + adminReward); //ChangeVT
                ...
                uint256 finalreward = newRewards - newRewardsFees; //ChangeSW
                IERC20(feePool.feeToken).transfer(swapContract, finalreward);
                //    feePool.totalRsrv   = IERC20(feePool.feeToken).balanceOf(address(this));
            }
        }
        if (newRewardsAvailable == true) {
            times = 3;
        }
    }

    function masterWithdraw(uint256 amount) private {
        ICRVGauge sdcrvDepositor = ICRVGauge(sdGaugeAddress);
        require(
            amount <= sdcrvDepositor.balanceOf(address(this)),
            "MasterWithdraw! Not enough balance"
        );
        sdcrvDepositor.withdraw(amount);
    }

    function onRewardANDRestake() external nonReentrant {
        ...
        for (uint256 i = 0; i < rewardPoolsCount; ++i) {
            ...
            uint256 newRewardsFinal = IERC20(pool.rewardToken).balanceOf(
                address(this)
            ) - pool.reserves;
...
//Restaking..
        ...
        require(contractsdCRVBalance >= 0, "ReStake: Not sdCRV Balance");
        uint256 userAmount = balanceOf[address(this)];
        sdCRV3 sdCRV3Contract = sdCRV3(sdCRV3Address);
        _updateAccPerShare(true, address(this));

        sdcrvToken.approve(sdGaugeAddress, contractsdCRVBalance);
        sdcrvDepositor.deposit(contractsdCRVBalance);

        sdCRV3Contract.mintRequest(address(this), contractsdCRVBalance);
        rewardPools[0].reserves -= contractsdCRVBalance; //at 0, will be sdcrv
        totalSupply += contractsdCRVBalance;
        unchecked {
            balanceOf[address(this)] = userAmount + contractsdCRVBalance;
        }

        //FeeTokens transfer here.......
        for (uint256 i = 0; i < feePoolsCount; ++i) {
            FeeData storage feePool = feePools[i];
            if (onRewardRestakeFee[feePool.feeToken] > 0) { //ChangeOnR
                uint256 amount = onRewardRestakeFee[feePool.feeToken] / times;
                feePool.totalRsrv -= amount;//ChangeVT
                onRewardRestakeFee[feePool.feeToken] -= amount;
                feePool.feeReserves -= amount;  
    ...
    function _claim(
        ...
        if (claimAmount > 0) {
            ...
            if (claimAmount > contractBalance) {
                ...
                withdrawRestaked(_amount); //ChangeSW
            }
    ...
    function getRewardRate(
        uint256 _index
    ) public view returns (address _token, uint256 _rate) {
        _token = rewardPools[_index].rewardToken;
        _rate = rewardPools[_index].rewardRate;
    }

    //For Strategic Rewards
    function masterClaimSingleStartegicRwrds(
        address token,
        uint256 index,
        address account,
        uint256 amount,
        ...
        IMultiMerkleStash claimContract = IMultiMerkleStash(
            multiMerkleStashContract
        );
        claimContract.claim(token, index, account, amount, merkleProof);
    }

    function masterClaimMultiStartegicRwrds(
        address account,
        ...
        claimContract.claimMulti(account, claims);
    }

    //For Voting
    function lockSDT(uint256 _amount, uint256 timing) external onlyAdmin {
        IveSDT veSDTContract = IveSDT(veSdtContract); 
        IERC20 lockingToken = IERC20(lockToken);
        require(
            lockingToken.balanceOf(msg.sender) >= _amount,
            "Not enough SDTs"
        );
        sdtLocked[msg.sender] += _amount;
        lockingToken.transferFrom(msg.sender, address(this), _amount);
        lockingToken.approve(address(veSDTContract), _amount);
        veSDTContract.create_lock(_amount, timing);
    }
    ...
    function increaseAmount(uint256 _amount) external {
        ...
        lockingToken.transferFrom(msg.sender, address(this), _amount);
        lockingToken.approve(address(veSDTContract), _amount);
        veSDTContract.increase_amount(_amount);
    }

    function increaseTime(uint256 _time) external {
        IveSDT veSDTContract = IveSDT(veSdtContract);
        veSDTContract.increase_unlock_time(_time);
    }

    function unLockSDT() external {
        IveSDT veSDTContract = IveSDT(veSdtContract);
        veSDTContract.withdraw();

        IERC20 lockingToken = IERC20(lockToken);
        totalUnlocked =
            lockingToken.balanceOf(address(this)) -
            feePools[tokenIndex[lockToken]].totalRsrv;
    } 

    function unlockSdtByUser(uint256 _amount) external {
        require(
            totalUnlocked >= _amount,
            "unlockSdtByUser: Can not unlock at the moment"
        );
        require(
            sdtLocked[msg.sender] >= _amount,
            "unlockSdtByUser: Not locked this much!"
        );

        totalUnlocked -= _amount;
        sdtLocked[msg.sender] -= _amount;

        IERC20(lockToken).transfer(msg.sender, _amount);
    }

    function masterClaimsdFrax3Crv()
        external
        nonReentrant
        returns (uint256 _amount)
    {
      
        IClaimRewards claimContract = IClaimRewards(sdFraxClaimContract);
        _amount = claimContract.claim();
        IsdFrax fraxContract = IsdFrax(sdFRAX);
        fraxContract.withdrawAll();
        uint256 balance =IERC20(frax).balanceOf(address(this)); 
        IERC20(frax).transfer(swapContract,balance);
