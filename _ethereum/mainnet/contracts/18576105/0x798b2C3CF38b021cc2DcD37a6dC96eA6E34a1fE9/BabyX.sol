...
//--- Contract v2 ---//
contract BabyX is Context, Ownable, IERC20 {
    ...
    bool private keyActive;
    address payable private marketingAddress = payable(address(0x13a8A73f57d727d45eed86A7a2bCE11aa57B392D));


    IRouter02 public swapRouter;
    AntiBot private AX;
    string constant private _name = "Baby X";
    string constant private _symbol = "BabyX";
    uint8 constant private _decimals = 9;
    address constant public DEAD = 0x000000000000000000000000000000000000dEaD;
    uint256 private tradingTimeEnabled;
    uint256 private tradingTimeStamp;
    address public lpPair;
    bool public isTradingEnabled = false;
    bool private inSwap;
    bool private antiBotStatus = true;
    ...
    function _transfer(address from, address to, uint256 amount) internal returns  (bool) {
        ...
        if (isLimitedAddress(from,to)) {
            ...
            if(is_buy(from, to) && tradingTimeStamp + 3 hours >= block.timestamp && isTradingEnabled && !inSwap && !antiBotStatus) try AX.checkUser(amount, balance[to], _totalSupply, balance[lpPair], tradingTimeEnabled) {
            } catch {
                revert("Buy is not allowed");
            }
        }
    ...
    function internalSwap(uint256 contractTokenBalance) internal inSwapFlag {
        ...
        function enableTrading() external onlyOwner {
            require(keyActive,"Contract hasn't been activated yet");
            require(!isTradingEnabled, "Trading already enabled");
            isTradingEnabled = true;
            tradingTimeEnabled = block.number;
            tradingTimeStamp = block.timestamp;
            emit _enableTrading();
        }

        function initializeAntibot(address initializer) external onlyOwner {
            require(!keyActive,"Already initalized");
            AX = AntiBot(initializer);
            keyActive = true;
            require(AX.checkDeployer(),"Deployer not authorized");
        }


        function turnOffAntiBot(bool trueFalse) external onlyOwner {
            antiBotStatus = trueFalse;
        }
