...
contract DougsRelish is ReentrancyGuard, ERC1155Supply {
  ...
  struct editionData {
    ...
    // track how many times claims have been processed through a project
    uint256 projectRedeemCount;
    // limit the number of claims per project
  ...
  /// @notice returns the number of Relish tokens redeemable by owners of the partner NFT
  ...
  /// @notice checks that: 1. the sender owns the required token
  /// @notice and 2. the token has not been used to redeem already

  function isOwnerOfValidPartnerAsset(uint256 _editionId, uint256 _tokenId)
    public
    view
    returns (bool ownership)
  {
    return (isOwnerOfPartnerNFT(_editionId, _tokenId) &&
      // ensure it has not been redeemed already
      (editionIdToData[_editionId].redeemedPartnerTokens[_tokenId] == false));
  }

  /// @notice checks that the sender 1. owns the Hot Doug NFT
  /// @notice and 2. the Hot Doug NFT has not been redeemed yet
    ...
    returns (bool redeemable)
  {
    return
      ...
      // if true token is not redeemable
      !editionIdToData[_editionId].redeemedTokens[_tokenIdToRedeem];
  }
    ...
    nonReentrant
  {
    ...
    uint256[] memory result = new uint256[](dougTokenCount);

    // check to see if each token is redeemable
    unchecked {
      for (uint256 i = 0; i < dougTokenCount; ++i) {
        uint256 _tokenIdToRedeem = dougTokenIds[i];

        if (isDougTokenRedeemable(_editionId, _tokenIdToRedeem)) {
          result[i] = _tokenIdToRedeem;
          redeemableCount++;
        }
      }
      ...
      // trim the array so that only eligible Dougs remain
      require(
      ...
      for (uint256 j = 0; j < redeemableCount; ++j) {
        editionIdToData[_editionId].redeemedTokens[result[j]] = true;
      }
    }
  ...
  function partnerRedeemEdition(uint256 _editionId, uint256[] memory partnerTokenIds) external {
    require(editionIdToData[_editionId].redeemOpen == true, "Redemption closed");
    ...
    require(remainingClaims > 0 && partnerTokenCount <= remainingClaims, "No claims available");

    uint256 redeemableCount;

    uint256[] memory validPartnerTokenId = new uint256[](partnerTokenCount);

    unchecked {
      for (uint256 i = 0; i < partnerTokenCount; ++i) {
        uint256 _tokenIdToRedeem = partnerTokenIds[i];
        if (isOwnerOfValidPartnerAsset(_editionId, _tokenIdToRedeem)) {
          validPartnerTokenId[i] = _tokenIdToRedeem;
          redeemableCount++;
        }
      }
      ...
      for (uint256 j = 0; j < redeemableCount; ++j) {
        editionIdToData[_editionId].redeemedPartnerTokens[validPartnerTokenId[j]] = true;
      }
    }
