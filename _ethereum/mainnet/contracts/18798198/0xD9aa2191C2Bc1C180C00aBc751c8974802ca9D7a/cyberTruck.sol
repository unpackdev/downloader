...
/**
 *Submitted for verification at Etherscan.io on 2023-1-14
 */
...
contract CybertruckSale is ReentrancyGuard, Ownable {
    ...
    {
        require(
            block.timestamp <= presale[presaleId].endTime,
            "presale Time is over"
        );
        ...
        if (userClaimData[_msgSender()].totalAmount > 0) {
            ...
            userClaimData[_msgSender()] = ClaimData(0, numberOfTokens, 0);
        }
        ...
        returns (uint256 ethAmount)
    {
        ...
        ethAmount =
            ...
            (getLatestPrice() * 10**IERC20Metadata(cybertruck).decimals());
    }
        ...
        returns (uint256 usdPrice)
    {
        usdPrice =
            ...
            10**IERC20Metadata(cybertruck).decimals();
    }
    ...
    function claimAmount() public returns (bool) {
        ...
        require(cybertruck != address(0), "Presale token address not set");
        require(
            amount <= IERC20(cybertruck).balanceOf(address(this)),
        ...
        bool status = IERC20(cybertruck).transfer(msg.sender, amount);
