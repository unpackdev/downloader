...
contract SpecialSlothItemV5 is Initializable, ERC721AQueryableUpgradeable, OwnableUpgradeable, ISlothItemV4, IERC2981, RevokableOperatorFiltererUpgradeable {
  ...
  function mintHalloweenJacKOLanternSet(address sender, uint256 quantity) external {
    ...
    for (uint i = 0; i < quantity; i++) {
      ...
      itemType[nextTokenId] = IItemTypeV2.ItemType.HAND;
      specialType[nextTokenId] = HALLOWEEN_CLOTH_SPECIAL_TYPE; 
      clothType[nextTokenId] = JACKOLANTERN_CLOTH_TYPE;
      nextTokenId += 1;
    }
