...
contract InflationManager is IInflationManager, Ownable {
        ...
        for (uint256 i; i < pools.length; i++) {
            if (controller.isActivePool(pools[i])) {
                ...
                uint256 poolUSDValue =
                    pool.cachedTotalUnderlying().convertScale(underlying.universalDecimals(), 18).mulDown(price);
    ...
    function computePoolWeight(address pool) public view returns (uint256 poolWeight, uint256 totalUSDValue) {
        ...
        for (uint256 i; i < pools.length; i++) {
            if (controller.isActivePool(pools[i])) {
                ...
                uint256 price = oracle.getUSDPrice(address(underlying));
                uint256 usdValue =
                    currentPool.cachedTotalUnderlying().convertScale(underlying.universalDecimals(), 18).mulDown(price);
        ...
        poolWeight = totalUSDValue == 0 ? ScaledMath.ONE / pools.length : poolUSDValue.divDown(totalUSDValue);
    }
    ...
    function handleRebalancingRewards(address account, uint256 deviationBefore, uint256 deviationAfter) external {
        ...
        for (uint256 i; i < _rebalancingRewardHandlers[msg.sender].length(); i++) {
            ...
            IRebalancingRewardsHandler(handler).handleRebalancingRewards(
                ITorusPool(msg.sender), account, deviationBefore, deviationAfter
            );
        }
    }

    function addPoolRebalancingRewardHandler(address poolAddress, address rebalancingRewardHandler) external override {
        require(msg.sender == owner() || msg.sender == controller.emergencyMinter(), "only owner or emergency minter");
        require(controller.isPool(poolAddress), "invalid pool");
        require(_rebalancingRewardHandlers[poolAddress].add(rebalancingRewardHandler), "handler already set");

        emit RebalancingRewardHandlerAdded(poolAddress, rebalancingRewardHandler);
    }

    function removePoolRebalancingRewardHandler(address poolAddress, address rebalancingRewardHandler)
        external
        override
    {
        require(msg.sender == owner() || msg.sender == controller.emergencyMinter(), "only owner or emergency minter");
        require(controller.isPool(poolAddress), "invalid pool");
        require(_rebalancingRewardHandlers[poolAddress].remove(rebalancingRewardHandler), "handler not set");
        emit RebalancingRewardHandlerRemoved(poolAddress, rebalancingRewardHandler);
    }

    function hasPoolRebalancingRewardHandlers(address poolAddress, address handler) external view returns (bool) {
        return _rebalancingRewardHandlers[poolAddress].contains(handler);
    }

    function rebalancingRewardHandlers(address poolAddress) external view returns (address[] memory) {
