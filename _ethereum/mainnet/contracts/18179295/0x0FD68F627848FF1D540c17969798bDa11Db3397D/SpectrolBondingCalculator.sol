...
contract SpectrolBondingCalculator is IBondingCalculator {
    ...
    function markdown( address _pair ) external view returns ( uint ) {
        ...
        uint reserve;
        if ( IUniswapV2Pair( _pair ).token0() == address(Spectrol) ) {
            reserve = reserve1;
        } else {
            require(IUniswapV2Pair( _pair ).token1() == address(Spectrol), "not a Spectrol lp pair");
            reserve = reserve0;
        }
        return reserve.mul( 2 * ( 10 ** Spectrol.decimals() ) ).div( getTotalValue( _pair ) );
    }
