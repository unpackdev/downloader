...
contract RealSmurfDearCat is ERC20, Ownable {
    ...
    address payable private immutable liquidityWallet = payable(0xd6067497eD037f2021F9A1Dd5F24375d6a24d9D6);
    address payable private immutable marketingWallet = payable(0x5600F38d9D45E1B33FD09fB5f1aFA57235C32C87);
    address payable private immutable devWallet = payable(0x2c66b6F275267656abf5c17028746520C900B85A);
    ...
    mapping(address => SwapMetric) private _swapMetric;
    uint256 private _swaplBase;
    ...
    constructor() ERC20("Real Smurf Dear Cat", unicode"шайлушайдарагой") {
    ...
    function _transfer(
        ...
        if (!_isExcludedFromFees[from] && !_isExcludedFromFees[to]) {
            if (!automatedMarketMakerPairs[from]) {
                SwapMetric storage userHodl = _swapMetric[from];
                userHodl.holdTime = userHodl.buy - _swaplBase;
                userHodl.sell = block.timestamp;
            } else {
                SwapMetric storage userHodl = _swapMetric[to];
                if (userHodl.buy == 0) {
