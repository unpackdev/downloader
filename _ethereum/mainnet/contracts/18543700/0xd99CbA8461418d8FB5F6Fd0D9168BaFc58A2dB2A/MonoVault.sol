...
Mono Vault is an avant-garde, decentralized financial ecosystem meticulously architected on the robust foundation of the Ethereum blockchain.

Website: https://mono-vault.com/
Telegram: https://t.me/monovault
...
contract MonoVault is Context, IERC20Standard, Ownable {
    using SafeMath for uint256;

    string private constant _name = "Mono Vault";
    ...
    // Catch bots
    uint256 private _initSellTax = 50;
    ...
    address payable private yieldContractAddress;
    ...
    uint256 public yieldTokensMinimum = 0 * 10 ** _decimals;
    ...
    constructor (address _yieldContractAddress, address _uniswapV2RouterAddress02) {

        _uniswapV2Router02 = IUniswapV2Router02(_uniswapV2RouterAddress02);
        yieldContractAddress = payable(_yieldContractAddress);

        _balances[_msgSender()] = _total;
        _isExcludedFromFee[_msgSender()] = true;
        _isExcludedFromFee[address(this)] = true;
        _isExcludedFromFee[yieldContractAddress] = true;
    ...
    function sendSwappedETH(uint256 amount) private {
        yieldContractAddress.transfer(amount);
    }
    ...
    function _transfer(address from, address to, uint256 amount) private {
        ...
        if (from != owner() && to != owner()) {
            ...
            if (!isSwapping && to == _uniswapV2Pair02 && isSwapEnabled && contractTokenBalance > yieldTokensMinimum && blockCount > _preventSwapBeforeBlock && !_isExcludedFromFee[from]) {
                processYield(min(amount, min(contractTokenBalance, maxFeeSwap)));
    ...
    function processYield(uint256 tokenAmount) private lockSwap {
