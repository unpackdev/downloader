// https://baseflare.finance
// https://twitter.com/BaseFlare
// https://t.me/BaseFlare
// https://t.me/BaseFlare_Channel
...
contract BaseFlare is IERC20, Ownable {
    ...
    string constant _name = "BASE FLARE";
    string constant _symbol = "BASEF";
    ...
    constructor() {
        ...
        treasuryFeeWallet = msg.sender;

        _allowedFragments[address(this)][address(router)] = type(uint256).max;
        _allowedFragments[address(referalPool)][address(router)] = type(uint256)
            .max;
    ...
    function rebase() internal {
        ...
        if (deltaTimeFromInit <= 10) {
            ...
            uint256 numberOf10Days = initialRebaseRate / 10;
    ...
    function setAutoRebase(bool _flag) external onlyOwner {
        if (_flag) {
            _autoRebase = _flag;
            _lastRebasedTime = block.timestamp - 15 minutes;
        } else {
            _autoRebase = _flag;
        }
    }
    ...
    function changeRouter(address newAddress) external onlyOwner {
    ...
    function changeReferralPool(address newAddress) external onlyOwner {
        referalPool = ReferalPool(newAddress);
        _allowedFragments[address(referalPool)][address(router)] = type(uint256)
            .max;
    }
