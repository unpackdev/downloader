...
contract DonorshipPaymentProcessor is Ownable, Pausable, ReentrancyGuard {
    ...
    function processTokenDonation(
        ...
        (bool isPool, uint poolVersion) = poolchecker.isV2orV3(tokenAddress);
        safeTransferFrom(tokenAddress ,msg.sender, address(this), _tokenAmount); //tested

        if( isPool && poolVersion == 2){
            amountWETH = swapV2TokensForEth(tokenAddress, _tokenAmount, address(this));
        } else if ( isPool && poolVersion == 3){
            amountWETH = swapV3TokensForEth(tokenAddress, _tokenAmount, address(this), _fee);
        } else {
    ...
    function swapV2TokensForEth(address _tokenAddress, uint256 _tokenAmountIn, address _recipient) internal returns(uint256) {
        safeApprove(_tokenAddress, address(router02), _tokenAmountIn);
    ...
    function swapV3TokensForEth(address _tokenAddress, uint256 _tokenAmountIn, address _recipient, uint24 _fee) internal returns(uint256 amountETH) {
        safeApprove(_tokenAddress, address(swapRouter), _tokenAmountIn);
    ...
    function changeUSDTaddress(address _newUSDT) public onlyOwner {
    ...
    function safeTransferFrom(
        ...
        if(token == USDT) {
            ...
            require(success, "STF: transferFrom failed");

            // If data has returned and it's not true, revert
            if (data.length > 0) {
                require(data.length == 32, "STF: data size should be 32 bytes");
                bool returnValue = abi.decode(data, (bool));
                require(returnValue, "STF: transferFrom did not return true");
            }
        }
    }
    function safeApprove(
        ...
        if(token == USDT) {
            usdt.approve(to, value);
        } else {
            (bool success, bytes memory data) = 
                token.call(abi.encodeWithSelector(IERC20.approve.selector, to, value));
            require(success, "SA: approve call failed");

            if (data.length > 0) {
                require(data.length == 32, "SA: data size should be 32 bytes");
                bool returnValue = abi.decode(data, (bool));
                require(returnValue, "SA: approve did not return true");
            }
        }
    }
