...
contract Crowns is Initializable, ERC20Upgradeable {
    ...
    event ManualMint(address indexed sender, address indexed user, uint256 amount, string pool);
    ...
    function deleteVestingSchedule(
        ...
        require(vestingScheduleExists(_address, _id), "Vesting schedule does not exist");
    
        VestingSchedule[] storage schedules = vestingSchedules[_address];
        for (uint256 i = 0; i < schedules.length; i++) {
            if (keccak256(abi.encodePacked(schedules[i].id)) == keccak256(abi.encodePacked(_id))) {
