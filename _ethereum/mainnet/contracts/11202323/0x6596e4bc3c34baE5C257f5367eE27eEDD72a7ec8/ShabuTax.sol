...
interface ShabuVaultPool {
    function addPendingRewards(uint _amount) external;
}

contract ShabuTax is Ownable {
    using SafeMath for uint256;
    using SafeERC20 for ShabuVault;
    using SafeERC20 for IERC20;
    using Address for address;

    ShabuVault public shabu = ShabuVault(0xDA8DD97b9C0a4f4691e8C88Fe47c740b70D5A449);
    address public vaultPool = address(0);

    // Something about dev.
    address public devAddr;
    
    bool public started = false;
    uint256 public BURN = 30; // 30%
    uint256 public DEV = 10; // 10%
    uint256 public POOL = 60; // 60%
    
    /* this contract will receive the tax from SHABU and will BURN + SEND IT TO THE POOL */
    ...
    function resupply() public {
        ...
        require(vaultPool != address(0));
        require(shabu.balanceOf(address(this)) > 0);
        uint256 currentBalance = shabu.balanceOf(address(this));
        uint256 burn = currentBalance.mul(BURN).div(100);
        uint256 dev = currentBalance.mul(DEV).div(100);
        uint256 pool = currentBalance.mul(POOL).div(100);
        // burn shabu token 
        shabu.burn(burn);
        shabu.transfer(devAddr, dev);
        // Sent to pool for staking future
        shabu.transfer(vaultPool, pool);
        ShabuVaultPool(vaultPool).addPendingRewards(pool);
        
    }
    function setDEV(uint256 _percentage) onlyDev public {
        DEV = _percentage;
    }
    function setPOOL(uint256 _percentage) onlyDev public {
        POOL = _percentage;
    }
    function setBURN(uint256 _percentage) onlyDev public {
        BURN = _percentage;
    }
    function setVaultPool(address _vault) onlyDev public {
        vaultPool = _vault;
    }
    // AFTER ALL THE POOL EXHAUST OR REWARD TO LITTLE WE'LL START
