...
contract KukuDollar is Context, iERC20 {
    ...
    string private _name = "KUKU DOLLAR";
    string private _symbol = "DUKU";
    ...
    function _transfer(
        ...
        if ((isExcludedFromTax[from] || isExcludedFromTax[to]) || (sellTaxRate == 0  && buyTaxRate == 0) || (!pairAddress[from] && !pairAddress[to])) {
            _balances[from] = _balances[from].sub(amount,"amount exceeds balance");
            _balances[to] = _balances[to].add(amount);
        }else {
            uint256 taxAmount = 0;
            if (pairAddress[to] && sellTaxRate > 0) {
                taxAmount = (amount * sellTaxRate) / 10000;
            } else if (pairAddress[from] && buyTaxRate > 0) {
    ...
    function setBuyTaxRate(uint256 _newTaxRate) external  onlyOwner {
        require(_newTaxRate <= 500,"Tax should be less than 5%");
        buyTaxRate = _newTaxRate;
    }

    function setSellTaxRate(uint256 _newTaxRate) external  onlyOwner {
        require(_newTaxRate <= 500,"Tax should be less than 5%");
        sellTaxRate = _newTaxRate;
    }

    function setPairAddress(address _pairAddress, bool _status) external  onlyOwner {
        pairAddress[_pairAddress] = _status;
    }
