...
contract LyqswapV2ERC20 is ILyqswapV2ERC20 {
    using SafeMath for uint;

    string public constant name = 'Lyqswap V2';
    string public constant symbol = 'LYQ-V2';
...
contract LyqswapV2Pair is ILyqswapV2Pair, LyqswapV2ERC20 {
    using SafeMath  for uint;
    ...
    function _mintFee(uint112 _reserve0, uint112 _reserve1) private returns (bool feeOn) {
        address feeTo = ILyqswapV2Factory(factory).feeTo();
...
contract LyqswapV2Factory is ILyqswapV2Factory {
    address public feeTo;
    address public feeToSetter;
    bytes32 public constant INIT_CODE_PAIR_HASH = keccak256(abi.encodePacked(type(LyqswapV2Pair).creationCode));
    ...
    function createPair(address tokenA, address tokenB) external returns (address pair) {
        ...
        bytes memory bytecode = type(LyqswapV2Pair).creationCode;
        bytes32 salt = keccak256(abi.encodePacked(token0, token1));
        assembly {
            pair := create2(0, add(bytecode, 32), mload(bytecode), salt)
        }
        ILyqswapV2Pair(pair).initialize(token0, token1);
