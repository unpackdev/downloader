...
interface IsSPT is IERC20 {
    function rebase( uint256 SpectrolProfit_, uint epoch_) external returns (uint256);
...
contract SpectrolStaking is Ownable {

    using SafeMath for uint256;
    using LowGasSafeMath for uint32;
    using SafeERC20 for IERC20;
    using SafeERC20 for IsSPT;

    IERC20 public immutable Spectrol;
    IsSPT public immutable sSPT;
    ...
    /**
        @notice stake Spectrol to enter warmup
        @param _amount uint
        @return bool
     */
    function stake( uint _amount, address _recipient, LOCKUPS _lockup ) external returns ( bool ) {
        ...
        Spectrol.safeTransferFrom( msg.sender, address(this), _amount );
        ...
        warmupInfo[ _recipient ] = Claim ({
            ...
            gons: info.gons.add( sSPT.gonsForBalance( _amount ) ),
        ...
        if( _lockup == LOCKUPS.MONTH1 ) { // 1 - 1.25 Multiplier
            ...
            lockupInfo[ _recipient ] = Lockup({
                gonsWarmup: IsSPT( sSPT ).gonsForBalance( _amount.mul(25).div(100) ),
                gonsAccount: IsSPT( sSPT ).gonsForBalance( _amount ),
        ...
        if( _lockup == LOCKUPS.MONTH3 ) { // 2 - 1.5 Multiplier
            ...
            lockupInfo[ _recipient ] = Lockup({
                gonsWarmup: IsSPT( sSPT ).gonsForBalance( _amount.mul(50).div(100) ),
                gonsAccount: IsSPT( sSPT ).gonsForBalance( _amount ),
                initialDeposit: _amount,
                multiplier: 150,
        ...
        if( _lockup == LOCKUPS.MONTH6 ) { // 3 - 2 Multiplier
            ...
            lockupInfo[ _recipient ] = Lockup({
                gonsWarmup: IsSPT( sSPT ).gonsForBalance( _amount ),
                gonsAccount: IsSPT( sSPT ).gonsForBalance( _amount ),
                initialDeposit: _amount,
                multiplier: 200,
        ...
        sSPT.safeTransfer( address(warmupContract), amountToTransfer );
        emit LogStake(_recipient, _amount);
        return true;
    }

    /**
        @notice retrieve sSPT from warmup
    ...
    function claim ( address _recipient ) external {
        ...
        if ( epoch.number >= info.expiry && info.expiry != 0 ) {
            ...
            uint256 amount = sSPT.balanceForGons( info.gons );
    ...
    /**
        @notice forfeit sSPT in warmup and retrieve Spectrol
     */
    function forfeit() external {
        Claim memory info = warmupInfo[ msg.sender ];
        delete warmupInfo[ msg.sender ];
        uint memoBalance = sSPT.balanceForGons( info.gons );
        warmupContract.retrieve( address(this),  memoBalance);
        Spectrol.safeTransfer( msg.sender, info.deposit);
    ...
    /**
        @notice redeem sSPT for Spectrol
        @param _amount uint
        @param _trigger bool
     */
    function unstake( uint _amount, bool _trigger ) external {
        ...
        require(sSPT.balanceOf(msg.sender) - _amount >= sSPT.balanceForGons( lock.gonsAccount ), "Not enough sSPT for lockup");
        sSPT.safeTransferFrom( msg.sender, address(this), _amount );
        Spectrol.safeTransfer( msg.sender, _amount );
    ...
    function unstakeLocked(bool _trigger) external{
        ...
        if (lock.multiplier >0){
            sSPT.safeTransferFrom( msg.sender, address(this), sSPT.balanceForGons( lock.gonsAccount ));
            warmupContract.retrieve( address(this), sSPT.balanceForGons( lock.gonsWarmup ));

            if (lock.lockTimestamp <= block.timestamp){
                Spectrol.safeTransfer( msg.sender, sSPT.balanceForGons( lock.gonsWarmup + lock.gonsAccount ));
            }
            else{
                Spectrol.safeTransfer( msg.sender, lock.initialDeposit );
            }
    ...
    /**
        @notice returns the sSPT index, which tracks rebase growth
        @return uint
     */
    function index() external view returns ( uint ) {
        return sSPT.index();
    ...
    function rebase() public {
        if( epoch.endTime <= uint32(block.timestamp) ) {
            ...
            sSPT.rebase( epoch.distribute, epoch.number );
            ...
            uint staked = sSPT.circulatingSupply();
    ...
    /**
        @notice returns contract Spectrol holdings, including bonuses provided
        @return uint
     */
    function contractBalance() public view returns ( uint ) {
        return Spectrol.balanceOf( address(this) ).add( totalBonus );
    }
