    ...
    OwnableUpgradeable
{
    ...
    function _redeemVoucherForNftFixedPrice(
        ...
        uint256 weiToExchange = (msg.value * exchangeRateNumerator) / EXCHANGE_RATE_DENOMINATOR;
        uint256 weiToCreator;
        if (
            nftContractLocation == NftContractLocation.INTERNAL &&
            saleMarketType == SaleMarketType.SECONDARY
        ) {
            weiToCreator = (msg.value * tokenCreatorFee[params.tokenId]) / 1000;
        }
        ...
        for (uint256 i = 0; i < params.exchangeVoucherPayoutAddresses.length; i++) {
            uint256 weiToAdditionalCreator = (msg.value *
        ...
        if (
            ...
            weiToReferrer = (msg.value * params.referralFee) / 1000;
        }
        uint256 weiToSeller = msg.value -
            weiToExchange -
            weiToCreator -
