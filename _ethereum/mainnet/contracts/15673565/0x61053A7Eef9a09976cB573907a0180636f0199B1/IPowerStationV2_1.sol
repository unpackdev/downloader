...
interface IDUOPowerStation {
    ...
    function poolAvailable(uint256 _pid) external returns(bool);

    function poolRemainingTime(uint256 _pid) external returns(uint256);

    // function utilsRemaining(address user, uint256 _pid)  external returns(uint256);

    function pendingRewards(address user, uint256 _pid)  external returns (uint256);

    function checkSoldShares(uint256 _pid) external returns(uint256);

    function poolExists(uint256 _pid)  external returns(bool);

    function stakerExistsInPool(uint256 _pid, address stakerAddr)  external returns(bool);

    function numberOfstakers(uint256 _pid)  external returns (uint256);
    ...
    function getTiming(uint256 _pid)  external returns(uint256[3] memory);

    function getStakerInfoNTime(uint256 _pid, address stakerAddr) external returns(uint256[6] memory);
