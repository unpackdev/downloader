...
contract WCAVIP is ERC721Enumerable, Ownable {
	...
	constructor(address _manager) ERC721("WCAPES VIP", "WCAVIP") {
		manager = _manager;
	}
	...
	function mint(string calldata uri) external onlyOwnerOrManager {
		NFTId.increment();
		_safeMint(owner(), NFTId.current());
		nfts[NFTId.current()] = uri;
	}

	function update(uint256 id, string calldata uri) external onlyOwnerOrManager {
		require(id > 0 && id <= NFTId.current(), "Invalid token id");
		nfts[id] = uri;
	}
