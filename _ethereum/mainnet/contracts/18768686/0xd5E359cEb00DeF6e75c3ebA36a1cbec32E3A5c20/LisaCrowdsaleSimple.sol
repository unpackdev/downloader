...
contract LisaCrowdsaleSimple is Initializable, LisaCrowdsaleBase {
    ...
    function initialize(
        ...
        salesAgreementURI = params.salesAgreement;
    }
    ...
    /**
     * @notice  Claim the AT tokens. Can only be called by the seller.
    ...
    function refund() public override nonReentrant returns (uint256) {
        ...
        uint256 refundBT = costBT(tokenAT.allowance(_msgSender(), address(this)));
        if (refundBT > 0) {
