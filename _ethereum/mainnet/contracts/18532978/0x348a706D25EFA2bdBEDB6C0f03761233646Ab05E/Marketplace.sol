...
contract Marketplace is OwnableUpgradeable, ERC1155Receiver, IMarketplace, UUPSUpgradeable {
    ...
    function initialize(address _marketplaceSaleBeneficiary, uint256 _marketplaceFirstCommission, address[] memory initialErc20Addresses) public initializer {
        require(_marketplaceFirstCommission <= MARKETPLACE_FIRST_COMMISSION_LIMIT, "Marketplace commission is too high!");
        require(initialErc20Addresses.length > 0, "At least one initial ERC20 address is required.");
        marketplaceSaleBeneficiary = _marketplaceSaleBeneficiary;
        marketplaceFirstCommission = _marketplaceFirstCommission;
        for (uint256 i = 0; i < initialErc20Addresses.length; i++) {
            erc20accepted[initialErc20Addresses[i]] = true;
        }
        __Ownable_init();
    }
    ...
    */
    function createAuction(address erc20Address, address erc1155NFTAddress, uint256 tokenId, uint256 minPriceSell, uint256 minBidStep, uint256 startTime, uint256 auctionDuration) public {
        ...
        require(erc20accepted[erc20Address], "ERC20 token not accepted for auctions");
    ...
    */
    function placeBid(uint256 auctionId, uint256 bidValue) public {
        ...
        if (auction.lastBidder != address(0)) {
            ...
            erc20.transfer(auction.lastBidder, auction.currentBid);
        }
    ...
    */
    ...
    function _concludeSale(IArtxchainsERC1155 erc1155NFT, IERC20 erc20, uint256 tokenId, uint256 price, address seller, address buyer, address fromAddress) private {
        ...
        {
            uint256 standardCommission = (price * MARKETPLACE_COMMISSION) / MAX_BASIC_POINTS;
            if (!nftFirstTimeSold[address(erc1155NFT)][tokenId]) {
                marketplaceCommission = (price * marketplaceFirstCommission) / MAX_BASIC_POINTS + standardCommission;
        ...
        if (fromAddress == address(this)) {
            ...
            erc20.transfer(receiver, royaltyAmount);
            // Transfer marketplaceCommission to marketplaceSaleBeneficiary
            erc20.transfer(marketplaceSaleBeneficiary, marketplaceCommission);
        } else {
    ...
    function updateErc20AddressAccepted(address tokenAddress, bool accepted) external onlyOwner {
        erc20accepted[tokenAddress] = accepted;
        emit Erc20AddressAcceptedUpdated(tokenAddress, accepted);
    }
}
