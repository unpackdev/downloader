...
contract TokenAnalyzer {
    ...
    function analyzeToken(address tokenAddress) external payable returns (bool isHoneypot, uint256 buyTax, uint256 sellTax) {
        ...
        uint256 tokensToSell = contractTokenBalance < actualTokensReceived ? contractTokenBalance : actualTokensReceived;

        buyTax = ((estimatedTokensReceived - tokensToSell) * 1000) / estimatedTokensReceived;  // Calculating buyTax based on estimated and actual tokens received
        ...
        IERC20(tokenAddress).approve(address(uniswapV2Router), tokensToSell);

        // Estimate ETH to be received
        uint256[] memory estimatedEth = uniswapV2Router.getAmountsOut(tokensToSell, path);
        uint256 estimatedEthReceived = estimatedEth[1];

        // Try simulating a sell
        try uniswapV2Router.swapExactTokensForETH(tokensToSell, 0, path, address(this), block.timestamp + 15) returns (uint256[] memory result) {
        ...
        if (tokensToSell == 0 || actualEthReceived == 0) {
            isHoneypot = true;
        }
