...
contract GovernanceMessageVerifier is IGovernanceMessageVerifier {
    ...
    event Event1(string s, bytes32 rootHash);
    event Event2(string s, bytes data);
    ...
    function verifyAndPropagateMessage(GovernanceMessageProof calldata proof) external {
        ...
        emit Event1('rootHash', rootHash);

        emit Event1('blockHash', blockHash);

        emit Event2('proof.rootHashProof', proof.rootHashProof);
