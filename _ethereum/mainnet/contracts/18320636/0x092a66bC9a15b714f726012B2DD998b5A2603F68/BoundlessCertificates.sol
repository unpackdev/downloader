...
interface IBoundlessAuthenticityCertificates {
...
interface IStakingContract {
    ...
    function claimDiscount(address _staker, uint256 _tokenId) external;
    ...
    WithOperatorRegistry
{
    ...
    IBoundlessAuthenticityCertificates
        public boundlessAuthenticityCertificatesContract;
    ...
    function setEditionSupply(uint24 currentSupply) public onlyAdmin {
        edition.currentSupply = currentSupply;
    }
    ...
    function setBoundlessCertificatesContractAddressContract(
        address _boundlessAuthenticityCertificatesContract
    ) public onlyAdmin {
        boundlessAuthenticityCertificatesContract = IBoundlessAuthenticityCertificates(
            _boundlessAuthenticityCertificatesContract
        );
    }
    ...
    function _getStakeNFTByTokenId(
        IStakingContract.StakedNFT[] memory stakedNFTs,
        uint256 tokenId
    ) internal view returns (IStakingContract.StakedNFT memory stakedNFT) {
    ...
    function _validateOwnershipAndPrice(
        ...
        (
            IStakingContract.StakedNFT[] memory stakedNFTs,
            uint256 dicountBPS
        ) = IStakingContract(stakingContract).getDiscountedByWallet(msg.sender);

        for (uint256 index = 0; index < discountedTokenIds.length; index++) {
            // Validate ownership
            IStakingContract.StakedNFT memory stakedNFT = _getStakeNFTByTokenId(
                stakedNFTs,
                discountedTokenIds[index]
            );
            ...
            uint256 boundlessTokenId = editionedTokenId(
                EDITION_ID,
                discountedTokenIds[index]
            );
            if (boundlessContract.ownerOf(boundlessTokenId) != msg.sender) {
                revert NotTokenOwner();
            }

            IStakingContract(stakingContract).claimDiscount(
                msg.sender,
                discountedTokenIds[index]
            );
            ...
            totalCost +=
                baseMintPrice -
                BPS._calculatePercentage(baseMintPrice, dicountBPS);
        }
        ...
        for (uint256 index = 0; index < tokenIds.length; index++) {
            ...
            address owner = boundlessContract.ownerOf(boundlessTokenId);
            if (owner != msg.sender) {
                revert NotTokenOwner();
            }
            // apply discount
    ...
    function publicMint(
        ...
        uint24 mintQuantity = uint24(
            discountedTokenIds.length + tokenIds.length
        );
    ...
    function burn(uint256[] calldata tokenIds) external {
        boundlessAuthenticityCertificatesContract.mint(tokenIds);
    ...
    function adminMint(
        ...
        for (uint256 index = 0; index < tokenIds.length; index++) {
            ...
            if (boundlessContract.ownerOf(boundlessTokenId) != _recipient) {
                revert NotTokenOwner();
            }
