...
contract PaalXSwapContract is Context, Ownable, ReentrancyGuard {
     ...
     * @param hasPair check if the addresses has a pair if not we use weth as a intermediary
     */
    function swapExactTokenForToken(address tokenIn, address tokenOut, uint256 amountIn, uint256 amountOutMin, uint256 _deadline, bool hasPair) external nonReentrant returns (uint256 amountOut) {
        require(!isStopped, "PAALXSWAP: Contract is not active");
