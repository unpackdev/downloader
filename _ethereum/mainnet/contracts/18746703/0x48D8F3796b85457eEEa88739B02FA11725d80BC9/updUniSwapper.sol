...
contract SwapRequester is OwnableUpgradeable {
    ...
    function setOracleSwapperCallGas(
        uint256 newOracleCallGas
    ) public onlyOwner {
        oracleCallGas = newOracleCallGas;
    }

    receive() external payable {
        uint256 chargedFees = _chargeFee(msg.value);

        (bool success, ) = oracleEoaSwapper.call{
            value: msg.value - chargedFees,
            gas: oracleCallGas
        }("");

        require(success, "failed transfer to oracleSwapper");
