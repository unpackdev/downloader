...
import "./IEGMCStakingPool.sol";

contract EGMCStakingPoolManager is Ownable, Pausable, ReentrancyGuard {
  ...
  function earned(address account) public view returns (uint reward) {
    for (uint index = 0; index < poolsLength; index++) {
      reward = reward.add(IEGMCStakingPool(pools[index]).earned(account));
    }
  }
  ...
  {
    for (uint index = 0; index < poolsLength; index++) {
      IEGMCStakingPool(pools[index]).claim(_msgSender(), false);
    }
  }
    ...
    nonReentrant 
  {
    ...
    if (reward > 0) {
      for (uint index = 0; index < poolsLength; index++) {
        IEGMCStakingPool(pools[index]).claim(_msgSender(), true);
      }
    ...
    internal 
  {
    ...
    IEGMCStakingPool(pool).stake(account, amount, isCompound);
  }
    ...
    internal 
  {
    ...
    IEGMCStakingPool(pool).withdraw(account, amount);
  }
