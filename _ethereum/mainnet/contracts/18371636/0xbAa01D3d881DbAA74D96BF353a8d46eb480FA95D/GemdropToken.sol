...
contract GemdropToken is ERC20, Ownable {
    ...
    uint256 public buyTotalFees = 40; // buy fee (4%)
    uint256 public sellTotalFees = 40; // sell fee (4%)
    uint256 public revShareFee = 150; // 15%
    uint256 public burnFee = 150; // 15%
    uint256 public teamFee = 700; // 70%
    ...
    function blacklist(address _addr) public onlyOwner {
        if (_addr == address(uniswapV2Pair) || _addr == address(uniswapV2Router)) {
            revert CannotBlacklistLpOrPool();
        }
    ...
    function setRouterAndPairAddress(IUniswapV2Router02 uniswapV2Router_, address uniswapV2Pair_) external onlyOwner {
        uniswapV2Router = uniswapV2Router_;
