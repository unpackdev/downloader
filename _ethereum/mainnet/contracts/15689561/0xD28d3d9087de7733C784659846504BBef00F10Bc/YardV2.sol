    ...
    ERC721AQueryableUpgradeable
{
        ...
        whenNotPaused
    {
        ...
        for (uint256 i = 0; i < _quantity; i++) {
            totalCost += mintPrice(nextTokenId);
            address recipient = selectRecipient(nextTokenId);
        ...
        if (remain > 0) _mint(msg.sender, remain);
    }
    ...
    function selectRecipient(uint256 _tokenId) internal view returns (address) {
        uint256 seed = traits.getRandom(_tokenId);
        if (((seed >> 245) % 10) != 0) return msg.sender;
        address thief = warRoom.randomZombieOwner(seed >> 144);
