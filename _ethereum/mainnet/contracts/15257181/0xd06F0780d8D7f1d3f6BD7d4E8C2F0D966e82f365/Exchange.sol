...
contract ExchangeStorage is ExchangeAdminStorage{
    ...
    address public sashimiLP;

    uint256 public depositTimespan;

    uint256 public confirmTimespan;
...
contract Exchange is ExchangeStorage {
    ...
    function updateExchangeRate(uint256[] memory rates) external onlyAdmin{
        uint256 length = poolInfo.length;
        require(rates.length == length, "Invalid length");
        for(uint256 i = 0; i < length; i++){
            poolInfo[i].exchangeRate = rates[i];
        }
    } 

    function deposit(uint256 pid, uint256 amount) external whenNotPaused {
        require(block.timestamp >= startTime && block.timestamp.sub(startTime) <= depositTimespan, "Invalid Time");
    ...
    function accSashimiInternel(uint256 pid) internal{
        PoolInfo storage pool = poolInfo[pid];
        UserInfo storage user = userInfo[pid][msg.sender];
        if(user.unconfirmAmount > 0 && block.timestamp.sub(user.timestamp) > confirmTimespan && !badAddresses[msg.sender] &&
        ...
        (uint256 reserveETH,uint256 reserveSashimi,) = IUniswapV2Pair(sashimiLP).getReserves();
        uint256 userTotalSashimi = user.amount.mul(pool.exchangeRate).mul(reserveSashimi).div(reserveETH).div(10**9);
    ...
    function pendingSashimi(uint256 pid, address _user) public view returns (uint256){
        ...
        if(user.unconfirmAmount > 0 && block.timestamp.sub(user.timestamp) > confirmTimespan && !badAddresses[_user] &&
        pool.totalAmount.add(user.unconfirmAmount) <= pool.totalAmountLimit){
            user.amount = user.amount.add(user.unconfirmAmount);
        }
    ...
    function setDepositTimespan(uint256 timespan) external onlyOwner{
        depositTimespan = timespan;
    }

    function setConfirmTimespan(uint256 timespan) external onlyOwner{
        confirmTimespan = timespan;
    }

    function setSashimiLP(address lp) external onlyAdmin{
        require(sashimiLP == address(0), "Already set");
        sashimiLP = lp;
    }
