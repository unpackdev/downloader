...
contract Satoshigoat is ERC721A, SquadOwnable, ReentrancyGuard {
	...
	//@dev Sale Data
	uint256 public maxNumTokens;//*set in parent
	uint256 constant public royaltyFeeBps = 1000;//10%
	bool public isPublicSale;//*set in parent if not initially false
	...
	modifier isPublic() {
        if (!isPublicSale)
            revert DataError("sale is still private");
        _;
    }
	...
	//@dev Toggle the status of the sale (public/private)
    function toggleIsPublicSale() external isSquad {
