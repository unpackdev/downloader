...
contract UniswapPriceSource {
    ...
    /// @param twapInterval The time interval in seconds to get the twap over
    /// @return price The price of the token in terms of USDC with 18 decimals
    function priceFromUsdtPoolInUsdc(IUniswapV3Pool pool, uint32 twapInterval) public view returns (uint256) {
        // get the price from the usdt pool first
        uint256 tokenEthPrice = priceFromUsdtPool(pool, twapInterval);
        return (usdtPriceInUsdc() * tokenEthPrice) / 10 ** 18;
    }
    ...
    /// @param twapInterval The time interval in seconds to get the twap over
    /// @return price The price of the token in terms of USDT with 18 decimals
    function priceFromUsdcPoolInUsdt(IUniswapV3Pool pool, uint32 twapInterval) public view returns (uint256) {
        // get the price from the usdc pool first
        uint256 tokenEthPrice = priceFromUsdcPool(pool, twapInterval);
        return (usdcPriceInUsdt() * tokenEthPrice) / 10 ** 18;
    }
