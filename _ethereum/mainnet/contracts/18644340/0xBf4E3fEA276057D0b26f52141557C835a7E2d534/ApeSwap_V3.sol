    ...
    TokenTransfer
{
    ...
    function loanInfo(
        uint256 _loanId
    ) external view override returns (LoanInfo memory) {
    ...
    function calculatePositionReturns(
        ...
        // set the active borrower so we pass the require statement in _execute_ApeSwap_ClosePosition
        _activeBorrower = loanBorrower[_params.bidId];
    ...
    function _execute_ApeSwap_ClosePosition(
        ...
        require(loanBorrower[_params.bidId] == _activeBorrower, "not borrower");

        LoanInfo memory info = _loanInfo[_params.bidId];
        ...
        emit PositionClosed(_params.bidId, remainingBalance_);
    }
    ...
    function _execute_TellerV2_RepayLoan(
        ...
        if (paymentAmount > 0) {
            if (
                _params.repayFrom != address(0) &&
                _params.repayFrom != address(this)
            ) {
                require(
                    _params.repayFrom == _activeBorrower,
                    "Can only use borrower funds to repay."
                );

                IERC20(_loanInfo[_params.bidId].principalToken).transferFrom(
                    _params.repayFrom,
                    address(this),
                    paymentAmount
                );
        ...
        //send additional collateral out
        if (
            _params.collateralRecipient != address(0) &&
            _params.collateralRecipient != address(this)
        ) {
            if (
                _loanInfo[_params.bidId].collateralTokenType ==
                ...
                address ownerOfNft = IERC721(
                    _loanInfo[_params.bidId].collateralToken
                ).ownerOf(_loanInfo[_params.bidId].collateralTokenId);

                if (ownerOfNft == address(this)) {
                    IERC721(_loanInfo[_params.bidId].collateralToken)
                        .transferFrom(
                            address(this),
                            _params.collateralRecipient,
                            _loanInfo[_params.bidId].collateralTokenId
                        );
                }
            } else if (
                _loanInfo[_params.bidId].collateralTokenType ==
                ...
                uint256 remainingCollateralBalance_ = IERC1155(
                    _loanInfo[_params.bidId].collateralToken
                ).balanceOf(
                        address(this),
                        _loanInfo[_params.bidId].collateralTokenId
                ...
                IERC1155(_loanInfo[_params.bidId].collateralToken)
                    .safeTransferFrom(
                        address(this),
                        _params.collateralRecipient,
                        _loanInfo[_params.bidId].collateralTokenId,
                        remainingCollateralBalance_,
                        data
                    );
            } else {
                uint256 remainingCollateralBalance_ = IERC20(
                    _loanInfo[_params.bidId].collateralToken
                ).balanceOf(address(this));

                if (remainingCollateralBalance_ > 0) {
                    IERC20(_loanInfo[_params.bidId].collateralToken).transfer(
                        _params.collateralRecipient,
                        remainingCollateralBalance_
                    );
                }
    ...
    function _preFlashSwapSubHop(
        ...
        if (
            ...
            uint256 currentBalance = IERC20(
                commitmentParams.principalTokenAddress
            ).balanceOf(address(this));
            require(
