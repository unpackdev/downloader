...
contract EloPresale {
     ...
     * @dev Set the presale claim mode 
     */
    function setTotalEth(uint roundId, uint256 totalEth) external onlyOwner {
        if ( roundId == 1) {
            require(totalEth >= _r1TotalEthRaised, "Total Ether amount is smaller than already deposited amount");
            _r1TotalEth = totalEth;
        } else if ( roundId == 2) {
            require(totalEth >= _r2TotalEthRaised, "Total Ether amount is smaller than already deposited amount");
            _r2TotalEth = totalEth;
        } else {
