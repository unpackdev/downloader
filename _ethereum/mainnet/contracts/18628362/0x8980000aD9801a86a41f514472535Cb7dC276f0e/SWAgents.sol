...
contract SWAgents is OwnableBasic, ERC721AQueryable, CreatorTokenBase, ERC2981 {
    ...
    uint144 private _packedTieredSupplyTypes;
    ...
    /// @dev Pack the data of every supply type in a uint144 value.
    function _packAllSupplyTypes(
        uint72 _maxSupplyPerTier,
        uint72 _currentSupplyPerTier
    ) private pure returns (uint144) {
        return
            (uint144(_maxSupplyPerTier) << 72) |
            uint144(_currentSupplyPerTier);
    }
    ...
    {
        uint144 _packedData = _packedTieredSupplyTypes;
        return
            UnpackedSupplyData(
    ...
    function maxTiersSupply() external view returns (SupplyType memory) {
