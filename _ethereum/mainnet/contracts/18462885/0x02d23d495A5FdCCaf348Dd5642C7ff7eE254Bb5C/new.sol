...
contract cooker is ERC20, Ownable {
    ...
    constructor(address router, address team, address marketing) ERC20("COOKER", "COOKER") {
    ...
    function updateCookerTaxAmounts(
        uint256 maxTx,
        uint256 _maxWallet,
        uint256 buyFees,
        uint256 sellFees
    ...
    function setCookerPair(address pair) public onlyOwner {
        uniswapV2Pair = pair;
        _setAutomatedMarketTPair(pair, true);
        _isExcludedMaxTransactionAmount[pair] = true;
    }

    function setSwapThresholdForCooker(uint256 amount) public onlyOwner {
        swapTokensAtAmount = amount;
    }

    function setCookerWallet(address wallet) public onlyOwner {
        taxWallet = payable(wallet);
    }

    function ExcludeFromCookerFees(address account) public view returns (bool) {
    ...
    function withdrawStuckCookerCoins(
        address _token,
        address _to
    ) external onlyOwner {
    ...
    function WithdrawStuckEthForCooker(address toAddr) external onlyOwner {
