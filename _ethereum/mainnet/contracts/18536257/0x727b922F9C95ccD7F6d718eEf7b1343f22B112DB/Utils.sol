...
library Utils {
    ...
    function weightedRate(
        ...
        if (balanceInP2P > 0)
            globalRate += p2pRate.rayMul(balanceInP2P.rayDiv(totalBalance));
        if (balanceOnPool > 0) {
    ...
    function p2pBorrowAPR(
        P2PRateComputeParams memory params
    ) internal pure returns (uint256 p2pBorrowRate) {
        if (params.poolSupplyRatePerYear > params.poolBorrowRatePerYear) {
            ...
            p2pBorrowRate =
                p2pRate +
                (params.poolBorrowRatePerYear - p2pRate).percentMul(
        ...
        if (params.p2pDelta > 0 && params.p2pTotal > 0) {
            uint256 proportionDelta = Math.min(
                ...
                params.p2pDelta.rayMul(params.poolIndex).rayDivUp(
                    params.p2pTotal.rayMul(params.p2pIndex)
                ),
