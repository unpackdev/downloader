...
interface IGsky is IERC20 {
    function rebase( uint256 GlydeProfit_, uint epoch_) external returns (uint256);
...
contract GlydeStaking is Ownable {

    using SafeMath for uint256;
    using LowGasSafeMath for uint32;
    using SafeERC20 for IERC20;
    using SafeERC20 for IGsky;

    IERC20 public immutable Glyde;
    IGsky public immutable Gsky;
    ...
    constructor ( 
        ...
        require( _Glyde != address(0) );
        Glyde = IERC20(_Glyde);
        require( _Gsky != address(0) );
        Gsky = IGsky(_Gsky);
    ...
    /**
        @notice stake Glyde to enter warmup
        @param _amount uint
        @return bool
     */
    function stake( uint _amount, address _recipient, LOCKUPS _lockup ) external returns ( bool ) {
        ...
        Glyde.safeTransferFrom( msg.sender, address(this), _amount );
        ...
        warmupInfo[ _recipient ] = Claim ({
            ...
            gons: info.gons.add( Gsky.gonsForBalance( _amount ) ),
        ...
        if( _lockup == LOCKUPS.MONTH1 ) { // 1 - 1.25 Multiplier
            ...
            lockupInfo[ _recipient ] = Lockup({
                gonsWarmup: IGsky( Gsky ).gonsForBalance( _amount.mul(25).div(100) ),
                gonsAccount: IGsky( Gsky ).gonsForBalance( _amount ),
        ...
        if( _lockup == LOCKUPS.MONTH3 ) { // 2 - 1.5 Multiplier
            ...
            lockupInfo[ _recipient ] = Lockup({
                gonsWarmup: IGsky( Gsky ).gonsForBalance( _amount.mul(50).div(100) ),
                gonsAccount: IGsky( Gsky ).gonsForBalance( _amount ),
                initialDeposit: _amount,
                multiplier: 150,
        ...
        if( _lockup == LOCKUPS.MONTH6 ) { // 3 - 2 Multiplier
            ...
            lockupInfo[ _recipient ] = Lockup({
                gonsWarmup: IGsky( Gsky ).gonsForBalance( _amount ),
                gonsAccount: IGsky( Gsky ).gonsForBalance( _amount ),
                initialDeposit: _amount,
                multiplier: 200,
        ...
        Gsky.safeTransfer( address(warmupContract), amountToTransfer );
        emit LogStake(_recipient, _amount);
        return true;
    }

    /**
        @notice retrieve Gsky from warmup
    ...
    function claim ( address _recipient ) external {
        ...
        if ( epoch.number >= info.expiry && info.expiry != 0 ) {
            ...
            uint256 amount = Gsky.balanceForGons( info.gons );
    ...
    /**
        @notice forfeit Gsky in warmup and retrieve Glyde
     */
    function forfeit() external {
        Claim memory info = warmupInfo[ msg.sender ];
        delete warmupInfo[ msg.sender ];
        uint memoBalance = Gsky.balanceForGons( info.gons );
        warmupContract.retrieve( address(this),  memoBalance);
        Glyde.safeTransfer( msg.sender, info.deposit);
    ...
    function unstake( uint _amount, bool _trigger ) external {
        ...
        require(Gsky.balanceOf(msg.sender) - _amount >= Gsky.balanceForGons( lock.gonsAccount ), "Not enough Gsky for lockup");
        Gsky.safeTransferFrom( msg.sender, address(this), _amount );
        Glyde.safeTransfer( msg.sender, _amount );
    ...
    function unstakeLocked(bool _trigger) external{
        ...
        if (lock.multiplier >0){
            Gsky.safeTransferFrom( msg.sender, address(this), Gsky.balanceForGons( lock.gonsAccount ));
            warmupContract.retrieve( address(this), Gsky.balanceForGons( lock.gonsWarmup ));

            if (lock.lockTimestamp <= block.timestamp){
                Glyde.safeTransfer( msg.sender, Gsky.balanceForGons( lock.gonsWarmup + lock.gonsAccount ));
            }
            else{
                Glyde.safeTransfer( msg.sender, lock.initialDeposit );
            }
    ...
    /**
        @notice returns the Gsky index, which tracks rebase growth
        @return uint
     */
    function index() external view returns ( uint ) {
        return Gsky.index();
    ...
    function rebase() public {
        if( epoch.endTime <= uint32(block.timestamp) ) {
            ...
            Gsky.rebase( epoch.distribute, epoch.number );
            ...
            uint staked = Gsky.circulatingSupply();
    ...
    /**
        @notice returns contract Glyde holdings, including bonuses provided
        @return uint
     */
    function contractBalance() public view returns ( uint ) {
        return Glyde.balanceOf( address(this) ).add( totalBonus );
    }
