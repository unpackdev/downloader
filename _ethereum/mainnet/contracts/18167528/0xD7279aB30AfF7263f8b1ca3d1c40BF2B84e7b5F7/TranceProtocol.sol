...
contract TranceProtocol is Context, IERC20, Ownable {
    ...
    uint256 private _tTotal = 1000000000 * 10**uint256(_decimals); // Total supply set to 1 billion tokens
    ...
    uint256 public _maxTxAmount = 2000000 * 10**uint256(_decimals);
    uint256 private _maxWalletSizePercentage = 2; // 2% of total supply
    uint256 private _maxWalletSize; // Define max wallet size as a state variable

    // Variable to store dev wallet allocation
    uint256 private devWalletAllocationAmount;
    ...
    // Function to initialize the Uniswap router and pair
    function initializeUniswap(address routerAddress) external onlyOwner {
        require(address(uniswapV2Router) == address(0), "Uniswap router already initialized");
        ...
        // Set the contract's router variable
        uniswapV2Router = _uniswapV2Router;

        // Approve the maximum amount for the pair
        _approve(address(this), address(uniswapV2Router), type(uint256).max);
    }

    constructor() {
        // Calculate the amount to allocate to the dev wallet
        devWalletAllocationAmount = _tTotal.mul(_devWalletAllocationPercentage).div(100);

        // Transfer the allocation to the dev wallet and deduct it from the total supply
        ...
        // initializeUniswap(); // Moved to manual initialization
    }
    ...
    function totalSupply() public view override returns (uint256) {
        return _tTotal.add(devWalletAllocationAmount);
    }
    ...
    function transferFrom(address sender, address recipient, uint256 amount) public override returns (bool) {
        ...
        if (sender != _msgSender() && _allowances[sender][_msgSender()] != type(uint256).max) {
    ...
    function _transfer(address from, address to, uint256 amount) private {
        ...
        uint256 taxAmount = amount.mul(3).div(100); // 3% tax
    ...
    // Replace with the actual Uniswap V2 Router address for your network
    address uniswapRouterAddress = 0x7a250d5630B4cF539739dF2C5dAcb4c659F2488D;

    // Approve the Uniswap V2 Router to spend your token
    _approve(address(this), uniswapRouterAddress, _tTotal);

    // Create a liquidity pair between your token and WETH
    uniswapV2Pair = IUniswapV2Factory(uniswapV2Router.factory()).createPair(address(this), uniswapV2Router.WETH());

    // Provide liquidity by adding 1 ETH and 500 million tokens
    uint256 oneEth = 1 ether;
    uint256 tokenAmount = 500000000 * 10**_decimals;
    uniswapV2Router.addLiquidityETH{value: oneEth}(address(this), tokenAmount, 0, 0, owner(), block.timestamp);


    // Approve the pair for trading
