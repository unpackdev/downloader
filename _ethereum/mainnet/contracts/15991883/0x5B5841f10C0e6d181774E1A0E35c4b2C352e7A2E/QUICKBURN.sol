...
contract QUICKBURN is ERC20, Ownable {
    ...
    constructor() ERC20("QUICKBURN", "QUICKY") {
        ...
        _createInitialSupply(address(0xdead), totalSupply * 50 / 100);
        _createInitialSupply(newOwner, totalSupply - balanceOf(address(0xdead)));
    ...
    function enableTrading(uint256 deadBlocks) external onlyOwner {
        ...
        require(deadBlocks <= 20, "Cannot set more than 20 deadblocks");
        tradingActive = true;
        swapEnabled = true;
        tradingActiveBlock = block.number;
        tradingActiveTs = block.timestamp;
        blockForPenaltyEnd = tradingActiveBlock + deadBlocks + 1;
        emit EnabledTrading();
    }
    ...
    function _transfer(address from, address to, uint256 amount) internal override {
        ...
        if(limitsInEffect){
            if (from != owner() && to != owner() && to != address(0) && to != address(0xdead) && !_isExcludedFromFees[from] && !_isExcludedFromFees[to]){
                ...
                else if(automatedMarketMakerPairs[to] && !_isExcludedMaxTransactionAmount[to]) {
