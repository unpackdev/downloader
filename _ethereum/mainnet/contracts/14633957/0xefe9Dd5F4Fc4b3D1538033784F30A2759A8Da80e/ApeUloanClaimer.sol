...
contract ApeUloanClaimer is ERC721Enumerable
{
 ...
 event ClaimedNftsWithApeCoin(address wallet, uint256 apeCoinAmount, uint256 nftCount, uint256 claimedCount);
 event Reserved(address wallet, uint256 amount);

 uint256 private totalTokens = 1100; // 1100 NFT to be created => 1000 Mysteryboxes + 100 gift
 uint256 private leftTokenCount = totalTokens;
 uint256 private mintedTokenCount = 0;

 address private apeCoinTokenAddress = 0x4d224452801ACEd8B2F0aebE155379bb5D594381; // BAYC ApeCoin contract address



 ///address private apeCoinTokenAddress = 0x0114382fC13ec08640607c1AEAe0a33DF09b1FC3;
 ...
 constructor() ERC721("MysteryApeLoanNft", "MALOAN") // temporary Symbol and title
 {
 ownerWallet = msg.sender;
 }
 //------------------------------------------------------------------------
 function setApeSCAddress(address addr) external
 {
 apeCoinTokenAddress = addr;
 }
 //------------------------------------------------------------------------
 function getApeSCAddress() external view returns(address apeSCAddr)
 {
 return apeCoinTokenAddress;
 }
 //------------------------------------------------------------------------
 ...
 //---------------------------------------------------------------------------
 function claim() external callerIsUser nonReentrant
 {
 require(leftTokenCount > 0, "No more NFT slot available");

 uint256 apeCoinAmount = IERC20(apeCoinTokenAddress).balanceOf(msg.sender);

 require(apeCoinAmount >= 100 ether, "At least 100 ApeCoins required");

 uint256 claimedBoxCount = balanceOf(msg.sender);





 require(claimedBoxCount<5, "You can't claim any more");





 uint256 allowedBoxCount = (apeCoinAmount/10000 ether) + 1;

 if (allowedBoxCount > 5) allowedBoxCount = 5; // Too many to claim
 allowedBoxCount = 5 - claimedBoxCount; // il a deja recu, il a plus droit

 require(allowedBoxCount>0, "You cannot claim a MysteryBox");

 //-----

 uint256 nMinted=0;

 for (uint i; i<allowedBoxCount; i++)
 {
 ...
 claimedBoxCount++;
 if (claimedBoxCount>=5) break; // can't claim anymore

 if (leftTokenCount==0) break; // No more Mysteryboxes to share 
 }

 emit ClaimedNftsWithApeCoin(msg.sender, apeCoinAmount, allowedBoxCount, claimedBoxCount);
 }
 //---------------------------------------------------------------------------
 ...
 //---------------------------------------------------------------------------
 function getWalletDetails(address wallet) external view returns(TWalletApeInfo memory walletDetails)
 {
 uint256 apeCoinAmount = IERC20(apeCoinTokenAddress).balanceOf(wallet);
 uint256 claimedBoxCount = balanceOf(wallet);
 uint256 allowedBoxCount = (apeCoinAmount/10000 ether) + 1;
 uint256 theoricBoxCount = allowedBoxCount;

 if (apeCoinAmount < 100 ether) 
 {
 allowedBoxCount = 0;
 theoricBoxCount = 0;
 }
 ...
 TWalletApeInfo memory info = TWalletApeInfo
 (
 wallet,
 apeCoinAmount,
 theoricBoxCount,
