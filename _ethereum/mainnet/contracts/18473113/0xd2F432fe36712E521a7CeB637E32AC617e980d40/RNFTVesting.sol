...
contract RNFTVesting is ReentrancyGuard {
    ...
    function userClaimable() external view returns (uint128 _total) {
        uint256[] memory tokens = rnftv2.getUserTokens(msg.sender);

        for (uint16 i = 0; i < tokens.length; i++) {
            if (userClaimedIds[msg.sender][uint16(tokens[i])]) {
                _total += uint128(BENEFICIARY_TOKENS - rootClaimedByTokenId[uint16(tokens[i])]);
            }
    ...
    function userClaimed() external view returns (uint128 _total) {
        uint256[] memory tokens = rnftv2.getUserTokens(msg.sender);

        for (uint16 i = 0; i < tokens.length; i++) {
            if (userClaimedIds[msg.sender][uint16(tokens[i])]) {
                _total += uint128(rootClaimedByTokenId[uint16(tokens[i])]);
            }
        }
    }
}