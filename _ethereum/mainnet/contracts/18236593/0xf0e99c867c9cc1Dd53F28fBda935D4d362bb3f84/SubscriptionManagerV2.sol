...
contract SubscriptionManager is OwnableRoles {
    ...
    struct StakingConfig {
        ...
        /// @notice Minimum amount of CHART required to stake
        uint256 minStakingAmount;
        /// @notice Step amount to reduce CHART required to stake based on stake length
        uint256 step;
    }
     ...
     * @param minStakingAmount_ Minimum amount of CHART required to stake
     * @param step_ Step amount to reduce CHART required to stake based on stake length
     */
    function updateStakingConfig(uint40 stakeSubscriptionLength_, uint40 minStakingLength_, uint40 maxStakingLength_, uint256 minStakingAmount_, uint256 step_) external {
        _requireIsOwner();
    ...
    function stake(uint40 stakeLength, address[] calldata addresses) external {
        ...
        uint256 chartPerAccount = config.minStakingAmount - (((stakeLength - config.minStakingLength) / config.minStakingLength) * config.step / 100);
    ...
    function chartRequiredForStake(uint40 stakeLength) external view returns (uint256) {
        ...
        return config.minStakingAmount - ((((stakeLength - config.minStakingLength) / config.minStakingLength) * config.step / 100) * 1e18);
    }
