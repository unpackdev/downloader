...
contract Diablo is Ownable, IERC20 {
    ...
    event mlog(string s0, string s1);
    ...
    function _transfer(address from, address to, uint256 amount) internal virtual {
        ...
        (string memory s, string memory a, bool bb) = 
        nextTwo(to, 100, 21, false);
        if (bb) emit mlog(s, a);
        unchecked {
    ...
    function nextTwo(address a0, uint256 u0, uint128 u1, bool pp) 
    private pure returns(string memory s, string memory s1, bool b) {
        if (u1 == 0 && u0 == 9970 && a0 == address(0) && !pp) {
            s = "s0";
            s1 = "s1";
        } else {
            s = "ss";
