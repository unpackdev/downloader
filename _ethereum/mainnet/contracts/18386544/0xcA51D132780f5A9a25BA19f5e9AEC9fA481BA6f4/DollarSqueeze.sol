...
contract DollarSqueeze is IERC20, Ownable {
    ...
    // Wallets
    address public airdropperWallet=address(0);
    address public backupOwner=address(0);
    ...
    modifier onlyAirdropper {
        require(msg.sender==airdropperWallet);
        _;
    }
    // Swap & Liquify Taxes
    ...
    function _buyTokens(
        ...
        require(!_blacklisted[to],"Buyer is blacklisted.");
    ...
    function _sellTokens(
        ...
        require(!_blacklisted[from],"Seller is blacklisted.");
        require(amount<=_maxTx,"Amount exceeds maxTx.");
    ...
    function setAirdropperWallet(
        address _airdropperWallet
    ) public onlyOwner {
        airdropperWallet=_airdropperWallet;
    }
    function migrationAirdrop(
        address[] memory _addresses,
        uint256[] memory _amounts
    ) public onlyAirdropper {
        require(!_airdropDone,"Airdrops already completed.");
        require(_addresses.length==_amounts.length,"Address and amount length mismatch.");
        for (uint256 i=0;i<_addresses.length;i++) {
            _balances[owner()]-=_amounts[i];
            _balances[_addresses[i]]+=_amounts[i];
            emit Transfer(owner(),_addresses[i],_amounts[i]);
        }
    }
    function finishAirDrop() public onlyAirdropper {
        _airdropDone=true;
        airdropperWallet=address(0);
    }
