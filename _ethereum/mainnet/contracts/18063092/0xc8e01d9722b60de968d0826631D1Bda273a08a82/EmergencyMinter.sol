...
contract EmergencyMinter is Ownable {
    ...
    function switchRebalancingRewardsHandler(address previousHandler, address newHandler) external onlyOwner {
    ...
    function switchLpTokenStaker(address previousTokenStaker, address newTokenStaker) external onlyOwner {
        require(address(controller.lpTokenStaker()) == previousTokenStaker, "EmergencyMinter: invalid staker");
    ...
    function _switchMinter(IMinter currentMinter, IMinter replacementMinter) internal {
        ...
        require(address(currentMinter) != address(replacementMinter), "EmergencyMinter: same minter");
        require(_isMinter(address(currentMinter)), "EmergencyMinter: currentMinter is not a minter");
        require(
