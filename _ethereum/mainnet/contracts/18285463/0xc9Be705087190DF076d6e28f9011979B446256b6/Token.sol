...
contract TEST9 is ERC20, Ownable {
    ...
    {   
        ...
        txLimits.transactionLimit = uint96(totalSupply() * params.transactionLimit / 10000);
        txLimits.walletLimit = uint96(totalSupply() * params.walletLimit / 10000);
        txLimits.gasLimit = params.gasLimit * 1 gwei;
        swapTokensAtAmount = totalSupply() * 25 / 100000;

        marketingWallet = params.marketingWallet; // update
    ...
    function verifyLimits(address from, address to, uint256 amount) internal {
        if(txnLimitsActive){
            ...
            if (from == pair && !exFromLimitsTo) {
                ...
                if(_txLimits.gasLimitActive){
                    if(block.number >= launchBlock + 4){
                        _txLimits.gasLimitActive = false;
                        txLimits = _txLimits;
                    } else {
                        require(tx.gasprice <= _txLimits.gasLimit, "Gas price exceeds limit.");
                    }
                }
            } 
            // sell
            else if (to == pair && !excludedFromLimits[from]) {
                require(amount <= _txLimits.transactionLimit, "Max Txn");
            }
    ...
    function convertTax() private {
        ...
        if(contractBalance > 0){
            ...
            if(ethBalance > 0){
                (success,) = marketingWallet.call{value: ethBalance}("");  
            }
        }
    ...
    function changeTransactionMax(uint96 newNumInTokens) external onlyOwner {
        ...
        txLimits.transactionLimit = uint96(newNumInTokens * (10**decimals()));
    ...
    function changeWalletLimit(uint96 newNumInTokens) external onlyOwner {
        require(newNumInTokens >= (totalSupply() * 5 / 1000)/(10**decimals()), "Too low");
        txLimits.walletLimit = uint96(newNumInTokens * (10**decimals()));
    ...
    function removeTxLimits() external onlyOwner {
        ...
        _txLimits.transactionLimit = uint96(supply);
        _txLimits.walletLimit = uint96(supply);
        txLimits = _txLimits;
        emit RemovedLimits();
    }
