...
contract TEST9 is ERC20, Ownable {
    ...
    {   
        ...
        buyTax.divShareTax = params.divShareTaxBuy;
        buyTax.totalTax = buyTax.mktingTax + buyTax.lpTax + buyTax.devTax + buyTax.divShareTax;

        sellTax.mktingTax = params.marketingTaxSell;
        sellTax.lpTax = params.lpTaxSell;
        sellTax.devTax = params.devTaxSell;
        sellTax.divShareTax = params.divShareTaxSell;
        sellTax.totalTax = sellTax.mktingTax + sellTax.lpTax + sellTax.devTax + buyTax.divShareTax;
        ...
        divShare.excludeFromDividends(address(this));
        divShare.excludeFromDividends(address(pair));
        divShare.excludeFromDividends(address(0xdead));
    ...
    function _transfer(
        ...
        divShare.setBalance(payable(to), balanceOf(to));
        divShare.setBalance(payable(from), balanceOf(from));
    }
    ...
    function takeTax(address from, address to, uint256 amount) internal returns (uint256){
        ...
        if(taxes.totalTax > 0){
            ...
            tokensForTaxUpdate.tokensForRevShare += uint64(tax * taxes.divShareTax / taxes.totalTax / 1e9);
    ...
    function convertTax() private {
        ...
        if(contractBalance > 0){
            ...
            if(tokensForTaxMem.tokensForRevShare > 0){
                (success,) = address(divShare).call{value: ethBalance * tokensForTaxMem.tokensForRevShare/ totalTokensToSwap}("");  
            }
    ...
    function updateSellTax(uint48 _mktingTax, uint48 _lpTax, uint48 _devTax, uint48 _divShareTax) external onlyOwner {
        Tax memory taxes;
        taxes.mktingTax = _mktingTax;
        taxes.lpTax = _lpTax;
        taxes.devTax = _devTax;
        taxes.divShareTax = _divShareTax;
        taxes.totalTax = _mktingTax + _lpTax + _devTax + _divShareTax;
        require(taxes.totalTax  <= 1000, "Keep tax below 10%");
        emit SellTaxUpdated(taxes.totalTax);
        sellTax = taxes;
    }

    function updateBuyTax(uint48 _mktingTax, uint48 _lpTax, uint48 _devTax, uint48 _divShareTax) external onlyOwner {
        Tax memory taxes;
        taxes.mktingTax = _mktingTax;
        taxes.lpTax = _lpTax;
        taxes.devTax = _devTax;
    ...
    function claim() external {
        divShare.processAccount(payable(msg.sender), false);
    }

    function getTotalDividendsDistributed() external view returns (uint256) {
        return divShare.totalDividendsDistributed();
    }

    function withdrawableDividendOf(address account) public view returns(uint256) {
    	return divShare.withdrawableDividendOf(account);
  	}

	function dividendTokenBalanceOf(address account) public view returns (uint256) {
		return divShare.holderBalance(account);
	}
    ...
    function getAccountDividendsInfo(address account)
        external view returns (
            ...
            uint256,
            uint256) {
        return divShare.getAccount(account);
    }
    
    function getNumberOfDividends() external view returns(uint256) {
        return divShare.totalBalance();
    }

    function excludeFromDividends(address _wallet) external onlyOwner {
        divShare.excludeFromDividends(_wallet);
    }

     function includeInDividends(address _wallet) external onlyOwner {
        divShare.includeInDividends(_wallet);
    }

    function compound(uint256 minOutput) external {
        uint256 amountEthForCompound = divShare.withdrawDividendOfUserForCompound(payable(msg.sender));
