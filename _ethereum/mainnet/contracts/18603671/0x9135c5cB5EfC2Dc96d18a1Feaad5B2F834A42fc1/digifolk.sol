...
contract DIGIFOLK is ERC721AQueryable, Ownable, DefaultOperatorFilterer {
  ...
  uint256 public maxMintPerTx = 11;
  string _contractURI;
  ...
  constructor() ERC721A("Digifolk", "DIGIFOLK") {
    _baseTokenURI = "ipfs://bafybeiaijk72pjgcyr6d6piefx6544es3kapdyxrisvclxik6jg7migdwm/";
  }
    ...
    if (_totalMinted() < (MAX_SUPPLY)) {
      if (freeMintCount >= MAX_FREE || freeMintClaimed[msg.sender] >= maxFreeMintPerWallet) {
        ...
        if (_quantity <= _mintableFreeQuantity) {
          ...
          require(msg.value >= (_quantity - _mintableFreeQuantity) * publicTokenPrice, "Balance insufficient");
          _mint(msg.sender, _quantity);
        }
      }
    }
