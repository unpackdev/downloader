...
contract MstWrapper is IMstWrapper, ERC20, ReentrancyGuard, Multicall {
    ...
    /**
     * @notice Migrate from wrapped tick to native tokenization
     */
    function migrate() external nonReentrant {
        ...
        /* Revert if 0 balance */
        if (amount == 0) revert NoSharesToMigrate();

        /* Burn shares */
        _burn(msg.sender, amount);

        /* Transfer deposit tokens owned by wrapper to msg.sender */
        address depositToken = ERC20DepositToken(address(_pool)).depositToken(_tick);
        require(IERC20(depositToken).transfer(msg.sender, amount));
