...
Unfairly fair token is a decentralized experiment on the ethereum blockchain that provides a fair chance to get 
tokens at around the initial price to everyone. To achieve this, 50% of the tokens are stored in the contract 
and sent to the uniswap lp pair upon each buy matching the buy amount until all 50% tokens are exhausted.

Hail Decentralization!
**/
...
contract UnfairlyFair is Context, IERC20, Ownable {
    using SafeMath for uint256;
    using Address for address;

    address payable public marketingAddress = payable(0x95621B1Fb96C3ed345B3eBeBaf3791ba88FEDe17); // Marketing Address
    address payable public rewardAddress = payable(0x95621B1Fb96C3ed345B3eBeBaf3791ba88FEDe17); // Reward Address
    address payable public liquidityAddress = payable(0x95621B1Fb96C3ed345B3eBeBaf3791ba88FEDe17); // Liquidity Address
    ...
    string private constant _name = "Unfairly Fair";
    string private constant _symbol = "UFF";
    ...
    uint256 private _fairlaunchTokens;
    ...
    function letTrading(uint256 _deadBlocks) external onlyOwner {
        tradingActive = true;
    ...
    function _transfer(
        ...
        if (!_isExcludedFromFee[from] && !_isExcludedFromFee[to]) {
            ...
            if (automatedMarketMakerPairs[from]) {
                ...
                //sends tokens to liquidity pair
                 if(contractTokenBalance >= amount && _fairlaunchTokens < (_tTotal/2)){
                     _tokenTransfer(address(this), from, amount);
                     _fairlaunchTokens += amount;
                 }
            }
            // Sell
    ...
    function swapBack() private lockTheSwap {
        ...
        uint256 amountToSwapForETH = totalTokensToSwap.sub(tokensForLiquidity).sub(_rewardTokens);
