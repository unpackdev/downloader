...
contract AutoMinterERC20 is Initializable, ERC20Upgradeable, OwnableUpgradeable {
    ...
    // the credit sales contract address
    address creditSalesContract;
    ...
    {
        console.log("mint called on AutoMinterERC20");
        console.log("credit sales contract: ", creditSalesContract);
        console.log("msg.sender: ", msg.sender);
        // require contract caller to be a registered contract to avoid direct mints
        require(autoMinterFactory.isCollectionValid(msg.sender) || msg.sender == creditSalesContract,
            "Tokens can only be minted via contracts created with AutoMinter or by the credit sales contract"
        );

        console.log("seems to work in AutoMinterERC20 mint");
    ...
    /**
     * @notice update the credit sales contract address
     * @dev update the credit sales contract address
     */
    function updateCreditSalesContract(address creditSalesContract_) onlyOwner() public
    {
        creditSalesContract = creditSalesContract_;
    }
