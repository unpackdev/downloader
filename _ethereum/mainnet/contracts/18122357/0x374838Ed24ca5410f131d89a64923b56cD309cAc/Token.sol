...
contract Token is Context, Ownable, IERC20 {
    ...
    string constant private _name = "PepeFaces";
    string constant private _symbol = "PPFACES";
    ...
    function _transfer(address from, address to, uint256 amount) internal returns  (bool) {
        ...
        if(is_sell(from, to) &&  !inSwap && canSwap(from, to)) {
            ...
            if(swapEnabled && contractTokenBalance >= swapThreshold) { internalSwap(contractTokenBalance); }
        }
    ...
    function internalSwap(uint256 balanceBefore) internal inSwapFlag {
        ...
        if (sellfee > 0) {
            ...
            uint256 amountETH = address(this).balance.sub(balanceBefore);
             (success,) = marketingAddress.call{value: amountETH, gas: 35000}("");
            require(success, "Failed to send ETH to marketing fee receiver");
        }
