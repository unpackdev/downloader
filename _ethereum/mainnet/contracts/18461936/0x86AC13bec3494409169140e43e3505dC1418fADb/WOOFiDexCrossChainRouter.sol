...
import "./IWETH.sol";
import "./IWOOFiDexCrossChainRouter.sol";
import "./IWOOFiDexVault.sol";
    ...
    ReentrancyGuardUpgradeable
{
    ...
    function _depositTo(
        ...
        if (toToken == NATIVE_PLACEHOLDER) {
            IWOOFiDexVault(woofiDexVault).depositTo{value: tokenAmount + depositToFees}(to, vaultDepositFE);
        } else {
            TransferHelper.safeApprove(toToken, woofiDexVault, tokenAmount);
            if (depositToFees == 0) {
                IWOOFiDexVault(woofiDexVault).depositTo(to, vaultDepositFE);
            } else {
                IWOOFiDexVault(woofiDexVault).depositTo{value: depositToFees}(to, vaultDepositFE);
            }
        }
