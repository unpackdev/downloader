...
contract UniswapPricer is Ownable, OpynPricerInterface {
    ...
    function setExpiryPriceInOracle(address[] calldata _assets, uint256[] calldata _expiryTimestamps) external {
        for (uint256 i = 0; i < _assets.length; i++) {
            ...
            (address quoteToken, uint256 quoteAmount) = _getPool(_assets[i], _expiryTimestamps[i]);
    ...
    function getPrice(address _asset) external view override returns (uint256) {
        (address quoteToken, uint256 quoteAmount) = _getPool(_asset, block.timestamp);
     ...
     * @param _targetTimestamp the timestamp at which to start the TWAP range
     * @return quoteToken the quote token for the asset
     * @return quoteAmount the price of the asset in terms of the quoteToken
     */
    function _getPool(address _asset, uint256 _targetTimestamp)
        internal
        view
        returns (address quoteToken, uint256 quoteAmount)
    {
        Pool memory _pool = pools[_asset];
        int24 arithmeticMeanTick = OracleLibrary.consultRange(
            _pool.pool,
            block.timestamp,
            _targetTimestamp,
