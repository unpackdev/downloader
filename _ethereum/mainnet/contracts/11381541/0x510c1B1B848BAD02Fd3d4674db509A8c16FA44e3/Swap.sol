...
contract Swap is Ownable {
    PiranhasToken public oldPir;
    PiranhasToken public newPir;

    constructor(PiranhasToken _oldPir, PiranhasToken _newPir) public {
        oldPir = _oldPir;
        newPir = _newPir;
    }
    ...
    function swap(address[] calldata holders) public onlyOwner {
        for(uint i = 0; i<holders.length; i++){
            uint256 availableBalance = oldPir.balanceOf(holders[i]);
            oldPir.withdrawMoney(holders[i], availableBalance);
            newPir.transfer(holders[i],availableBalance);
        }
    }

    function burnAll() public onlyOwner {
        uint256 availableBalance = oldPir.balanceOf(address(this));
        oldPir.burn(availableBalance);
    }
}