    ...
    IRadiantStaking
{
    ...
    modifier _onlyAuthorizedUser() {
        if (!allowedUsers[msg.sender]) revert NotAuthorizedUser();
        _;
    }
    ...
    ) external nonReentrant _onlyAuthorizedUser {
    ...
    function addAuthorizeUser(address _authorizeUser) external onlyOwner {
        if (allowedUsers[_authorizeUser]) revert AlreadyRegistered();
        allowedUsers[_authorizeUser] = true;
    }
