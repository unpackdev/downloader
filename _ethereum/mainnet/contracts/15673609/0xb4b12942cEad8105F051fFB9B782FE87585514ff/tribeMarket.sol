...
// File contracts/tribeMarket.sol
...
contract tribeMarket is ReentrancyGuard, Pausable, Ownable {
    uint256 constant public PERCENTS_DIVIDER = 10000;

    IERC721 erc721;     // instance of the Tribe contract
    ...
    // A record of the highest tokens bid
    ...
    /* Initializes contract with an instance of Tribe contract, and sets deployer as owner */
    constructor(address initialAddress) {
        IERC721(initialAddress).balanceOf(address(this));
        erc721 = IERC721(initialAddress);
    }
    ...
    /* Allows the owner of a Tribe to stop offering it for sale */
    function cancelForSale(uint id) external onlyTokenOwner(id) {
        offers[id] = Offer(false, id, msg.sender, 0, address(0x0));
        emit Cancelled(id);
    }

    /* Allows a Tribe owner to offer it for sale */
    function offerForSale(uint id, uint minSalePrice) external onlyTokenOwner(id) whenNotPaused {
        offers[id] = Offer(true, id, msg.sender, minSalePrice, address(0x0));
        emit Offered(id, minSalePrice, address(0x0));
    }

    /* Allows a Tribe owner to offer it for sale to a specific address */
    function offerForSaleToAddress(uint id, uint minSalePrice, address toAddress) external onlyTokenOwner(id) whenNotPaused {
        offers[id] = Offer(true, id, msg.sender, minSalePrice, toAddress);
        emit Offered(id, minSalePrice, toAddress);
    }
    

    /* Allows users to buy a Tribe offered for sale */
    function buyApes(uint id) payable external whenNotPaused nonReentrant() {
        ...
        require (seller == erc721.ownerOf(id), 'seller no longer owner of token');

        offers[id] = Offer(false, id, msg.sender, 0, address(0x0));
        
        // Transfer Tribe to msg.sender from seller.
        erc721.safeTransferFrom(seller, msg.sender, id);
    ...
    /* Allows users to enter bids for any Tribe */
    function placeBid(uint id) payable external whenNotPaused nonReentrant() {
        require (erc721.ownerOf(id) != msg.sender, 'you already own this token');
    ...
    /* Allows Tribe owners to accept bids for their Tokens */
    function acceptBid(uint id, uint minPrice) external onlyTokenOwner(id) whenNotPaused nonReentrant() {
        ...
        erc721.safeTransferFrom(msg.sender, bidder, id);
