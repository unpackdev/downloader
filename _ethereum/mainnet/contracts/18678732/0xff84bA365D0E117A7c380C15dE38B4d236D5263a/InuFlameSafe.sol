...
// File @openzeppelin/contracts/interfaces/IERC721Receiver.sol@v5.0.0
...
// File contracts/INonfungiblePositionManager.sol
...
// File contracts/InuFlameSafe.sol

// Original license: SPDX_License_Identifier: MIT
pragma solidity ^0.8.19;

contract InuFlameSafe is IERC721Receiver {
    
    address public caller;
    INonfungiblePositionManager public v3PositionManagerContract;
    IUniswapV3Pool public inuPool;
    address internal BURN_ADDRESS = 0xdEAD000000000000000042069420694206942069;
    IInuBurn inuBurn;
    IERC20 public InuFlame;
    ...
    constructor(address _token0, address _v3PositionManagerContract, address _pool, address _inuBurn, address _caller) { 
        InuFlame = IERC20(_token0);
        v3PositionManagerContract = INonfungiblePositionManager(_v3PositionManagerContract);
        inuPool = IUniswapV3Pool(_pool);
        caller = _caller;
        inuBurn = IInuBurn(_inuBurn);
    }
    ...
    function _distribute() internal {
        uint256 inuFLameToBurn = InuFlame.balanceOf(address(this));
        uint256 wethToTransfer = WETH.balanceOf(address(this));

        InuFlame.transfer(BURN_ADDRESS, inuFLameToBurn);
        //Transfer WETH to Butn contract
        WETH.transfer(address(inuBurn),wethToTransfer);
    ...
    function estimateUncollectedFees(uint256 _tokenId) external view returns (uint256, uint256) {
        ...
        uint256 global0 = inuPool.feeGrowthGlobal0X128();
        uint256 global1 = inuPool.feeGrowthGlobal1X128();
    ...
    // unstuck Token
    function rescueErc(address _tokenAddress) external onlyCaller {
    ...
    function setBurnContract(address _burnContract) external onlyCaller {
        require(_burnContract != address(0), "zero address");
        inuBurn = IInuBurn(_burnContract);
    }
