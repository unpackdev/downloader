...
abstract contract Shareholders {
    ...
    event PayoutErc20(address indexed _to, address _tokenAddress, uint256 _value);
    ...
    /**
     * @dev Once the royalty contract has a ERC-20 balance, call this to payout a specific token to the shareholders
     */
    function payout(address tokenAddress) public {
        IERC20 token = IERC20(tokenAddress);
        uint256 balance = token.balanceOf(address(this));
        require(balance > 0, "No balance found for token address");
        for (uint256 i = 0; i < shareholders.length; i++) {
            // 10,000 shares represents 100.00% ownership
            uint256 amount = (balance * shareholders[i].share) / TOTAL_SHARES;
            // transfer the amount of tokens to the shareholder's address
            require(token.transfer(shareholders[i].shareholder_address, amount), "Transfer failed.");

            emit PayoutErc20(shareholders[i].shareholder_address, tokenAddress, amount);
        }
    }
