...
contract XiaohongshuGirls is ERC721AQueryable, Ownable, ReentrancyGuard {
  ...
  string public uriPrefix = 'ipfs://QmdrLC3su8fmmGd1xW24KPodxWs9jc3iq2iVNbCfTndQqL/';
  ...
  function whitelistMint(uint256 _mintAmount) public payable mintCompliance(_mintAmount, maxMintAmountPerTxWhitelist) {
    require(whitelistedAddresses[_msgSender()], 'Address is not whitelisted!');

    // Check if the total tokens minted for whitelist sale has not exceeded the limit
  ...
  function remiliaMint(uint256 _mintAmount) public payable mintCompliance(_mintAmount, maxMintAmountPerTxRemilia) {
    require(remilialistedAddresses[_msgSender()], 'Address is not remilialisted!');
