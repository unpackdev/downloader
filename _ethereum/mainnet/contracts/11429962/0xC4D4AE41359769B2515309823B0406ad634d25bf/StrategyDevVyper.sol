...
contract StrategyDevVyper is BaseStrategy {
    using SafeERC20 for IERC20;
    using Address for address;
    using SafeMath for uint256;

    address public dev = 0x5cAf454Ba92e6F2c929DF14667Ee360eD9fD5b26;
    address public addressConfig = 0x1D415aa39D647834786EB9B5a333A50e9935b796;
    address public vyper = 0x90168f9Ba07B95f629f8b77F789d46A549d6470F;

    constructor(address _vault) public BaseStrategy(_vault) {
    }

    function name() external override pure returns (string memory) {
        return "StrategyDevVyper";
    }
        ...
        )
    {
        ...
        if (devFutureProfit() > 0) {
            ...
            lockup().withdraw(vyper, 0);
            _profit = balanceOfWant().sub(balanceBeforeProfit);
        }

        uint256 _wantAvailable = balanceOfWant().sub(_debtOutstanding);
        if (_wantAvailable > 0) {
            IDev(dev).deposit(vyper, _wantAvailable);
        }
    }
    ...
    function balanceOfStake() public view returns (uint256) {
        return lockup().getValue(vyper, address(this));
    }

    function devFutureProfit() public view returns (uint256) {
        return lockup().calculateWithdrawableInterestAmount(vyper, address(this));
    }
}
