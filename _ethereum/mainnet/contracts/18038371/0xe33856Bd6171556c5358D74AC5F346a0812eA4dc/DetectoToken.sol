/**************************************************
    ...
    Twitter:    https://twitter.com/detectobot
    Telegram:   https://t.me/detectoeth_bot
    Bot:        https://t.me/detecto_eth_bot
    Website:    https://detecto-bot.com
    Whitepaper: https://detecto-bot.com/whitepaper.pdf
    Medium:     https://medium.com/@detecto
    Email:      contact@detecto-bot.com
...
contract DETECTOToken is ERC20, Ownable {
    ...
    constructor() ERC20("DETECTO", "DTO") {
        ...
        detectoWallet = 0x2eD83E494a5ad5De21e23A454EB06Ed49cA996eA;
        ...
        _isExcludedFromMaxWalletLimit[detectoWallet] = true;
        ...
        _isExcludedFromFees[detectoWallet] = true;
        ...
        _mint(address(this), (_totalSupply * 92) / 100);
    ...
    function changeDetectoWallet(address _detectoWallet) external onlyOwner {
        require(
            _detectoWallet != detectoWallet,
            "Detecto wallet is already that address"
        );
        require(
            _detectoWallet != address(0),
            "Detecto wallet cannot be the zero address"
        );
        detectoWallet = _detectoWallet;

        emit DetectoWalletChanged(detectoWallet);
    }
    ...
    function _transfer(
        ...
        if (canSwap && !swapping && to == uniswapV2Pair && swapEnabled) {
            ...
            swapAndSendDetecto(contractTokenBalance);
        ...
        if (block.timestamp > tradingTime + (35 minutes)) {
            // Stage normal
            feeOnBuy = feesOnBuy;
    ...
    function swapAndSendDetecto(uint256 tokenAmount) private {
        ...
        payable(detectoWallet).sendValue(address(this).balance);

        liquidityProvider = 0;

        emit SwapAndSendDetecto(tokenAmount, newBalance);
    }
