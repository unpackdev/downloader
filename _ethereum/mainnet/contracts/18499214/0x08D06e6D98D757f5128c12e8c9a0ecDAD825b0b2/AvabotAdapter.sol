...
abstract contract AvabotAdapter is Maintainable {
    using SafeERC20 for IERC20;

    event AvabotAdapterSwap(address indexed _tokenFrom, address indexed _tokenTo, uint256 _amountIn, uint256 _amountOut);
    ...
    function recoverERC20(address _tokenAddress, uint256 _tokenAmount) external onlyMaintainer {
        require(_tokenAmount > 0, "AvabotAdapter: Nothing to recover");
        IERC20(_tokenAddress).safeTransfer(msg.sender, _tokenAmount);
        emit Recovered(_tokenAddress, _tokenAmount);
    }

    function recoverNative(uint256 _amount) external onlyMaintainer {
        require(_amount > 0, "AvabotAdapter: Nothing to recover");
    ...
    function isUniV2like() external pure virtual returns(bool) {
        return false;
    }
}
