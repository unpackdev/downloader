...
contract GovernanceDataLayout is LibraryLock {
    using SafeERC20 for IERC20;
    using SafeMath for uint256;
    
    NyanV2 public nyanV2;
    address public nyanV2LP;
    
    struct status {
        bool isAdmin;
    }
    mapping(address => status) public currentAdmins;
    ...
    mapping(address => LPVotes) public stakerVotes;
    ...
    mapping(address => nyanVoted) public nyanVotes;
    ...
    mapping(address => proposal) public eachProposal;
    ...
    event nyanWithdrawn(address indexed voter, uint256 amount);
...
contract Governance is GovernanceDataLayout, Proxiable {
    constructor() public {
        
    }

    function govConstructor(address _nyanAddress, address _nyanV2LP) public {
        require(!initialized);
        currentAdmins[msg.sender].isAdmin = true;
        nyanV2 = NyanV2(_nyanAddress);
        nyanV2LP = _nyanV2LP;
        initialize();
    }
    ...
    /** @notice Updates the logic contract.
      ...
      */
    function updateCode(address newCode) public _isAdmin delegatedOnly  {
        updateCodeAddress(newCode);
        
        emit logicContractUpdated(newCode);
    }

    function setNyanV2(address _addr) public _isAdmin delegatedOnly {
        nyanV2 = NyanV2(_addr);
    }
    
    function setNyanV2LP(address _addr) public _isAdmin delegatedOnly {
        nyanV2LP = _addr;
    }
    ...
    function getProposal(address _proposal) public view returns(
        string memory,
        address,
        string[] memory,
        string[] memory,
    ...
    function getVoteCount(address _staker) public view returns(uint256) {
    ...
    function voteForProposal(uint256 _amount, uint256 _optionIndex, address _proposalAddr) public delegatedOnly {
        uint256 totalVotes;
        uint256 dNyan;
        uint256 rewards;
        uint256 blockChecked;
        uint256 blockStaked;
        (totalVotes,dNyan, rewards, blockChecked, blockStaked) = nyanV2.userStake(msg.sender);
        require(block.number < eachProposal[_proposalAddr].endBlock, "Voting is already over!");
        if (!stakerVotes[msg.sender].votesInitialized) {
            stakerVotes[msg.sender].totalVotes = totalVotes;
            stakerVotes[msg.sender].votesInitialized = true;
        }
        if (block.number.sub(stakerVotes[msg.sender].lastBlockChecked) > 5500) {
            stakerVotes[msg.sender].totalVotes = totalVotes;
            stakerVotes[msg.sender].currentVotesUsed = 0;
            stakerVotes[msg.sender].lastBlockChecked = block.number;
        }
        // uint256 currentVotes = stakerVotes[_staker].totalVotes.sub(stakerVotes[_staker].currentVotesUsed);
        require(stakerVotes[msg.sender].currentVotesUsed.add(_amount) <= stakerVotes[msg.sender].totalVotes);
        stakerVotes[msg.sender].currentVotesUsed = stakerVotes[msg.sender].currentVotesUsed.add(_amount);
       
        nyanVotes[msg.sender].votes = nyanVotes[msg.sender].votes.add(_amount);
        nyanVotes[msg.sender].proposalAddr = _proposalAddr;
        nyanVotes[msg.sender].proposalQuestion = eachProposal[_proposalAddr].proposalQuestion;
        nyanVotes[msg.sender].option = eachProposal[_proposalAddr].options[_optionIndex];
        ...
        emit votedForProposal(msg.sender, eachProposal[_proposalAddr].proposalQuestion, nyanVotes[msg.sender].option);
    }
    
    
    function addAdmin(address _admin) public _isAdmin delegatedOnly {
        currentAdmins[_admin].isAdmin = true;
        
        emit newAdminAdded(msg.sender, _admin);
    }
    
    function removeAdmin(address _admin) public _isAdmin delegatedOnly {
