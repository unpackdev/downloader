...
contract ConvexAllocator is Ownable {
     ...
     *  @param pid uint256
     */
    function addPidRewardItem(address _rewardPool, uint256 pid) external onlyPolicy {
     ...
     *  @param pid uint256
     */
    function removePidRewardItem(address _rewardPool, uint256 pid) external onlyPolicy {
    ...
    function queueFtmAddress(uint _chainId,address _ftmAddress) external onlyPolicy{
        require(_ftmAddress != address(0) && _chainId != 0,"invalid chainid or address");
    ...
    function setFtmAddress() external onlyPolicy{
        require(ftmAddressCandidate!=address(0) && chainIdCandidate!=0,"put new address in queue first");
