    ...
    ReentrancyGuardUpgradeable
{
    ...
    IStaking public tribotStaking;

    // New Storage
    mapping(address => mapping(uint256 => uint256))
        public totalRewardShareTribot;
    mapping(address => mapping(uint256 => uint256)) public totalRewardShareUsdt;
    mapping(address => mapping(uint256 => uint256)) public totalRewardSharePlus;
    ...
    function depositRewards(address _token, uint256 _amount) external payable {
        ...
        totalRewardShareTribot[_token][totalRewardCount[_token]] = _amount
            .mul(25)
            .div(100);
        totalRewardShareUsdt[_token][totalRewardCount[_token]] = _amount
            .mul(25)
            .div(100);
        totalRewardSharePlus[_token][totalRewardCount[_token]] = _amount
            .mul(50)
            .div(100);

        rewardAddTime[_token] = block.timestamp;
        usdtStaking.updateRewardRecord(_token, totalRewardCount[_token]);
        if (address(tribotStaking) != address(0)) {
            tribotStaking.updateRewardRecord(_token, totalRewardCount[_token]);
        }
    ...
    function getRewardPoolShare(
        ...
        require(address(0) != _user, "Invalid User!");
        (uint256 usdtStakedAmount, , ) = usdtStaking.calculateTotalStakedInfo(
            _user
        );
        (uint256 tribotStakedAmount, , , , , , , , ) = tribotStaking.users(
            _user
        );

        if (usdtStakedAmount != 0) {
            currentShare += 2500;
        }
        if (tribotStakedAmount != 0) {
            currentShare += 2500;
        }
    }
    ...
    function setribotStaking(IStaking _staking) external onlyOwner {
        tribotStaking = _staking;
    }
}
