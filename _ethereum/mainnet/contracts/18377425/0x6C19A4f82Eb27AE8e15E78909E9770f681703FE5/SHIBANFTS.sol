    ...
    DefaultOperatorFilterer
{
    ...
    function mint(uint256 _mintAmount) public payable {
        // Checks
        require(!paused, "SHIBANFTS: minting has not started yet");
        require(_mintAmount > 0, "SHIBANFTS: you have to mint at least one");
        require(
            _mintAmount <= maxMintAmount,
            "SHIBANFTS: you can only mint 10 at a time"
        );
        require(
            lastSupply >= _mintAmount,
            "SHIBANFTS: the collection is sold out"
        );
        if (msg.sender != owner()) {
            require(
                msg.value >= cost * _mintAmount,
                "SHIBANFTS: total cost isn't match"
            );
        }
    ...
    function randomGiveaway(
        ...
        // Checks
        require(_winner != address(0), "SHIBANFTS: zero address");
        require(_amount > 0, "SHIBANFTS: you have to mint at least one");
        require(lastSupply != 0, "SHIBANFTS: the collection is sold out");
    ...
    function tokenURI(
        ...
        require(_exists(_id), "SHIBANFTS: URI query for nonexistent token");
