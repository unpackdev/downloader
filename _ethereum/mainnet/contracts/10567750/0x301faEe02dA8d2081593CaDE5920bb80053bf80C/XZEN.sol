//  Official Website = https://xzen.io/
...
contract XZEN {
	...
	uint256 constant private Total_XZEN = 125e21; 
	uint256 constant private Burn_Ratio = 0;   
	uint256 constant private Burn_Threshold = 2;     
	uint256 constant private Freeze_Size = 1e20; 
	
	string constant public name = "XZEN";
	string constant public symbol = "XZEN";
	...
	constructor() public {
		...
		info.totalSupply = Total_XZEN;
		info.users[msg.sender].balance = Total_XZEN;
		emit Transfer(address(0x0), msg.sender, Total_XZEN);
	...
	function _transfer(address _from, address _to, uint256 _tokens) internal returns (uint256) {
		...
		if (totalSupply() - _burnedAmount < Total_XZEN * Burn_Threshold / 100 || isWhitelisted(_from)) {
			_burnedAmount = 0;
		}
