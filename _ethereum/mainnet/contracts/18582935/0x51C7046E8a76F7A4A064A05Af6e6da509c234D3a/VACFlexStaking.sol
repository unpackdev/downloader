        ...
        \$$$  /  $$ |  $$ |$$ |  $$\             $$ |      $$ |$$   ____| $$  $$<  
         \$  /   $$ |  $$ |\$$$$$$  |            $$ |      $$ |\$$$$$$$\ $$  /\$$\ 
          \_/    \__|  \__| \______/             \__|      \__| \_______|\__/  \__|
            ...
            $$\   $$ |  $$ |$$\ $$  __$$ |$$  _$$<  $$ |$$ |  $$ |$$ |  $$ |       
            \$$$$$$  |  \$$$$  |\$$$$$$$ |$$ | \$$\ $$ |$$ |  $$ |\$$$$$$$ |       
             \______/    \____/  \_______|\__|  \__|\__|\__|  \__| \____$$ |       
                                                                  $$\   $$ |       
                                                                  \$$$$$$  |       
                                                                   \______/
*/
    ...
    AccessControlEnumerable
{
    ...
    function updateCashFlowFee(uint256 _newFee)
        external
        onlyRole(DEFAULT_ADMIN_ROLE)
    {
    ...
    function updateCashFlow(address newCashFlow)
        external
        onlyRole(DEFAULT_ADMIN_ROLE)
    {
    ...
    {
        emit UpdateAddress(1, usd, _newUsd);
        usd = _newUsd;
    }
    ...
    {
        emit UpdateAddress(2, vac, _newVac);
        vac = _newVac;
    }

    function _updateCashFlow(address _newCashFlow)
        internal
        onlyValidContractAddr(_newCashFlow)
    {
        emit UpdateAddress(3, cashFlow, _newCashFlow);
        cashFlow = _newCashFlow;
    }
}

contract VACFlexStaking is AdminPanel, ReentrancyGuard {
    ...
    constructor(
        ...
        _grantRole(DEPOSITOR_ROLE, _cashFlow);
        _updateCashFlow(_cashFlow);
    }

    function stakeAndClaimReward(uint256 amount)
        external
        whenNotPaused
        nonReentrant
        onlyNonBlackListed(_msgSender())
    {
        ...
        if (user.stakedAmount > 0) {
            uint256 pending = pendingReward(msgSender);
            require(pending > 0 || amount > 0, "SACR: N T S || C");
            if (pending > 0) _claimReward(msgSender, pending);
        }

        if (amount > 0) {
            IERC20(vac).safeTransferFrom(msgSender, address(this), amount);
            user.stakedAmount += amount;
            totalStaked += amount;
            emit Stake(msgSender, amount, block.timestamp);
        }

        user.rewardDebt = (user.stakedAmount * usdPerShare) / PRECISION;
    }

    function claimCapital(uint256 amount)
        external
        whenNotPaused
        nonReentrant
        onlyNonBlackListed(_msgSender())
    {
        ...
        if (stakedAmount > 0) {
            uint256 pending = pendingReward(msgSender);
            require(pending > 0 || amount > 0, "CC: N T C");
            if (pending > 0) _claimReward(msgSender, pending);
        }

        if (amount > 0) {
            require(amount <= stakedAmount, "CC: F E L!");
            user.stakedAmount -= amount;
            totalStaked -= amount;
            IERC20(vac).safeTransfer(msgSender, amount);
            emit ClaimedCapital(msgSender, amount, block.timestamp);
        }
        ...
        returns (bool)
    {
        ...
        if (totalStaked > 0) {
            ...
            usdPerShare += (amount * PRECISION) / totalStaked;
        }
        return true;
    }
    ...
    function pendingReward(address account) public view returns (uint256) {
        return
            ((userInfo[account].stakedAmount * usdPerShare) / PRECISION) -
            userInfo[account].rewardDebt;
    }

    function _claimReward(address account, uint256 amount) private {
        uint256 feeAmount;
        if (cashFlowFee > 0) {
            feeAmount = (amount * cashFlowFee) / DIVISOR;
            amount -= feeAmount;
            require(amount > 0, "CR: N R T C");
            IERC20(usd).safeApprove(cashFlow, feeAmount);
            require(ICashFlow(cashFlow).deposit(feeAmount), "CR: D T CF F!");
        }
        IERC20(usd).safeTransfer(account, amount);
        userInfo[account].totalRewardClaimed += amount;
        emit ClaimedReward(account, amount, feeAmount, block.timestamp);
    }
}