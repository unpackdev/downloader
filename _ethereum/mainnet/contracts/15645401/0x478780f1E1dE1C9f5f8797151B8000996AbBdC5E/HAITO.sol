/*
HAITO is a crypto currency designed to pay you in USDC dividends. 

Website: https://www.HAITO.cc

Twitter: https://twitter.com/HAITOCRYPTO

https://t.me/HAITOCRYPTO


VERY LOW TOTAL SUPPLY @1,000,000 Tokens
OWNERSHIP RENOUNCED within the first 10days 

100% RUG PULL PROOF- FOREVER LOCKED LIQUIDITY POOL (all LP tokens will be sent to the DEAD address)

ANTI WHALE MECHANISM IS IN PLACE FOR THE VERY EARLY DAYS
...
3% of buy or sell transactions will Buy USDC and distribute to holders with more than 100 HAITO tokens or more.  
...
配当(HAITO)はUSDCトークンを配当として100以上の配当トークンを保持する方に分配する仮想通貨です。

非常に少ない最大流通量 1,000,000トークン

ローンチ後、１０日以内にオーナーシップ放棄

100%アンチラグプルシステム ー 永久ロックの流動性プール (LPトークンが 0x…DEADのオーナーレスアドレスに送られ、永久に流動性プールは持続します）

アンチクジラシステムが最初の数日間作動します。

ソリディティコード（コントラクト）のセキュリティ監査済み

0%プリセール、100%全てのトークンが市場で販売されるため、一番フェアなローンチ方法です。

3%の売買代金がUSDCトークンを購入するため使用され、そのUSDCを100HAITOトークン以上を保持する方に配当致します。

1%の売買代金が流動性プールに送られ、LPトークンが0x…..DEADのオーナーレスアドレスに送られます。
流動性プールのサイズは、総合計売買代金が増えるにつれ、大きくなっていきます。そのため、総売買代金が上がるにつれてHAITOトークンの価格が安定して行きます。
...
contract HAITO is Context, IERC20, Ownable {
    using SafeMath for uint256;
