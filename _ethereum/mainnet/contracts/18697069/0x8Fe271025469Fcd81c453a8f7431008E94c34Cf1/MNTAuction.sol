...
contract MNTAuction is Initializable, OwnableUpgradeable, UUPSUpgradeable, ReentrancyGuardUpgradeable {
    ...
    mapping(address => mapping(uint256 => Vat)) private nftContractVats;
    ...
    enum CHARGES {SELLER, ROYALTIES, FEES, VAT}
    ...
    event MNTClaimDone(address nftContractAddress, uint256 tokenId, address _nftSeller, address _nftHighestBidder, uint256 _nftHighestBid);
    ...
    event MNTHighestBidTaken(address nftContractAddress, uint256 tokenId, address _nftSeller, address _nftHighestBidder, uint256 _nftHighestBid);

    event MNTRoyaltiesInfo(address _receiver, uint256 _royalties);

    event MNTAuctionPaymentDetail(address _nftContractAddress, uint256 _tokenId, address _recipient, uint256 _amount, uint256 _percentage, uint256 _typeOfCharge, bool success);

    event MNTAuctionPaymentGlobal(address _nftContractAddress, uint256 _tokenId, address _seller, address _nftHighestBidder, uint256 _highestBid, uint256 _vatExcludedAmount, uint256 _fees, uint256 _royalties, uint256 _vatTax, uint32 _vatRate);
    ...
    /// Returns the vat excluded amount
    function _getAmountVatExcluded(uint256 _amount, address _nftContractAddress, uint256 _tokenId)
    internal
    view
    returns (uint256)
    {
        return (_amount * 10000) / (10000 + nftContractVats[_nftContractAddress][_tokenId].vatRate);
    }
    ...
    bidAmountMeetsBidRequirements(_nftContractAddress, _tokenId, _tokenAmount)
    {
        ...
        uint256 _vatExcludedAmount = _getAmountVatExcluded(msg.value, _nftContractAddress, _tokenId);
        emit MNTBidMade(
            _nftContractAddress,
            _tokenId,
            msg.sender,
    ...
    {
        nftContractVats[_nftContractAddress][_tokenId].vatRecipient = _vatRecipient;
        nftContractVats[_nftContractAddress][_tokenId].vatRate = _vatRate;
    ...
    function _transferNftAndPaySeller(
        ...
        uint32 vatRate = nftContractVats[_nftContractAddress][_tokenId].vatRate;
        uint256 vatPaid = _payVatTaxes(_nftContractAddress, _tokenId, _nftHighestBid);

        // Back to amount VAT excluded
        uint256 vatExcludedAmount = _nftHighestBid - vatPaid;

        uint256 royalties = _payRoyalties(_nftContractAddress, _tokenId, vatExcludedAmount);
        uint256 feesPaid = _payPlatformsFees(_nftContractAddress, _tokenId, vatExcludedAmount);

        bool successSeller = _payout(_nftContractAddress, _tokenId, _nftSeller, (_nftHighestBid - vatPaid - feesPaid - royalties));
        emit MNTAuctionPaymentDetail(_nftContractAddress, _tokenId, _nftSeller, (_nftHighestBid - vatPaid - feesPaid - royalties), 0, uint(CHARGES.SELLER), successSeller);

        emit MNTAuctionPaymentGlobal(_nftContractAddress, _tokenId, _nftSeller, _nftHighestBidder, _nftHighestBid, vatExcludedAmount, feesPaid, royalties, vatPaid, vatRate);
    ...
    function _payRoyalties(
        ...
        if (_checkRoyalties(_nftContractAddress)) {
            // Get % of royalties
            (address royaltiesReceiverDummy, uint256 royaltiesPercentage) = IERC2981(_nftContractAddress).royaltyInfo(_tokenId, 10000);
            ...
            if (royaltiesAmount > 0) {
                ...
                emit MNTAuctionPaymentDetail(_nftContractAddress, _tokenId, royaltiesReceiver, royaltiesAmount, royaltiesPercentage, uint(CHARGES.ROYALTIES), success);
            }
        }
        return royalties;
    }
    ...
    function _payPlatformsFees(
        ...
        for (uint256 i = 0; i < nftContractAuctions[_nftContractAddress][_tokenId].feeRecipients.length; i++) {
            uint256 fee = _getPortionOfBid(_nftHighestBid, nftContractAuctions[_nftContractAddress][_tokenId].feePercentages[i]);
            feesPaid += fee;
            bool success = _payout(_nftContractAddress, _tokenId, nftContractAuctions[_nftContractAddress][_tokenId].feeRecipients[i], fee);
            emit MNTAuctionPaymentDetail(_nftContractAddress, _tokenId, nftContractAuctions[_nftContractAddress][_tokenId].feeRecipients[i], fee, nftContractAuctions[_nftContractAddress][_tokenId].feePercentages[i], uint(CHARGES.FEES), success);
        }
    ...
    /// @param _nftContractAddress _nftContractAddress
    /// @param _tokenId _tokenId
    function _payVatTaxes(
        address _nftContractAddress,
        uint256 _tokenId,
        uint256 _nftHighestBid
    ) internal returns (uint256){

        uint256 vatExcludedAmount = _getAmountVatExcluded(_nftHighestBid, _nftContractAddress, _tokenId);

        uint256 vat = _nftHighestBid - vatExcludedAmount;
        if(vat>0) {
            bool success = _payout(_nftContractAddress, _tokenId, nftContractVats[_nftContractAddress][_tokenId].vatRecipient, vat);
            emit MNTAuctionPaymentDetail(_nftContractAddress, _tokenId, nftContractVats[_nftContractAddress][_tokenId].vatRecipient, vat, nftContractVats[_nftContractAddress][_tokenId].vatRate, uint(CHARGES.VAT), success);
        }
    ...
    /// @param _nftContractAddress _nftContractAddress
    /// @param _tokenId _tokenId
    function mntClaim(address _nftContractAddress, uint256 _tokenId, address _vatRecipient, uint32 _vatRate)
    external
    ...
    /// @param _nftContractAddress _nftContractAddress
    /// @param _tokenId _tokenId
    function mntTakeHighestBid(address _nftContractAddress, uint256 _tokenId, address _vatRecipient, uint32 _vatRate)
    external
    onlySeller(_nftContractAddress, _tokenId)
    {
    ...
    function _fillInfoStruct(
        ...
        uint256 vatExcludedAmount = ( nftContractAuctions[_nftContractAddress][_tokenId].nftHighestBid * 10000) / (10000 + nftContractVats[_nftContractAddress][_tokenId].vatRate);
        ...
        InfoType memory info = InfoType(
            ...
            vatExcludedAmount,// Must return price excl VAT
