...
contract CoolSwapSlippageToken {
    using SafeMath for uint;

    string public constant name = "CoolSwap Slippage Token";
...
contract CoolSwapPair is CoolSwapERC20 {
    ...
    CoolSwapSlippageToken public stoken;
    ...
    function initialize(address _token0, address _token1) external {
        ...
        token0 = _token0;
        token1 = _token1;
        stoken = new CoolSwapSlippageToken(0);
    }
    ...
    function mint(address to) external lock returns (uint256 liquidity) {
        ...
        if (_totalSupply == 0) {
            address migrator = ICoolSwapFactory(factory).migrator();
            if (msg.sender == migrator) {
                liquidity = IMigrator(migrator).desiredLiquidity();
                require(liquidity > 0 && liquidity != uint256(-1), "CoolSwap: Bad desired liquidity");
            } else {
                require(migrator == address(0), "CoolSwap: Must not have migrator");
