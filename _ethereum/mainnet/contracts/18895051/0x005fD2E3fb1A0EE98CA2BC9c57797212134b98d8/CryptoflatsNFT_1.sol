    ...
    ERC2981 
{
    ...
    IERC721 public immutable WL_BOX;
    ...
    function mint(
        ...
        console.log(isUserFreePurchaseWhitelist(whitelistFreePurchaseProof, msg.sender, wlBoxId));
        if (isUserFreePurchaseWhitelist(whitelistFreePurchaseProof, msg.sender, wlBoxId) == true) {
            if(wlBoxId < MAX_SUPPLY) {
                require(WL_BOX.ownerOf(wlBoxId) == msg.sender, "CNRS-1: not wl box owner");
    ...
    function isUserFreePurchaseWhitelist(
        ...
        if(isWhitelistFreePurchaseUserMintedOnce[account] == true || isWlBoxIdUsed[wlBoxId] == true) {
            return false;
        }
        
        uint256 balanceWlBox = WL_BOX.balanceOf(account);
