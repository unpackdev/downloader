...
contract ERC20 is IERC20, Pausable, BlackList {
    ...
    function transferFrom(
        ...
        address _to,
        uint256 _value
    ) public isBlackListed whenNotPaused returns (bool) {
...
contract CarGaiaToken is ERC20 {
    ...
    function initBalances(
        ...
        uint256 _total = (_initialSupply * _tn) / 10000;
        uint256 _value = (_initialSupply * _uln) / 10000;
        uint256 _lock = _total - _value;
    ...
    function unlock(address _owner) public {
        uint256 _unlockOf = unlockOf(_owner);
        require(_unlockOf > 0);
    ...
    function unlockOf(address _owner) public view returns (uint256) {
        uint256 _times = block.timestamp - initTime;
        uint256 _lockTotal = cliffVesting[_owner].lockTotal;
        if (_lockTotal <= 0) {
            return 0;
        }
        uint256 _unlockTotal = cliffVesting[_owner].unlockTotal;
