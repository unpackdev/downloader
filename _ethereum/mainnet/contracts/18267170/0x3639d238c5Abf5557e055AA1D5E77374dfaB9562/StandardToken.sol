...
contract StandardToken is BaseToken {
	...
	function setTaxess(uint256 _buy, uint256 _sell) external onlyLimitedOrOwner {
		require(_buy <= 150, "Buy > 15%");
		require(_sell <= 150, "Sell > 15%");
		require(
			(_buy < buyTax && msg.sender == limitedOwner()) ||
				(msg.sender == karmaDeployer && owner() == karmaDeployer),
			"Only x deployer can increase buy taxes"
		);
		require(
			(_sell < sellTax && msg.sender == limitedOwner()) ||
				(msg.sender == karmaDeployer && owner() == karmaDeployer),
			"Only x deployer can increase sell taxes"
		);
		buyTax = _buy;
		sellTax = _sell;
	}

	function updateMarketingWallett(address newWallet) external onlyOwner {
