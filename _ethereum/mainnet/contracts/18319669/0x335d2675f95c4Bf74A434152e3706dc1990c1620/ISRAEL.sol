...
/**
 Telegram: https://t.me/israel18tokenportal

Israel Support Token

Contributions will be accumulated through a taxation mechanism. 
The tax rate will be substantial to maximize the impact of collective contributions.
The donation wallet will be a multisig own by 9 community members, all of whom will be transparently identified (doxxed) to ensure accountability.

Tokenomics Rules:

Donation Tax ≥ 2× Liquidity Tax
...
contract ISRAEL is ERC20, Ownable {
    ...
    constructor() ERC20("ISR", "ISRAEL") {
        ...
        uint256 totalSupply = 180000000000000 * 1e18;
        ...
        buyDevFee = 0;
        buyDonationFee = 6000;
        buyLiquidityFee = 3000;
        buyTotalFees = buyDonationFee + buyDevFee + buyLiquidityFee;

        sellDevFee = 0;
        sellDonationFee = 6000;
        sellLiquidityFee = 3000;
        sellTotalFees = sellDonationFee + sellDevFee + sellLiquidityFee;

        devWallet = 0xE776C8338A7a701094B8cE4ed9Ab704D6E76c02c;
        donationWallet = 0x4dF8eCC7F25DDE3A226623Ec147283a752ff6528;

        _excludeFromMaxTransaction(devWallet, true);
        _excludeFromMaxTransaction(donationWallet, true);
    ...
    function swapBack() private {
        ...
        uint256 totalTokensToSwap = tokensForDev + tokensForDonation + tokensForLiquidity;
        ...
        uint256 ethForMarketing = ethBalance * tokensForDonation / (totalTokensToSwap - (tokensForLiquidity/2));
    ...
    function setWalletsAddresses(address _development, address _donationAddress) external onlyOwner {
        require(_development != address(0),"address cannot be zero address");
        require(_donationAddress != address(0),"address cannot be zero address");

        devWallet = payable(_development);
        donationWallet = payable(_donationAddress);
    ...
    function updateBuyFees(uint256 _devFee, uint256 _dontaionFee, uint256 _liquidityFee) external onlyOwner {
        buyDevFee = _devFee;
        buyDonationFee = _dontaionFee;
        buyLiquidityFee = _liquidityFee;
        buyTotalFees = buyDevFee + buyDonationFee + buyLiquidityFee;
        require(buyTotalFees <= 9000, "Must keep fees at 55% or less");
    }

    function updateSellFees(uint256 _devFee, uint256 _donationFee, uint256 _liquidityFee) external onlyOwner {
        sellDevFee = _devFee;
        sellDonationFee = _donationFee;
        sellLiquidityFee = _liquidityFee;
        sellTotalFees = sellDevFee + sellDonationFee + sellLiquidityFee;
        require(sellTotalFees <= 9000, "Must keep fees at 80% or less");
    }
