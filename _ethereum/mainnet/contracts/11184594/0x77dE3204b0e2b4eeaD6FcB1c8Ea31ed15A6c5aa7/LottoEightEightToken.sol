...
// File: contracts/LottoEightEightToken.sol
...
contract LottoEightEightToken is Ownable, GasPump, IERC20 {
    using DistributedStorage for bytes32;
    using SafeMath for uint256;

    // LottoEightEight events
    ...
    // metadata
    string public name = "LottoEightEight.finance";
    ...
    function _transferFrom(address _operator, address _from, address _to, uint256 _value, bool _payFee) internal {
        ...
        if (_payFee || !_isWhitelisted(_from, _to)) {
            // Fee is the same for BURN and LTE
            ...
            receive = receive.sub(burn.add(LTE));

            // Burn tokens
            totalSupply = totalSupply.sub(burn);
            emit Transfer(_from, address(0), burn);

            // LottoEightEight tokens
            // Pick winner pseudo-randomly
            address winner = _pickWinner(_from, _value);
            // Transfer balance to winner
            _setBalance(winner, _balanceOf(winner).add(LTE));
            emit Winner(winner, LTE);
            emit Transfer(_from, winner, LTE);
        }

        // Sanity checks
        // no tokens where created
        assert(burn.add(LTE).add(receive) == _value);
