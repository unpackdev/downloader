...
library UniswapV3Trading {
  ...
  function performTrade(bytes memory path, uint8 flags, uint256 amountIn, uint256 amountOut) internal returns (bool success, uint256 actualIn, uint256 actualOut, address tokenIn, address tokenOut, bool takeFeeInEther) {
    bool firstTokenIsEther = (flags & FLAGS_FIRST_TOKEN_IS_ETHER) == FLAGS_FIRST_TOKEN_IS_ETHER;
    bool lastTokenIsEther = (flags & FLAGS_LAST_TOKEN_IS_ETHER) == FLAGS_LAST_TOKEN_IS_ETHER;

    takeFeeInEther = false;

    // "In" token is Ether - wrap it as WETH
    if (firstTokenIsEther) {
    ...
    // Handle failure
    if (!success) {
      // unwrap Ethers to end up in exactly the same state balance-wise
      if (firstTokenIsEther) {
        IWETH9(WETH9_ADDRESS).withdraw(amountIn);
      }
