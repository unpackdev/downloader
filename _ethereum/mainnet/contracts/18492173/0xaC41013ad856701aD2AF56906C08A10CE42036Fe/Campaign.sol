    ...
    ReentrancyGuardUpgradeable
{
    ...
    function getTaskToCampaignId(uint24 id) public view returns(uint24){
        return taskToCampaignId[id];
    }

    function getClaimedTasks(uint24 taskId, address sender) public view returns(uint8){
        return claimedTasks[taskId][sender];
    }

    function getMultipleClaim(uint24 taskId) public view returns(uint8){
        return multipleClaim[taskId];
    }

    function getCheckClaimedHL(uint48 hlNonce) public view returns(uint8){
        return checkClaimedHL[hlNonce];
    }
