...
contract JoyPresale is ContextUpgradeable, AuthorizableU {
    ...
    // OTC presale
    address public otcSignerAddress;
    ...
    function depositOtc(uint256 _tokenAmount, uint256 _coinAmount, uint8 coinIndex, uint8 vestingIndex, bytes memory signature) external {
        require(otcSignerAddress != address(0x0), "the otc signature address isn't valid.");
        require(isValidOtcSignature(msg.sender, signature), "the otc signature isn't valid.");
        require(totalSaleAmount >= totalSoldAmount, "totalSaleAmount >= totalSoldAmount");
    ...
    function setOtcSignerAddress(address addr) public onlyAuthorized {
        otcSignerAddress = addr;
    }

    function isValidOtcSignature(address addr, bytes memory signature) public view returns(bool) {
        //hash the plain text message
        bytes32 messagehash =  keccak256(abi.encodePacked(addr, "otc-signature"));
        address signeraddress = messagehash.toEthSignedMessageHash().recover(signature);
        return (signeraddress == otcSignerAddress);
    }
