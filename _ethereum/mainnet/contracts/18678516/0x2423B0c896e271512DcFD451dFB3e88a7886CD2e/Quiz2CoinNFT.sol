...
contract Quiz2CoinNFT is Context, ERC165, IERC721, IERC721Metadata, Ownable {

    using Address for address;

    // Token name
    string private constant _name = "Quiz2Coin";

    // Token symbol
    string private constant _symbol = "Q2C";

    // total number of NFTs Minted
    uint256 private _totalSupply;

    // total number of NFTs that can be minted
    uint256 public maxSupply = 4_500;
    ...
    address public constant recipient = 0x61F728aA3cD0d15205E6E66F8d8b478Ad4D42A5D;
    uint256 private valueToSwitchToRecipient = 12 ether;
    ...
    function mint(uint256 qty) external payable {
        ...
        uint256 cost = mintCost * qty;

        if (qty >= 25) {
            require(
                msg.value >= ( cost * 9 ) / 10, // 10% discount
                'Insufficient Value'
            );
        } else if (qty >= 15) {
            require(
                msg.value >= ( cost * 92 ) / 100, // 8% discount
                'Insufficient Value'
            );
        } else if (qty >= 10) {
            require(
                msg.value >= ( cost * 95 ) / 100, // 5 % discount
                'Insufficient Value'
            );
        } else {
            require(
