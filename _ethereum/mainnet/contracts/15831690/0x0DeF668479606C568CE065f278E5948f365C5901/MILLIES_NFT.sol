...
contract MILLIES_NFT is ERC721Enumerable, Ownable, AccessControlEnumerable {
    ...
    **********************************************/
    ...
    string private notRevealedUri = "https://nftstorage.link/ipfs/bafybeiclmo7slevfmvem753odrx652njkeilquzxb4cflti45qrteq7kce/MilliesNotRevealed.json";
    ...
    //  WL Spots: 2000
    uint256 public maxWlSpots = 2000;
    ...
    mapping(address => bool) private walletsInWhitelist;
    // Contador de wallets en Whitelist
    Counters.Counter private counterWalletsInWhitelist;
    ...
    **********************************************/
    ...
    function mint(uint256 _mintAmount) external payable {
        ...
        if(isWhitelistEnabled) {
            ...
            require(walletsInWhitelist[_to], "Exception in mint: You are not whitelisted.");
    ...
    function claimFreemint() external {
        ...
        require(freeMinters[_msgSender()], "Exception in claimFreemint: You do not have access to free mining.");
    ...
    **********************************************/
    ...
    // Internal function - Add/Delete wallet from Whitelist
    function addToWhitelist(address _to, bool toggle) internal {
        require(counterWalletsInWhitelist.current() <= maxWlSpots, "Exception in addToWhitelist: Max spot reached.");

        walletsInWhitelist[_to] = toggle;

        if(walletsInWhitelist[_to]) 
            counterWalletsInWhitelist.increment();
        else
            counterWalletsInWhitelist.decrement();
    }

    // Public function - Use wallet array, easy wallet import.
    function bulkAddToWhitelist(address[] memory _to, bool toggle) external {
        require(hasRole(ADMIN_ROLE, _msgSender()), "You do not have permissions.");
        for(uint i = 0; i < _to.length; i++)
            addToWhitelist(_to[i], toggle);
    }
    ...
    function numOfWalletInWhitelist() external view returns (uint) {
        return counterWalletsInWhitelist.current();
    }

    function isWhitelisted(address wallet) external view returns(bool) {
        return walletsInWhitelist[wallet];
    }
    ...
    **********************************************/
    ...
    function addToFreeMint(address _to, bool toggle) internal {
        require(freeMinters[_to] != toggle, "Error in addToFreeMint: This wallets have the same value.");
        freeMinters[_to] = toggle;
        addToWhitelist(_to, true);
    }

    function bulkAddToFreeMint(address[] memory _to, bool toggle) external {
        require(hasRole(ADMIN_ROLE, _msgSender()), "You do not have permissions.");
        for(uint i = 0; i < _to.length; i++)
            addToFreeMint(_to[i], toggle);
    }
    ...
    function isFreeminter(address wallet) external view returns(bool) {
        return freeMinters[wallet];
    }
    ...
    **********************************************/
    ...
    function remainingForMintOneWallet(address wallet) external view returns(uint256) {
        ...
        if(isWhitelistEnabled) {
            if(walletsInWhitelist[wallet]) resultado = maxMintAmountinWL - addressMintedBalanceInWL[wallet].current();
            else resultado = 0;
        } else {
    ...
    **********************************************/
    ...
    function setMaxWhitelistSpot(uint256 _maxAmount) external {
        require(hasRole(ADMIN_ROLE, _msgSender()), "You do not have permissions.");
        maxWlSpots = _maxAmount;
    }
