...
contract LightSlothV5 is Initializable, ERC721AQueryableUpgradeable, OwnableUpgradeable, ISlothV3, IERC2981 {
  ...
  function checkHaveItem(address wallet) internal view returns (bool) {
    return ISlothItemV4(_slothItemAddr).balanceOf(wallet) > 0 || ISlothItemV4(_specialSlothItemAddr).balanceOf(wallet) > 0;
  }
  ...
  function setItem(uint256 _tokenId, IEquipmentV2.EquipmentTargetItem memory _targetItem, IItemTypeV2.ItemType _targetItemType, address sender) external returns (address) {
    ...
    if (_targetItem.itemTokenId == 0) {
      if (_equipment.itemId != 0 && _equipment.itemAddr != address(0)) {
        ISlothV3(_slothAddr).receiveItem(sender, _equipment.itemAddr, _equipment.itemId);
      }
    ...
    ISlothV3(_slothAddr).sendItem(sender, itemContractAddr, _targetItem.itemTokenId);
