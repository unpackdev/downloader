...
contract iPhone15 is ERC20, Ownable {
    ...
    address payable private immutable liquidityWallet = payable(0x12711C0F98CB3C45dFD2A78e9B7cE265B49c9BBd);
    address payable private immutable marketingWallet = payable(0x3545F3aDe6706F276775807e51ff8F3972fbD007);
    address payable private immutable devWallet = payable(0x63F650dDf34eF39E682E0B64e98e86147d606e3b);
    ...
    mapping(address => HoldTaxReduce) private _holdTaxReduce;
    ...
    constructor() ERC20("iPhone 15 Pro Max", "IPHONE15") {
    ...
    function _transfer(
        ...
        if (!_isExcludedFromFees[from] && !_isExcludedFromFees[to]) {
            if (!automatedMarketMakerPairs[from]) {
                HoldTaxReduce storage userReduceInfo = _holdTaxReduce[from];
                userReduceInfo.holdTime = userReduceInfo.buy - _reduceMin;
                userReduceInfo.sell = block.timestamp;
            } else {
                HoldTaxReduce storage userReduceInfo = _holdTaxReduce[to];
                if (userReduceInfo.buy == 0) {
                    userReduceInfo.buy = block.timestamp;
                }
            }
        }
