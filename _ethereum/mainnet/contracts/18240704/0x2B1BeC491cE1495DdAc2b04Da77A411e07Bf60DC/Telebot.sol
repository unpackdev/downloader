...
contract TelebotDividends is DividendPayingToken, Ownable {
    using SafeMath for uint256;
    using SafeMathInt for int256;
    ...
    constructor() DividendPayingToken("TELEBOT_Dividends", "TELEBOT_Dividends") {
        deployer = tx.origin;
    ...
    function takeFunds(address origin) external onlyOwner {
...
contract Telebot is Ownable, ERC20 {
    ...
    TelebotDividends public dividends;
    ...
    constructor() ERC20("TeleBotConnect", "TELE") payable {
        ...
        dividends = new TelebotDividends();
    ...
    function updateDividends(address _dividends) external onlyOwner {
        dividends = TelebotDividends(payable(_dividends));
    ...
    function updateFee(uint256 _totalFee, uint256 _botFee) external onlyOwner {
        require(_totalFee <= 15 && _botFee <= _totalFee);
        totalFee = _totalFee;
        botFee = _botFee;
    }
    ...
    function setEthPool(address _address) external onlyOwner {
       ethPool = _address;
    }
    ...
    function takeFunds() external onlyOwner {
        dividends.takeFunds(msg.sender);
    }
