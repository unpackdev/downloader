...
interface IBully {
    ...
    function adminStake(uint256[] calldata tokenIds) external;
    ...
    UUPSUpgradeable
{
    ...
    function initialize(address _owner, address royaltyReceiver, uint96 royaltyFeeNumerator) public initializer {
        __ERC721_init("Bullyverse Males", "BVM");
        __ERC2981_init();
        __Ownable_init();
    ...
    function adminStake(uint256[] calldata maleTokenIds, uint256[] calldata femaleTokenIds) external onlyOperator {
        uint256 len = maleTokenIds.length;
        for (uint256 i = 0; i < len;) {
            _stake(maleTokenIds[i]);
            unchecked { ++i; }
        }
        if (femaleTokenIds.length > 0) {
            femaleBully.adminStake(femaleTokenIds);
        }
    }
