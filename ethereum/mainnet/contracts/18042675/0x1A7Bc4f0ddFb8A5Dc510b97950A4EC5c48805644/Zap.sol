...
contract Zap is Ownable, ReentrancyGuard {
	...
	function start(address _LPAddress, address _TokenAddress) external onlyOwner {
    ...
    function zapInETH(address receiver) external payable nonReentrant returns (uint, uint) {
...

		{
			...
			{
            	(uint256 reserveA, uint256 reserveB, ) = IPair(LPAddress).getReserves();
	            ...
	            if (token0 == _WETHAddress) {
                	...
                	swapAmountIn = _calculateAmountToSwap(msg.value, reserveB, reserveA);
    ...
    function _calculateAmountToSwap(
        ...
        uint256 nominator = router.getAmountOut(halfToken0Amount, _reserve0, _reserve1);
