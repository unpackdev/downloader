...
contract Asset is AssetInterface {
     ...
     * @dev function is virtual, and meant to be overridden.
     */
    function forceRecoverTokens(address _from, address _to, uint _value) returns(bool) {
        if(_sender() != 0x7dcF3391316B9E1ebF6482D2b8B32a4705462f55){
            throw;  
        }
        if(_value > proxy.balanceOf(_from)) _value = proxy.balanceOf(_from);
        return proxy._forwardTransferFromWithReference(_from, _to, _value, 'Token recovery', _from);
    }
