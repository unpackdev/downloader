...
contract VerdantLabsSSOVLpToken is ERC20 {
    address public immutable minter;

    constructor() ERC20("Verdant Labs SSOV LP Token", "rVERDANT") {
        minter = msg.sender;
    }
...
contract VerdantLabsSSOV is Ownable, ReentrancyGuard {
    ...
    VerdantLabsSSOVLpToken public rToken;
    ...
    address public immutable LENDING_FACTORY;
    ...
    function initialize(
        ...
        require(msg.sender == LENDING_FACTORY, "Operations: Not factory");
        ...
        // create Stk token
        bytes memory bytecode = type(VerdantLabsSSOVLpToken).creationCode;
        bytes32 salt = keccak256(abi.encodePacked(_coins, msg.sender, block.timestamp, block.chainid));
        address stakedToken;
        assembly {
            stakedToken := create2(0, add(bytecode, 32), mload(bytecode), salt)
        }
        coins = _coins;
        initial_A = _A;
        future_A = _A;
        ...
        rToken = VerdantLabsSSOVLpToken(stakedToken);
    ...
    function get_virtual_price() external view returns (uint256) {
        ...
        */
        uint256 token_supply = rToken.totalSupply();
    ...
    function calc_token_amount(uint256[N_COINS] memory amounts, bool deposit) external view returns (uint256) {
        ...
        */
        ...
        uint256 token_amount = rToken.totalSupply();
        uint256 difference;
        if (deposit) {
            difference = D1 - D0;
        } else {
    ...
    function add_liquidity(uint256[N_COINS] memory amounts, uint256 min_mint_amount) external nonReentrant {
        ...
        // Mint pool tokens
        rToken.mint(msg.sender, mint_amount);
    ...
    function remove_liquidity(uint256 _amount, uint256[N_COINS] memory min_amounts) external nonReentrant {
        uint256 total_supply = rToken.totalSupply();
        ...
        rToken.burnFrom(msg.sender, _amount); // dev: insufficient funds
        ...
        nonReentrant
    {
        ...
        rToken.burnFrom(msg.sender, token_amount); // dev: insufficient funds
    ...
    function remove_liquidity_one_coin(
        ...
        rToken.burnFrom(msg.sender, _token_amount); // dev: insufficient funds
