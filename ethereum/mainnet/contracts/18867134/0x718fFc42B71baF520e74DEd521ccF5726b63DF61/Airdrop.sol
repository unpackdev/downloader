...
/** 
  * Moonbirds Airdrop
  * Each Moonbird can claim 10,000 BIRD tokens
  */
...
contract Airdrop is Ownable {
    ...
    function claim(address _contractAddress, uint256[] calldata _tokenIds) public {
        ...
        uint256 numBirdTokens = _tokenIds.length.mul(numTokensPerNFT);
        require(tokenContract.balanceOf(address(this)) >= numBirdTokens, "Not enough tokens remaining in faucet");
        require(tokenContract.transfer(msg.sender,numBirdTokens),"Error sending coins to caller");
        
        emit TokensClaimed(msg.sender, _contractAddress, numBirdTokens);
    }
