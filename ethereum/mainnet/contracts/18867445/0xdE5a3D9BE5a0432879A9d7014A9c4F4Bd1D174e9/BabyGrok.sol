    ...
    ERC20Upgradeable
{
    ...
    ////////STAKING FUNCTIONS/////////
    //stake BabyGrok tokens to contract and claims any accrued interest
    ...
    function StakeTokens(uint amt) external synchronized {
        ...
        require(BabyGrokBalance() >= amt, "Error: insufficient balance"); //ensure user has enough funds
  ...
  **/
    ...
    function _claimInterestUnstake() internal {
        ...
        require(
            ...
            "staking contract has not enough BabyGrok token"
        );
    ...
    function calcStakingRewards(address _user) public view returns (uint) {
        uint BabyGrokBurnt = staker[_user].totalBurnt;
        uint staked = staker[_user].stakedBalance;
        uint apyAdjust = 10000;
        if (BabyGrokBurnt > 0) {
            if (BabyGrokBurnt >= staked.sub(staked.div(10))) {
                apyAdjust = 1000;
            } else {
                uint burntPercentage = ((BabyGrokBurnt.mul(100) / staked));
    ...
    //BabyGrok balance of caller
    function BabyGrokBalance() public view returns (uint256) {
        return balanceOf(msg.sender);
    }

    // can only burn equivalent of x10 total staking interest
    // minimize vamp bots by keeping max burn pamp slippage low
    function BurnBabyGrok(uint amt) external synchronized {
        require(
