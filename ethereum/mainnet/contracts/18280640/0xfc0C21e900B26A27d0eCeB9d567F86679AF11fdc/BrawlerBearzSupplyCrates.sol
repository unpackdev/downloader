    ...
    IBrawlerBearzSupplyCrates
{
    ...
    function _processFulfillment(
        ...
        uint256 consolationPrizeQuantity = config.consolationPrizes.length;
        uint256 dropQuantity = config.quantity * openAmount;
        uint256[] memory ids = new uint256[](
            dropQuantity + consolationPrizeQuantity
        );
        uint256 seed;
        uint256 indexChosen;
        uint256 i = 0;
        ...
        if (consolationPrizeQuantity > 0) {
            for (uint256 j = 0; j < consolationPrizeQuantity; ) {
                ids[j + dropQuantity] = config.consolationPrizes[j];
                unchecked {
                    ++j;
                }
            }
     ...
     */
    function setSupplyCrateConfig(
        SupplyCrateConfig calldata config
