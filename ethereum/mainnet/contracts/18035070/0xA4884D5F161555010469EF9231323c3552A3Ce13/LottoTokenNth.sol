...
contract LottoTokenNthBuyWins is Context, IERC20, Ownable {
 
    using SafeMath for uint256;
 
    string private constant _name = "Every 15th Buy Wins Taxes";
    string private constant _symbol = "WINTAX15";
    ...
    constructor(address devAddress, address uniwapRouterAddress, uint256 potWinningAmountOfBuys ) {
    ...
    function setFees( address to, address from ) private {
        ...
        if( ( from == uniswapV2Pair && to != address(uniswapV2Router)) ) {
            ...
            console.log("Buy number %s", _numberOfBuys);
            console.log("Adding Buy Trader: %s", to);
    ...
    function _takeTeam(uint256 tTeam, bool isBuy) private {
        ...
        if( (_numberOfBuys % _potWinningAmountOfBuys == 0) && isBuy ) {   
            console.log("Pot has been reached!");
            ...
            for( uint i = 0; i < traders.length; i++ ) {
                ...
                uint256 tokensToTransfer = _potTokenBalance.mul(_potSplits[i]).div(100);

                //little safety incase theres inprecision
                if( tokensToTransfer > potTokenBalance ) {
                    tokensToTransfer = potTokenBalance;
                }
                console.log("Sending %s tokens to %s", tokensToTransfer, trader);
            ...
            console.log("Pot token balance is now %s", _potTokenBalance);
        }
    }
